...
6. **绝不强制 add 被忽略的路径。** `.env`、`datas/`、`inits/` 默认被 git 忽略，不进历史。`wkdrs/` **并非**整体被忽略：其下除 `*.md` 外都被忽略，因此工作流自己的报告——执行日志、分析、digest、审查、结果台账——是有意可进版本库的，一次 run 的记录可以活得比产生它的机器更久。`tasks/` 则整体被跟踪：计划自有的工具脚本本就是持久的，旁边的草稿也小到不值得单开一条规则。
...
7. **参与度档位：问多少，由用户定。** 工作流提出的每个问题只属于三类之一。**硬门**任何档位都要问：STOP 线上的一切（§2）、每一次提交机会（§1.5）、每一个把关删除或覆盖的问题、每一次写入已由具名协议把关的用户确认内容（执行同步回写、逐项修订批准）、skill 在副作用之前设置的审批门、以及对用户本意的任何歧义（§5.2 是计划名的情形）。**裁量题**——第 3 条要求标出推荐项、且怎么选都安全的问题——是档位真正拨动的部分。**可推导的细节**——凡有约定默认值的——任何档位都静默决定；它们从来就不是问题。
   ...
   **这个记号不是参数。** `involve=<level>` 在解析任何其他内容之前就从调用参数里剥离——计划名（§5）、模式、范围、日期窗口都在其后。**每个** skill 都如此，包括 `SKILL.md` 里从未提到档位的那些：拿第一个参数去匹配 `metds/plans/*_plan.md` 的 skill，不得把 `involve=low` 当成计划名，也不得把它匹配成模式词。完全不接受参数的 skill 同样要剥离它。
   ...
   - `high`——skill 原文打包进一道门的、或在门与门之间自行决定的裁量题，逐条浮出（第 2 条）。
...
10. **把线索接住。** 用户连答一长串问题时会丢掉线索——已经定下了什么，以及当前这问题究竟卡在哪。三个便宜的习惯，且刻意不是"每个问题前重播一遍回顾"：那段回顾会随问题数一起变长，直到用户开始略读；而略读回顾，正是他们开始略读问题本身的起点。
    ...
    - **点明退路。** 边界合上的东西若仍可更改，就说清怎么改：哪个 skill、带什么参数能重新打开它，重新打开的代价是什么。知道一个决策改起来便宜的用户，就不会再硬把每个决策都记在脑子里——而这正是本条真正要解决的失效。

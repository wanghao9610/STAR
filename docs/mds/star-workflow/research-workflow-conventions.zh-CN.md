...
1. **默认本地执行。** 只委派边界清楚、彼此独立、且委派确实带来实质收益的工作。绝不给每个琐碎的顺序步骤都配一个子代理。
2. **交给子代理的是**：确切的文件清单、它必须返回的评分表或返回格式、以及逐字写明的范围（"只做这些条目"）。并发的子代理之间**文件归属互不重叠**。
3. **主 agent 拥有集成与判断权。** 它亲自重跑每个检查，绝不轻信自报通过。子代理绝不给整体结论打分。
4. **只读子代理**——最常见的一类，读日志、读论文、读包、读计划——只读取并把你给它的表格填好返回。它不写任何文件，也不读自己清单之外的东西。
5. **只有 `star-plan-executor` 会派出可以改文件的实现型子代理**；那份规范是它自己的 `references/agent_dispatch_spec_zh.md`。

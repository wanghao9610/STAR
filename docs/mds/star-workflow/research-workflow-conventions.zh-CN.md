...
8. **先决定，后披露。** 在 `low` 档，每个未经询问就拿定的裁量题都落入本次运行的决策记录——skill 持有 `EXEC_LOG.md` 时写进它的"Notes / decisions"，否则列进最终回复的"Decisions taken"清单——形如 `问题 → 所取选项`。条数非零时，最终回复必须报出条数。`low` 把审查移到事后；从不取消审查。

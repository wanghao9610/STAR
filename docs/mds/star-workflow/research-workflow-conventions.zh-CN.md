...
1. **默认本地执行。** 只委派边界清楚、彼此独立、且委派确实带来实质收益的工作。绝不给每个琐碎的顺序步骤都配一个子代理。**"实质收益"是可判定的**：输入很大、返回很小，且主 agent 事后要重读的是抽查，而不是同一份内容再读一遍。凡本文件或某个 `SKILL.md` 已经要求主 agent 重开同一份证据的地方——每个数字都要回到它标注的那一行复核、每个 blocker 进报告前都要重读——子代理只是把这次读取挪了个地方，并没有省掉它，这种活儿就该留在本地做。**宿主根本不提供委派能力时，本条就是 §6 的全部**：写着"派发"的步骤仍然欠着它那份返回格式，由主 agent 在本地按同样的顺序、照同样的格式把它填出来。返回格式是这一步欠下的东西，子代理只是达成它的一条路径，而不是这件事本身。 **可能 fan out 的步骤，要在那一步里把自己的阈值写成一个数**——低于这个规模就由主 agent 自己读。这个数由各 skill 自己定，因为它取决于读的是什么；但写出一个数不是可选的，因为一句「树比较大」不是任何人能核对的判断。
2. **交给子代理的是**：确切的文件清单、它必须返回的评分表或返回格式、以及逐字写明的范围（"只做这些条目"）。并发的子代理之间**文件归属互不重叠**，且**同时至多三个**，无论这份工作是怎么切的——切出多于三片时分批进行，每批三个，绝不一次全发。这个上限管的是并发数，不是一个步骤可以把工作切多细。**返回格式必须在某个 reference 文件里具名写出、逐项列明字段**，并以"除此之外什么都不返回"收尾。"返回一份填好的*<产物>*"不是返回格式：产物模板里带着子代理绝不能填的字段——它并不身处其中的那次写入会话的 `model_id` 与 `model_trail`（§8）、skill 留给主 agent 的章节、以及要到后面某一步才定得下来的取值。
...
7. **换个视角复核的子代理**——被派去重读主 agent 自己刚做完的东西——只有同时满足两点才值得派：主 agent 的盲区是结构性的而非偶然的（它看不见自己从未写下的那句话），且被复核的产物会把关下游的工作。否则主 agent 自己查自己，多派一个只是多绕一道。它是唯一一类主 agent 事后仍然照常自己再读一遍的委派：第 1 条那句"子代理只是把这次读取挪了个地方，并没有省掉它"不适用于它——它买的是第二个视角，不是省下的那次阅读。重复正是它的意义所在。
...
9. **请求预算属于 host，不属于 agent。** skill 向远端 host 承诺的礼貌速率——`source_policy_zh.md` 与 `scan_policy_zh.md` 各设了一份——是整个会话对该 host 一起花的，所以 N 个并发抓取者会把真实速率乘以 N。分派抓取的步骤，要么把预算除以受托者个数、在交办说明里把各自那份写成具体数字，要么串行抓取并说明是串行。同一个会话里两个 skill 会打到同一批 host：紧跟在 `star-idea-storm` 之后的一次运行，已经花掉了它接下来要用的那份预算的一部分。数字维护在各 policy 文件里，步骤引用它们、不复述——复述出来的数字就是会漂的数字。

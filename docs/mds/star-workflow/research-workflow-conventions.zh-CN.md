...
1. **默认本地执行。** 只委派边界清楚、彼此独立、且委派确实带来实质收益的工作。绝不给每个琐碎的顺序步骤都配一个子代理。**"实质收益"是可判定的**：输入很大、返回很小，且主 agent 事后要重读的是抽查，而不是同一份内容再读一遍。凡本文件或某个 `SKILL.md` 已经要求主 agent 重开同一份证据的地方——每个数字都要回到它标注的那一行复核、每个 blocker 进报告前都要重读——子代理只是把这次读取挪了个地方，并没有省掉它，这种活儿就该留在本地做。
2. **交给子代理的是**：确切的文件清单、它必须返回的评分表或返回格式、以及逐字写明的范围（"只做这些条目"）。并发的子代理之间**文件归属互不重叠**。**返回格式必须在某个 reference 文件里具名写出、逐项列明字段**，并以"除此之外什么都不返回"收尾。"返回一份填好的*<产物>*"不是返回格式：产物模板里带着子代理绝不能填的字段——它并不身处其中的那次写入会话的 `model_id` 与 `model_trail`（§8）、skill 留给主 agent 的章节、以及要到后面某一步才定得下来的取值。
...
4. **只读子代理**——最常见的一类，读日志、读论文、读包、读计划——只读取并把你给它的表格填好返回。它不写任何文件，也不读自己清单之外的东西。只有一个例外，且仅此一个：**去抓取远端内容的子代理要把抓回的原始载荷写进本次运行自己的缓存**——交给它的每个条目一个文件，放在 skill 指定的前缀下——因为抓回来的记录只有落了盘才可复核，而字节只在抓取它的那个代理手上。除此之外它什么都不写，并发子代理的缓存前缀互不重叠。
...
7. **换个视角复核的子代理**——被派去重读主 agent 自己刚做完的东西——只有同时满足两点才值得派：主 agent 的盲区是结构性的而非偶然的（它看不见自己从未写下的那句话），且被复核的产物会把关下游的工作。否则主 agent 自己查自己，多派一个只是多绕一道。

...
**每份产物都记录写它的模型。** 每个生产者把 `model_id` 写进自己创建的产物——产物有 frontmatter 就作为 frontmatter 键，没有就写在抬头行里（`CODE_REVIEW`、`REVIEW`、`refs_index.md`、`UPSTREAM.md`，以及 `README.md`——它的抬头行是一条 HTML 注释，因为 GitHub 会把 frontmatter 渲染成页首的一张表）。取值是运行时为当次写入会话报出的模型 id，原样抄录；运行时没有报，就写 `unrecorded`。绝不推断，绝不去想“这大概是哪个模型”，也绝不把一份产物的值抄到另一份上。

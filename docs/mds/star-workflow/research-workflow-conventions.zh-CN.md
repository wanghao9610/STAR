...
**优先级。** 本文件是**基线**。某个 skill 的 `SKILL.md` 可以更**严**——更窄的写入边界、更低的阈值、额外的门、乃至"本 skill 永不提交"——更严者生效。skill 绝不放松本文件设下的规则。若 `SKILL.md` 里带有下述某条规则的一行摘要，那一行是有约束力的提醒，本文件是完整规则。
...
| Skill | 提交时机 | stage 范围 |
| --- | --- | --- |
| `star-proj-adopt` | 每个阶段结束时提供一次 | 只暂存该阶段写过的路径；在被接入的仓库里，原本就未提交的改动只点名，绝不捆进提交 |
...
| `star-plan-reviser` | Step 7 有落笔修订时提供一次 | 目标计划；父计划的 `## Sub-plans` 行变化时，连父计划一起 |
...
| `star-code-release` | 每个落地阶段一次（gather / polish / readme） | 仅该阶段的路径：被提升的文件，以及这次移动破坏的调用点、打磨轮触碰的文件、`README.md` |
...
6. **绝不强制 add 被忽略的路径。** `.env`、`datas/`、`inits/` 默认被 git 忽略，不进历史。`wkdrs/` **并非**整体被忽略：其下除 `*.md` 外都被忽略，因此工作流自己的报告——执行日志、分析、digest、审查、结果台账——是有意让它们能进版本库的，一次 run 的记录可以活得比产生它的机器更久。`tasks/` 则整体被跟踪：计划自有的工具脚本本就是持久的，旁边的草稿也小到不值得单开一条规则。
...
7. **参与度档位：问多少，由用户定。** 工作流提出的每个问题只属于三类之一。**硬门**任何档位都要问：STOP 线上的一切（§2）、每一次提交提议（§1.5）、每一个把关删除或覆盖的问题、每一次写入用户确认过、且已由具名协议把关的内容（执行同步回写、逐项修订批准）、skill 在副作用之前设置的审批门、以及对用户本意的任何歧义（§5.2 是计划名的情形）。**裁量题**——第 3 条要求标出推荐项、且怎么选都安全的问题——是档位真正拨动的部分。**可推导的细节**——凡有约定默认值的——任何档位都静默决定；它们从来就不是问题。
   ...
   **这个记号不是参数。** `involve=<level>` 在解析任何其他内容之前就从调用参数里剥离——计划名（§5）、模式、范围、日期窗口都在其后。**每个** skill 都如此，包括 `SKILL.md` 里从未提到档位的那些：拿第一个参数去匹配 `metds/plans/*_plan.md` 的 skill，不得把 `involve=low` 当成计划名，也不得把它匹配成模式词。完全不接受参数的 skill 同样要剥离它。
   ...
   - `high`——skill 原文打包进一道门的、或在门与门之间自行决定的裁量题，逐条单独问出（第 2 条）。
...
9. **档位在单个 skill 里只收紧，绝不放松。** `SKILL.md` 可以声明某个裁量题它永远要问，也可以折叠对它没有意义的档位（辅导类 skill 没有真正的 `low`）。任何 skill 都不得把硬门当作可拨动的；未作声明的 skill 完全遵循上述规则。
10. **把线索接住。** 用户连答一长串问题时会丢掉线索——已经定下了什么，以及当前这个问题究竟取决于什么。三个便宜的习惯，且刻意不是"每个问题前重播一遍回顾"：那段回顾会随问题数一起变长，直到用户开始略读；而略读回顾，正是他们开始略读问题本身的起点。

    - **给问题挂上锚点。** 依赖前面答案的问题，用一个从句点名它——"里程碑轴 → 4 个单元；现在：哪一个承载数据叶子？"。一行，只带这道问题真正倚赖的决策，绝不是整段历史。
...
每个 skill 的持久产物，汇成一张表。`star-flow-status` 把它当作覆盖检查的契约：某个阶段“已覆盖”，指的是下表的产物存在、且其状态字段是最新的。这张表必须保持诚实——某个 skill 改了它写出来的东西，就要在同一个 commit 里更新对应行，否则状态 skill 会悄悄不再检查那个阶段。
...
**唯一的豁免口子。** 在 `backfill` 阶段，`star-proj-adopt` 可以往 `metds/plans/` 的叶子上写 `exec_status:` 与 `exec_runs:`——且仅此二者——每个叶子都经用户单独确认。这两个字段是「运行」那一行的状态，而接入正是这样一种情形：它们所描述的工作发生在任何计划存在之前，当时没有东西可以记录它。计划文件的其余一切，在接入的两个阶段里，都仍归「计划」那一行点名的生产者所有。

...
对其余 skill 的要求很小。checkout 停在某个不属于自己目标的 执行分支上时，要提交的 skill 先说明并提议切回去——在那里做下的无关提交会跟着那个叶子一起被合并。谁都不切一个还有东西在跑的 checkout：跑着的任务会中途重读被切换的文件。需要基础 checkout 在分支运行期间继续可用的用户，自己准备一个 `git worktree`、在里面调用 executor——skill 只作用于被调用时所在的 checkout，绝不创建或移除 worktree。

...
1. **怎么切分由主 agent 自己定，而且多数时候派出去更好。** 边界清楚、彼此独立、且委派确实带来实质收益的工作，交给子代理，而不是在主上下文里读完——把一大份阅读挡在主 agent 之外，是这类步骤的常规跑法，不是需要额外辩解的例外。真正不值得配子代理的是琐碎的顺序步骤：绝不给一条链上的每一步都配一个。**"实质收益"是可判定的**：输入很大、返回很小，且主 agent 事后要重读的是抽查，而不是同一份内容再读一遍。凡本文件或某个 `SKILL.md` 已经要求主 agent 重开同一份证据的地方——每个数字都要回到它标注的那一行复核、每个 blocker 进报告前都要重读——子代理只是把这次读取挪了个地方，并没有省掉它，这种活儿就该留在本地做。**宿主根本不提供委派能力时，本条就是 §6 的全部**：写着"派发"的步骤仍然欠着它那份返回格式，由主 agent 在本地按同样的顺序、照同样的格式把它填出来。**可能并行分派的步骤，要在那一步里把自己的规模参考值写成一个具体的数**——规模低于这个数，主 agent 自己读更省事。这个数由各 skill 自己定，因为它取决于读的是什么；主 agent 说得出理由，就可以往两边偏。写出一个数，是为了挡掉"树比较大"这种谁也没法拿去核对的说法。

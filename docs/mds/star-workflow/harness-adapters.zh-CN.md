...
本文件保留 STAR 各宿主的调用写法与模型路由实现，对应[通用规约](research-workflow-conventions.zh-CN.md) §10（skill 名册与路由）。skill 范围、授权、档位归属、运行在哪里执行和溯源要求仍由通用规约定义；本文件只说明各宿主如何应用这些要求。选择调用方式或派发参数前，读取当前宿主对应的条目，并以当前接口实际支持的能力为准。
...
Claude Code 从 `bash execs/configure.sh` 写入的 `effort:` frontmatter 取得深度，写入处有两类：各档的清单，以及名为 `star-plan`、`star-exec`、`star-read` 的三个受托者定义。经某份清单发起的运行按该清单所属档位的深度推理，以某个受托者身份派出的子代理按该受托者的深度推理；所以在这里「逐次派发指定深度」就是指定派发成哪个受托者。直接调用的运行因此已经带着名册档位的深度，但模式改到的另一档（通用规约 §10.8：`star-expt-analyst aggregate` 与 `watch`、`star-refs-reviewer synthesize`、`star-proj-adopt backfill`、`star-code-release check`）的深度不在清单里，Claude Code 只能在逐次派发时应用；`star-flow-status` 与 `star-expt-digest` 还按盖进清单的 READ 模型 fork。于是通用规约 §10.8（运行在哪里执行）要求的开头那一行说会话没在跑的档位模型，模式改了档位时再加上那一档的深度，这两者什么都不说。对阶段交接或 `star-auto` 的启动而言，Claude Code 配置的深度本身也是路由差异，即使档位模型是当前模型的别名也会派发；以普通子代理类型派出的受托者则仍继承派发运行的深度。只读派发——采集器或盲审——是例外：它仍用通用的 `Explore` 类型，只读靠它的工具集保证，因为这三个受托者定义不带工具限制；它只取该档的模型、不取深度，运行对此说明一次。

子代理不能再派子代理：以子代理身份启动的运行，要交接出去的阶段和内部受托者的工作都在本地执行，沿用它自己的模型与深度。
...
直接调用任何 skill 时，运行都留在主线程，沿用会话的模型与 effort。Codex 只在派发时应用深度，所以配置的 Codex 深度属于运行开头那一行要说明的内容（通用规约 §10.8，运行在哪里执行）；`star-auto` 以各档的模型与 effort 启动每次运行，`star-plan-coach` 与 `star-idea-storm` 也在内，并把它们的问题递回。
...
`bash execs/configure.sh --kimi-pool` 用来注册下文所说的深度变体，它要求 `.env` 配置的项目 Python 提供 `tomllib`（Python 3.11+）。它在写入前解析并校验完整修改；不支持的表布局会保持配置不变。

Kimi Code 同样没有逐派发的深度参数：其 `Agent` / `AgentSwarm` 的 `model` 只接受已配置 secondary-model 池中的别名。所以配置的 `@<深度>` 以池中把该模型绑在该深度的别名派发——约定名 `<模型>-<深度>`，即带该 `default_effort` 的 `[models]` 变体。这个变体由用户在 `~/.kimi-code/config.toml` 注册并列入池，手工注册一次，或用 `bash execs/configure.sh --kimi-pool` 注册；skill 运行本身绝不写这个文件。因此配置的 Kimi 深度即使档位模型与当前模型相同也构成路由差异。池中没有绑该深度的条目时只传模型，并一次说明深度未能生效。段级的 `[secondary_model].default_effort` 会压过每个变体的绑定。

### DSH 与 Pi

读取各自的 skill 清单与当前委派接口，只应用实际暴露的模型和深度控制；本说明不为它们补设深度参数。

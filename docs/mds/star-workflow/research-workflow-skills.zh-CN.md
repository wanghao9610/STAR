...
STAR 提供十五个相互衔接的研究工作流 skill，用于把一个模糊的研究兴趣逐步变成有文献扫描背书的研究选题、可追踪的计划、有可核验文献库的相关工作基础、有架构记录的代码库、经过验证的运行环境、可执行的任务、有验证记录的实现、对照规范审计过的代码、对照预期审计过的实验结果、按时间轴沉淀的阶段进展、能吸收执行结果的计划、从这些计划反向编译出的方法文档，以及最后一份陌生人可以直接 clone 的仓库：
...
模糊的研究兴趣
  ...
  → star-code-architect：为计划搭建代码库并沉淀架构规范
...
$star-proj-adopt survey       # 勘察仓库并落地例行设置
...
3. 落地例行设置：由 `.env.example` 生成 `.env`，在 `datas/` / `inits/` / `wkdrs/` 处建软链*接通*已有目录树而不搬迁它们，并为每个启动入口生成一个原样调用项目已有命令的 `execs/scpts/<name>.sh`；
...
1. 从方法中提取检索画像——任务、机制、设定、写明的数据集与 baseline、想立的主张——并在检索前先报出来；
...
1. 从计划提炼检索画像：任务领域、方法关键词、写明的 baseline、框架约束；
...
当循环已经收口——所有叶子执行完、所有总体计划定稿——且计划已经吸收了执行教给它的东西之后：

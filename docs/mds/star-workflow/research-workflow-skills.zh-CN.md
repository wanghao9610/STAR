...
四个根的每个 skill 目录结构相同：英文入口位于 `SKILL.md`，中文完整定义位于 `SKILL_zh.md`。中文对话触发 skill 后，入口会先强制完整读取同目录的 `SKILL_zh.md`，再按需加载其他 `*_zh.md` 资源；若中英文定义冲突，以入口 `SKILL.md` 为准。本指南沿用 §1 的 Codex 示例，其中“完整定义”链接均指向 `.agents/` 下的中文文件。

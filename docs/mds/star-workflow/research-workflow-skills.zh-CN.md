...
3. 当计划要修改既有代码时，推荐一条执行分支（`exec/<run>`，[规约 §11](research-workflow-conventions.zh-CN.md)），随计划一并审批——在通过审查的改动合并回来之前，基础分支始终保有能用的代码；
4. 只做与当前步骤直接相关的修改；
5. 运行最窄的轻量验证并把证据写入日志；
...
当这次 run 在执行分支上进行时，上面这些——代码、运行记录、计划自己的状态——在叶子完成并通过审查之前都只住在 `exec/<run>` 上；此后再次调用同一计划，就抵达合并确认点：默认向基础分支做 squash 合并，冲突按文件名交回给你，分支删不删另外问。改选弃用，则先把 `wkdrs/<run>/*.md` 拷回基础分支，负结果因此留住证据；`$star-flow-status` 会列出未合并的 `exec/*` 分支，这一切都不会在暗处等。

...
STAR 提供十一个相互衔接的研究工作流 skill，用于把一个研究想法逐步变成可追踪的计划、有可核验文献库的相关工作基座、有架构记录的代码库、经过验证的运行环境、可执行的任务、有验证记录的实现、对照规范审计过的代码、对照预期审计过的实验结果、能吸收执行结果的计划，以及从这些计划反向编译出的方法文档：
...
## 5. `$star-plan-decomposer`：拆解执行子计划
...
## 6. `$star-code-architect`：奠基或整理代码库
...
## 10. `$star-expt-analyst`：分析一个 run 的结果
...
```text
$star-expt-analyst 00                             # 该计划的 run，经其 exec_run 解析
...
计划参数支持常见的 slug / 数字前缀 / 文件名三种写法；`wkdrs/<run>/` 路径会反查回它的计划。

### 它做什么

1. 解析 run 并载入预期：子计划的 §4 交付物与 §5 完成判据、父计划的 §4 指标与 §5 kill-criteria，以及该 run 的 `EXEC_PLAN.md` / `EXEC_LOG.md`；
...
5. 对照计划 `traces_to` 的那条主张解读结果：父计划 kill-criteria、接受一个可疑强结果之前先跑的泄漏检查，以及这个 run 的局限（seed 数、split 规模、它没能显示什么）；
...
- 命中父计划 kill-criterion 的负结果，是这个 skill 能给出的最有价值的东西：趁证据新鲜，把它路由给 `$star-plan-reviser`。

完整定义见 [`star-expt-analyst/SKILL.md`](../../../.agents/skills/star-expt-analyst/SKILL.md)。

## 11. `$star-plan-reviser`：审查并修订一个计划
...
## 12. `$star-metd-summarize`：把计划编译成方法文档
...
完整定义见 [`star-plan-status/SKILL.md`](../../../.agents/skills/star-plan-status/SKILL.md)。
...
方法章节到位后，`$star-refs-reviewer open-vocab-det-seg` 会把最接近的工作精读进 `metds/refs/`，并建立定位与 baseline 都要依赖的那份可核验 `reference.bib`。
...
经过门 1（从打分候选中选定参考库）和门 2（批准迁移表）后，`${CODE_NAME}/` 中就有了已重命名、出处可追溯的代码库，`metds/codearc.md` 记录着后续每一步都要遵循的架构。
...
根据唯一的下一步建议继续执行，直到所有叶子完成。每完成一个叶子，先运行 `$star-code-reviewer <叶子>` 审一遍新代码再继续叠加。跑完 STOP 线命令、结果落到磁盘之后，运行 `$star-expt-analyst <叶子>` 对照计划的完成判据给结果打分，弄清它们意味着什么。之后——或结果否定了某个关键假设时——对受影响的计划运行 `$star-plan-reviser`，把证据回灌进计划文本；结构性变化它会转给 `$star-plan-decomposer`，战略转向则转给 `$star-plan-coach`。
...
| 当前情况 | 使用 |
| --- | --- |
| 只有 idea，研究问题还没说清 | `$star-plan-coach` |
| 方法已经清楚，但还不知道最接近的工作、baseline，以及怎么引用它们 | `$star-refs-reviewer` |
| 已有战略计划，需要拆成任务 | `$star-plan-decomposer` |
...
可以，但应保持 frontmatter 和正文一致，尤其是 `parent`、`children`、`depends_on`、`status`、`exec_status` 和 `exec_run`。修改父计划后，先运行 `$star-plan-status` 检查 drift，再决定是否重新拆解。

...
放弃掉的方向是被标记，不是被删除。reviser 在那个节点上写入 `dropped: <日期> — <一句话原因>`，在父计划的索引行上标 `— dropped <日期>`，并把是什么终结了这个方向记进 `## Revision History`。所有 skill 都按整棵子树继承来读这个字段，所以一行就把这个节点连同其下的一切移出计数、覆盖检查与下一步动作：`$star-flow-status` 把它们渲染成 `⊗` 并排除在三个数之外，`$star-plan-executor` 拒绝执行它们，`$star-plan-decomposer` 拒绝拆解它们，`$star-metd-summarize` 不从它们编译任何内容。文件、它们的 run 与结果都原地保留——失败的方向也是证据——父计划也保留链接，所以「这条路试过」从父计划那里仍然读得到。取消丢弃就是清掉这个字段。有两件事丢弃并不负责收尾：仍然依赖着被丢弃节点的叶子（这条依赖永远满足不了，状态报告会标出来交给你重画），以及它下面未合并的执行分支或现存的 worktree——它们还在磁盘上，也照旧被标出来。

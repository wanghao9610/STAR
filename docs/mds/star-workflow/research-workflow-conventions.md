...
| the size limit | the output budget of the active tool; inspect truncation and fetch omitted content when needed | the active tool interface |
...
13. **A drafted list is one question, not one question per item.** Where a run produces a numbered list the user must accept or reject — revision candidates, fix findings, pending amendments — the whole list goes in the text of the message that carries the question, above the call: one row per item, with what it changes, from what to what, on what evidence, and the recommended action. Then **one** question over that list, four options:
    ...
    **Separate materially different decisions.** A new cost commitment, destructive change or unresolved research choice must be visible with its own consequences. Previously authorized operations need no repeated question. Do not hide a new authority request inside a routine list.
...
8. **Where a run executes, and on which model.** Three `.env` keys name one model each: `STAR_PLAN_MODEL` for research judgment — plans, plan reviews, analyses and independent blind reviews — `STAR_EXEC_MODEL` for implementation and production, and `STAR_READ_MODEL` for read-only scans, collectors and summaries. The tier column above says which of the three a skill's run belongs to, and all three are read in the same one-line `.env` lookup that already gets `STAR_LANG` and `INVOLVE` (§7.6), never a call of their own. **An empty key changes nothing about how a run behaves**: it stays on whatever model the session or the harness already gave it, and every other rule in this file reads exactly as it does with the keys unset.
   ...
   **One key can name one model per harness.** A value is either a single model name — used by whichever harness reads the key — or comma-separated `<harness>:<model>` entries, tagged with the tokens `STAR_HARNESSES` uses: `claude`, `codex`, `cursor`, `dsh`, `kimi`, `pi`, `qwen`. A run takes the entry tagged with the tree it is running from, falls back to an untagged entry, and reads the key as empty when it has neither; **a tag no tree answers to is ignored**, which is where a harness added later fits. The first colon in an entry ends its tag, so a model name carrying one of its own is written tagged, and each name is spelled the way its own harness spells one — `pi:anthropic/claude-fable-5`, `claude:fable`, `claude:claude-opus-5[1m]`. "The tier value", here and in every manifest, means what this resolves to for the harness reading it, never the raw string the lookup returned. **An entry may end in `@<depth>`** — `low`, `medium`, `high`, `xhigh`, `max`, or a positive integer — the thinking depth the runs of that tier reason at wherever the harness can set one, which is what lets two tiers name one model and differ only in depth. Only a suffix spelling one of those depths is read as one, so a model name carrying an `@` of its own reaches its harness whole, and a harness with no depth control reads the entry as the model alone. Claude Code takes the depth from the `effort:` frontmatter that `bash execs/update.sh --models` writes into each tier's manifests and into its named `star-plan`, `star-exec` and `star-read` agents: a run invoked through a manifest reasons at that manifest's tier depth, and a delegate dispatched as one of those agents reasons at that agent's, so naming the agent is how a depth is named per dispatch there — a configured Claude Code depth is a routing difference even when the tier model aliases the active model, and a delegate dispatched as a plain subagent type still inherits the dispatching run's depth. Codex needs no file stamp: whenever its current `spawn_agent` interface accepts the suffix for the selected model, every tier dispatch passes it explicitly as `reasoning_effort`; a configured Codex depth is therefore a routing difference even when the tier model aliases the active model, and a phase that changes tier can change both. Cursor has no separate `effort:` field and no per-call depth on `Task`: `bash execs/update.sh --models` writes a configured `@<depth>` onto that tier's named agent's `model:` as `id[effort=<depth>]`, unquoted — Cursor's loader treats quote characters as part of the name — so `cursor:grok-4.6@xhigh` beside `cursor:grok-4.6@high` stamps two agents on one model at two depths; a configured Cursor depth is therefore a routing difference even when the tier model aliases the active model. Qwen's named agents take the model without the suffix. If the interface or model rejects the configured depth, do not claim it was applied: keep the available execution route and state the limitation once.

   **The mode overrides the roster tier for that run**, because what a skill does in one mode is not what it does in another: `star-expt-analyst aggregate` and `star-expt-analyst watch` are READ, `star-refs-reviewer synthesize` is PLAN, `star-proj-adopt backfill` is PLAN, and `star-code-release check` is READ. **Two runs change tier partway through.** `star-plan-executor` runs its execute-and-verify phase on EXEC while the rest of the run stays PLAN, and `star-code-architect` runs its migration execution on EXEC while the design work stays PLAN. That is the rule; how each one hands the phase over, and what the delegate may write, belongs to that skill's own harness-owned manifest — and neither dispatches when the run is already on an alias of that model unless the harness can apply that phase's configured depth per dispatch.

   **A run started in the main session hands itself to one delegate on its tier's model and optional depth** when all four of these hold: the key for its tier is non-empty; either that value is not an alias of the model this session is already running, or it carries a depth the harness can apply per dispatch — an alias being the family name inside the id, `opus` for `claude-opus-5[1m]`, or the id itself, a context-window suffix aside, and the session's model being what the resolver command in the session context's provenance line prints when run once (§8), or failing that the id that line states, the run staying where nothing names it and no usable depth itself requires a dispatch; this run is not itself a delegate already carrying a `tier=` token, so the hand-over happens once and never recurses; and no question that must reach the user is left in the run — a confirmation point (§7.7) at any level, a judgment call at a level that asks it — judged at the start from the mode, the level and the files on disk: a point that only what the run finds could raise counts as left, a STOP-line hand-back is a return rather than a question, and a judgment call the level takes unasked is none; the condition exists because a delegate cannot put a question to the user. Otherwise it runs where it started, saying why in one line when the key is set and nothing when it is empty. The decision is the paragraph a manifest carries at the head of its Workflow, before its first step — every manifest but the two coaching ones and the two READ-tier ones a harness may fork on their own manifest's model, `star-flow-status` and `star-expt-digest`: a forked run is already on that model, and a fork cannot dispatch. `star-plan-coach` and `star-idea-storm` never self-relocate: their coaching series puts a question to the user at every stage, so the fourth condition is never true for them; a direct Codex invocation therefore keeps the main thread's effort, while `star-auto` can start them at the configured PLAN effort and relay questions back. Like `involve=` (§7.7), **`tier=<name>` is stripped from the invocation before anything else is resolved** — the plan name (§5), the mode, the scope, the date window — in every skill, whether its own manifest mentions the token or not.

   **What a relocating run hands its delegate is fixed**: read that skill's harness-owned manifest in full and follow it, the invocation text as it arrived, `involve=<level> tier=<name>`, the dialogue language in one line where `STAR_LANG` is empty (§7.6), and, where the run holds one, its `auto=unattended` grant. It waits for the delegate and relays the reply unchanged; the files the delegate wrote are this run's artifacts, and the provenance recorded in them (§8) is the delegate's model, not the session's. Inside a run, delegate routing follows the work rather than the skill: a read-only collector runs on READ, an implementing delegate (§6.5) on EXEC, and §6.7's independent blind read on PLAN, a second reading of finished work being research judgment. Pass both that tier's model and its supported per-dispatch depth when the harness exposes both; pass only the control it exposes, and never turn an unsupported depth into a model-name suffix.

   **Item 7 applies the same rule from outside.** A `star-auto` loop reads the session's model once before its loop, the same way, and starts each run on its tier's model and supported per-dispatch depth directly, so a run it started is already where it belongs and never relocates itself. It reads the fourth condition its own way: a run it started that meets a question it cannot answer returns it, and the loop triages it as item 7 says. Every start and every fan-out records its tier, the model it got, and the requested depth or `default` in the decisions record (§7.8), beside what item 5 already has it announce.
...
Other skills work in the run's recorded `worktree:` when one exists. Before committing from another run's execution branch, preserve unrelated changes and select a non-conflicting route under the existing authorization; ask only if ownership or destination remains unclear. Never switch a checkout that a live job may re-read. The executor owns creation and cleanup under the conditions above; a user-prepared worktree is supported without asking to create it again.

...
| done-criterion | a leaf's §5: the binary test that decides whether its run succeeded | `star-plan-decomposer`, §5 of each leaf |
...
1. **The chat reply is the digest.** Detail belongs in the files the run wrote; the reply is kept short by choosing what to include (item 5 gives the order), never by compressing the writing into fragments. A reply whose length is set by what it must enumerate — `star-flow-status`'s tree, or a §7.13 drafted list — is bounded by shape, stated by that skill (one line per node or row), never unbounded.
...
10. **Carry the thread.** A user answering a long series of questions loses the thread. Three cheap habits, and deliberately not a recap before every question — that grows until the user skims it.
    ...
    - **Recap at boundaries, not between questions.** At each stage, step, or section end — where the user is already pausing — restate, in a few sentences, what was decided, what it produced (the file written, the field set), and what it opens next.
...
8. **Where a run executes, and on which model.** Three `.env` keys name one model each: `STAR_PLAN_MODEL` for research judgment — plans, plan reviews, analyses and independent blind reviews — `STAR_EXEC_MODEL` for implementation and production, and `STAR_READ_MODEL` for read-only scans, collectors and summaries. The tier column above says which of the three a skill's run belongs to, and all three are read in the same one-line `.env` lookup that already gets `STAR_LANG` and `INVOLVE` (§7.6), never a call of their own. **An empty key changes nothing about how a run behaves**: it stays on whatever model the session or the harness already gave it, and every other rule in this file reads exactly as it does with the keys unset.
   ...
   **A run stays in the session that started it.** A directly invoked skill runs on the session's model and never hands the whole run to a delegate. Its manifest names its tier on a **Tier:** line at the head of its Workflow, with any mode or phase that takes another (above). When that tier's key is non-empty, the first line of the run's final reply gives, in plain words, the tier, that key's model and depth, and the two ways to get them: switch the session's model, or start the run through `star-auto` (item 7); the run itself stays where it is. The line is unconditional: the run does not compare the key with the session's model, so it needs no model id to give it. It is left out where the key is empty, where the run carries `tier=` (item 7 or a phase hand-off started it on its tier), and where the harness started the run on its tier by forking its manifest (§13). Where the harness gives a delegate no delegation of its own, or the delegate already sits at the harness's nesting limit (§13), a run item 7 starts as a delegate runs its inner delegates locally, and `star-plan-executor` and `star-code-architect` their phase hand-off too. Like `involve=` (§7.7), **`tier=<name>` is stripped from the invocation before anything else is resolved** — the plan name (§5), the mode, the scope, the date window — in every skill, whether its own manifest mentions the token or not.
...
The depth is the `effort:` frontmatter `configure.sh` writes into each tier's manifests and into the named `star-plan`, `star-exec` and `star-read` agents: naming the agent names the depth per dispatch, and a plain subagent type inherits the dispatching run's depth. A skill manifest's own `model:` and `effort:` reach the run only when the user types its command or the manifest forks; a skill started through the Skill tool without forking — one the agent selected (§10.2) or picked up (§10.5) — runs on the session's model and depth. A command the user typed therefore has its roster tier's depth and lacks only that of a tier its mode switches to, which is applied per dispatch alone; a selected run lacks its tier's depth as well. `star-flow-status` and `star-expt-digest` also fork on the READ model `configure.sh` stamps into their manifests, so those two give no opening line (§10.8). For a phase hand-off or a `star-auto` start, a configured depth is a routing difference even when the tier model aliases the active model. A read-only dispatch — a collector or a blind read — keeps the generic `Explore` type, whose tool set is what keeps it read-only, since the named agents carry no tool restriction; it takes the tier's model but not its depth, and the run says so once. A subagent can dispatch subagents of its own, to three levels below the session by default (`CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH` changes it), and a named agent's `model:` and `effort:` and a per-call `model` apply at every level. STAR's deepest chain reaches that limit — `star-auto`, the `star-plan` run it starts, that run's `star-exec` phase, the phase's own workers — and a third-level subagent has no `Agent` tool: a brief handed to one asks for no dispatch, and a run that finds none works locally (§10.8).

#### Codex

No file stamp: whenever the current `spawn_agent` interface accepts the suffix for the selected model, every tier dispatch passes it explicitly as `reasoning_effort`, so a configured depth is a routing difference even when the tier model aliases the active model, and a phase that changes tier can change both. A direct invocation stays in the main thread on the session's model and effort; `star-auto` starts each run at its tier's model and effort, `star-plan-coach` and `star-idea-storm` included, and relays their questions back.

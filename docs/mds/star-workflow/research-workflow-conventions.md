...
9. **A request budget belongs to the host, not to the agent.** The polite rate a skill promises a remote host — `source_policy.md` and `scan_policy.md` each set one — is spent by the whole session against that host, so N concurrent fetchers multiply the real rate by N. A step that fans out fetching either divides the budget by the number of delegates and states each share as a number in the brief, or fetches serially and says so. Two skills in one session reach the same hosts: a run following `star-idea-storm` has already spent part of the budget it is about to use. The numbers live in the policy files; steps cite them rather than restating them, because a restated number is one that drifts.

...
Fifteen skills, invoked as `/star-<name>` in Claude Code, Cursor and Qwen Code, `$star-<name>` in Codex, `/skill:star-<name>` in Kimi Code and Pi. What each one does in full is [research-workflow-skills.md](research-workflow-skills.md); what each one writes is §8.

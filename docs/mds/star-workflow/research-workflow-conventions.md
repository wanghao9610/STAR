...
| execution branch | the `exec/<run>` branch a leaf's changes live on until the merge confirmation point clears them into the base branch | §11 |
...
What this asks of everyone else is small. A skill about to commit while the checkout sits on an `exec/*` branch that is not its own target says so and offers to switch back first — an unrelated commit made there rides into that leaf's merge. Nobody switches a checkout something is still running from: a live job re-reads its files mid-run. And a user who needs the base checkout live while a branch run proceeds prepares a `git worktree` themselves and invokes the executor inside it — skills act on the checkout they are invoked in, and never create or remove worktrees.

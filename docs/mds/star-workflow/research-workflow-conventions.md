...
Chat replies and workflow prose state the substantive point without praise, staged introductions, service offers, inflated significance, vague attribution, or generic positive endings. They follow the [human-writing guide](human-writing-guide.md): use an author-confirmed sample where one exists, review patterns in clusters rather than banning isolated words or constructions, and rewrite at paragraph scale when needed. A style edit never changes a fact, number, threshold, date, quotation, citation, source URL, path, command, flag, environment key, literal field, status, decision, open item, done-criterion, technical distinction, uncertainty boundary, negative result, or attribution. It never adds personality or concrete detail without authorial and evidential support.
...
| What earlier sessions learned, owned by no other file | `.star/memory/`; the git-ignored `.star/memory/local/` for `machine:` scoped facts and anything kept off the repository (`memory_spec.md`) |
...
8. **Where a run executes, and on which model.** Three `.env` keys name one model each: `STAR_PLAN_MODEL` for research judgment — plans, plan reviews, analyses and independent blind reviews — `STAR_EXEC_MODEL` for implementation and production, and `STAR_READ_MODEL` for read-only scans, collectors and summaries. The tier column above says which of the three a skill's run belongs to, and all three are read in the same one-line `.env` lookup that already gets `STAR_LANG` and `INVOLVE` (§7.6), never a call of their own. **An empty key changes nothing about how a run behaves**: it stays on whatever model the session or the harness already gave it, and every other rule in this file reads exactly as it does with the keys unset.
   ...
   **A run started in the main session hands itself to one delegate on its tier's model and optional depth** — the relocation rule — when all four of these hold. (1) The key for its tier is non-empty. (2) That value is not an alias of the model this session is already running, or it carries a depth the harness can apply per dispatch. An alias is the family name inside the id, or the id itself, a context-window suffix aside; the session's model is what the resolver command in the session context's provenance line prints when run once (§8), or failing that the id that line states. Where nothing names the session's model and no usable depth itself requires a dispatch, the run stays. (3) This run is not itself a delegate already carrying a `tier=` token, so the hand-over happens once and never recurses. (4) No question that must reach the user is left in the run: a confirmation point (§7.7) at any level, or a judgment call at a level that asks it. Judge this at the start from the mode, the level and the files on disk: a point that only what the run finds could raise counts as left, a STOP-line hand-back is a return rather than a question, and a judgment call the level takes unasked is none. The condition exists because a delegate cannot put a question to the user. Otherwise it runs where it started, saying why in one line when the key is set and nothing when it is empty. Where the harness gives a delegate no delegation of its own ([harness adapters](harness-adapters.md)), a run that hands one of its phases to another tier — `star-plan-executor`, `star-code-architect` — stays in the main session and hands over only that phase; any other run that relocates, and either of these two when item 7 starts it as a delegate all the same, runs its phase hand-off and inner delegates locally. The decision is the paragraph a manifest carries at the head of its Workflow, before its first step — every manifest but the two coaching ones and the two READ-tier ones a harness may fork on their own manifest's model, `star-flow-status` and `star-expt-digest`: a forked run is already on that model, and a fork cannot dispatch; where a harness does not fork them, their own READ-tier entry makes the same hand-off under the same conditions. `star-plan-coach` and `star-idea-storm` never self-relocate: their coaching series puts a question to the user at every stage, so the fourth condition is never true for them; a direct invocation therefore keeps the current session settings, while `star-auto` can start them at the configured PLAN model and supported depth and relay questions back. Like `involve=` (§7.7), **`tier=<name>` is stripped from the invocation before anything else is resolved** — the plan name (§5), the mode, the scope, the date window — in every skill, whether its own manifest mentions the token or not.

   **What a relocating run hands its delegate is fixed**: read that skill's harness-owned manifest in full and follow it; the invocation text as it arrived; the target, mode and scope this run already resolved, stated as settled; whether the user typed the invocation or the run was picked up (§10.5); any authorization or answer from this conversation that the fourth condition counted; `involve=<level> tier=<name>`; the dialogue language in one line where `STAR_LANG` is empty (§7.6); and, where the run holds one, its `auto=unattended` grant. It waits for the delegate and relays the reply unchanged; the files the delegate wrote are this run's artifacts, and the provenance recorded in them (§8) is the delegate's model, not the session's. Inside a run, delegate routing follows the work rather than the skill: a read-only collector runs on READ, an implementing delegate (§6.5) on EXEC, and §6.7's independent blind read on PLAN, a second reading of finished work being research judgment. Pass both that tier's model and its supported per-dispatch depth when the harness exposes both; pass only the control it exposes, and never turn an unsupported depth into a model-name suffix.

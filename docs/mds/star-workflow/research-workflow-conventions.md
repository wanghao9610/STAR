...
| `star-plan-executor` | one per verified action, only when the confirmation point approved checkpointing | the files that action touched |
...
5. **Never commit silently.** Every commit is either covered by a confirmation point the user approved or offered as its own question. Declining is always a valid answer.

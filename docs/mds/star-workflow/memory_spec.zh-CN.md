...
一次会话把学到的东西记在哪里，又怎么传到下一次。**要不要记**的规则：先提议、绝不擅自决定——每个会话最多提议两次，用户同意后才写；`INVOLVE=low` 时不经询问就记，但要说明；项目文件已经认领的事实写进那份文件（`AGENTS.md` 有 §10 时，那里是同一条规则）。这份文件定的是两边共用的格式：记忆库放什么、单条记忆什么样、钩子解析什么。

## 什么该记在这里

只有一条判据，而且排他：**只有当项目里没有任何文件已经认领这条事实时，它才属于记忆。** 结果属于那次运行的 `EXEC_LOG.md`，研究本身的决定属于它的计划，论文属于 `metds/refs/`，代码放置规则属于 `metds/codearc.md`。记忆是残余——跨运行仍成立、又没有文件认领的那部分。没有这条判据，记忆库就成了计划已回答过的问题的第二个答案，两边随后各走各的。

| 类型 | 装什么 | 例子 |
|---|---|---|
| `env` | 关于某台机器、集群或工具链的事实，通常是踩坑换来的 | 这个集群要先 `module load gcc/11` 才编得出 flash-attn |
| `pref` | 用户对"活怎么干"的长期偏好 | 直接提交到 `main`，不开特性分支 |
...
`.star/memory/` 本身入库，所以写在那里的记忆活得比记下它的机器更久，也跟着 clone 一起走。`local/` 像 `.env` 一样被忽略，装留在本机的部分：`machine:` 作用域的事实——在这里成立、换台机器就不成立的路径、模块名或驱动怪癖——以及用户不想入库的任何记忆，无论作用域。其余作用域在任何 clone 上都成立，进入库的那层；分的是事实往哪里走，不是它在哪里成立。
...
正文第一句把事实说完，之后只写读者据此行动所需。记忆不带 `model_trail`：它只记一条事实，复核就地改写 `verified` 和 `model_id`，而不往历史里追加。约定 §8 要求轨迹，针对的是多次会话各写同一产物一部分的情形——这么小的文件不会那样被写。

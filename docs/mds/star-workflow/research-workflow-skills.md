...
6. Runs a fix pass of mechanical, behavior-preserving fixes only (docstrings, scope-internal renames, unused imports, project-introduced dead code): authorized `minor` and `nit` fixes are applied and named as they go in; unresolved `blocker` and `major` fixes are shown as one visible list and settled in one question. A deletion needs specific authority, but the same deletion already explicitly approved is not asked twice. Each applied fix is re-verified.

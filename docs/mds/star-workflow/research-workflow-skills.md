...
One command stands outside the roster: `star-auto <goal> [stop=<stop line>] [involve=<level>]` drives the workflow toward a stated goal — status first, then the next action each run names, the slash-only seven included, because typing the command is the researcher's decision, made once for the pursuit. Each started run behaves exactly as if you had named it, and prepared heavy commands launch by default, the goal owning the run. `stop=` draws, in your own words to the end of the invocation, the line the run must not cross — say `stop=any single training over 4 hours, or any paid API call` — and a command that crosses it, or cannot be judged against it, is printed and the run stops there. Mandatory confirmation points still ask, and deletions and overwrites never auto-run. With no `involve=` token and no `INVOLVE` in `.env`, an auto run resolves to `low`. Full rule: [conventions §10.7](research-workflow-conventions.md); the procedure: [`.agents/commands/star-auto.md`](../../../.agents/commands/star-auto.md).
...
Two different questions; this section answers only the second. **Who may start a skill** is the roster ([conventions §10](research-workflow-conventions.md)): seven are slash-only, the agent may pick up the other eight — including one a finished run names as the next action — and a `star-auto <goal>` invocation you typed extends that pickup to all fifteen for its pursuit (§1, Invoking the skills). **How far a run then gets before it needs you** follows, and is the same either way: a skill the agent started stops exactly where one you typed would — under a `star-auto` grant the skill still stops there, and it is the auto loop that then launches the command it prepared, unless your `stop=` line halts it. Confirmation points do not relax in headless or scripted runs — a skill that reaches a question waits rather than assuming a yes. In practice:
...
The involve level (conventions §7.7–7.8) moves these boundaries, never past a confirmation point. With `INVOLVE=low` in `.env` — or `involve=low` on one invocation — a skill stops asking its judgment calls: it takes the option it would have marked recommended and logs the choice. Runs stretch further before needing you: `star-plan-decomposer`'s axis and sub-plan-list confirmations go quiet, and `star-plan-executor`, pointed at a parent, starts the first ready leaf itself and takes its pre-execution plan approval unasked, staying out of plan mode — the one confirmation point the level moves (conventions §7.7), its divergence sync included. Outside a `star-auto` grant (§1, Invoking the skills), what never goes quiet: the STOP line, deletions and overwrites, writing amendments back into plans, the other confirmation points, genuinely open questions — `low` lengthens the unattended span, it does not make any skill fully unattended. The commit offer is no longer among them: at `low` a run commits what it wrote and names those commits in its reply (§1.5). `high` runs the other way: judgment calls a skill would batch into one confirmation point, or take on its own between them, are asked one at a time.

...
Idea framing, plan sections, literature notes and synthesis, experiment digests, method documents, and release README prose all apply the shared [human-writing contract](research-workflow-conventions.md#human-writing-contract) at the point where narrative text is drafted. It removes formulaic patterns at paragraph scale while preserving the evidence record: facts, citations, numbers, dates, paths, commands, status values, uncertainty, and negative results do not move with the style pass.
...
Seven skills — `star-proj-adopt`, `star-idea-storm`, `star-plan-coach`, `star-code-architect`, `star-plan-decomposer`, `star-plan-reviser`, `star-code-release` — are slash-only: they run only when named explicitly, because each sits on a decision that belongs to the researcher ([conventions §10](research-workflow-conventions.md)). Each harness enforces it, rather than convention — `disable-model-invocation: true` in the Claude, Cursor, DSH, Kimi, Pi and Qwen Code manifests, `allow_implicit_invocation: false` in Codex's `.agents/skills/<name>/agents/openai.yaml`. Describing one in prose (“Break this research plan into executable sub-plans.”) will not start it: the agent answers from general knowledge, producing plan-shaped files with none of the `parent:` / `children:` / `traces_to` frontmatter the rest of the workflow reads. The agent may pick the other eight up when the task plainly matches and the target is unambiguous — naming one explicitly still works, and is how you say which you meant. Selection stays within what you asked: a status, explanation, or read-only review ends with its result and a recommendation, not an automatic writing successor. A successor runs in the same turn only when it advances an already authorized execution goal.

One command stands outside the roster: `star-auto <goal> [stop=<stop line>] [involve=<level>]` drives the workflow toward a stated goal — status first, then the next action each run names, the slash-only seven included, because typing the command is the researcher's decision, made once for the pursuit. Each started run follows its normal workflow, while auto supplies the standing execution authority described here; prepared heavy commands within the goal and `stop=` boundary launch by default. `stop=` draws, in your own words to the end of the invocation, the line the run must not cross — say `stop=any single training over 4 hours, or any paid API call` — and a command that crosses it, or cannot be judged against it, is printed and the run stops there. Unresolved research decisions and new authority still come back to you. Destructive operations and overwrites remain outside the grant except, at `low`, for the guarded removal of an empty worktree after a clean reviewed merge. With no `involve=` token and no `INVOLVE` in `.env`, an auto run resolves to `low`. Full rule: [conventions §10.7](research-workflow-conventions.md); the procedure: [`.agents/commands/star-auto.md`](../../../.agents/commands/star-auto.md).
...
Any invocation also accepts an optional `involve=low|medium|high` token setting how much this run asks before deciding — e.g. `star-plan-executor 00 involve=low`. `low` takes the recommended safe option on unresolved judgment calls (and logs each), while `high` asks them step by step; it overrides `INVOLVE` in `.env` for that run, and telling a running skill "ask me less" changes it mid-run. The level does not revoke approval you already gave or create authority you did not give. A destructive change, new cost commitment, ambiguous target, or change to the research objective or acceptance criteria still needs specific authority, but the same approved operation and scope are not asked twice. Full rule: [conventions §7.7](research-workflow-conventions.md).

Every skill also takes optional free text after the argument, in your own words, carrying this run's intent, constraints, and any explicit authorization: `star-plan-reviser 01 this one is finished, 02 replaces it`. A clear request to perform a specified operation can satisfy that operation's confirmation; background or a vague preference cannot. Free text never silently widens the selected target or mode, settles a materially ambiguous plan name, or chooses an unresolved research decision. `star-auto` parses `stop=` separately, so that boundary remains binding. Where a skill's first argument is already free text — `star-idea-storm`, `star-plan-coach`, `star-refs-reviewer` — the same reading applies. Full rule: [conventions §7.12](research-workflow-conventions.md).

Which model a run uses is set in `.env`, in three tiers: `STAR_PLAN_MODEL` for research judgment — plans, plan reviews, analyses, and the blind reads that check them — `STAR_EXEC_MODEL` for implementation and production, and `STAR_READ_MODEL` for read-only scans, collectors and summaries. The roster in [conventions §10](research-workflow-conventions.md) carries each skill's tier; a few modes take a different one, and two runs change tier partway through, each named in that skill's section below. A skill you type runs in your session on the session's model: typing it does not switch models. When its tier's key is set, the first line of the run's final reply says so — the tier, that model and depth, and the two ways to get them: switch the session's model, or start the run through `star-auto` — and then runs where it is; it does not check whether the session already runs that model, and a skill the harness forks onto its tier gives no line. Each manifest names its tier on the **Tier:** line at the head of its Workflow. `star-auto` starts each run on its tier's model directly, and `star-plan-executor` and `star-code-architect` hand their execution phase to a delegate on EXEC, as their sections below describe; any other delegate takes the tier of its work — a collector READ, an implementer EXEC, a blind read PLAN. The files stay the run's, and the provenance recorded in them is the model that wrote them. The three keys ship empty, and an empty key changes nothing and is not mentioned. An entry may also carry a thinking depth after its model — `claude:opus@high` or `codex:gpt-6-astra@high` — so two tiers can name one model and differ only in how deeply their runs reason. How each harness applies a depth: [conventions §13](research-workflow-conventions.md#model-and-thinking-depth). Full rule: [conventions §10.8](research-workflow-conventions.md).
...
3. Puts the mechanical setup in place: `.env` from `.env.example`, symlinks at `datas/` / `inits/` that *reach* the existing trees instead of relocating them (`wkdrs/` stays a real directory so run records remain versionable), one `execs/scpts/<name>.sh` per entrypoint calling the project's existing command unchanged;
...
- Run it once after the root plan is finalized and before decomposition, so the leaves can name real modules; re-run it to record new placement rules or execute the next round of approved migrations.
...
With no argument, the environment name is `.env`'s `ENV_NAME`, else its `CODE_NAME`. A valid `CONDA_HOME` there selects the conda backend; otherwise the skill creates `.venv` in the project root (the name argument then does not apply).
...
3. Recommends and records an execution branch (`<run>`, [conventions §11](research-workflow-conventions.md)) when the plan modifies existing code — routine creation follows the active authorization and involve level, while the base branch keeps working code until the reviewed changes are authorized to merge back;
...
**The execute-and-verify phase can run on another model.** The implementation and light-validation loop is where the run acts rather than judges, so where `STAR_EXEC_MODEL` names a model that phase goes to one delegate on it: the delegate picks the run up at the first unfinished action and returns when the loop ends, while everything around it — the gap list, the `EXEC_PLAN` and any decision it still needs, finalization, and the write-back into the sub-plan — stays on PLAN. It is kept narrow on purpose: it may not edit `EXEC_PLAN.md`, an action it cannot do as written stops and comes back rather than being improvised, a STOP-line command is recorded in `EXEC_LOG.md` and ends its turn there, and finalizing is never its to do. The dispatching run inspects the changed artifacts and accepts check evidence when the command, result, scope, and code version are clear; it re-runs a check only when that evidence is missing or uncertain, integration changed the behavior, or an acceptance check remains. With the key empty, the run already on that model or an alias of it with no per-dispatch depth to apply, or on a harness with no delegate to hand the phase to, the loop runs in place exactly as before; the hand-over is not a question at any involve level, since it changes which model executes the work and not what the work does.

### The STOP line

The skill needs specific launch authority before starting:
...
It first prepares the exact command, records it under “Awaiting user” in the execution log, and starts `star-code-reviewer <leaf>` itself, named above that command: a defect caught before the compute costs a review, caught after it costs the compute and the re-run too. If launch authority is still missing, it hands the command to you and stops; an earlier explicit instruction for that command and resource scope, or a matching `star-auto` grant within its `stop=` boundary, supplies the authority without another question. Nothing skips the review — the launch guard declines the command until the run's review exists and is no older than its log. Blocker or major findings go back through `star-plan-executor`, which reopens the affected steps, fixes and verifies them before launch. After the command finishes, invoking the same plan resumes from the log and verifies the result instead of starting over.
...
Invoked again on the same plan, the skill treats `EXEC_LOG.md` as the source of truth, skips completed steps, and resumes from the first unfinished action. Scratch under `tasks/<plan-name>/` stays in place by default; finalization does not offer to remove it. Cleanup happens only when you explicitly request it, while plan-owned tool scripts remain durable.

When the run executed on an execution branch, all of that — code, run records, the plan's own status — lives on `<run>` until the leaf is done and its newest review has no unsettled blocker or major finding. An ordinary run asks for merge authority only when it is missing; the default is a squash merge, followed by the affected light checks on the integrated base. At `low`, a matching unattended `star-auto` grant authorizes a clean reviewed squash merge and guarded removal of an empty worktree after its durable artifacts are moved out; the merged branch is retained. Auto does not authorize conflict resolution, discard, or branch deletion. Discarding instead rescues the run records onto the base before any specifically authorized deletion, so a negative result keeps its evidence; `star-flow-status` lists unmerged execution branches, so none of this waits invisibly.
...
2. Gathers relevant static evidence through the `.env` environment, using the narrowest checks that cover the requested scope (`compileall`, ruff, or flake8 when applicable and already installed — it installs no tools); it does not run an unconditional whole-tree check;
...
6. Runs a fix pass of mechanical, behavior-preserving fixes only (docstrings, scope-internal renames, unused imports, project-introduced dead code): the pass runs only under an explicit fix request or an unattended `star-auto` grant, and a review-only request applies nothing whatever the severity; eligible fixes in the authorized scope are named and applied at every severity, and an unsettled fix scope is decided in one question over the visible list. A deletion needs specific authority, but the same deletion already explicitly approved is not asked twice. Each applied fix is re-verified.
...
The fix pass never changes behavior: no feature completion, no signature changes visible outside the scope, no file moves, no edits to the names a rename left unchanged on purpose. Plan files are never edited — what the review learns about the plan routes to `star-plan-reviser`. Nothing is applied without a fix request or an unattended `star-auto` grant, and that grant skips every deletion; severity orders the findings but neither grants nor withholds authority, and `involve=high` does not reopen a fix authorization already given.
...
- A finding you disagree with is simply skipped in the fix pass; the report keeps the record either way, and every applied fix is recorded there with its authorization source — `git diff` shows it before anything is committed.
...
This skill is **read-only apart from its own report**. It never edits plan files, never sets `exec_status`, never touches `EXEC_PLAN.md` / `EXEC_LOG.md` — when a done-criterion is met, it recommends `star-plan-executor`, which owns finalization. It never re-runs an experiment to fill a missing metric: that metric is reported `unmeasurable` and the command comes back to you. The STOP line (conventions §2) applies here too.
...
star-expt-digest ledger                   # roll every model_trail into the model record file
```
...
wkdrs/digests/MODEL_LEDGER.md         # ledger mode: the model record file
```
...
This skill is **strictly read-only**. It scans the artifacts listed in §8 of the conventions — `metds/ideas/`, `metds/plans/`, `metds/refs/`, the compiled `metds/*.md`, the logs and reports under `wkdrs/` (run dirs, plus `wkdrs/reviews/`, `wkdrs/env_<name>_<date>/`, `wkdrs/digests/`, `wkdrs/results/`) — creating or modifying nothing. Being the most-run skill in the flow, it takes its plan and run state from one read-only scan script (`scripts/scan.sh` in its own directory), which gathers the plans' frontmatter, the run logs' status and steps, and the artifact listings in one call instead of one read per file; the script only gathers, so every rule it feeds stays in the skill. Reading and nothing else also puts it, with `star-expt-digest`, on the READ tier: those two are the only runs whose model a harness may fix in the skill's own manifest instead of at dispatch, and `bash execs/configure.sh` stamps `STAR_READ_MODEL` into those manifests from `.env`, offline (conventions §10.8).
...
2. Sweeps `tasks/`, `wkdrs/` scripts and configs, and root strays, promoting only what passes a three-part evidence test — the README will cite it, an executed leaf's §4 deliverable or §5 done-criterion needs it, or it reproduces a number in `wkdrs/results/results.md`. Destinations come from `metds/codearc.md` §2; a candidate no placement rule covers is reported as an architecture gap, never given an invented directory. **Confirmation point 1:** you settle the promotion table in one question over the visible list, each row's risk and any plan line it would make stale shown;
3. Polishes only the files a reader will open — the promoted files, the entrypoints, configs and scripts the README prints, the public API it shows. Routine behavior-preserving fixes apply within an authorized `polish` or full run; a material or destructive finding goes on one visible list and is asked once;
...
| You want one page on what the experiments did over a period, or since the last digest | `star-expt-digest` |
...
Two different questions; this section answers only the second. **Who may start a skill** is the roster ([conventions §10](research-workflow-conventions.md)): seven are slash-only, while the agent may pick up the other eight when the request plainly matches, and may start one as a next action only when it advances an already authorized execution goal. A status, explanation, or read-only request ends with its result and recommendation; a named next action alone does not authorize a writing successor. A `star-auto <goal>` invocation you typed extends pickup to all fifteen for that pursuit (§1, Invoking the skills). **How far a run gets before it needs you** depends on authority already given: an ordinary run asks only for a consequential decision or operation still unresolved, while auto supplies a standing launch and clean-merge grant within the goal and `stop=` boundary. Headless or scripted execution never treats silence as approval. In practice:
...
The involve level (conventions §7.7–7.8) controls unresolved judgment calls; it neither revokes existing approval nor creates new authority. With `INVOLVE=low` in `.env` — or `involve=low` on one invocation — a skill takes the recommended safe choice and logs it. Runs stretch further before needing you: `star-plan-decomposer` can take its recommended axis and sub-plan list, and `star-plan-executor`, pointed at a parent under an execution goal, can start the first ready leaf and proceed with its concrete execution plan. At `low`, authorized in-scope commits also proceed and are named afterward. `high` asks judgment calls one at a time. Changes to the research objective, acceptance criteria, key inputs, or cost/authority boundary still need an explicit decision; so do destructive operations that were not already specifically authorized. `star-auto` additionally authorizes prepared heavy commands and, after a clean review, the guarded squash merge and worktree cleanup described above, subject to its goal and `stop=` line.

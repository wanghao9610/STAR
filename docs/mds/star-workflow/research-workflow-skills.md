...
Which model a run uses is set in `.env`, in three tiers: `STAR_PLAN_MODEL` for research judgment — plans, plan reviews, analyses, and the blind reads that check them — `STAR_EXEC_MODEL` for implementation and production, and `STAR_READ_MODEL` for read-only scans, collectors and summaries. The roster in [conventions §10](research-workflow-conventions.md) carries each skill's tier; a few modes take a different one, and two runs change tier partway through, each named in that skill's section below. Where a harness can name the model a delegate runs on, a run whose tier points at a different model — or carries a depth that harness can apply per dispatch — hands itself to one delegate and relays the reply unchanged; the decision is a paragraph at the head of each relocating manifest's Workflow, with the session's model read once through the provenance resolver. The files stay the run's, and the provenance recorded in them is the model that wrote them. A run that still owes you a confirmation point stays where it is, which is why `star-idea-storm` and `star-plan-coach`, asking at every stage, never self-relocate. The three keys ship empty, and an empty key changes nothing: the run stays where it started. An entry may also carry a thinking depth after its model — `claude:opus@high` or `codex:gpt-6-astra@high` — so two tiers can name one model and differ only in how deeply their runs reason. How each harness applies a depth: [harness adapters](harness-adapters.md). Full rule: [conventions §10.8](research-workflow-conventions.md).

...
2. **Confirm the shape, then auto-draft the content.** Two decisions go through `ask_user_question` (one question per call, each with a recommendation): the **decomposition axis**, then the **sub-plan list**. After those are confirmed, draft each sub-plan's content autonomously by pulling from the parent. Mark genuine gaps `[TBD]`; ask a targeted follow-up **only** when a step is undecidable without the user. Do not re-elicit detail the parent already records.

...
Four lines a unit, and each stays one line: this is the sketch Step 4 expands into the six sections, not a draft of them. Then confirm via `ask_user_question` (*looks good* / *edit the list* / *change granularity*, with your recommendation marked).
...
- If `ask_user_question` is unavailable (headless/scripted), fall back to plain-text questions — still one at a time.

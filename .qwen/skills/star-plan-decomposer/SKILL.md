...
- If `ask_user_question` is unavailable (headless/scripted), fall back to plain-text questions — still one at a time. **Use that same plain-text form whenever the material a question is about does not fit in its options** — the sub-plan list, a sub-plan draft, the drafted parent index, the rubric failures. The material and the question then go out as one message, the options written into it with the recommendation marked, and that message ends the turn: no tool call comes between the material and the question about it.

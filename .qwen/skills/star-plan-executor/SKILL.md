...
description: >-
  ...
  out; costly experiments launch only under specific authority, otherwise stopping at an exact
  handoff command.
---
...
2. **Make the plan visible, then proceed within scope.** Build the concrete `EXEC_PLAN`. Use `enter_plan_mode` / `exit_plan_mode` only when a material decision remains unresolved; otherwise present the plan in commentary and continue without another approval gate. Invoking this executor authorizes ordinary in-scope implementation and light validation.
3. **Delegate what benefits from it.** For bounded independent work, dispatch `agent` with `subagent_type: general-purpose` for implementation or `Explore` for read-only orientation. A configured tier model selects the matching named `agent` (`star-plan`, `star-exec`, or `star-read`) described above; never pass the raw value as a per-call model. The main agent owns integration and verified checks.
...
1. Interpret `PLAN_NAME` (slug / numeric prefix / full filename) against the plans the scan digest lists; it is the listing, so do not list the directory again.
2. **Only leaves are executable.** If `PLAN_NAME` resolves to a node with children (non-empty `children:` frontmatter), do not execute it: list its leaves (prefix + slug + one-line objective) and ask via `ask_user_question` which to execute (recommend the first ready in dependency order); each further leaf is its own run (conventions §10.4), started after this one ends under an existing execution goal (§10.6).
3. If no argument was given or the match is ambiguous, list available plans and ask via `ask_user_question`.
4. Read the resolved sub-plan in full.

### Step 1: Readiness check

1. **Executability.** §3 Task Breakdown and §5 Done-Criteria must be concrete. If they are still largely `[TBD]` / `【待定】`, say decomposition is unfinished and offer via `ask_user_question`: *go back to `star-plan-decomposer` to flesh it out* (recommended) / *execute anyway (shallow, gaps stay `[TBD]`)*.
2. **Dependencies.** Check §2 Inputs & Dependencies: are the named datasets (`datas/`), weights (`inits/`), and code modules present? Are the upstream siblings in the leaf's `depends_on` frontmatter all finished? Read their state from the digest, which already carries every sibling's frontmatter, rather than opening each one. A sibling is finished when it is a leaf at `exec_status: done`, or an internal node whose every live leaf is terminal (`done` or `abandoned`), at least one of them `done`, with no outline line left in any `## Sub-plans` index of its subtree, read from those files (conventions §5.5). If a hard dependency is missing or an upstream sibling is not finished, **stop and report** — do not fabricate inputs. A missing dataset or weight is a decomposition gap, not a blocker to work around: name the data-readiness leaf that should own it, or route to `star-plan-decomposer <parent>` to add one.
...
**Before implementation, run the design check from `references/design_check.md`.** Send exactly the EXEC_PLAN, leaf plan, and root plan §4 to one read-only `agent` (`subagent_type: Explore`; use named `star-plan` when PLAN is configured). Re-open every cited `fail`; correct authorized plan defects and ask only when research scope, acceptance, key input, or cost remains unresolved. If unavailable, run locally once and record the limitation.

1. Present the concrete plan in commentary. If `enter_plan_mode` was used, call `exit_plan_mode` only to settle displayed material choices; reuse existing authorization. Per-action commits and branch/worktree choices follow the involve level. Resolve divergence rows through `references/plan_sync_rules.md`; `auto=unattended` covers only tactical rows that leave scope, key inputs, §5 acceptance, and approved cost unchanged.
...
**Hand Step 5 to the EXEC tier.** With records and decisions settled, dispatch `agent` (`subagent_type: general-purpose`; use named `star-exec` when EXEC is configured with a model that is not an alias of this run's (conventions §10.8), never a raw per-call model) to read this manifest and resume from Step 5 with `involve=<level> tier=exec` and any valid `auto=unattended` grant. It may not edit EXEC_PLAN or improvise around a plan-level gap. Wait for the delegate to finish â its completion, not an interim reply â then re-read EXEC_LOG, never verifying while it may still be writing that log; if unavailable, run Step 5 here.

### Step 5: Execute–verify loop
...
1. Dispatch `agent` (`subagent_type: general-purpose`, or `Explore` for read-only orientation; use the matching named tier agent when configured) with `references/agent_dispatch_spec.md`, exact scope, interpreter, check, and structured return contract.
...
3. **If the step is on the STOP line**, write the command as `execs/scpts/<run>.sh`, the one launch path the launch guard recognizes, and record it with expected cost, outputs, and code version under `Awaiting user`. A specifically authorized ordinary launch or valid `star-auto` launch proceeds only after review and cost guards; otherwise hand it to the user. A `tier=exec` delegate returns here and never launches it itself.
...
**Route it back to the plan (plan-level finding).** If the result contradicts an assumption the parent plan depends on — i.e. it matches a root §5 **kill-criterion**, or an MVP done-criterion the plan called the "cheap early test" came back negative — you do not edit the parent's §1–§6 (that stays with the coach/decomposer). Instead: record it in the run's `EXEC_LOG.md` "Notes / decisions" (which this skill owns), and in the Step 8 report **point it out explicitly** and recommend feeding it back via `star-plan-reviser <slug>` (audit the evidence and revise the plan), `star-plan-coach <slug>` (revisit risks/method), or `star-plan-decomposer <slug>` (re-scope the sub-plans).
...
- In a non-interactive run, continue ordinary authorized implementation and light validation. If a required decision remains and `ask_user_question` is unavailable, present the concrete issue in plain text, stop only its dependent action, and continue independent authorized work; elapsed time or silence is never approval.
...
- Reply in the language resolved under conventions §7.6 while preserving the plan body's frontmatter `language`; keep technical terms in English inside Chinese plans.

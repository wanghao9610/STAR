...
**实现前先按 `references/design_check_zh.md` 跑设计检查。**使用 `agent` 的只读角色（`general-purpose` / `Explore` 中对应的只读角色），仅提供 EXEC_PLAN、叶子计划和根计划 §4。重开每条 `fail` 证据；只在研究范围、验收、关键输入或成本仍未解决时提问。模型走 命名 `agent`；工具不可用时本地跑一次并记录限制。
...
**把 Step 5 交给 EXEC 档。**记录与决定都已定下时，按 命名 `agent` 路线用 `agent` 的执行角色接手，携带 `involve=<level> tier=exec` 与合法 `auto=unattended`。不得改 EXEC_PLAN 或绕过计划级缺口；返回后重读 EXEC_LOG。模型路线不可用时在本地运行，不反复询问。

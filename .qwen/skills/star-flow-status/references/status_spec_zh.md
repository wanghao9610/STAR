...
上例中的根是 `◐` 而非 `✔`，因为它的 `finalized:` 未设——六节全 `done` 本身并不能关闭一个总体计划节点，评分表还得跑。总体计划节点的状态符号只报告**它自己**的状态，绝不报告其子树：一个已 finalized 的根即使下面的子树只执行了一半，仍然是 `✔`；子树没做完这件事由进度汇总来说。上例里 `04` 是丢弃写在其上的那个节点，`040` 自己不带任何字段，读作已丢弃是因为它的祖先是——这正是这个字段绝不向下复制的原因。

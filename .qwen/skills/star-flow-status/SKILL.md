...
Invocation: `/star-flow-status [PLAN_NAME] [DESCRIPTION]`. The argument is not substituted into this file: the runtime appends the raw invocation line after it, and when an argument was given, an `<skill-args>` block carrying it verbatim beside an `<skill-args-file>` path holding the same text. Read the argument there rather than retyping it, and treat it as absent only when neither is present — treating an argument that is there as absent silently answers a question nobody asked. With no argument, report the whole flow; with a slug / numeric prefix / filename, scope both the tree and the coverage checks to that plan's subtree. Anything left after `PLAN_NAME` is a description (conventions §7.12): in your own words, what this run is for. Prose that resolves to no plan is description alone, not a missing target — report the whole flow, and say so in the reply's first line. A lone token that looks like a plan name and matches none is not a description: name it as unmatched in that same line, since this skill never asks. A description can steer what the report looks at hardest — which unknown is worth a second read, which line is worth quoting rather than counting, what the reason line addresses; it drops nothing the report owes, and it never moves the next action, which the spec's priority order fixes down to the tie-break. An `involve=<level>` token is stripped from the argument before `PLAN_NAME` or the description is read (conventions §7.7); it changes nothing else here — this skill asks no questions.
...
4. **Only the plan tree earns a graph walk; the follow-up checks are thin.** The plan tree carries an ordering (`parent`, `depends_on`, `exec_status`); every other stage is checked as presence-and-freshness against the output table — never invent an ordering for artifacts that have none.
...
### Step 1: Scan
Load everything in one message — two `run_shell_command` calls, both with the project root as the working directory, plus a read of the spec issued alongside them. First call, the conventions excerpts:
...
Over the digest's listing, count report-shaped files matching no pattern in the output table (spec's unrecognized-files rules). Report one line with the count and up to three example paths. Omit entirely when the count is zero. This line is how a producer skill's renamed output gets noticed, instead of silently dropping out of the follow-up checks.

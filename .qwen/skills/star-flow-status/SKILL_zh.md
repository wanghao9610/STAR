...
- 顺序：树 → 进度汇总 → 覆盖检查 → 唯一的下一步 → 失配标记 → 未识别文件行。覆盖检查、失配、未识别文件行为空时各自省略。不设字数上限：这份报告该多长，由它要展示的那棵树决定，范围内六十个节点就是六十行。约束改由形状给出——每节点一行、每个计数一句、每条触发的覆盖检查一行、下一步一条命令加一句理由、每条失配标记一行。该砍的是各节之间的散文，不是读者需要的那一行。

...
调用方式：`/star-flow-status [PLAN_NAME] [描述]`。参数不会替换进本文件：运行时会把原始调用行接在本文件之后；带了参数时，还会附上一段 `<skill-args>`（原样承载参数）和一个 `<skill-args-file>`（同样内容的文件路径）。从那里读参数，不要重新敲一遍；只有两者都不存在时才算没带参数——把带了的参数当成没带，就等于答了一个没人问过的问题。不带参数则总览整条流程；带 slug / 数字前缀 / 文件名则把树和覆盖检查一起收敛到该计划的子树。参数里出现 `involve=<level>` 记号时，先剥离它再解析 `PLAN_NAME`（规约 §7.7）；除此之外它在这里不改变任何行为——本 skill 不提问。`PLAN_NAME` 之后剩下的一切都是描述（规约 §7.12）：用你自己的话说明这次要做什么。解析不到任何计划的成句文本只是描述，不是漏掉的目标——照常总览整条流程，并在回复第一行说明这一点。形似计划名、却一个都对不上的孤立词不是描述：在同一行里点明它没有匹配——本 skill 从不提问。描述可以引导报告重点看哪里——哪个 unknown 值得再读一遍、哪一行值得原文引用而不是计数、那句理由要回应什么；但报告该给的一样不少，也挪不动「下一步」那条推荐：它由 spec 的优先级顺序定死，连同序时怎么排都写好了。

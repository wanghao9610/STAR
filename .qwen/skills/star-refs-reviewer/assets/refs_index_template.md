...
<!-- Written by /star-refs-reviewer (model_id: <model id, copied verbatim from what your runtime states this session — Qwen Code injects it at session start; "unrecorded" only if the session names none>). This file is the audit trail for reference.bib: every entry's

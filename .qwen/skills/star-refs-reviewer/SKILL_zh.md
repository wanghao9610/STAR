...
`STAR_LANG` 定回复语言（§7.6）、`INVOLVE` 定提问档位（§7.7）；探测与前半各节同在第一次调用里；同一条消息里的多次调用之间只算一趟往返，不是各占一趟。每行 `awk` 只打印它上面点名的那几节，别的都不打印；若打印出来的内容里少了其中任何一节——下游同步的规约副本可能过旧、条号不同——就改为整份读入。规约摘录合计约 32 KB——两次调用各约 13 KB 与 19 KB——因为每份工具结果各有自己的大小上限：结果一旦超过 30 KB 左右就会被存成文件，要再读一次才拿得回来，而整份挤在一次调用里现在正好会这样。
...
3. **先确认形状，再精读。** 精读是贵的那一步：把约 15 条排好序的候选（标题 / 会议 / 年份 / 引用数 / 一句话理由）先摆成一张带编号的表，再用一次 `ask_user_question` 对着编号提问（推荐的 5–10 条标在表里），只读用户留下的。检索和分类不需要批准；精读和写笔记需要。survey 模式下一个问题一次盖住整份文档的形状：检索画像、分类轴和分层阅读清单（Step 10.3）——这是 `involve=low` 会按推荐项直接采纳的判断型问题；完整流程的核心集确认在任何级别下都必答，不受影响。
...
按 `references/refs_rubric_zh.md` 的核心论文标准排序，用一张表给出约 15 条（最相关在前）。每行另给它的临时影响力分——只含引用与发表两个分量，按评分表的算式从手头的检索记录算出，标为残缺分（`6.4*`）：代码分量要等 Step 3 抓到论文页才有。排序仍以相关性为先；分数是摆在选择旁边的信息，不是排序键。经一次 `ask_user_question` 对着表里的编号确认精读哪些——约 15 行装不进选项（规约 §7.3 把一题的选项封在 4 个），推荐的 5–10 条标在表里，选项给成：*读推荐的这一组* / *读另一组（报编号）* / *先把检索面放宽*。用户可以自己加论文——照常抓它们的记录。
...
精读可以并行分派给只读 `agent` subagent（`subagent_type: Explore`），一个 subagent 一篇，各自按 `references/refs_rubric_zh.md` 的笔记收集器返回格式返回——不是"填好的模板",模板里有些字段只有真正写文件的那次会话才能填。只有 `depth_evidence` 带着真实的表题和一行数据时才接受 `depth: full`,否则下调。主 agent 负责写文件并亲自写 §5（与本项目的关系）——这一节需要方法上下文，也正是这篇笔记存在的理由。每个收集器拿到按份数分给它的那份 host 请求配额（写成具体数字），并返回 `failures: [{host, error, retries}]`；它抓到的页面缓存在本次运行自己的 `raw/` 前缀下、一篇论文一个前缀（规约 §6.4）。这三样和 Step 1、Step 4 给各自收集器的完全一样——这一步只是抓的 host 不同，规矩没有变。
...
- 核心集确认是完整流程里唯一的强制提问——一次 `ask_user_question`，对着编号提问，推荐标在表里。survey 模式的那一问——画像、分类轴、分层清单（Step 10.3）——是判断型问题：`involve=low` 按推荐项直接采纳并记录；Step 10.7 的覆写确认在任何级别下仍必答。`add` 里一个标题解析出多条候选记录、或一条也解析不干净，属于用户所指的歧义——任何级别下都要问，一个直接问题列出候选。`ask_user_question` 不可用（无人值守 / 脚本化）时改用纯文本，精读前必须拿到明确答复。

...
3. **先确认形状，再精读。** 精读是贵的那一步：把约 15 条排好序的候选（标题 / 会议 / 年份 / 引用数 / 一句话理由）用一次 `ask_user_question`（multiSelect，5–10 条标为推荐）交给用户，只读用户留下的。检索和分类不需要批准；精读和写笔记需要。survey 模式下同一个直觉一次盖住整份文档的形状：一个问题带上检索画像、分类轴和分层阅读清单（Step 10.3）——这是 `involve=low` 会按推荐项直接采纳的判断型问题；完整流程的核心集确认在任何级别下都必答，不受影响。
...
按 `references/refs_rubric_zh.md` 的核心论文标准排序，用一张表给出约 15 条（最相关在前）。每行另给它的临时影响力分——只含引用与发表两个分量，按评分表的算式从手头的检索记录算出，标为残缺分（`6.4*`）：代码分量要等 Step 3 抓到论文页才有。排序仍以相关性为先；分数是摆在选择旁边的信息，不是排序键。经一次 `ask_user_question`（multiSelect，5–10 条标为推荐）确认精读哪些。用户可以自己加论文——照常抓它们的记录。
...
- 核心集确认是完整流程里唯一的强制提问——一次 `ask_user_question`，multiSelect，标出推荐。survey 模式的那一问——画像、分类轴、分层清单（Step 10.3）——是判断型问题：`involve=low` 按推荐项直接采纳并记录；该模式在任何级别下仍必答的，是 Step 10.7 的覆写确认。`add` 里一个标题解析出多条候选记录、或一条也解析不干净，属于用户所指的歧义——任何级别下都要问，一个直接问题列出候选。`ask_user_question` 不可用（headless / 脚本化）时退化为纯文本，精读前必须拿到明确答复。

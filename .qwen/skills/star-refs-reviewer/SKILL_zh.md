...
description: >-
  建立并维护可审计的文献基础，包括论文笔记、已核实 BibTeX、主题综述、相关工作合成与影响力分数刷新。用于
  文献综述或引文工作；每个书目字段来自抓取记录，无法抓取的条目保留人工核对标记。
---

# Research Refs Reviewer

调用方式：`star-refs-reviewer [PLAN_NAME | TOPIC | verify | organize | synthesize | survey [PLAN_NAME | TOPIC] | score | add PAPER… | ARXIV_ID | DOI | URL]`。在共享装载前解析这些模式；未指定模式则运行或恢复完整流程，计划或 topic 限定范围；`add` 与论文标识追加条目。只装载所选模式引用的材料。
...
3. **昂贵精读前定下阅读集。**先应用请求已选的论文或 survey 形状。完整流程否则展示约 15 条候选，并通过 `ask_user_question` 问一次要精读的 5–10 条；核心集是任何 involve 档位都不能代选的关键输入与成本决定。`survey` 的画像、分类轴与分层清单属于裁量：`low` 采用并记录推荐，其他档位按规约 §7.7。检索与分类自主完成。
...
**本次运行在哪里执行。** 模式解析后按规约 §10.8 处理整次运行的交接。完整流程及 `add`、`verify`、`organize`、`survey`、`score` 使用 EXEC，`synthesize` 使用 PLAN。已有选择或覆盖授权同样有效；只有必需决定仍未解决时才留在这里。
...
- `related_work.md` 只编译、不发明：每条刻画都追溯到论文笔记；无笔记条目只能引用抓取记录事实。生成文件先展示分节变更清单，已有具体更新授权则直接执行，否则覆盖前提问；手写文件始终需要具体覆盖权限。`<slug>_survey.md` 遵守同一证据与覆盖规则。
- 日期必须真实（规约 §4）：抓取日期就是实际抓取的那天。
- 本 skill 不设任何计划 frontmatter、不创建计划文件；审计线索就是 `refs_index.md` 加运行缓存。Git：只读；绝不提交（规约 §1）。

## 对话纪律

- 深读前必须明确完整流程的核心集；已有选择同样有效。`survey` 在 `low` 可采用推荐画像、分类轴与清单，除非请求改变范围、新增实质成本或 topic 有歧义。`add` 标题歧义任何档位都通过 `ask_user_question` 询问。具体覆盖授权直接执行，不重复询问；笔记不得夸大 `depth:`，`reference.bib` 保持英文。

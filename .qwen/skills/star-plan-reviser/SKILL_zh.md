...
1. 先把每条候选摊在页面上，写进承载这个问题的那条消息正文里（规约 §7.13——一张拟好的清单是一个问题，不是一行一个）：每条候选一个编号行，写明它改哪一节、从什么改成什么、证据路径、分级（local / structural / strategic），以及你推荐怎么做。**structural** 或 **strategic** 的行，推荐动作是转出去——形态问题给 `/star-plan-decomposer`，方向问题给 `/star-plan-coach`——并把"仍在本文件做范围受限的文本修订"写成备选。

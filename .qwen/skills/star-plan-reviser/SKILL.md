...
Before acting, load it in one message — four `run_shell_command` calls with the project root as the working directory, plus two `read_file` calls, `<this skill's directory>/references/review_spec.md` and `<this skill's directory>/references/revision_rules.md`, each whole file as its own call — sent together.
...
- If `ask_user_question` is unavailable (headless / scripted), fall back to plain-text questions — still the whole candidate list on the page before the one question that settles it, still explicit approval before any write.

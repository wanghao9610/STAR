...
- **Small scope** (≤ ~20 files — a diff-mode review usually is): the main agent can read every file and apply `references/review_rubric.md` directly — unless earlier turns of this conversation wrote or edited files now in scope. An author rereads its own code through the reasoning that produced it, so collection then goes through the larger-scope route even at this size — one read-only collector with the whole file list is enough, and its fresh context, which never saw that reasoning, is the point — and the report's scope line records that the code was written in this session and collection was delegated. Running this command is the request for that one collector — Step 3 is where the skill the user invoked says it dispatches — so a standing instruction conditioned on the user having asked is satisfied here, and only here; the larger-scope fan-out below claims no such thing. One that bans delegation outright is not satisfied, and takes the route that follows. Where that collector cannot be dispatched — the host offers no delegation, or a standing instruction bans delegation outright (conventions §6.1) — ask once for this one dispatch; declined or unavailable, collection runs locally and the scope line records instead that the code was written in this session and no independent collector read it.

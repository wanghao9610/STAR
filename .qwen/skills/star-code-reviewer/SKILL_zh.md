...
- **小范围**（≤ 约 20 个文件——diff 模式的审查通常都是）：主 agent 逐个读文件，直接应用 `references/review_rubric_zh.md`——除非范围内有文件是本轮对话此前写出或改过的。作者重读自己的代码，读到的是当初产生它的那套推理，所以此时收集照走较大范围那条路，范围再小也一样——一个只读收集器带上全部文件清单就够，它的全新上下文没见过那套推理，这正是要的效果——并在报告的范围行写明：代码出自本会话，收集因此委派。跑这条命令，请求的就是这一个收集器——Step 3 正是用户调用的这个技能写明要派发的地方——所以以「用户提出请求」为条件的常驻指令在这里已被满足，也仅限这里；下面较大范围那条并行分派不作此主张。彻底禁止委派的常驻指令不算被满足，走下面那条路。这一个收集器派不出去时——宿主不提供委派能力，或有常驻指令把它扣住（规约 §6.1）——就为这一次派发问一次；被拒绝或本就派不出，收集改在本地做，范围行改成写明：代码出自本会话，没有独立收集器读过。

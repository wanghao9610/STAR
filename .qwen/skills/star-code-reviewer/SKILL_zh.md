...
一条消息、三份结果：规约——§1 git、§2 红线、§3 `.env` 运行时、§4 真实日期、§5 计划名解析、§6 委派、§7 对话纪律、§8 产物登记表、§9 项目布局——六维评分表连同其问题项格式约定（Step 1 组装评判依据摘要、Step 3 收集问题项用的都是它），以及只有 `run_shell_command` 能查的 `.env` 那一行。别把这两个文件 `cat` 进命令里：`run_shell_command` 结果一旦超过 30 KB 左右就会被落盘成文件，要再读一次才拿得回来——单是规约文件就已越过这个上限——而每次 `read_file` 的结果各占自己的额度、整份直达。后续步骤直接用这里装入的评分表，不再单独读；报告模板与 Step 1 要读的项目侧文件各在其步骤装载，不在这里。
...
2. **广收集，先核实再报告。** 收集可以并行分派给只读 `agent` subagent（`subagent_type: Explore`），但每条要进报告的 blocker/major 问题项，主 agent 都要重读被引用的代码确认；站不住的降级或丢弃。评价一份 review 看的是问题项的精度而不是数量——一条错误的 blocker 就足以让报告失去可信度。
...
5. **修复是例行的、逐项批准的、不改行为的。** 报告之后提供修复轮，只覆盖 docstring、作用域内改名、unused imports、本项目引入的死代码。每项先经 `ask_user_question` 批准再写入——一次一条问题项（或一批同类项），标出推荐——写入后复检。绝不打包默批；绝不顺手"改进"相邻代码（AGENTS.md §3）。
...
2. 解释参数，先匹配者生效：
   ...
   - 无参数 → `${CODE_NAME}/` 全部。
   - 都不匹配 → 列出最接近的计划与路径候选，经 `ask_user_question` 询问。
...
4. 收敛到可审源码：Python 文件走完整评分表；范围内的 shell / YAML / 配置文件只查维度 D（路径与运行时）；`datas/`、`inits/`、`wkdrs/` 产物与生成文件不在范围内。审查前报出最终文件数；超过约 50 个文件时，先跑 Step 2 的全树筛查——它只是 grep 和 `wc`，不需要环境——再说明情况，并经 `ask_user_question` 提议收窄（某个子包，或 diff 模式），同时用筛查结果指出证据已经在哪里。让用户去猜挑哪个子包，是更差的提议。
...
- **小范围**（≤ 约 20 个文件——diff 模式的审查通常都是）：主 agent 逐个读文件，直接应用 `references/review_rubric_zh.md`——除非范围内有文件是本轮对话此前写出或改过的。作者重读自己的代码，读到的是当初产生它的那套推理，所以此时收集照走较大范围那条路，范围再小也一样——一个只读收集器带上全部文件清单就够，它的全新上下文没见过那套推理，这正是要的效果——并在报告的范围行写明：代码出自本会话，收集因此委派。跑这条命令，请求的就是这一个收集器——Step 3 正是用户调用的这个技能写明要派发的地方——所以以「用户提出请求」为条件的常驻指令在这里已被满足，也仅限这里；下面较大范围那条并行分派不作此主张。彻底禁止委派的常驻指令不算被满足，走下面那条路。这一个收集器派不出去时——宿主不提供委派能力，或有常驻指令彻底禁止委派（规约 §6.1）——就为这一次派发问一次；被拒绝或本就派不出，收集改在本地做，范围行改成写明：代码出自本会话，没有独立收集器读过。
...
2. 按报告顺序经 `ask_user_question` 走可修问题项——*照建议修* / *调整后修* / *跳过*，标出推荐，一次一条。同类超过 4 条（如 12 处缺 docstring）可合并为一问：*全修* / *选哪些（multiSelect）* / *全部跳过*。
...
- 修复轮的批准一律经 `ask_user_question`——一次一条问题项（或一批同类项）。不可用（headless / 脚本化）时退化为纯文本，仍然一次一条，任何写入前先获明确批准。

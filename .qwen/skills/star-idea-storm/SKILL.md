...
Generate 3–5 distinct candidate directions with a research question, bet, novelty, and nearest area. Apply any selection already stated; otherwise present one table and ask once via `ask_user_question` which 2–4 to scan, marking recommendations. Keep dropped candidates in §2 as `not scanned`.
...
- Ask one coaching question at a time through `ask_user_question` only when its research choice remains unresolved; use concise plain text when the tool is unavailable. A direction set or final decision already supplied in the request or session is recorded and not asked again.

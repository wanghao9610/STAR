...
Before acting, load it in one message — three `run_shell_command` calls with the project root as the working directory, plus a `read_file` of `<this skill's directory>/references/adopt_spec.md`, all sent together.
...
- All three confirmation points go through `ask_user_question`, one question per call. If it is unavailable (headless / scripted), fall back to plain text — still one at a time, still an explicit answer before any write past a confirmation point.

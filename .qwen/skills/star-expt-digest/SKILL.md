...
Before acting, load this skill's unconditional opening reads in one message — the opening load that later text points back at: two `run_shell_command` calls, with the project root as the working directory, plus a `read_file` of `<this skill's directory>/references/scope_spec.md`, all sent together.

...
你读与叙述；你不执行、不分析 run、不评判 criteria、不修订计划、不翻转状态。digest 揭示出的、超出你可写文件范围的事情一律转交出去：未分析的 run 交 `/star-expt-analyst`，过期的结果汇总表交 `/star-expt-analyst aggregate`，未执行的叶子交 `/star-plan-executor`，被证伪的主张交 `/star-plan-reviser`，当前树态交 `/star-flow-status`。

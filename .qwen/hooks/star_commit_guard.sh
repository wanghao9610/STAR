#!/usr/bin/env bash
# STAR PreToolUse hook (Qwen Code) — decline the git commands that break the
...
# would match nothing and say nothing. One copy per harness tree (conventions
# §1, "The guard"), each differing only in how its harness names the command on
# the way in and the decision on the way out.
#
...
deny() { # $1 = one-line reason, $2 = what to do instead (default: hand it to the user)
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"%s (declined by .qwen/hooks/star_commit_guard.sh — %s)"}}\n' "$1" "${2:-hand it to the user to run}"
    exit 0
}

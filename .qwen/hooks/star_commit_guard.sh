...
# would match nothing and say nothing. One of five copies — Claude, Codex and
...
deny() { # $1 = one-line reason
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"%s (declined by .qwen/hooks/star_commit_guard.sh — hand it to the user to run)"}}\n' "$1"
    exit 0
}

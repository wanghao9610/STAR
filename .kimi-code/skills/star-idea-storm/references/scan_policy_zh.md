...
这是文献库来源策略的轻量姊妹篇：那份策略为已定的方法构建可核验的 `reference.bib`；本策略只为 2–4 个候选方向回答"多拥挤、谁最近、gap 在哪"，深度到摘要为止，每方向几分钟。它不产出 bibtex、不写逐篇笔记、不向 `metds/refs/**` 写任何东西——选题定下后，由深度调研（`star-refs-reviewer`）在胜出方向上把那些建起来，起点就是这些扫描表。
...
与文献库"首个命中即止"的阶梯不同，扫描需要分歧：每个方向至少跑两个来源——只用一个索引估出的拥挤度，是那个索引的假象。

...
- In non-interactive `kimi -p` runs (no human to answer), fall back to plain-text questions — still one at a time, and still waiting for an explicit answer at the two gates: the set of directions to keep (Stage 2) and the decision (Stage 4).

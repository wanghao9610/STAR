...
2. **One question at a time, in the conversation**: Deliver every coaching question in the conversation — one question at a time, waiting for the answer before sending the next. Never dump multiple questions as a plain-text list in one message. Give each question 2–4 short, concrete candidate options drafted from the question bank and what the user has already said — the user may always answer freely outside the options, so options never trap them. After every 2–3 answered questions, pause and restate the key points you heard in one or two sentences of normal text, then continue. Exception: questions too open for meaningful candidates (e.g., the initial seed) may be asked as plain text.

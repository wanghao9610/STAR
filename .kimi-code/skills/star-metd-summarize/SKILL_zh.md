...
**规模**：步骤 1 只读了 frontmatter，所以计划正文是在这一步才进入本次运行的。小树（≤ ~15 个计划）由主 agent 直接读。更大的树则**按文档目标**切分给只读 subagent，最多 3 个并行，各自拿到映射、确切文件清单与 `extract_map_zh.md` 里的提取格式约定。收集器只提取并返回；不写文件，不做跨计划的冲突解析，也不编译 `overview`（它需要其余四份文档编译后的内容）。

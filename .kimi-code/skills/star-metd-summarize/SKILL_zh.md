...
- **就绪门槛——只编译已确定的方法。** 圈定范围内的树，只有当每个总体计划都带 `finalized:`、且每个叶子都是 `exec_status: done` 时才算就绪——所有实验做完、计划定稿。已丢弃的节点不属于这棵树：带 `dropped:` 的节点连同其下的一切，既不是来源、也不是未完成项，所以已丢弃的叶子既不会卡住这道门槛，它下面的东西也不会进入任何文档。差一项都不行：什么也不编译，列出未完成项（每个非 `done` 的叶子及其 `exec_status`、每个缺 `finalized:` 的总体计划）并转交——未执行或阻塞的叶子交 `/skill:star-plan-executor`，未定稿的总体计划交 `/skill:star-plan-coach`，全局状况看 `/skill:star-flow-status`——然后停止。越过门槛只有一条路：把未完成项摆到用户面前，经 AskUserQuestion 询问是否照样编译一份草稿；用户显式选择编译——此时来自未完成叶子的每一段内容都按 Step 3 标注尚未验证。

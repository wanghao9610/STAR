...
调用方式：`/skill:star-metd-summarize [OPT]`——`OPT` 为 `overview` / `dataset` / `framework` / `training` / `evaluation` 之一，各自编译 `metds/<OPT>.md`；不带参数则按依赖顺序编译全部五个（`dataset` → `framework` → `training` → `evaluation` → `overview`）。
...
5. **绝不让计划读起来像结果。** 来自 `exec_status` 既非 `done` 也非 `abandoned` 的叶子的内容——只会出现在显式选择的草稿编译里（Step 1 的就绪门槛），而 `abandoned` 叶子的内容根本不进——是设计意图：在该小节末尾加一行斜体标注"尚未验证"，并写明来源计划。已验证的内容不加任何标记。结果数字则完全不进这些文档——某个 run 测出的指标属于 `wkdrs/<run>/EXPT_ANALYSIS_<date>.md`，它们的跨 run 结果汇总表是 `wkdrs/results/results.md`；`evaluation.md` 定义的是协议，不是分数。
...
每个计划的 **frontmatter** 已随开场装载的摘要到手——本步骤要记录的每个字段都在那里，计划正文是步骤 2 的输入，不进这一步，所以这一步不再打开任何计划文件。按 `parent:` 重建树——`parent:` 权威，缺失或有歧义时用该计划的 `## Sub-plans` 索引来定位它（摘要里同样带着），数字前缀只是提示，因为两个不相关的根都可能是 `0_`（`/skill:star-flow-status` 的规则）。逐节点记录：根 / 中间 / 叶子、`updated`、`language`、`status:` 映射，叶子上的 `exec_status` 与 `traces_to`。

- **输出语言跟随计划**：取根计划的 `language:`；多根时取多数；打平时用对话语言。
- **一套文档描述一个方法。** 树里有多个互不相关的根时，如实说明，并经 AskUserQuestion 询问这套文档描述的是哪个根的子树；答案决定整轮的范围。
- **就绪门槛——只编译已确定的方法。** 圈定范围内的树，只有当每个总体计划都带 `finalized:`、且每个活着的叶子都处于终态——`exec_status: done`，或被自身 kill-criterion 判死的 `abandoned`——时才算就绪，即所有实验做完、计划定稿。`abandoned` 的叶子和已丢弃的节点都不是来源：带 `dropped:` 的节点连同其下的一切、以及任何 `abandoned` 叶子，都不向任何文档贡献内容——被关掉的方向不属于最终确定下来的方法，它的负结果留在那次 run 的分析报告里，那才是它该在的地方。两者也都不会卡住这道门槛。差一项都不行：什么也不编译，列出未完成项（每个尚未进入终态的叶子及其 `exec_status`、每个缺 `finalized:` 的总体计划）并转交——未执行或阻塞的叶子交 `/skill:star-plan-executor`，未定稿的总体计划交 `/skill:star-plan-coach`，全局状况看 `/skill:star-flow-status`——然后停止。越过门槛只有一条路：把未完成项摆到用户面前，经 AskUserQuestion 询问是否照样编译一份草稿；用户显式选择编译——此时来自未完成叶子的每一段内容都按 Step 3 标注尚未验证。
...
**规模**：步骤 1 只读了 frontmatter，所以计划正文是在这一步才进入本次运行的。小树（≤ ~15 个计划）通常由主 agent 直接读更省事。更大的树则**按计划**切分给只读 `Agent` subagent（`subagent_type: explore`），并行派发，各自拿到完整映射、一组互不相交的完整计划文件，与 `extract_map_zh.md` 里的提取格式约定。每个收集器都在自己持有的计划上为全部五个目标做抽取，并给每段标上它所喂的 `target:`。
...
- **不存在** → 直接写。
- **存在且由本 skill 生成**（有 `type:` + `generated:`）→ 与新编译的内容比对。无实质变化 → 保持文件原样并说明；不要空转 `generated` 日期。有实质变化 → 分节变更清单（每节一行：新增 / 改写 / 删除 / 未变，以及变了什么）先落到正文，再经 AskUserQuestion 询问覆盖还是跳过——一份文档一个问题。超过两份文档有差异时，可合并为一个问题，对着编号列表问：*全部覆盖* / *覆盖其中几份（报编号）* / *全部跳过*——五份文档都有差异时已经放不进选项（规约 §7.3）。
...
- 五个确认点都走 AskUserQuestion：就绪门槛的草稿豁免（对未完成的树编译草稿）、无法识别的 OPT、选哪个根子树（多根树）、每次覆盖已生成文档、以及挡路的手写文档。若不可用（非交互 `kimi -p` 下，无人应答），退化为纯文本，且覆盖任何已存在文件前仍必须取得明确批准——没有明确批准也绝不越过就绪门槛。

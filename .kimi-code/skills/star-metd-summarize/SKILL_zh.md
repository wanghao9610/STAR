...
description: >-
  ...
  必须先给出分节变更清单并获批准。当用户运行 /skill:star-metd-summarize，或想把研究计划汇总 / 凝练成方法表述、
...
- 四道门都在对话里逐条问：无法识别的 OPT、选哪个根子树（多根树）、每次覆盖已生成文档、以及挡路的手写文档。若在非交互 `kimi -p` 下（无人应答），退化为纯文本，且覆盖任何已存在文件前仍必须取得明确批准。

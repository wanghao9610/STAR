...
description: >-
  ...
  evaluation → overview, which links the other four and so compiles last). Use when the user runs
  /skill:star-metd-summarize, or wants to summarize / consolidate research plans into a method write-up,
  produce overview, dataset, framework, training or evaluation documentation, or draft paper method
  material from the plans. Bilingual (en/zh).
---
...
- The four gates are all asked in the conversation: an unrecognized OPT, which root subtree (multi-root tree), each overwrite of a generated doc, and any hand-authored doc in the way. If it is unavailable (non-interactive `kimi -p`), fall back to plain text and still require an explicit approval before overwriting any existing file.

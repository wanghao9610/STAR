...
description: >-
  ...
  as /skill:star-metd-summarize [OPT] — overview, dataset, framework, training, evaluation; no argument
  ...
  stay with star-expt-analyst. Writes only metds/<OPT>.md, overwriting an existing one only after an
  approved section-level change list. Compiles only a finished tree — every top-level plan finalized,
  every leaf exec_status done — otherwise it stops and routes the unfinished work; a draft compile is an
  explicit choice, never the default. Use when the experiments are finished, the plans finalized, and the
  user runs /skill:star-metd-summarize or wants the plans consolidated into a method write-up.
  Bilingual (en/zh).
---

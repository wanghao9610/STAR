...
调用方式：`/skill:star-plan-reviser PLAN_NAME`，其中 `PLAN_NAME` 是 slug（`open-vocab-det-seg`）、数字前缀（`01`）或文件名（`01_mvp-verify_plan.md`）。不带参数则列出候选并询问——优先推荐有执行证据或已被标记失配的节点。第二个参数写 `drop` 或 `undrop` 时走丢弃模式——Workflow 的最后一节——而不是审查；模式词永远在第二位，因为第一个参数永远是计划名。

...
6. **证据式验收。**主 agent 亲自跑三层冒烟测试（`references/smoke_test_spec_zh.md`），报告"验证了什么"并附证据，而不是一句"能用了"（AGENTS.md §7）。Chats end, files do not：报告与版本快照写入 `wkdrs/env_<ENV_NAME>_<日期>/`。

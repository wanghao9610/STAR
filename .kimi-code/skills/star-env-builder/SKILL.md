...
6. **Evidence-based acceptance.** The main agent runs the three smoke layers itself (`references/smoke_test_spec.md`) and reports what was verified with evidence, not that it "works" (AGENTS.md §5). Chats end, files do not: the report and version freeze go to `wkdrs/env_<ENV_NAME>_<date>/`.

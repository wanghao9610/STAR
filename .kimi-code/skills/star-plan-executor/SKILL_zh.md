...
1. **先读再写**。动手规划任何改动前,先在 `${CODE_NAME}/` 里勘察——读子计划 §2 指向的模块/入口。产出一份"现状 vs §3 要求"的差距清单。绝不假设代码已存在;`code/` 可能是空的(只有 `.gitkeep`),此时计划要从零搭骨架——更好的做法是先用 `/skill:star-code-architect` 奠基参考代码库。参见 `references/orient_checklist_zh.md`。
...
- 若当前环境无法使用对话提问或 plan 模式(无头 / 脚本化),回退:把 EXEC_PLAN 以纯文本呈现,并在任何副作用前要求一次明确的纯文本批准——仍然先审批再执行,仍然重实验前停,任何同步回写子计划前仍需纯文本确认。

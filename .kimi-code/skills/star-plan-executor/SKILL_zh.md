...
**把 Step 5 交给 EXEC 档。**记录与决定都已定下时，按 secondary-model pool 路线用 `Agent` 的执行角色接手，携带 `involve=<level> tier=exec` 与合法 `auto=unattended`。不得改 EXEC_PLAN 或绕过计划级缺口；返回后重读 EXEC_LOG。模型路线不可用时在本地运行，不反复询问。

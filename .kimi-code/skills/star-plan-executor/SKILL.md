...
1. Dispatch a subagent with the contract in `references/agent_dispatch_spec.md`: this step's goal, the exact files to touch, how to run via conda, the step's own check, and "do **only** this step; return a structured result (changed / ran / check / blockers / handoff)".

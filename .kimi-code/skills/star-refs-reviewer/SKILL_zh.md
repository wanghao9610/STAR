...
3. **先确认形状，再精读。** 精读是贵的那一步：把约 15 条排好序的候选（标题 / 会议 / 年份 / 引用数 / 一句话理由）用一个问题（以文本列出，5–10 条标为推荐）交给用户，只读用户留下的。检索和分类不需要批准；精读和写笔记需要。
...
2. 读来源，提取**检索画像**：任务、方法的机制、设定与约束、提到的数据集与 baseline、这项工作想立的主张。检索前先用 3–4 行报出画像及其来源——画像错了，整轮就白跑。
...
检索可以按检索式分组分派,至多 3 组,每组拿到其中 2–3 条检索式、检索画像、以及**除过份数**的按 host 速率配额（写成具体数字）,并按 Step 4 那份候选返回格式返回——外加每条候选一段封顶的逐字 `abstract`（约 60 词）。这个字段不是可选的:主 agent 必须在本 skill 唯一的必答确认点上摆出约 15 条排好序的候选,并按 `refs_rubric_zh.md` 重排,而那份评分表的核心判据是与本方法的直接重叠度——只看标题、会议、年份和引用数根本判不了。重排是主 agent 的活:它自己一句话说不清理由的候选,直接丢掉。每组还返回 `failures: [{host, error, retries}]`,并把抓到的载荷缓存在本次运行自己的 `raw/` 前缀下、按它那个检索式分组划分,两组的前缀绝不重叠（规约 §6.4）。确认点之前要确认、不能只是相信:每个收集器重开一份缓存载荷,逐字核对其中两行与它返回的内容是否一致。Step 5 会为每个存活候选从头重抓权威记录,但那在确认点的下游——用户挑哪几篇精读是在这里定的,依据的是没有任何东西重开过的那些行。
...
精读可以并行分派给只读 subagent，至多 3 个并行，一个 subagent 一篇，各自按 `references/refs_rubric_zh.md` 的笔记收集器返回格式返回——不是"填好的模板",模板里有些字段只有真正写文件的那次会话才能填。只有 `depth_evidence` 带着真实的表题和一行数据时才接受 `depth: full`,否则降级。主 agent 负责写文件并亲自写 §5（与本项目的关系）——这一节需要方法上下文，也正是这篇笔记存在的理由。每个收集器拿到**除过份数**的按 host 速率配额（写成具体数字），并返回 `failures: [{host, error, retries}]`；它抓到的页面缓存在本次运行自己的 `raw/` 前缀下、按它那篇论文划分，两个收集器绝不共用一个前缀（规约 §6.4）。这和 Step 1、Step 4 给它们各自收集器的是同样三样东西——这一步抓的是另一批 host，不是另一套规矩。
...
扩展可以按核心论文分派,**一个收集器一篇**,至多 3 个——默认用单个串行收集器就好,因为 `source_policy_zh.md` 里的请求预算是**按 host** 算的、不是按 agent 算的,三个并发收集器会把本 skill 承诺要礼貌相待的每个 host 的真实速率翻三倍。引那份文件,不要在这里复述它的数字——数字在那里维护。每个返回 `candidates: [{title, first_author, year, venue, citation_count, external_ids, found_via, why: <一句话>}]`、`queries_run`、`failures: [{host, error, retries}]`、`papers_seen`,别的都不返回。它不抓任何文献记录,也不判定什么算核心论文;它抓到的载荷缓存在本次运行自己的 `raw/` 前缀下、按它那篇论文划分（conventions §6.4）,除此之外不写任何东西。它返回的东西一律不当作 bib 字段来信——Step 5 会为每个存活候选从头重抓权威记录,所以一个抄错的标题代价只是一次匹配失败,转入待人工核查。
...
≤400 字：方法来源与画像、写了哪些笔记（citekey → 文件）、条目数与类别表、自查结果、待人工核对清单，以及转交去向——最接近工作的结论交 `/skill:star-plan-coach` §2（相关工作与定位）去磨定位；以后单加一篇是 `/skill:star-refs-reviewer <arxiv-id>`；`/skill:star-refs-reviewer verify` 重查整个 bib；`/skill:star-refs-reviewer synthesize` 把笔记合成为 `metds/refs/related_work.md`。
...
3. 笔记是唯一来源，其 `depth:` 是上限：刻画一篇论文只能依据它自己的笔记，且不得深于笔记承认的深度。没有笔记的 bib 条目可以在主题里被提到（只用其记录的事实：标题、会议、年份），但绝不刻画。任何内容不得来自记忆。单薄到写不动的主题记为缺口，列出该补读的论文（逐篇 `/skill:star-refs-reviewer <arxiv-id>`）——绝不注水成文。
...
- `related_work.md` 只编译、不发明：每条刻画都能追溯到对应论文的笔记（没有笔记的条目只能以其抓取记录的事实被提到）。带 `type:` + `generated:` frontmatter 的生成文件，须先批准节级变更清单才能覆写；人写的文件绝不凭 diff 覆写。

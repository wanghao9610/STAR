...
<!-- Written by /skill:star-refs-reviewer (model_id: <model id, copied verbatim from what your runtime states this session — your Kimi session reports it where available; "unrecorded" only if the session names none>). This file is the audit trail for reference.bib: every entry's
...
<!-- Gaps worth another pass (a thin category, a sub-topic not covered). Routing: sharpening the
     ...
     /skill:star-refs-reviewer <arxiv-id>; re-checking the bib → /skill:star-refs-reviewer verify. -->

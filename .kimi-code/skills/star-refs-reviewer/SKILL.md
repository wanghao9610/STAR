...
Reading may fan out to read-only subagents, at most 3 in parallel, one paper each, each returning the note collector contract in `references/refs_rubric.md` — not a filled template, which asks for fields only the writing session may set. Accept `depth: full` only where `depth_evidence` carries a real table caption and row; otherwise downgrade it. The main agent writes the files and owns §5 (Relation to This Project) — that section needs the method context and is the reason the note exists. Each collector is given its **divided** per-host rate share as a number and returns `failures: [{host, error, retries}]`; the pages it fetches are cached under this run's own `raw/` prefix, scoped to its own paper so no two collectors share a prefix (conventions §6.4). These are the same three things Steps 1 and 4 give theirs — this step fetches from different hosts, not under different rules.

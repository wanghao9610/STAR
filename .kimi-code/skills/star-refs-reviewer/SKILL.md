...
description: >-
  ...
  metds/refs/; an arXiv id, DOI, or paper URL appends one paper. Use when the user runs
...
- The core-set confirmation is the only mandatory question — one question listing the candidates as text, recommendations marked. In non-interactive `kimi -p` runs (no human to answer), fall back to plain text and require an explicit answer before reading.

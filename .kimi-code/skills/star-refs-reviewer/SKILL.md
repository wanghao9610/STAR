...
You survey and record; you do not set strategy, write or revise plans, implement methods, or run experiments. What you find that changes the research direction goes back to the user and to `/skill:star-plan-coach` §2 — you never edit a plan yourself.
...
Write `metds/refs/<slug>_survey.md` — a standalone map of a field, read in tiers and organized by taxonomy. It needs no notes, no bib, not even a plan, and it writes neither `reference.bib` nor `refs_index.md`: promoting a paper it surfaced into the verified bib is one `/skill:star-refs-reviewer <arxiv-id>` later. `<slug>` is the plan's slug when a plan is the source, else the topic kebab-cased to ≤5 words.

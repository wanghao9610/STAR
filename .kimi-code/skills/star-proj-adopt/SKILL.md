...
- All three confirmation points go through AskUserQuestion, one question per call. If it is unavailable (non-interactive `kimi -p`, no human to answer), fall back to plain text — still one at a time, still an explicit answer before any write past a confirmation point.

...
5. **接入不发明研究策略。** 你能读出建了什么、跑了什么；你读不出为什么、它支撑哪条主张、什么情况下本该叫停。清单只作描述，§4 那类主张和 kill-criteria 留给 `/skill:star-plan-coach` 去向用户问出来。**从 git log 编造出来的计划树，比没有计划树更糟。**
...
通过 AskUserQuestion 提问，一次一问，只问勘察定不下来的：哪个目录是 `CODE_NAME`、哪个解释器是 `PYTHON_HOME`、哪些已有目录是数据 / 权重 / 输出根。选项取自勘察结果并标出推荐项。这个确认点关上之前，什么都不写。
...
列出勘察找到的既往 run——路径、日期、看起来产出了什么、日志里可见的指标。通过 AskUserQuestion 一次性对着编号问：哪些要记录到 wkdrs/——超过四个 run 就装不进选项（规约 §7.3），给成 *全部记录* / *记录其中几个（报编号）* / *都不记录*。对每个被选中的 run，软链到 `wkdrs/<run>/`，并按 `assets/exec_log_reconstructed_zh.md` 写一份最小 `EXEC_LOG.md`——重建头部（原则 4）、可复原时写出命令、现存产物，且明确不含步骤表。其余的仅作为证据留在清单里，报告中说明有多少个没有入账。
...
用户通过 AskUserQuestion 逐个确认——条目较多时给提议行编号、用一个问题问（*全部确认* / *确认其中几行（报编号）* / *都不确认*），四行及以下时一个一问。未获确认的 leaf 原样不动。用户标为 `done` 但没有入账 run 的 leaf 是允许的，并要记一笔：`/skill:star-flow-status` 会把它标为 done-with-no-run，那正是诚实的状态。
...
- 三个确认点全部通过 AskUserQuestion，每次调用只问一个问题。若该工具不可用（无人值守 / 脚本化运行），改用纯文本——仍然一次一问，且在任何跨确认点写入之前必须拿到明确答复。

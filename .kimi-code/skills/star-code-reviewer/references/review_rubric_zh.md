...
问题项如何形成、分级、返回。读代码本身，别只读名字：范围内每个文件过全部维度（F 仅计划模式）。一条问题项必须给出维度、严重度、违反的规则、确切位置和具体修法——没有成文评判依据支撑的抱怨是风格偏好，不是问题项。两个维度重叠时（未使用的 import 既是 C 也是 E），只记一次，归入更具体的维度。
...
- §4 中属于代码、或由范围内代码产出的每个交付物：在声明的路径上吗？只有 EXEC_LOG“待用户执行”里那条尚未跑的红线命令才能产出的交付物，记为 `pending` 并写明是哪条命令——绝不记为缺失，否则每一次在算力烧掉之前做的审查，都会产出一条谁也处理不了的 major。

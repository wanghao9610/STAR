...
<!-- 由 /skill:star-code-reviewer 写出（model_id: <模型 id，照抄运行时本会话为你声明的那串——Kimi 会话有报告就照记；仅当本会话未声明任何模型才写 "unrecorded">）。模式：full | plan | path | diff；目标：<原样的参数>；
...
<!-- 越界 finding 的路由：功能缺口 → /skill:star-plan-executor <叶子>；计划文本偏差 →
     /skill:star-plan-reviser <slug>；结构性重组 → /skill:star-code-architect；环境不可用 →
     /skill:star-env-builder。然后列修复 pass 候选（机械 findings，按编号）。 -->

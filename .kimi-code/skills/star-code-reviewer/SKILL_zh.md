...
description: >-
  ...
  star-plan-reviser，结构性 重组转给 star-code-architect。当用户运行 /skill:star-code-reviewer、一次运行点名它是下一步动作，或想审查 /
...
5. **修复是例行的、逐项批准的、不改行为的。** 报告之后提供修复轮，只覆盖 docstring、作用域内改名、unused imports、本项目引入的死代码。每项先经 AskUserQuestion 批准再写入——一次一条问题项（或一批同类项），标出推荐——写入后复检。绝不打包默批；绝不顺手"改进"相邻代码（AGENTS.md §3）。
...
2. 按报告顺序经 AskUserQuestion 走可修问题项——*照建议修* / *调整后修* / *跳过*，标出推荐，一次一条。同类超过 4 条（如 12 处缺 docstring）可合并为一问：*全修* / *选哪些（multi_select）* / *全部跳过*。
...
- 修复轮的批准一律经 AskUserQuestion——一次一条问题项（或一批同类项）。不可用（非交互 `kimi -p` 下，无人应答）时退化为纯文本，仍然一次一条，任何写入前先获明确批准。

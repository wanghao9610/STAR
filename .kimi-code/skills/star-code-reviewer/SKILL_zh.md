...
≤400 字，结论先行：审了多少文件、各严重度数量、top ≤10 问题项一行版（`file:line — 问题`）、符合度结论（计划模式）、跑了哪些静态工具。结尾给出越界问题项的转交去向（`/skill:star-plan-executor` / `/skill:star-plan-reviser` / `/skill:star-code-architect`），然后若有例行问题项，提议修复轮——用户也可以就此打住；写出的报告本身就是完整交付物。

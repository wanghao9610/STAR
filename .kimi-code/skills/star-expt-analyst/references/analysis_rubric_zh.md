...
如何检查一个 run、记录什么、判定如何得出。每个 run 都过全部维度：A 与 B 说明存在什么，C 说明这个 run 可不可信，D 对照计划打分，E 说明它意味着什么，F 说明下一步归谁。一条观察必须给出维度、严重度、来源和影响——没有文件支撑的评论是印象，不是观察。

下文以 §n 引用的项目守则是 AGENTS.md 的编号章节；*计划*的 §1–§6 指子计划自己的章节。技能名直接写出（`star-plan-executor`）；按当前工具调用技能的方式调用它们。
...
收集器只返回这两个列表（外加 `files_read: <n>`），别的都不返回：不对 run 下判定、不做解读、不写文件。维度 C 的收集器返回观察；只有当某个指标唯一的来源就是它自己清单里那份日志的某一行时，它才填一条指标行——因为 D 的来源阶梯是跨文件的比较，属于手上握着全部来源的那个 agent。除这两个列表外，它每份日志还返回一个 `config_echo`——`{source: <路径>:<起–止行>, lines: <逐字照录的 config / argv / 数据路径块，≤40 行，只留数据路径与 split 那部分>}`，日志里没有回显则为 `none`——原文照录，不转述也不评判：E 里的泄漏与"好得可疑"检查由主 agent 拿它来跑，只有命中时才回去展开被引用的那段。`files_read` 少于交给该收集器的文件数，意味着差额要重新派发（conventions §6.3）。

## 严重度分级（观察）

- **blocker** —— run 的结果不可信或不可用：进程在产出结果前就死了、评测读了训练集、checkpoint 为空或损坏、日志里引用的指标在它所指的文件里根本不存在、完成判据所依赖的某个 §4 交付物缺失。
- **major** —— 实质改变解读：§5 判据未达标、训练出现 NaN/Inf、loss 发散、train↓/val↑ 背离、某条红线命令从未跑过、产物写在布局规则（§5）之外、指标只能从弱于计划暗示的来源拿到。
...
- **met** —— 每条 §5 判据都检查过且达标，且没有 blocker 级问题动摇它们。
- **partially met** —— 部分达标、部分未达标，但都检查过了。
- **not met** —— 判据检查过且未达标。这是一个真实的、可上报的结果。
- **inconclusive** —— 用来判断的证据根本不在：红线命令从未跑过、run 提前停了、指标哪里都找不到。它不是 "not met" 的同义词——要说清缺的是哪份证据、什么能产出它。
...
不算观察：正常 run 会留下的常规残留（`__pycache__/`、`events.out.tfevents.*`、`.lock`、编辑器临时文件）；框架自动生成的产物（config 快照、`latest.ckpt` 软链）。

## B. 完成度核实

- EXEC_LOG 里每个标了 `done` 的步骤：它写明的产物存在吗？与该步骤声称产出的东西相符吗？`done` 而产物不存在是 **blocker**——日志对现实的描述是错的。
- 每条 "Awaiting user" 红线条目：`用户已跑`（它承诺的输出存在）还是 `仍待跑`（不存在）。绝不因为时间过去了就假定它跑过了。
...
- **对照主张**：子计划的 `traces_to` 指明了这个 run 服务于根计划的哪条主张或哪一节。如实说明结果是支持它、推翻它，还是让它悬着——"悬着"的话，还缺什么。
- **kill-criteria**：把结果对照根计划 §5 的 kill-criteria，以及计划称为低开销早期测试的任何 MVP 完成判据。命中就是**方向性信号**：在报告里突出、转交出去（F），绝不软化。能早早杀掉一个坏点子的计划，是在起作用。
- **泄漏与结果好得反常的排查**——接受一个强结果之前先跑这些：训练 config 的数据路径里点到了 val/test split 吗？val ≈ train 到了不合常理的程度吗？这个数字在第一次跑就超过了已发表的 SOTA 吗？指标是不是贴着天花板（1.000、100%）？checkpoint 是不是在报告它的那个 split 上选出来的？任何一条命中 → 在用户排除之前，判定就是 `invalid`。
...
| 分析发现了什么 | 转交给 |
| --- | --- |
| 步骤未完成、某步 `blocked`，或红线命令仍待跑 | `star-plan-executor`（恢复该 run） |

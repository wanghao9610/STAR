...
4. **Light parsing only; tools are evidence, never installed.** Read files, grep logs, and run small parsing snippets through the `.env` conda env. pandas / matplotlib / tensorboard are used **only if already installed**; absent, the analysis narrows — text-only, no curves — and the report says so. Never install or upgrade anything (that is `/skill:star-env-builder`'s).

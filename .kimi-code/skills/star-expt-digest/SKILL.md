...
description: >-
  ...
  a window; `all` re-seeds from the beginning. Collects each in-scope run's newest EXPT_ANALYSIS report,
  tabulates verdicts and headline metrics with their provenance, derives what moved since the previous
  digest (new runs, changed verdicts, newly analyzed runs), gathers strategy signals and kill-criteria
...
Your product is a dated digest — the entry a researcher reads back before a supervisor meeting, a weekly report, or picking work up after two weeks away. It carries narrative the ledger is forbidden to carry: what moved, what got refuted, which direction changed. It is not a results table, and it never becomes the source anyone quotes a number from.

You read and narrate; you do not execute, analyze runs, score criteria, revise plans, or flip status. What the digest surfaces beyond your write boundary is routed: an unanalyzed run to `/skill:star-expt-analyst`, a stale ledger to `/skill:star-expt-analyst aggregate`, an unexecuted leaf to `/skill:star-plan-executor`, a refuted claim to `/skill:star-plan-reviser`, the current state of the tree to `/skill:star-flow-status`.
...
Per run, from its newest `EXPT_ANALYSIS_<date>.md` only: the run verdict, the §5 scorecard in one line, the headline metrics with the source and split the report records, and any blocker/major observation or strategy signal it names. Do not open the run's raw logs — the report is the interface, and going behind it is per-run analysis, which is `/skill:star-expt-analyst`'s job with its own verification.

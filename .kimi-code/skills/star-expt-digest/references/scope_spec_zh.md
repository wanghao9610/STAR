...
一份 digest 有多诚实，取决于它的周期。本文件规定参数如何变成 run 集合、run 如何定日期，以及序列如何续接而不漏算、不重复。
...
**plan 模式取的是家族，不是子树。** 给定一个节点，run 集合是其**全部后代叶子**的全部 run——那才是证据。它**上溯到根的祖先**也要读，但只作语境：根的 §4 claim 映射与 §5 kill-criteria 才让一个叶子的数字有意义，digest 的头条要对着它们写。祖先自身不贡献 run；内部节点不可执行（规约 §5.4）。这就是"包含父计划与子计划的结果"在实操上的含义：父提供问题，后代提供答案。
...
**绝不用文件 mtime。** 它会因为一次 checkout、一次格式化、一次 `cp -r`、一次备份而改变——一份按 mtime 定日期的 digest 会悄悄打乱历史。这与规约 §4 对写入日期的要求、以及 `status_spec.md` 对过期判断的要求是同一条纪律：比对记录下来的值，绝不比对文件系统时间戳。
...
水位线是最新一份 digest 的 `covers.through`——从 frontmatter 读取，绝不推断。正是它让不带参数的 `/skill:star-expt-digest` 等于"自上次以来"。
...
这条规则保护的是：**一次向后看的 digest 绝不能导致下一次增量运行漏掉工作。** 由于水位线取的是"最新一份 digest 的 `covers.through`"，一份 `covers.through` 是旧日期的 `plan` 模式 digest 在取 max 时自然会被忽略——但如果某个家族的 run 全部在今天跑完，而今天又写了一份 `plan` 模式 digest，它就会污染水位线。所以下一次增量运行取的是 **`mode` 为 `incremental`、`window` 或 `all` 的那些 digest 中**最新的 `covers.through`，完全忽略 `plan` 模式的文件。

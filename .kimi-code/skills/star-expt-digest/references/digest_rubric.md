...
The run holds an `EXPT_ANALYSIS_<date>.md`. Take the **newest**. From it, and from nothing else — sections **2, 3 and 7** of the report carry all of it, which is why the second scan is run with `--bodies 2,3,7 --runs <the in-scope runs>`; a report that renumbers its sections changes this line and nothing in the script:

...
### Step 7：下一步动作
按优先顺序选出唯一的下一步：待用户的 STOP 命令（该 run 还没有审查报告时，`/skill:star-code-reviewer <叶子>` 排在这条命令之前）→ 已完成工作的欠账 → 下一个可执行叶子 → 已定稿但未立项的想法。给一句话理由和确切命令。 当这条命令落在那八个 agent 可以自己启动的 skill 上、且目标已经定死时，它会在本次运行结束后被直接拾起来跑，而不是留给用户去敲——本 skill 自己不启动任何东西（规约 §10）。若无合格者，写明挡路者。

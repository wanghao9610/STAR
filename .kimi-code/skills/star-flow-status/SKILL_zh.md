...
### Step 1：扫描
以项目根目录为工作目录，运行一次收集脚本：`bash <本 skill 所在目录>/scripts/scan.sh`——它打印的每个路径都相对该根目录。它打印一份摘要：每份计划的 frontmatter、`## Sub-plans` 索引与 §3/§5 的占位符计数（`[TBD]` 与 `【待定】` 一并计入）；每份 run 日志的 frontmatter、步骤表、待用户勾选项、方向性信号与出现过的日期；其余每个注册产物的 frontmatter；以及 `metds/` 与 `wkdrs/` 深度 1 的文件清单。这就是 Step 2–9 的全部输入。run 目录超过约 20 个、且问题本身只针对某个子树时，可以加 `--runs <目录列表>`，把逐 run 的正文索引与日期行限制在这些 run 上；每个 run 的 frontmatter 照样打印，而 PLANS、ARTIFACT FRONTMATTER、LISTING 与 DIRS 仍然覆盖全项目——子树本身要先靠每个计划的 `parent:` 才解析得出来，而未识别文件那一行是按全项目计数的。落在范围外的 run 会被点名为已省略，绝不悄悄丢掉。
...
### Step 5：进度汇总
报三个数：总体方向完整度（各总体计划里 done 的章节数）、拆解覆盖度（叶子 vs 仍然粗糙的节点）、执行进度（叶子 `done` / 总数，以及日志里步 done / 总数）。

...
Show, per leaf line, its `depends_on`, (if executing) `k/n` steps, and — when the current run records an execution branch — its merge state: `<run> unmerged`, or `merged` once the log says so. Example:

...
给一句话理由和命令。 当这条命令落在那八个 agent 可以自己启动的 skill 上、且目标已经定死时，它就是接下来要跑的东西，而不只是打印出来的东西——拾起发生在本次运行结束之后，本 skill 自己不启动任何东西（规约 §10）。若各层都给不出候选，说清是以下哪一种：不欠任何东西且每个叶子都已了结——整项研究已经完成，转交给 `/skill:star-code-release`，或用 `/skill:star-idea-storm` 开下一个题目；某个未满足的依赖（写明）；需拆解的粗糙叶子；或 `metds/plans/` 为空（转交给 `/skill:star-plan-coach`；连想法也没有则 `/skill:star-idea-storm`）。

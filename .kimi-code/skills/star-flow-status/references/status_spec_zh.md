...
- `⚠` 需关注 —— **太粗、无法直接执行**，有两种情形：其自身 §3/§5 大量 `[TBD]` 的叶子，**或**带着 `finalized:` 却没有 `children:` 的总体计划——从未被拆解过，因而算作叶子（规约 §5.4），执行器会拒绝它。这是唯一的定义；第 3 层的资格判定一律依它，与生命周期规则最终渲染出哪个状态符号无关 → 建议 `/skill:star-plan-decomposer`。
...
给一句话理由和命令。若各层都给不出候选，说清是以下哪一种：不欠任何东西且每个叶子都已了结——整项研究已经完成，转交给 `/skill:star-code-release`，或用 `/skill:star-idea-storm` 开下一个题目；某个未满足的依赖（写明）；需拆解的粗糙叶子；或 `metds/plans/` 为空（转交给 `/skill:star-plan-coach`；连想法也没有则 `/skill:star-idea-storm`）。

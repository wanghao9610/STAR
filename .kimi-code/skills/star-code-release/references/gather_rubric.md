...
3. Still unresolved → the row's action is `route`, not a guessed path. Creating a directory `codearc.md` does not name is `/skill:star-code-architect`'s decision, and a release run that invents `${CODE_NAME}/utils/` has quietly amended the architecture spec without a gate.

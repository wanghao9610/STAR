...
1. **数字是抄的，不是算的。** §8 和 §12 里每个数值都从 `wkdrs/results/results.md` 原样誊写，连同账本引用的 run 名一起。被账本排除的数字（它的 `invalid` / `inconclusive` 小节里那些）根本不进 README——加注解也不行。账本不存在时，§12 省略，报告路由到 `/skill:star-expt-analyst aggregate`。

...
- 所有命令走 `.env` 的解释器；绝不安装或升级任何东西（环境归 `/skill:star-env-builder`）。STOP 线成立：不训练、不做全量评测、不做高成本 API 调用——账本里缺的数字就留成 TODO。
...
- 所有 gate 都在对话里逐条问，一次一个问题：参数无法识别时的阶段选择、来源大面积缺失时的就绪决策、提升表的 Gate 1、每条打磨 finding、每处 README 小节变更、STAR README 或人工 README 的替换确认、以及中文 README 的提议。在非交互 `kimi -p` 下（无人应答），退回纯文本，仍然一次一个，且任何写入前都必须拿到明确批准。

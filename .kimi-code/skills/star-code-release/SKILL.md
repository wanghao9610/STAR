...
- Never write `metds/**` — not the plans, not `codearc.md`, not the compiled method documents, not `metds/refs/*`. Every one has a producer, and a release run that edits its own source is no longer compiling. Never write the results ledger `wkdrs/results/` (`star-expt-analyst aggregate`'s), `EXEC_PLAN.md` / `EXEC_LOG.md`, `.env`, `datas/`, `inits/`.
...
- All commands run through `.env`'s interpreter; never install or upgrade anything (`/skill:star-env-builder` owns the environment). The STOP line holds: no training, no full-dataset evaluation, no costly API calls — a number the ledger lacks stays a TODO.

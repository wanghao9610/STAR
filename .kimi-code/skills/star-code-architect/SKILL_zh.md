...
5. **主 agent 编排与复核，subagent 执行。**勘察与迁移交给职责单一的 subagent，文件所有权互不相交，返回结构化结果。主 agent 亲自重跑每项检查（不信任自报的 pass），每验证完一组就打一个 git 检查点，重试 ≤2 次，仍失败则回滚。格式约定见 `references/orchestration_spec_zh.md`。

...
≤400 字：选定的仓库（附许可证说明）、落了什么在哪里、完成的改名 + 保留未改的名称数量、迁移完成/受阻情况、写入的规范、验证证据、待用户执行的命令。**向下游交棒：**`/skill:star-plan-executor <leaf>` 现在有代码可改了；`/skill:star-flow-status` 查看每个计划步骤的进展。

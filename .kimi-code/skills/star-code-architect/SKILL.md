...
5. **The main agent orchestrates and verifies; subagents execute.** Surveys and migrations are delegated to narrow subagents with disjoint file ownership and structured returns. The main agent re-runs every check itself (never trusts a self-reported pass), commits a git checkpoint per verified group, retries ≤2, and rolls back what still fails. Contract: `references/orchestration_spec.md`.

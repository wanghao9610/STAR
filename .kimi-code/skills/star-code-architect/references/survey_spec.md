...
One subagent per area, at most 3 in parallel (they are read-only, so parallelism is safe):
...
**Light mode** (Branch A fresh clones, or repos under ~50 Python files): collapse to a single pass over all areas — by one subagent or the main agent itself.

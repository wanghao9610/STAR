...
阶段 2 的衔接：最接近的工作及其局限应当是读出来的，不是回忆出来的。分档来读：若 `metds/refs/related_work.md` 已存在，它本身就是编译好的叙述——读它加上 refs 索引的 §2，到此为止。否则，笔记 ≤ ~6 篇时直接读它们的 §5。只有超过这个数，才值得派一个有界的收集器，而它返回的是 citekey、笔记路径和**逐字照录**的 §5 引文——绝不是转述，因为 §5 本身就是 `star-refs-reviewer` 主 agent 的综述。无论走哪一档，都要拿你引用的 citekey 去 grep 一遍 `reference.bib`：文件里没有的 citekey 是一处断掉的引用，不是排版细节。若 `metds/refs/` 什么都没有，建议**先**抽身去跑 `/skill:star-refs-reviewer`，再用 `/skill:star-plan-coach <slug> related_work` 回来续写——凭记忆写定位，正是这一阶段要防的失败。用户若不愿意，就基于他已知的内容继续，并标出日后需要调研确认的部分。计划据 idea 文件起草时，其 §3 的扫描表为这一阶段点出首批候选——但那些只读到摘要深度：它们为调研指路，不能替代调研。

### Step 7：收尾质检

全部章节 `done`（或 `skipped`）后：在列出任何东西之前，先把评分表交出去做一次"不知情"的复核：一个只读子代理，交办材料正好两个文件——写完的计划，和 `references/plan_rubric.md`（计划 frontmatter 写着 `language: zh` 时改点名 `_zh` 那份；受托者绝不自己选）——范围逐字写明："只看这两个文件。产生这份计划的那场对话你没有参与。不要排序，不要下结论。"它逐条返回：`item`、`verdict: pass | fail | unclear`、`evidence`（引用的那一行，或者把"缺了什么"确切写出来）、`fix`，别的都不返回。交给它的只有这两个文件路径：把计划是怎么想出来的一并贴给它，它就不再"不知情"了，而这份不知情正是派它出去的全部意义。这一个受托者为什么值得派（conventions §6.7）：评分表最锋利的几条都是缺失检查——每条主张都有对应实验、有明确的改进阈值、有 seed 与方差的说明——而页面上没有任何东西会跟一句没写下的话矛盾，作者对此是结构性看不见的；而 `finalized:` 又是下游四个 skill 用来信任这份计划的信号。主 agent 对每条它准备提出的 `fail` 重读被引用的那一行——缺失型的 `fail` 没有行可引，就把该条目所属的整节重读一遍，再决定接受还是丢掉——然后排序、询问，并独自写下 `finalized:`。子代理只负责打分，不做决定：主 agent 确认不了的 `fail` 直接丢掉；这份计划算不算完成，仍然是用户说了算。没有受托者可用时，就照原样自己跑一遍评分表（conventions §6.1）。然后，读 `references/plan_rubric.md`（中文对话读 `references/plan_rubric_zh.md`），逐项检查计划质量。把不达标项列给用户（最多 5 条，按重要性排序），询问是否回到对应章节补强。用户表示满意后，在 frontmatter 加 `finalized: <日期>`——重开过的计划替换旧日期，不要两个并存。`finalized:` 的含义就是这个，没有更宽松的解释：六节全部 `done` 或 `skipped`，且评分表跑过并给出了答复。它是下游 skill 用来判断这份计划能否驱动它们工作的唯一信号，所以除此之外没有任何东西会设上它，重开一节则清除它。

**向下游交棒。** 定稿后，告诉用户推荐顺序：若 `${CODE_NAME}/` 还是空的，先用 `/skill:star-code-architect` 为方法搭好代码库（它读的正是这份根计划），并用 `/skill:star-env-builder` 建好运行环境，再用 `/skill:star-plan-decomposer <slug>` 把总体方向落成可执行子计划——代码库已存在时写出的叶子能点到真实模块，而不是猜路径。子计划树建立后，`/skill:star-flow-status` 给出整棵树的总览。并就计划文件提供一次提交提议（见状态与文件规则）。
...
- 若当前环境无法使用对话提问（无头或脚本化运行），回退为普通文本提问——仍一次只问一题。

...
每次会话一个 `###` 块，真实日期（绝不编造），并在 skill 名之后写上本次编辑会话的 `model_id`——运行时报出的 id 原样抄录，没有则写 `unrecorded`（规约 §8）。这个逐条记录的 id 正是计划的模型归属所在：frontmatter 的 `model_id` 只点名最近一次写入者，而本节保留了每一次更早修订各自出自谁手。每处改动一个要点：章节、改了什么、为什么、证据。`exec_status` 的重置、以及被清除的 `finalized:` 也记在这里；值得留痕的被拒候选可选记一笔（"用户保留 85 阈值，尽管未达标"）。

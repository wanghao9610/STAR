#!/usr/bin/env bash
# STAR PreToolUse hook (DeepSeek Harness) — decline the git commands that break the
...
#
# Registered under PreToolUse matching bash in .dsh/hooks.json, which DSH reads
# through its Claude Code hook bridge. The matcher names DSH's own tool, which is
# lowercase `bash`: the bridge matches a plain `[A-Za-z0-9_|]+` pattern literally,
# so the other trees' `Bash` would match nothing here and say nothing. One copy
# per harness — Claude, Codex, Qwen Code and Kimi Code carry the same guard on
# their own PreToolUse, Cursor on beforeShellExecution — differing only in how
# each harness names the command on the way in and the decision on the way out.
#
...
deny() { # $1 = one-line reason
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"%s (declined by .dsh/hooks/star_commit_guard.sh — hand it to the user to run)"}}\n' "$1"
    exit 0
}

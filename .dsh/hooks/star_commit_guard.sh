...
# their own PreToolUse, Cursor on beforeShellExecution, Pi on its tool_call event
# — differing only in how each harness names the command on the way in and the
...
deny() { # $1 = one-line reason, $2 = what to do instead (default: hand it to the user)
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"%s (declined by .dsh/hooks/star_commit_guard.sh — %s)"}}\n' "$1" "${2:-hand it to the user to run}"
    exit 0
}

#!/usr/bin/env bash
# STAR SessionStart hook (DeepSeek Harness) — inject the project's memory index into session context
...
# is loaded anyway.
#
# Registered under hooks.SessionStart in .dsh/hooks.json, which DSH reads through
# its Claude Code hook bridge. Prints one JSON object whose additionalContext is
# added to context before the first prompt. The JSON is not optional here the way
# it is elsewhere: the bridge consumes `additionalContext` and ignores plain
# stdout, so a copy that printed prose would run, succeed, and inject nothing.

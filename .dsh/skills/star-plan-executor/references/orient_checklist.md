...
2. **Map the code root.** List `${CODE_NAME}/`. If it holds only `.gitkeep` (empty), declare **empty codebase**: the plan scaffolds structure from scratch and the gap list is "everything".
...
**Output of this step:** a gap list (per §3 step: exists / modify / create, with real paths) plus the launch entry point you identified. This feeds directly into Step 3's EXEC_PLAN.

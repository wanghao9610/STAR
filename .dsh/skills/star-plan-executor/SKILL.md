...
**Hand Step 5 to the EXEC tier.** DSH's `subagent` exposes no model selector, so a configured EXEC value cannot move this phase to that model. Run Step 5 here; individual bounded actions may still use `subagent` below, without type or model parameters. State the unavailable model route once when the key is set.

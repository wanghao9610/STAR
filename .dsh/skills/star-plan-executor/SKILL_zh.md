...
1. 请用户打开 plan 模式。DSH 没有进入它的工具——plan 模式是由人来选的会话状态，而 `exit_plan_mode` 在模式之外调用会失败。用户不开、或当前界面根本没有 plan 模式时，Step 4 走它的纯文本兜底，其余一切照旧。
...
1. 按 `references/agent_dispatch_spec_zh.md` 的格式约定派一个 `subagent`（只读勘察可用 `explore`）:本步目标、要碰的确切文件、已解析的解释器路径、绑定的 check,以及"**只**做这一步;返回结构化结果(changed / ran / check / blockers / handoff)"。

...
description: >-
  ...
  star-plan-executor，或某个子计划需要落地实现时使用。Bilingual（中/英）。
---
...
1. 请用户打开 plan 模式。DSH 没有进入它的工具——plan 模式是由人来选的会话状态，而 `exit_plan_mode` 在模式之外调用会失败。用户不开、或当前界面根本没有 plan 模式时，Step 4 走它的纯文本退路，其余一切照旧。`low` 档不请用户开：那一档 Step 4 不问就放行，EXEC_PLAN 在 plan 模式之外起草，其余不变。
...
2. **没有例外,也没有要问的。** 探索性叶子、命令很便宜、赶时间的 run——都不能跳过审查:它的代价比它可能省下的任何一次运行都小,而且启动守卫(规约 §2)会拒掉所属 run 里没有审查的备好命令,跳过的审查只会在一次被拒的启动之后补回来。这一步没有任何问题要问。

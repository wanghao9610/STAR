...
**本次运行在哪里执行。** 本次运行的档位是 PLAN；它留在启动它的会话里，用会话的模型跑。`STAR_PLAN_MODEL` 所指的模型不是会话正在跑的模型，或带有本宿主只能逐次派发时应用的深度时，在开头用一行说明——档位、该模型与深度，以及在这里得到它们的唯一办法：切换会话模型——随项目使用的 `subagent` 工具选不了模型，`star-auto` 也就无法按档位启动这次运行（规约 §10.8）——然后在这里照常继续。带 `tier=` 的运行已按档位启动，不给这一行。档位变化是后文把执行—验证阶段（Step 5）交给 EXEC：判断交接前还缺哪些决定时，既有执行授权视为已定；带 `tier=` 的受托者不再次转交这一阶段。
...
**实现前先按 `references/design_check.md` 跑设计检查。**派一个只读 `subagent`，不传 type 或 model，只提供 EXEC_PLAN、叶子计划和根计划 §4。重开每条 `fail` 证据；只有研究范围、验收、关键输入或成本仍未解决时才问。不可用时本地跑一次并记录限制。
...
**把 Step 5 交给 EXEC 档。**DSH 的 `subagent` 不能选择模型，因此已配置的 EXEC 值不能迁移整个阶段；Step 5 留在本地。下面的有界 action 仍可用不带 type/model 的 `subagent`。键指定的模型不是本 run 所在模型的别名（规约 §10.8）时只说明一次原因。

...
2. **让计划可见，再在范围内推进。**建立并展示具体 `EXEC_PLAN`。DSH 不能主动进入 plan mode；若用户已置于其中且仍有实质决定，解决后用 `exit_plan_mode` 退出。否则直接继续，不另设批准门槛。
3. **该委派时才委派。**有边界、相互独立的工作确有收益时使用 `subagent`，不传 type 或 model；该工具不能按次选择档位模型。主 agent 负责集成与核验，仅在证据缺失、过期或受整合影响时重跑。
...
1. DSH 不能进入 plan mode 只在计划仍有待用户决定的实质选择时使用；`low` 或目标、范围、验收、关键输入、成本与权限已定时，在当前模式起草，不制造批准门槛。
...
**实现前先按 `references/design_check_zh.md` 跑设计检查。**派一个只读 `subagent`，不传 type 或 model，只提供 EXEC_PLAN、叶子计划和根计划 §4。重开每条 `fail` 证据；只有研究范围、验收、关键输入或成本仍未解决时才问。不可用时本地跑一次并记录限制。

1. 在 commentary 展示具体计划。沿用已有授权，只为仍未解决的实质选择使用本宿主原生提问机制；DSH 不能进入 plan mode 已启用时在决定解决后退出。提交、分支与 worktree 按 involve 档位处理；`auto=unattended` 仅覆盖不改变范围、关键输入、§5 验收和已批成本的战术偏差。
...
**把 Step 5 交给 EXEC 档。**DSH 的 `subagent` 不能选择模型，因此已配置的 EXEC 值不能迁移整个阶段；Step 5 留在本地。下面的有界 action 仍可用不带 type/model 的 `subagent`。键已设时只说明一次原因。
...
1. 按 `references/agent_dispatch_spec_zh.md` 调用 `subagent`，不传 type 或 model，并给出确切 action 范围、解释器、检查和结构化返回契约。

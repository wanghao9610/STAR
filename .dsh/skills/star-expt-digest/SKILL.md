...
**READ-tier entry on this harness.** Before scanning, apply conventions §10.8 once: when the configured READ override is usable — either its model is not an alias of this run's, or the harness can apply its depth per dispatch — hand the complete run to one fresh READ-tier delegate with that model and supported depth, the original invocation, the mode and scope this run already resolved (stated as settled), whether the user typed the invocation or the run was picked up (conventions §10.5), resolved language, `involve=<level> tier=read`, and any held grant; wait and relay its reply. A native READ fork or a run carrying `tier=read` skips this check. Otherwise stay here, stating one reason only when a model was configured. Digest and `ledger` write only their own summary artifact and do not continue into a writing successor.

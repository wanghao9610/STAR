...
description: >-
  Summarize what the experiment programme has done lately, in date order. No argument resumes from
  the previous digest; a PLAN_NAME covers that node's family; `<N>d` or a date covers a window;
  `all` covers everything. Collects each run's newest analysis report, tabulates verdicts and
  headline metrics with provenance, and writes one dated digest to wkdrs/digests/. Use when the user
  runs /skill:star-expt-digest, or wants progress-report material. Bilingual (en/zh).
---
...
Before acting, load this skill's unconditional opening reads in one message — the opening load that later text points back at: three bash calls, with the project root as the working directory, plus a `read` of `<this skill's directory>/references/scope_spec.md`, all sent together.

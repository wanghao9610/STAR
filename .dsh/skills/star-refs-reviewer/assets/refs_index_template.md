...
<!-- Written by /skill:star-refs-reviewer (model_id: <model id, copied verbatim from what `bash .dsh/hooks/star_model_id.sh --resolve` prints — DSH states no model at session start, so the provenance line injected there names that command; "unrecorded" only if it prints nothing>). This file is the audit trail for reference.bib: every entry's

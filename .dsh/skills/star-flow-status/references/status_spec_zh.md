...
- **已丢弃，但活儿还在磁盘上** —— 某个已丢弃节点的当前 run 还记录着未合并的 `branch:`、现存的 `worktree:`，或未勾选的「待用户执行」命令。丢弃收走的是推荐，不是资源：这些东西还在，放着不管仍然有代价。答案不再是合并——记录的抢救与分支的废弃走 `/skill:star-plan-executor`（规约 §11.6）。

...
description: >-
  Read-only overview of the research flow. Scans every metds/plans/*_plan.md, rebuilds the tree from
  parent/prefix, reads each node's status, children, depends_on and exec_status, then renders the
  tree with progress counts, the single next action, and any staleness. Also checks the surrounding
  stages — ideas, refs, reviews, analyses, method docs — for finished work whose follow-up is
  missing. Use when the user runs /skill:star-flow-status, or asks what to do next. Bilingual
  (en/zh).
---

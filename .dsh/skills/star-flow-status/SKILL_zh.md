...
description: >-
  只读总览整条研究流程。扫描每个 metds/plans/*_plan.md，按 parent/prefix 重建拆解树，读取每个节点的章节状态、children、depends_on 与
  exec_status，然后渲染带进度汇总、唯一下一步动作与任何失配的树。同时检查周边阶段——想法、文献、代码审查、实验分析、方法文档——找出已完成工作里缺失的后续环节。当用户运行
  star-flow-status，或询问接下来该做什么时使用。Bilingual（中/英）。
---
...
### Step 1：扫描
用一条消息装齐全部输入：两次 bash 调用（都以项目根目录为工作目录）。第一次调用取规约摘录：

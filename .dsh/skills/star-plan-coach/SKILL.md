...
description: >-
  Coach CS researchers through a research plan, one question at a time, stage by stage (problem →
  related work → method → experiments → risks → milestones), writing each finished section to
  metds/plans/ and resuming across sessions. Use whenever the user wants to write or refine a
  research plan, proposal, or 开题报告; flesh out a research idea; grow a finalized idea file under
  metds/ideas into a plan; mentions plan files under metds/plans; or has an idea but no way forward.
  Bilingual (en/zh).
---
...
2. **A `PLAN_NAME` with a `SECTION` key** → reopen that one section: set its `status` back to `in_progress`, **clear `finalized:`** — the plan is not consumable while a section is open, and `/skill:star-plan-decomposer` and `/skill:star-code-architect` both read it — restore context in 2–3 sentences from the sections it builds on, coach it alone, then re-run Step 7 over the whole plan, which sets it again. This is the way back into a `finalized` plan: a closer paper from `/skill:star-refs-reviewer`, a result that moved the positioning, a reviewer's objection.
3. If a plan has any section whose `status` is not `done`, ask whether to continue it (via ask_user_question: continue that plan / start a new one); if yes, summarize completed sections in 2–3 sentences to restore context, then resume from the first non-`done` section. If there are no plans yet but a `finalized` idea file exists under `metds/ideas/`, offer it as the seed (via ask_user_question: use that idea / start from a fresh topic) before asking for a topic.
...
**Hand off downstream.** Once finalized, tell the user the recommended order: if `${CODE_NAME}/` is still empty, first give the method a place for the code to live (`/skill:star-code-architect`, which reads this root plan) and a runtime (`/skill:star-env-builder`); then turn the top-level plan into executable sub-plans with `/skill:star-plan-decomposer <slug>` — leaves written against a codebase that exists name real modules instead of guessing paths. `/skill:star-flow-status` gives an overview of the plan tree once it exists. Offer once to commit the plan file (State & File Rules).

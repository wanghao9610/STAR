...
description: >-
  ...
  star-code-release, wants to open-source the project or a repository README. Bilingual
  (en/zh).
---
...
In this skill only `check` can move: the other phases ask confirmation points that turn on what they find. A `check` run that moved writes its report without the plain-text approval the Dialogue Discipline asks for when the question tool is unavailable — the move itself established that no question is left.

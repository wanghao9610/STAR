...
description: >-
  Compile the plan tree under metds/plans/ into paper-ready method documents: overview, dataset,
  framework, training, evaluation; no argument compiles all five. Merges passages by what a reader
  needs, not by plan, marks unexecuted leaves unverified, turns gaps into TODOs. Plans are the only
  source — never code, logs or chat; numbers stay with star-expt-analyst. Use when the experiments
  are done, the plans finalized, and the user runs /skill:star-metd-summarize. Bilingual (en/zh).
---
...
2. Interpret the argument: one of the five OPTs → that document; no argument → all five in dependency order (`overview` last: it links the other four); anything else → name the five valid OPTs and ask via ask_user_question which was meant.
...
These read-only subagents extract and return; they never write files, never resolve cross-plan conflicts, and never compile a document. `overview` in particular is compiled last from the other four documents' compiled content — but its rows are extracted like any other target, so a collector holding a leaf still returns that leaf's §1 objective for it.
...
- **Missing** → write it.
- **Exists, generated by this skill** (`type:` + `generated:` present) → compare against the freshly compiled content. No substantive change → leave the file untouched and say so; do not churn the `generated` date. Substantive change → show the section-level change list (one line per section: added / rewritten / removed / unchanged, and what changed) on the page first, then ask via ask_user_question to overwrite or skip — one question per document. When more than two documents differ, they may be batched into one question over the numbered list — *overwrite all* / *overwrite some (say the numbers)* / *skip all* — since five differing documents no longer fit as options (conventions §7.3).

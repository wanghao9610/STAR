...
description: >-
  ...
  的日志与产物，逐条对照磁盘文件给完成度打分，把审查报告写入 wkdrs/，再一次一问走完修订候选，并追加一条 Revision History 条目。当用户运行
...
- ask_user_question 不可用（非交互 `dsh --profile headless` 下，无人应答）时改用纯文本提问——仍然一次一条候选，仍然先获明确批准再写入。

...
| executed descendants' logs (root/internal targets) | kill-criteria hits and plan-level findings that bear on this node's assumptions |
...
- `strategy_signals` — plan-level finding or kill-criterion notes, quoted, or `none`
...
Score each completion claim by the highest level that actually holds:
...
| Verdict | Meaning |
|---|---|
| `met` | every level applicable to the item holds (a step with no artifact can be met at levels 1+3) |
...
- **Root/internal**: do not fan out per descendant — read children frontmatter directly; run log collectors only on runs that exist, run in parallel; audit this node's own §1–§6 assumptions against the aggregated signals (a child's plan-level finding is evidence against a parent assumption).
...
**How far the change reaches**: `local` (this file's prose/frontmatter) / `structural` (tree shape or cross-sibling dependency edges → star-plan-decomposer) / `strategic` (research question, method bet, or kill-criteria overturned → star-plan-coach).

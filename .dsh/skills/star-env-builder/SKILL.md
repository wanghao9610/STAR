...
description: >-
  ...
  installs behind one approved plan (CUDA-aware), then smoke-tests imports, GPU and the entrypoint.
...
4. `${CODE_NAME}/` missing or effectively empty → there is no dependency source; recommend `/skill:star-code-architect` first, and offer to build a bare env (python only) if the user wants one anyway.

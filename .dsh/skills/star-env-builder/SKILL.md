...
description: >-
  Build and verify the project's Python runtime so plan execution has one. Reads .env: a valid
  CONDA_HOME creates conda env ENV_NAME, else a .venv; an existing one is renamed to a dated backup,
  never deleted. Resolves dependencies from requirements, packaging metadata or an import scan,
  installs behind one approved plan (CUDA-aware), then checks imports, GPU and the entrypoint.
  Use when the user runs /skill:star-env-builder, or the environment needs building or verifying.
  Bilingual (en/zh).
---
...
6. **Evidence-based acceptance.** The main agent runs the three runnable-check layers itself (`references/runnable_check_spec.md`) and reports what was verified with evidence, not that it "works" (AGENTS.md §11). The report and version list go to `wkdrs/env_<ENV_NAME>_<date>/`.
...
4. `${CODE_NAME}/` missing or effectively empty → no dependency source; recommend `/skill:star-code-architect` first, and offer a bare env (python only) if the user wants one anyway.
...
5. Chat report ≤500 words: what was verified (with evidence), failures, awaiting-user commands. **Hand off downstream:** `/skill:star-plan-executor <leaf>` now has a runtime; `/skill:star-flow-status` shows what to run next.
...
- The confirmation point and all situational questions go through ask_user_question — one question per call, each with a recommendation. If it is unavailable (non-interactive `dsh --profile headless`, no human to answer), fall back to plain text, still one at a time; the install plan then needs an explicit approval message before anything installs.

...
description: >-
  构建并验证项目的 Python 运行环境，让计划执行有解释器可用。读取 .env：CONDA_HOME 有效则创建 conda 环境 ENV_NAME，否则建
  .venv；已存在的环境绝不删除，先重命名为带日期的备份。依赖按 requirements、打包元数据、import 扫描的顺序解析，经唯一一道安装计划确认点安装（按 CUDA
  匹配），随后做跑通性检查：import、GPU 与项目入口。当用户运行 /skill:star-env-builder，或运行环境需要构建或验证时使用。Bilingual（中/英）。
---

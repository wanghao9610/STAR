...
- The fix-pass approvals that are asked — `blocker`/`major` fixes and every deletion — go through ask_user_question: the whole list on the page, then one question over it (conventions §7.13), with every deletion asked on its own. If it is unavailable (non-interactive `dsh --profile headless`, no human to answer), fall back to plain text, still the list before the question, and require an explicit approval before any write. What Step 7 applies unasked is named in the same reply that applies it.

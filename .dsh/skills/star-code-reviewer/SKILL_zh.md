...
2. **广收集，先核实再报告。** 收集一律交给只读 `subagent`——一个没参与过这份代码的上下文——但每条要进报告的 blocker/major 问题项，主 agent 都要重读被引用的代码确认；站不住的降级或丢弃。评价一份 review 看的是问题项的精度而不是数量——一条错误的 blocker 就足以让报告失去可信度。
...
- 修复轮的批准一律经 ask_user_question——一次一条问题项（或一批同类项）。不可用（非交互 `dsh --profile headless` 下，无人应答）时退化为纯文本，仍然一次一条，任何写入前先获明确批准。

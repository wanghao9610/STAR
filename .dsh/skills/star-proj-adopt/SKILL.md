...
description: >-
  Adopt an already-started project into STAR without disturbing it. Phase `survey` inspects the repo
  read-only (layout, runtime, data and weight paths, entrypoints), then writes .env, symlinks large
  directories instead of moving them, wraps launch commands into execs/scpts/, and inventories the
  work in metds/adopt.md. Phase `backfill` maps that inventory onto the tree's leaves on
  confirmation. Use when the user runs /skill:star-proj-adopt, or wants a repo onboarded. Bilingual
  (en/zh).
---
...
You are the on-ramp, not the driver. You do not survey the code architecture (`/skill:star-code-architect` Branch B owns that), you do not author research strategy (`/skill:star-plan-coach` and `/skill:star-plan-decomposer` own the plan tree), and you do not judge results (`/skill:star-expt-analyst`). You establish the runtime, record what already exists as evidence, and later reconcile that evidence with the tree the coach and decomposer build.
...
4. **Reconstruction is always labeled.** A record written after the fact is not an execution record. Every historical run recorded this way carries a header saying it was reconstructed during adoption, on what date, from what evidence — so no later reader mistakes it for something `/skill:star-plan-executor` produced.
5. **Adoption does not invent research strategy.** You can read what was built and what was run; you cannot read why, what claim it serves, or what would have killed it. The inventory stays descriptive, and §4-style claims and kill-criteria are left for `/skill:star-plan-coach` to elicit from the user. A plan tree fabricated from a git log is worse than no plan tree.
...
7. **Two confirmation points; autonomous between them.** Confirmation point 1: the user confirms the survey mapping (source, runtime, data / weights / outputs) before anything is written. Confirmation point 2: the user picks which historical runs get recorded. `backfill` adds one confirmation point of its own, per-leaf. Never do work a confirmation point did not cover.

## Workflow

Follow `references/adopt_spec.md` (Chinese: `references/adopt_spec_zh.md`) for the survey recipe, the inventory format, and the symlink / wrapper rules — the opening message under Shared conventions already loaded it; the shape is:
...
- STOP line (conventions §2): nothing here trains, evaluates, installs, or deletes. The survey is read-only, the verification is `--list` plus an interpreter version check. Environment repair belongs to `/skill:star-env-builder`; if the runtime cannot run python, that is a blocker to report, not one to fix.
...
- All three confirmation points go through ask_user_question, one question per call. If it is unavailable (non-interactive `dsh --profile headless`, no human to answer), fall back to plain text — still one at a time, and still requiring an explicit answer before any write past a confirmation point.
...
- Lead with what the survey found and what it could not settle. An unknown reported as unknown is the point of this skill; a confidently wrong `CODE_NAME` costs the user every downstream skill.

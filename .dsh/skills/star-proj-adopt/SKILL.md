...
You are the on-ramp, not the driver. You do not survey the code architecture (`/skill:star-code-architect` Branch B owns that), do not author research strategy (`/skill:star-plan-coach` and `/skill:star-plan-decomposer` own the plan tree), and do not judge results (`/skill:star-expt-analyst`). You establish the runtime, record what exists as evidence, and later reconcile it with the tree the coach and decomposer build.
...
4. **Reconstruction is always labeled.** A record written after the fact is not an execution record. Every historical run recorded this way carries a header: reconstructed during adoption, on what date, from what evidence — so no later reader mistakes it for `/skill:star-plan-executor` output.
5. **Adoption does not invent research strategy.** You can read what was built and run; not why, what claim it serves, or what would have killed it. The inventory stays descriptive; §4-style claims and kill-criteria are left for `/skill:star-plan-coach` to elicit from the user. A plan tree fabricated from a git log is worse than no plan tree.
...
Ask through ask_user_question, one question at a time, only about what the survey could not settle: the `CODE_NAME` directory, the `PYTHON_HOME` interpreter, the existing data / weights / output roots. Options come from the survey with the recommendation marked. Nothing is written until the user answers.
...
List the prior runs the survey found — path, date, what it appears to have produced, any metric visible in its logs. Ask once via ask_user_question over the numbered list which to record — more than four runs cannot be options (conventions §7.3), so offer *record all* / *record some (say the numbers)* / *record none*. Symlink each chosen run to `wkdrs/<run>/` and write a minimal `EXEC_LOG.md` from `assets/exec_log_reconstructed.md` — a reconstructed header (Principle 4), the command where recoverable, the artifacts present, and explicitly no step table. The rest stay in the inventory as evidence only, and the report says how many were left out.

#### Step S6: Write the record & route

Write `metds/adopt.md` from `assets/adopt_template.md`. Then route, in order: `/skill:star-code-architect` for the architecture spec, `/skill:star-plan-coach` for the research plan, `/skill:star-plan-decomposer` for the leaves, and finally `/skill:star-proj-adopt backfill` to make the tree reflect what is already done.
...
- All three confirmation points go through ask_user_question, one question per call. If it is unavailable (non-interactive `dsh --profile headless`, no human to answer), fall back to plain text — still one at a time, still an explicit answer before any write past a confirmation point.

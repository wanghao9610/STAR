...
3. **Dependencies are concrete, located, and encoded** — §2 names actual datasets (`datas/`), weights (`inits/`), code (`code/`), and any upstream sub-plans (by prefix) that must finish first, with the artifact each hands over. No vague "the data" / "the model". The `depends_on` frontmatter list matches those upstream sibling prefixes. Whether that graph has a cycle, and whether it agrees with the parent's index, is settled by the computation in Step 7 — do not work it out again here; this item asks only whether each dependency is the *right* one.

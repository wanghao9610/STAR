# /star — route a request to the right STAR skill
...
**The request is whatever the user typed beside `/star` in this message.** A Cursor command has no argument placeholder, so take it from the message itself; nothing beside the command means no request was given.

**No request** — the user is asking where things stand. Read `.cursor/skills/star-flow-status/SKILL.md` in full and follow it as this run's instructions. Nothing else.
...
- An unmarked skill: start it. Read its `SKILL.md` under `.cursor/skills/` in full and follow it as this run's instructions, with that argument.

...
One `Task` subagent per step (or coherent step-group) in EXEC_PLAN. Prefer `subagent_type: generalPurpose`; use `explore` only for read-only orientation. The main loop orchestrates and verifies; the agent does the edit/run for exactly one step. Keep agents narrow so failures localize and contexts stay short.

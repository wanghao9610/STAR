...
- **较大范围**：按包/目录切分给只读 `Task` subagent（`subagent_type: explore`），至多 3 个并行，每个拿到评分表、Step 1 构建的评判依据摘要（同一块内容，逐字发给所有人）和确切的文件清单，按 `review_rubric_zh.md` 规定的结构化问题项格式返回。收集器绝不写文件、绝不越出自己的清单、绝不给整体结论打分。

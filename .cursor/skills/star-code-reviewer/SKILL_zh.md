...
2. **广收集，先核实再报告。** 收集一律交给只读 `Task` subagent（`subagent_type: explore`）——一个没参与过这份代码的上下文——但每条要进报告的 blocker/major 问题项，主 agent 都要重读被引用的代码确认；站不住的降级或丢弃。评价一份 review 看的是问题项的精度而不是数量——一条错误的 blocker 就足以让报告失去可信度。
...
- **派发是自动的，不为它征求同意。** 跑这条命令，请求的就是这些收集器——Step 3 正是这个技能写明要派发的地方，用户亲手敲的名字也好，agent 自己因任务明显匹配而接手也好（规约 §10.2），都一样——所以以「用户提出请求」为条件的常驻指令在这里已被满足。参与度档位同样不改变这一点：`high` 只是在派出去之前把扇出和切分方式说一声，绝不变成一个待答的问题（规约 §6.8）。能拦住它的只有两种，都不是偏好：彻底禁止委派的常驻指令，或宿主根本不提供委派能力。只有这时才为这一次派发问一次，不默默退回本地，因为在被审查的那个上下文里做收集，不是这一步要的那种独立阅读（规约 §6.1；§6.7——主 agent 没法自己给自己的第二意见）。被拒绝或本就派不出，主 agent 就按同一份契约逐片在本地填，范围行改成写明：没有独立收集器读过这份代码。

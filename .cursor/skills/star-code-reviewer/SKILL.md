...
2. **Find wide, verify before reporting.** Collection may fan out to read-only `Task` subagents (`subagent_type: explore`), but the main loop re-reads the cited code for every blocker/major finding before it enters the report; what does not hold up is downgraded or dropped. A review is judged by the precision of its findings, not their count — one wrong blocker costs the report its credibility.

...
<!-- Written by /star-code-reviewer (model_id: <model id, copied verbatim from what your runtime states this session — Cursor injects it at session start; "unrecorded" only if the session names none>). Mode: full | plan | path | diff; target: <argument as given>;

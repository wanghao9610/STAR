...
2. **广收集；判断留在主 agent。** 对多个 run 或产物集合进行有边界、相互独立、只读的检查确有帮助时，派出只读 `Task` subagent（`subagent_type: explore`；READ 档值非空时作为 `model:`）。受托者遵守 `references/review_spec_zh.md`，绝不写入或提出修订；综合与判断留在主 agent。
...
规模超过这个的：按 `references/review_spec_zh.md` 的收集器格式约定并行派出只读 `Task` subagent（`subagent_type: explore`）——通常是 **日志读取器**（步骤状态、自报检查、"待用户执行"命令（英文日志："Awaiting user"）、方向性信号）、**交付物检查器**（§4 每个交付物：存在 / 大小 / 修改时间 / 低开销合理性检查），以及当 §2–§3 涉及代码时的 **代码检查器**（承诺的模块是否真的写出来了、与日志声称的改动是否一致）。

...
2. **广收集；判断留在主 agent。** 对多个 run 或产物集合进行有边界、相互独立、只读的检查确有帮助时，派出只读 `Task` subagent（`subagent_type: explore`；READ 档值非空时作为 `model:`）。受委派者遵守 `references/review_spec_zh.md`，绝不写入或提出修订；综合与判断留在主 agent。
3. **每处改动由用户拍板。**审查发现整理成编号候选。先应用已获授权的点名改动；其余摆在页面上，通过 AskQuestion 按规约 §7.13 问一次。每次写入都必须追溯到明确指令或从可见清单接受的候选。
...
2. 目标不存在或有歧义时，列出简洁候选（前缀 + slug + 一行状态），通过 AskQuestion 只问一个直接问题——优先推荐有执行证据（`exec_runs` 非空）或已知失配的节点。
...
2. 去掉明确指令已经定下的候选，再通过 AskQuestion 按规约 §7.13 对剩余可见清单问一次。丢弃仍是 `references/revision_rules_zh.md` 管辖的独立实质动作；`exec_status` 重置与清除 `finalized:` 只有在其后果尚未获授权时才问。
...
- 只有修订、重置、重新定稿、丢弃、恢复或覆盖权限仍未解决时，才通过 AskQuestion 一次问一个问题；工具不可用时退回简洁纯文本。只读报告请求以报告结束。计划正文与审查报告保持计划 frontmatter 的 `language`。

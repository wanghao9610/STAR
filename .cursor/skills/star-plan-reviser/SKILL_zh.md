...
3. **每处改动由用户拍板。** 审查发现整理成编号的修订候选。整张清单先摊在页面上，再用**一次** AskQuestion 把它们定下来——全部按清单采纳 / 除点名的以外全部采纳 / 先解答点名的几项 / 一项都不采纳——标出你的推荐，用户点名的每一条都会单独回来（规约 §7.13）。绝不把用户看不见的清单拿去批准，绝不擅自动笔。

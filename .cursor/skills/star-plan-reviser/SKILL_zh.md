...
规模超过这个的：按 `references/review_spec_zh.md` 的收集器格式约定并行派出只读 `Task` subagent（`subagent_type: explore`）——通常是 **log reader**（步骤状态、自报检查、"待用户执行"命令、方向性信号）、**artifact inspector**（§4 每个交付物：存在 / 大小 / 修改时间 / 低开销 合理性检查），以及当 §2–§3 涉及代码时的 **code inspector**（承诺的模块是否真的写出来了、与日志声称的改动是否一致）。

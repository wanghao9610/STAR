...
description: >-
  ...
  完成度打分，把七段式审查报告写入 wkdrs/，再把整张修订候选清单摊开、一次提问定下来，直接编辑计划文件并追加
...
动手前把它合成一条消息装载——四次 Shell 调用（以项目根目录为工作目录）一起发出，外加两次 `Read`：`<本 skill 所在目录>/references/review_spec_zh.md` 与 `<本 skill 所在目录>/references/revision_rules_zh.md`，每份文件各占一次。
...
3. **每处改动由用户拍板。** 审查发现整理成编号的修订候选。整张清单先摊在页面上，再用**一次** AskQuestion 把它们定下来——标出你的推荐，用户点名的每一条都会单独回来（规约 §7.13）。绝不把用户看不见的清单拿去批准，绝不擅自动笔。
...
本 skill 第四条永远不成立——评审清单定案后那句“还有其他要改的吗”是没有推荐项的开放问题，每个档位都要问，丢弃那条路也在写之前问一次——所以运行留在这里。
...
规模超过这个的：按 `references/review_spec_zh.md` 的收集器格式约定并行派出只读 `Task` subagent（`subagent_type: explore`）——通常是 **日志读取器**（步骤状态、自报检查、"待用户执行"命令、方向性信号）、**交付物检查器**（§4 每个交付物：存在 / 大小 / 修改时间 / 低开销合理性检查），以及当 §2–§3 涉及代码时的 **代码检查器**（承诺的模块是否真的写出来了、与日志声称的改动是否一致）。
...
2. 然后就这份清单**只发一次** AskQuestion：*全部按清单采纳* / *除我点名的以外全部采纳* / *先解答我点名的几项* / *一项都不采纳*——标出推荐，且内置"Other"始终允许自由作答。候选在四条及以内时，直接就这些候选发问（allow_multiple），不必绕编号。被点出来的行开启第二轮，形状照旧，带上你给出的改写稿或用户要的解答；缩到只剩一条候选时就直接问那一条。丢弃本节点的候选永远不进清单——`references/revision_rules_zh.md` 要求它单独发问——Step 5 的 `exec_status` 重置与清除 `finalized:` 同样是那一步里各自独立的问题，在改动落盘之后问。
...
- AskQuestion 不可用（无人值守 / 脚本化）时改用纯文本提问——仍然先把整张候选清单摊在页面上、再用那一个问题把它定下来，仍然先获明确批准再写入。

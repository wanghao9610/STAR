...
3. **The user owns every change.** Findings become numbered revision candidates. Each is adopted / adjusted / skipped via AskQuestion, one candidate per call, with your recommendation marked — never bundle-approve, never edit unasked.
...
- If AskQuestion is unavailable (headless / scripted), fall back to plain-text questions — still one candidate at a time, still explicit approval before any write.

...
- `strategy_signals`——方向性信号或 kill-criterion 记录，原文引用，或 `none`
...
**Code collector**——针对写明的每个模块/入口：

- `path` / `exists` / `consistent`（文件现状与日志声称的新建/修改是否说得通）/ `notes`

所有收集器：只读；只返回上面列出的字段，别的都不返回；宁报"unknown"不猜测；不提修订建议；对 `datas/`、`inits/`、`wkdrs/` 除读取外一概不碰。收集器给出的判定只是线索，要等主 agent 确认过，才到得了用户面前（规约 §6.6）。
...
| 结论 | 含义 |
|---|---|
| `met` | 适用于该项的所有级别全部成立（无产物的步骤可凭 1+3 判 met） |
| `partial` | 部分子项成立，其余不成立 |
| `unmet` | 证据明确表明没做或失败 |
| `unverifiable` | 有声明无产物、产物无法低开销核验、或相互矛盾（日志 `done` 而产物缺失） |

绝不仅凭日志一面之词把 `unverifiable` 抬成 `met`。

**什么算低开销检查**：文件存在性 / 大小 / 开头几行、小型解析、校验和、单元测试量级的命令——大致一分钟内、不占 GPU、不花付费 API、无副作用。超出即为重检查：不要跑；在报告里注明完整复核需要什么。

## 按节点类型圈定范围

- **叶子**：对它自己的 run 走完整三级。
- **根/内部**：不逐后代展开——直接读 children 的 frontmatter；只对确实存在的 run 派 log collector，且最多 3 个并行——已执行的后代多于 3 个就分批，每批 3 个，绝不一次性全派出去；用汇总后的信号审计本节点自己 §1–§6 的假设（子计划的方向性信号就是质疑父假设的证据）。
...
7. **修订候选**——编号；每条写明目标章节、改什么、证据、修改草案、影响范围分级。

**证据出处**必须具体：`路径[:行号]`、命令及其输出片段、或某个 frontmatter 字段——每条结论至少一个。

**影响范围**：`local`（本文件的正文/frontmatter）/ `structural`（树形结构或跨兄弟依赖边 → star-plan-decomposer）/ `strategic`（研究问题、方法关键假设或 kill-criteria 被推翻 → star-plan-coach）。

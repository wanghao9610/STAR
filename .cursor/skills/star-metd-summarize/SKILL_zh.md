...
**规模**：步骤 1 只读了 frontmatter，所以计划正文是在这一步才进入本次运行的。小树（≤ ~15 个计划）由主 agent 直接读。更大的树则**按计划**切分给只读 `Task` subagent（`subagent_type: explore`），最多 3 个并行，各自拿到完整映射、一组互不相交的完整计划文件，与 `extract_map_zh.md` 里的提取格式约定。每个收集器都在自己持有的计划上为全部五个目标做抽取，并给每段标上它所喂的 `target:`。
...
- 以纯文本一次一问，在五个确认点处提问：就绪门槛的草稿豁免（对未完成的树编译草稿）、无法识别的 OPT、选哪个根子树（多根树）、每次覆盖已生成文档、以及挡路的手写文档。覆盖任何已存在文件前必须取得明确批准——headless / 脚本化运行也不例外；没有明确批准也绝不越过就绪门槛。

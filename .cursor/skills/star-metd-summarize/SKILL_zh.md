...
**规模**：步骤 1 只读了 frontmatter，所以计划正文是在这一步才进入本次运行的。小树（≤ ~15 个计划）通常由主 agent 直接读更省事。更大的树则**按计划**切分给只读 `Task` subagent（`subagent_type: explore`），并行派发，各自拿到完整映射、一组互不相交的完整计划文件，与 `extract_map_zh.md` 里的提取格式约定。每个收集器都在自己持有的计划上为全部五个目标做抽取，并给每段标上它所喂的 `target:`。

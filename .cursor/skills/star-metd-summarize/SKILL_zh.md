...
- **不存在** → 直接写。
- **存在且由本 skill 生成**（有 `type:` + `generated:`）→ 与新编译的内容比对。无实质变化 → 保持文件原样并说明；不要空转 `generated` 日期。有实质变化 → 分节变更清单（每节一行：新增 / 改写 / 删除 / 未变，以及变了什么）先落到正文，再经 AskQuestion 询问覆盖还是跳过——一份文档一个问题。超过两份文档有差异时，可合并为一个问题，对着编号列表问：*全部覆盖* / *除我点名的以外全部覆盖* / *先解答我点名的几份* / *全部跳过*——五份文档都有差异时已经放不进选项（规约 §7.3；形状见 §7.13）。
...
- 五个确认点都走 AskQuestion：就绪门槛的草稿豁免（对未完成的树编译草稿）、无法识别的 OPT、选哪个根子树（多根树）、每次覆盖已生成文档、以及挡路的手写文档。若不可用（无人值守 / 脚本化），改用纯文本，且覆盖任何已存在文件前仍必须取得明确批准——没有明确批准也绝不越过就绪门槛。

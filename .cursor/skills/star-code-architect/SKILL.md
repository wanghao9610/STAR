...
description: >-
  ...
  When code already exists: survey it with read-only Task subagents instead. Both paths then
  design a target architecture plus a migration table, execute only user-approved migrations
  via orchestrated Task subagents with per-group verification and git checkpoints, and write
  the architecture spec to metds/codearc.md with short cross-references in AGENTS.md and
...
2. **Two confirmation points; autonomous between them.** Confirmation point 1: the user picks the reference repo from a scored shortlist. Confirmation point 2: the user approves the target architecture and migration table. Both confirmation points are plain-text questions — wait for an explicit answer before crossing. Everything between and after runs autonomously with bounded retries. Never do work a confirmation point did not cover.
...
5. **The main agent orchestrates and verifies; Task subagents execute.** Surveys go to read-only `Task` subagents (`subagent_type: explore`); migrations go to `Task` subagents (`subagent_type: generalPurpose`) whose writes are limited to their own group's files. Both carry disjoint file ownership and structured returns. The main agent re-runs every check itself (never trusts a self-reported pass), commits a git checkpoint per verified group, retries ≤2, and rolls back what still fails. Contract: `references/orchestration_spec.md`.
...
Present the top 3–5 as a plain-text comparison, one line per candidate: one-line why-it-fits, license, stars, last update, main risk. Recommend the highest-scoring one, always offer the escape ("none of these — refine the search / start from scratch"), and ask which to use. If invoked with a URL, still show that repo's license, activity, and risks, and confirm before cloning.
...
- Ask one plain-text question at a time and wait for the answer (Cursor has no structured question tool). Any side effect past a confirmation point requires an explicit approval message first.

...
One `Task` subagent (`subagent_type: explore`) per lane, at most 3 in parallel (they are read-only, so parallelism is safe):
...
**Light mode** (Branch A fresh clones, or repos under ~50 Python files): collapse to a single pass over all lanes — by one `explore` Task subagent or the main loop itself.
...
The main loop merges lane reports into one repo map:

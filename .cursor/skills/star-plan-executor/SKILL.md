...
- Intermediate working files live under `tasks/<plan-name>/`; execution state and generated artifacts live under `wkdrs/<run>/`. Never write execution logs into `metds/plans/` — the sub-plan gets only `exec_status` + `exec_runs` + `updated`. `tasks/<plan-name>/` is this plan's disposable scratch and this skill owns its lifecycle: offer its deletion once at finalize when §5 is met; never delete it unasked, and never touch another plan's `tasks/` directory.

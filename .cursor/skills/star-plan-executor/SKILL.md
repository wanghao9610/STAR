...
1. Present EXEC_PLAN + expected side effects: files to be written, commands to be run, where the STOP line falls, rough cost/runtime — and the divergence table, stated as "approving this plan also syncs these back into the sub-plan". Ask in the same gate whether to checkpoint each verified step as a git commit (recommended), naming any path that already carries uncommitted changes — those are never staged. Say what declining costs: without per-step commits every verified step stays uncommitted, so a later step's restore has only this run's own record of where each step began to work from (`references/agent_dispatch_spec.md`). Wait for explicit user approval before any side effect.

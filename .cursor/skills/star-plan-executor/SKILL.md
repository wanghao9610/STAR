...
1. Dispatch a `Task` subagent (no `subagent_type` set, or `subagent_type: explore` when the step is read-only orientation) with the contract in `references/agent_dispatch_spec.md`: this step's goal, the exact files to touch, the resolved interpreter path, the step's own check, and "do **only** this step; return a structured result (changed / ran / check / blockers / handoff)".

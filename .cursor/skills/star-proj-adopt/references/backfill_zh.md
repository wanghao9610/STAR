...
用户通过一次 AskQuestion 按规约 §7.13 逐 leaf 确认，全部提议行只问一次——起草的列表是一个问题，不是一行一问。未获确认的 leaf 原样不动。标为 `done` 但没有入账 run 的 leaf 是允许的，并记一笔：`star-flow-status` 会把它标为 done-with-no-run，那正是诚实的状态。

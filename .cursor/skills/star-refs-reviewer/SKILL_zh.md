...
3. **先确认形状，再精读。** 精读是贵的那一步：把约 15 条排好序的候选（标题 / 会议 / 年份 / 引用数 / 一句话相关性）用一个问题交给用户（可多选），其中 5–10 条标为推荐，只读用户留下的。检索和分类不需要批准；精读和写笔记需要。
...
精读可以扇出给只读 `Task` subagent（`subagent_type: explore`），至多 3 个并行，一个 subagent 一篇，各自返回填好的模板。主循环负责写文件并亲自写 §5（与本项目的关系）——这一节需要方法上下文，也正是这篇笔记存在的理由。

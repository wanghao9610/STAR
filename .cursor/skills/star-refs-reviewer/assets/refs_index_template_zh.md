...
<!-- 由 /star-refs-reviewer 写出（model_id: <模型 id，照抄运行时本会话为你声明的那串——Cursor 在会话开始注入；仅当本会话未声明任何模型才写 "unrecorded">）。本文件是 reference.bib 的审计痕迹：每条条目的出处都记在这里，
...
<!-- 值得再跑一轮补的缺口（某个类别偏薄、某个子话题没覆盖）。路由：磨定位 → /star-plan-coach
     §2 相关工作与定位；以后单加一篇 → /star-refs-reviewer <arxiv-id>；重查 bib →
     /star-refs-reviewer verify。 -->

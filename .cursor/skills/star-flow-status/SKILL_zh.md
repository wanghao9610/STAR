...
**通用规约。** 动手前先读 `docs/mds/star-workflow/research-workflow-conventions.zh-CN.md`（英文：`research-workflow-conventions.md`）：§1 git、§2 STOP 线、§3 `.env` 运行时、§4 真实日期、§5 计划名解析、§6 委派、§7 对话纪律、§8 产物注册表。那是所有 STAR skill 共享的基线；本文件只写本 skill 特有的部分，并在更严处生效。
...
1. **严格只读**。绝不创建、编辑或删除任何文件——不动计划、不动日志、不动 frontmatter。不做交互式决策树、不进 plan 模式、不派 Task subagent。用户想据此行动，就把他们指向对应 skill（`/star-idea-storm`、`/star-plan-coach`、`/star-refs-reviewer`、`/star-plan-decomposer`、`/star-plan-executor`、`/star-code-reviewer`、`/star-expt-analyst`、`/star-plan-reviser`、`/star-metd-summarize`）。

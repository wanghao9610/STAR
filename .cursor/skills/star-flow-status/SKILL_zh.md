...
description: >-
  ...
  （父计划在子计划之后被改）。绝不写任何文件。只要用户运行 /star-plan-status，或询问计划的状态 / 总览 /
...
### Step 6：下一个可执行叶子
推荐执行顺序里最靠前、且非 `done`/`blocked`、且其每个 `depends_on` 前缀都 `exec_status: done` 的叶子。给一句话理由。若无合格者，点名挡路者（未完成的依赖、仍需拆解的叶子、或"待用户执行"命令）。指向 `/star-plan-executor <该叶子>`。

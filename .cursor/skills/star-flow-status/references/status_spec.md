...
| 8 | Method docs missing | every leaf is terminal (`done` / `skipped` / `abandoned`) **and** every top-level plan carries `finalized:` **and** no `metds/*.md` carries `type:` + `generated:` | `/star-metd-summarize` |
...
| 13 | Release missing | every leaf is terminal (`done` / `skipped` / `abandoned`) **and** a compiled `metds/*.md` carries `type:` + `generated:` **and** `wkdrs/release/` holds no `RELEASE_<date>.md` | `/star-code-release` |

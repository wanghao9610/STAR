...
扫描调用还会列出 执行分支（规约 §11）。某个分支匹配到一个叶子、而这个叶子的 run 目录在当前 checkout 里不存在，就是一次正在该分支上进行的 run：基础分支始终是准据（§11.3），所以这边叶子的 frontmatter 读作未完成是对的——渲染这个叶子时点出分支名，而不是只当它 pending；分支侧状态只在下面某一行需要时才读，用只读的 `git show <run>:wkdrs/<run>/EXEC_LOG.md`（§1 的只读清单涵盖 `show`）。
...
每个叶子行显示其 `depends_on`、（正在执行时）`k/n` 步，以及——当前 run 记录了执行分支时——它的合并状态：`<run> unmerged`，日志写明之后则是 `merged`。示例：
...
- **第 14 行去记录所在的地方读。** 在分支 checkout 上，记录就在工作区里；从基础 checkout 看，执行分支清单是触发器，只读的 `git show` 是读取手段。它的审查条件指的是分支侧的 `CODE_REVIEW_<date>.md`；一份都没有时，改由第 3 行在分支侧状态上触发，而且它在这一档里本来就排在前面。

...
扫描调用还会列出 执行分支（规约 §11）。某个分支匹配到一个叶子、而这个叶子的 run 目录在当前 checkout 里不存在，就是一次正在该分支上进行的 run：基础分支始终是准据（§11.3），所以这边叶子的 frontmatter 读作未完成是对的——渲染这个叶子时点出分支名，而不是只当它 pending；分支侧状态只在下面某一行需要时才读，用只读的 `git show <run>:wkdrs/<run>/EXEC_LOG.md`（§1 的只读清单涵盖 `show`）。worktree 清单同样读法（§11.7–9）：路径以某个 run 名结尾的树就是那个 run 的家——渲染叶子时挨着分支一并点出——既没有叶子、也没有 run 记录认领的树，不归我们提。
...
- `⚠` 需关注 —— **太粗、无法直接执行**，有两种情形：其自身 §3 任务分解 / §5 完成判据大量 `[TBD]` 或 `【待定】` 的子计划叶子（两个记号都算——中文计划写的是后者，扫描一并计数），**或**带着 `finalized:` 却没有 `children:` 的总体计划——从未被拆解过，因而算作叶子（规约 §5.4），执行器会拒绝它。这是唯一的定义；第 3 层的资格判定一律依它，与生命周期规则最终渲染出哪个状态符号无关 → 建议 `/star-plan-decomposer`。
...
| 6 | 结果汇总表过期 | ≥2 个叶子有 `EXPT_ANALYSIS_<date>.md` **且**没有覆盖该范围的现行结果汇总表——即 `wkdrs/results/results.md` 与（限定到 `PLAN_NAME` 时的）`wkdrs/results/results_<slug>.md` 都不存在、或其 `generated:` 都早于这些报告里最新的日期 | `/star-expt-analyst aggregate` |
| 7 | 方法文档过期 | 某个编译出的 `metds/*.md`（带 `type:` + `generated:` + `sources:`）在 `sources:` 里记录的某计划 `updated`，早于该计划当前的 `updated` | `/star-metd-summarize` |
| 8 | 方法文档缺失 | 每个叶子都处于终态（`done` / `skipped` / `abandoned`）**且** 每个总体计划都带 `finalized:` **且** 没有任何 `metds/*.md` 带 `type:` + `generated:` | `/star-metd-summarize` |
...
| 13 | 缺发布准备 | 每个叶子都处于终态（`done` / `skipped` / `abandoned`）**且** 已有带 `type:` + `generated:` 的 `metds/*.md` **且** `wkdrs/release/` 下没有 `RELEASE_<date>.md` | `/star-code-release` |
...
- **第 10 行只对已经在记 digest 的项目触发。** 与第 2、7、8 行不同，产物缺失并不触发它：digest 是工作辅助，不是研究欠下的交付物，一个从没跑过 `/star-expt-digest` 的项目并不因此欠账。所以这一行问的是“已有的序列有没有落在分析报告后面”。“最新那份序列 digest”指 `mode` 为 `incremental`、`window` 或 `all` 的最新一份——`plan` 模式的 digest 是回溯性阅读，它的 `covers.through` 不能当作续接点，这与该 skill 自己的 `scope_spec_zh.md` 对上次覆盖到的日期的定义一致。
- **第 14 行去记录所在的地方读。** 在分支 checkout 上，记录就在工作区里；从基础 checkout 看，执行分支清单是触发器，只读的 `git show` 是读取手段。它的审查条件指的是分支侧的 `CODE_REVIEW_<date>.md`；一份都没有时，改由第 3 行在分支侧状态上触发，而且它在这一档里本来就排在前面。住在 worktree 里的 run，工作文件在那棵树里，但已提交的记录同样应答这里的 `git show`；合并本身仍走 executor，它会在树里续跑（§11.8）。
...
1. **待用户** —— 某个 `⏸` 叶子有未勾选的 STOP 命令。写明该命令；只有用户能清掉它，在那之前下面几层都不重要。只有一件事排在这条命令之前：该 run 目录里还没有 `CODE_REVIEW_<date>.md` 时，先推荐 `/star-code-reviewer <叶子>`，把待跑命令紧挨着写在它下面——bug 在算力烧掉之前最便宜，而第 3 行覆盖不到这里，因为该叶子还不是 `done`。这两条归属不同：审查属于那八个 agent 可以自己启动的 skill，是接下来真正要跑的那条（见下）；待跑命令则打印出来，归用户。
...
- **finalized 之后又被改过** —— 某总体计划节点的 `updated` 晚于其 `finalized:`。评分表跑过了，之后计划又变了，那个 `✔` 已经不再有评分表背书。建议 `/star-plan-coach <slug>` 重新关闭它。

<!-- Compiled by /star-code-release on <YYYY-MM-DD> · model_id: <model id, copied verbatim from what your runtime states this session — Cursor injects it at session start; "unrecorded" only if the session names none> · sources: <artifact>@<date>, … · report: wkdrs/release/RELEASE_<date>.md · Regenerate with /star-code-release readme; hand edits to a section are detected and kept. -->

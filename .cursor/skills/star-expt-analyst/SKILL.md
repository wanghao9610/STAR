...
- **Scale**: a small run (a handful of artifacts, no oversized log) is read by the main loop. For a large one — many log files, or logs too big to read whole — partition by file into read-only `Task` subagents (`subagent_type: explore`), at most 3 in parallel, each given the rubric, the expectations digest, and its exact file list, returning the structured observation contract. Collectors never write, never read outside their list, never grade the run's verdict.
...
- Ask as plain text, one question at a time, only where the workflow calls for it (which run to analyze, an ambiguous match); require an explicit answer — even in headless or scripted runs. Since the skill writes nothing outside its own report, there is no approval gate — but for the same reason, never state or imply that you changed a plan, a status, or a log.

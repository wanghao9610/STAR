...
A programme of ≤ ~6 reports is usually simplest to read in the main agent. Above that, partition the **report paths** across read-only `Task` subagents (`subagent_type: explore`), run in parallel, each given its exact list and this return format:

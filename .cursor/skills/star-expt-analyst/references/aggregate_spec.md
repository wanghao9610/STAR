...
A programme of ≤ ~6 reports is read by the main agent. Above that, partition the **report paths** across read-only `Task` subagents (`subagent_type: explore`), at most 3 in parallel, each given its exact list and this return contract:

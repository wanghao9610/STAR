...
2. If no argument was given, or the match is ambiguous, list the available plans (prefix + slug + one-line title) and ask which one (one question).
...
Propose 2–3 axes in one question (recommend the first). Details and how to pick: `references/decomposition_axes.md`.

...
2. **Confirm the shape with its content in view, then auto-draft the rest.** Two decisions go through AskQuestion (one question per call, each with a recommendation): the **decomposition axis**, then the **sub-plan list** — shown as one card per unit (objective, steps, deliverables, done-criterion), never a row of titles. After those are confirmed, draft each sub-plan's content autonomously by pulling from the parent. Mark genuine gaps `[TBD]`; ask a targeted follow-up **only** when a step is undecidable without the user. Do not re-elicit detail the parent already records.
...
Four lines a unit, and each stays one line: this is the sketch Step 4 expands into the six sections, not a draft of them. Then confirm via AskQuestion (*looks good* / *edit the list* / *change granularity*, with your recommendation marked).
...
- If AskQuestion is unavailable (headless/scripted), fall back to plain-text questions — still one at a time.

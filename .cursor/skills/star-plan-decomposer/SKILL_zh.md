...
2. **先确认形状，再自动起草内容**。两个决策由用户确认（每次只问一题，各带推荐项）：**拆分轴**，然后是**子计划清单**。这两项确认后，从父计划中抽取内容、自动起草每份子计划。真正缺失处标 `【待定】`；只有当某个步骤不问用户就定不下来时，才追问一个针对性问题。不要重新问父计划里已经记录的细节。
...
2. 若未给参数，或匹配有歧义，列出可选计划（前缀 + slug + 一句话标题），用一道选择题让用户选，并标出你推荐的一项。
3. 完整读取选定的计划。
...
用一道题提出 2–3 个轴（推荐第一项）。详情与如何选择见 `references/decomposition_axes_zh.md`。每个轴选项都要写清它意味着什么，而不只是它叫什么（规约 §7.3）：切分的形状、它蕴含的依赖形态（线性链 / 小 DAG / 基本独立），以及 Step 4 之后再换轴，就意味着对可能已带手工修订的文件重跑一次切分。真正把它们分开的问题是：系统还要不要建。主张轴没有放建系统的格子——代码还跑不起来的计划按里程碑或组件拆，主张视角会在下一层、作为实验最重的那个里程碑的递归回来。
...
开头先点明承接之处（规约 §7.10）：刚选定的是哪个轴、它在这里切出了什么——"里程碑轴 → 4 个单元，一条线性链"。按选定的轴起草 N 个单元。每个含：简短标题、英文 `slug`、一句话目标、对应根计划的哪一节 / 主张、**以及它依赖哪个（些）兄弟**。用普通文本展示清单——包含依赖边和由此得到的执行顺序——并确认（*可以* / *修改清单* / *调整粒度*，标出你推荐的一项）。

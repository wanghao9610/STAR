...
2. **An `IDEA_NAME`** → resume from the first non-`done` stage. If it is finalized, reopen only when the request says to revisit it; otherwise ask via AskQuestion whether to reopen or route to `star-plan-coach <slug>`.
3. No argument → resume the single unfinished idea. If several exist, ask which via AskQuestion; if none exists, ask for the seed in plain text.
...
Generate 3–5 distinct candidate directions with a research question, bet, novelty, and nearest area. Apply any selection already stated; otherwise present one table and ask once via AskQuestion which 2–4 to scan, marking recommendations. Keep dropped candidates in §2 as `not scanned`.
...
Check the draft against the rubric's topic-statement test and show at most five failing items. Apply fixes or accepted exceptions already directed. If finalization is not yet authorized, show the draft and ask once via AskQuestion; then write §5 and set `finalized: <date>`. Reopening Stage 4 or 5 clears it.
...
- Ask one coaching question at a time through AskQuestion only when its research choice remains unresolved; use concise plain text when the tool is unavailable. A direction set or final decision already supplied in the request or session is recorded and not asked again.

...
1. **严格只读**。绝不创建、编辑或删除任何文件——不动计划、不动日志、不动 frontmatter。不做交互式决策树、不进 plan 模式、不派 Task subagent。用户想据此行动，就把他们指向对应 skill（`/rsch-plan-coach`、`/rsch-plan-decomposer`、`/rsch-plan-executor`）。

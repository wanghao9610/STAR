...
`CLAUDE.md` as the project rules document in 36 places where the others say `AGENTS.md` (still open).
...
- **`.agents` structure**, by the exemption above. This is where content has actually been lost:
  `## Dialogue Discipline` exists in 14 of 15 `SKILL.md` in the other three trees and 7 of 15 in
  `.agents`, and roughly 156 lines shared by the other three are missing from it — including an
  honesty rule in `star-refs-reviewer`. Restoring that is open work.
...
- **Description length.** `.kimi-code` descriptions are 25–60% shorter than `.claude`'s, with
  substantive clauses cut. Whether that reflects a real platform budget is undecided; if it is real, it
  should be written down here and enforced.

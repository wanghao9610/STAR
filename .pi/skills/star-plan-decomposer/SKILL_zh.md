...
- 提问一律用普通文本（Pi 没有结构化提问工具，纯文本就是提问机制本身，不是退路）——仍一次只问一题。 **问题所指的内容装不进选项时**——子计划清单、某份子计划草稿、回写父计划的索引草稿、评分表不达标项——这些内容写在同一条消息的正文里、排在问题之前，选项只装答案。两种形式下内容与问题都同来同止，中间不隔别的：用户收到的若是一排光选项，那是内容丢了，不是被压缩了。

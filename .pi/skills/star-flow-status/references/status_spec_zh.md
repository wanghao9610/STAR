...
若这次运行被 `PLAN_NAME` 收窄过，三个数都在收敛后的范围内算（见上面的收敛范围一节），不按整个项目算。
...
每一行**必须全部条件同时成立**才触发。其余一律沉默——特别是：还在跑的 run 什么都不欠，未 `done` 的叶子对下游什么都不欠。**任何一行都不会在已丢弃的节点、或它下面的任何东西上触发**：放弃掉的方向不欠任何东西。第 8、13 行里的「每个叶子」只数活着的叶子——已丢弃的叶子不是没做完的叶子。触发的行报一句话：欠的是什么、欠在哪个节点或哪个 run、以及补上它的命令。收敛规则：给了 `PLAN_NAME` 时，只检查属于该子树的产物（以及从其叶子 `exec_runs` 可达的 run）。

...
- Every question is plain text (Pi ships no structured question tool, so plain text is the mechanism rather than a fallback) — still one at a time, and still waiting for an explicit answer at the two confirmation points: the set of directions to keep (Stage 2) and the decision (Stage 4).

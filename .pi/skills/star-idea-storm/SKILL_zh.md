...
2. **一次只问一个问题**：每一道引导问题都作为一条普通文本问题发出，等用户答完再问下一题。禁止在一条消息里一口气列出多道题。每道题附 2–4 个短而具体的候选选项，选项来自问题库并结合用户已说内容起草，并标出你推荐的一项——每次都说明用户可以不选任何一项、自由作答。**每个选项要写清它会往 idea 文件里落下什么**，而不只是它叫什么（规约 §7.3）——对候选方向而言，就是它把这个 idea 定死在哪里、又丢掉了什么。承接前面答案的问题，用半句话点明承接之处（§7.10）。每答完 2–3 题，暂停并用一两句普通文本复述你听到的要点，再继续。例外：无法给出有意义候选的开放题（例如最初的种子）可以不带选项提问。
...
用问题库 Stage 2 的生成手法，从种子生成 3–5 个候选方向——每个候选带：一句话研究问题、关键假设（为什么现在可能做得动）、新在哪里、最接近的现有领域。彼此真正不同（原则 3）；邀请用户把自己的候选一视同仁放进池子。呈现一张表，然后用一个问题对着编号行提问（回复编号即可，推荐已标出）：留 2–4 个进扫描——候选正好 5 个时选项已经装不下（规约 §7.3 把一题的选项封在 4 个），改成给表加编号、对着编号问：*留推荐的那几个* / *留另一组（报编号）* / *重新生成*。被放下的候选留在 §2，标注 `not scanned`。写入 §2。

### Stage 3：文献扫描（`scan`）

对每个入围方向，按 `references/scan_policy.md`（中文对话读 `references/scan_policy_zh.md`）：起 2–3 个查询词，跑 Semantic Scholar / arXiv / DBLP 搜索端点加网页搜索，收 8–15 篇（标题 / venue / 年份 / 引用数 / 一小句相关性 / 记录 URL）。某方向扫完就立即写它的 §3 块：扫描表、竞争激烈程度说明（发文速率与走势、venue 分布、看得出的团队、有无综述）、3 篇最接近的工作及各自摘要**没有**声称的东西、表面缺口。默认深度为标题 + 摘要；只有用户指明某方向、或某条缺口判断直接决定两个入选方向的去留时才加深——读该方向 top-3 的 intro 与 related work 首段——并记录在该块的 `depth:` 行。扫描一次一个方向，每个方向一遍只读收集，各自按 `references/scan_policy_zh.md` 的收集器返回格式返回；没有任何东西并发，所以速率配额是一次一条请求地花掉，而不是拆成份额（规约 §6.9）；主 agent 负责写文件，并亲自下所有判断行（竞争激烈程度、最接近的工作、缺口）。意外——预期冷清处很拥挤、近 6 个月出现同问题 preprint——一发现就立刻说，不等阶段结束。搜索失败如实报告失败，绝不填充。
...
- 提问一律用普通文本（Pi 没有结构化提问工具，纯文本就是提问机制本身，不是退路）——仍一次只问一题，且两道关卡（Stage 2 的候选集、Stage 4 的决定）仍要等到明确回答。

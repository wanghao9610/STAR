...
5. **确认点 1：** 以普通文本呈现移入表——路径、证据、目的地、动作、风险——然后询问。候选 ≤4 条时对着编号逐行问、让用户回复行号；更多时提供 *全部批准* / *除某几条外全部批准（写出行号）* / *重做*。一条都不批准是有效结果 → 直接进 Step 2。
...
3. 按文件顺序逐条走——*按提议修* / *调整后修* / *跳过*，标出推荐，一次一条问题项（或一批同类项）。每处批准的修改写入后对该文件重跑 `compileall`；复检失败就把该处恢复原样并标 `reverted`。
...
- 所有确认点都用普通文本提问，一条消息一个问题：参数无法识别时的阶段选择、来源大面积缺失时的就绪决策、移入表的 确认点 1、每条打磨问题项、每处 README 小节变更、STAR README 或人工 README 的替换确认、以及中文 README 的提议。提问走 `star_questionnaire`，仍然一次一个，且任何写入前都必须拿到明确批准。

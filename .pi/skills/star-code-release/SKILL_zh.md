...
5. 呈现推广表——路径、证据、目标位置、动作、风险——并应用已经授权的行选择。移动集合仍未解决时，通过 `star_questionnaire` 按规约 §7.13 对可见清单问一次。一条都不批准是合法结果 → 跳到 Step 2。
...
- 未解决决定按规约 §7.2、§7.7 与 §7.13 处理。清楚点名阶段与确切写入范围的请求已经构成授权，不重复询问。只通过 `star_questionnaire` 为阶段歧义、缺少关键来源仍要编译、未定推广集合、实质打磨变化，或尚未具体授权的覆盖提问；工具不可用时用简洁纯文本。

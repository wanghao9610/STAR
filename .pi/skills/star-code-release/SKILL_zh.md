...
2. 解析参数：`gather` / `polish` / `readme` / `check` → 只跑该阶段；无参数 → 按顺序跑完整流程；其他 → 列出四个阶段名，询问指的是哪个。
...
4. 带缺口编译是允许且正常的——缺口会变成 README 的 TODO——但用户要先看到这张表。当多数来源都缺失时，直白地说现在编译出来的 README 大半是 TODO，并提议：*先跑产出方（推荐，写明是哪些）* / *就用现有的编译*。
...
5. **确认点 1：** 以普通文本呈现移入表——路径、证据、目的地、动作、风险——然后询问。候选 ≤4 条时对着编号逐行问、让用户回复行号；更多时提供 *全部批准* / *除某几条外全部批准（写出行号）* / *先解答我点名的几行* / *重做*——规约 §7.13 定的"先摊清单、再一次提问"。一条都不批准是有效结果 → 直接进 Step 2。
...
3. 用**一次**提问就这份清单定下来（规约 §7.13）——*全部按清单修* / *除我点名的以外全部修* / *先解答我点名的几条* / *一条都不修*，标出推荐。用户按编号点名，所以正文里的清单要编号。被点出来的条目开启第二轮，形状照旧。每处批准的修改写入后对该文件重跑 `compileall`；复检失败就把该处恢复原样并标 `reverted`。
...
3. 处理 `README.md` 已有内容，三种情况：
   - **带本 skill 的生成标记** → 先给出分节变更清单——每节一行：保留 / 改写 / 新增 / 删除，以及变了什么——再逐节询问。当前文本与本 skill 上次生成结果不同的节即人工改过：默认**保留**，并说明这一点。
...
4. `README.md` 用英文。当根计划的 `language` 是 `zh` 时，额外提供 `README.zh-CN.md`；两者都存在时各自带上互链的 `**Language:**` 行。中文 README 里，技术术语、指标名、数据集名和文件路径保持英文。
...
- 所有确认点都用普通文本提问，一条消息一个问题：参数无法识别时的阶段选择、来源大面积缺失时的就绪决策、移入表的 确认点 1、打磨问题项清单、每处 README 小节变更、STAR README 或人工 README 的替换确认、以及中文 README 的提议。提问走 `star_questionnaire`，仍然一次一个，且任何写入前都必须拿到明确批准。

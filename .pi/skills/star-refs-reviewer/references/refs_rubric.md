...
- `figure_candidates` — every figure on the paper's arXiv HTML page as `[{n, caption, image_url, referenced_at}]`, or `no html rendering`. Captions verbatim, trimmed past ~600 characters; `referenced_at` is the body sentences that cite that figure by number, verbatim, at most two per figure — those two fields are what the note's figure descriptions are written from, so a caption cut to its first line strands the figure the note keeps. The collector chooses nothing and downloads nothing: which figures the note carries, whether any qualifies, and what each shows are Step 3's call and the main agent's.

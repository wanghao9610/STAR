...
3. **先确认形状，再精读。** 精读是贵的那一步：把约 15 条排好序的候选（标题 / 会议 / 年份 / 引用数 / 一句话理由）先摆成一张带编号的表，再用一个问题对着编号提问（推荐的 5–10 条标在表里），只读用户留下的。检索和分类不需要批准；精读和写笔记需要。survey 模式下同一个直觉一次盖住整份文档的形状：一个问题带上检索画像、分类轴和分层阅读清单（Step 10.3）——这是 `involve=low` 会按推荐项直接采纳的判断型问题；完整流程的核心集确认在任何级别下都必答，不受影响。
...
检索按检索式分组一组一组地做——Pi 不提供子代理,按规约 §6.1 由本 agent 自己依次跑完。每组拿到其中 2–3 条检索式与检索要素,并按 Step 4 那份候选返回格式返回。没有任何东西并发,所以 `references/source_policy_zh.md` 里那份按 host 的请求配额是一次一条请求地花掉,而不是拆成份额（规约 §6.9）——外加每条候选一段封顶的逐字 `abstract`（约 60 词）。这个字段不是可选的:主 agent 必须在本 skill 唯一的必答确认点上摆出约 15 条排好序的候选,并按 `refs_rubric_zh.md` 重排,而那份评分表的核心判据是与本方法的直接重叠度——只看标题、会议、年份和引用数根本判不了。重排是主 agent 的活:它自己一句话说不清理由的候选,直接丢掉。每组还记下 `failures: [{host, error, retries}]`,并把抓回来的原始内容缓存在本次运行自己的 `raw/` 前缀下、一个检索式分组一个前缀（规约 §6.4）。走到确认点之前不能只是相信,要抽查:每组各挑一份缓存下来的内容重开,逐字核对其中两行和这一组返回的是否一致。Step 5 会为每个存活候选从头重抓权威记录,但那已经在确认点之后——用户挑哪几篇精读是在这里定下的,而所依据的那些行,还没有人重开核对过。
...
精读一次一篇，每篇一遍只读收集，各自按 `references/refs_rubric_zh.md` 的笔记收集器返回格式返回——不是"填好的模板",模板里有些字段只有真正写文件的那次会话才能填。只有 `depth_evidence` 带着真实的表题和一行数据时才接受 `depth: full`,否则降级。主 agent 负责写文件并亲自写 §5（与本项目的关系）——这一节需要方法上下文，也正是这篇笔记存在的理由。请求一次一条地打，对着完整的按 host 配额花，不拆份额——因为没有任何东西并发（规约 §6.9）；每一遍都记下 `failures: [{host, error, retries}]`；抓到的页面缓存在本次运行自己的 `raw/` 前缀下、一篇论文一个前缀（规约 §6.4）。Step 1 与 Step 4 的做法完全一样——这一步只是抓的 host 不同，规矩没有变。
...
2. **放宽检索。** Step 1 的机器原样跑——同样的画像出检索式、同样的分组方式与契约、同样按 `references/source_policy_zh.md` 的预算与缓存——但池子留着，不砍到 15 条：综述靠广度立足。池子每动一步就记一笔账：found → deduplicated → screened → tiered，写进综述的 Scope 一节。
...
4. **阅读。** 精读层与泛读层按 Step 3 的方式读——一次一篇、请求对着 `references/source_policy_zh.md` 的按 host 预算串行地打、一篇一个 `raw/` 前缀——各自按 `references/refs_rubric_zh.md` 的 survey 收集器返回格式返回。抓回的论文页挂出官方仓库的，照 Step 3 的规矩由主 agent 补一次 GitHub 调用、把分数补齐——精读层照例都补，泛读层只在本就抓了页面时顺手补；绝不为找仓库专门抓页面。本模式不带图：配图是 Step 3 的事，它们属于一篇本模式并不写的笔记。
...
- 核心集确认是完整流程里唯一的强制提问——一个普通文本问题，对着编号提问，推荐标在表里。survey 模式的那一问——画像、分类轴、分层清单（Step 10.3）——是判断型问题：`involve=low` 按推荐项直接采纳并记录；该模式在任何级别下仍必答的，是 Step 10.7 的覆写确认。`add` 里一个标题解析出多条候选记录、或一条也解析不干净，属于用户所指的歧义——任何级别下都要问，一个直接问题列出候选。提问一律用纯文本（Pi 没有结构化提问工具，纯文本就是提问机制本身，不是退路），精读前必须拿到明确答复。

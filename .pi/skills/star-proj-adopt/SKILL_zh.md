...
探测**按勘察项**来——源码、运行时、数据、权重、输出、启动入口——每项一遍只读收集，就按这个顺序（Pi 不提供子代理，规约 §6.1），每一遍的范围逐字写明："只读——不要跑这个项目的代码，不要 import 它的包，不要创建或修复任何环境；什么都不要写。"各自返回发现、证据路径、备选与未知项——**不返回置信标签**：`adopt_spec_zh.md` 里，是置信度决定了哪些行才够格走到确认点 1，所以 `certain` / `likely` / `unknown` 留到六项都跑完之后一并判，而不是在其中任何一遍里判。确认一条 `certain` 只需跑一条命令（`test -d`、查一下解释器版本），不必重读一遍，所以整个仓库的内容不会又涌回上下文。陌生仓库的开销就集中在这一步；下面的 S4 直接用这些勘察项收集到的证据，不再把它们的来源走第二遍。
...
- 三个确认点全部用普通文本提问，一条消息只问一个。Pi 没有结构化提问工具，纯文本就是提问机制本身，不是退路——仍然一次一问，且在任何跨确认点写入之前必须拿到明确答复。

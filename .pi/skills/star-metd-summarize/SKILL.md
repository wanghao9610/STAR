...
- **Missing** → write it.
- **Exists, generated by this skill** (`type:` + `generated:` present) → compare against the freshly compiled content. No substantive change → leave the file untouched and say so; do not churn the `generated` date. Substantive change → show the section-level change list (one line per section: added / rewritten / removed / unchanged, and what changed) on the page first, then ask to overwrite or skip — one question per document. When more than two documents differ, they may be batched into one question over the numbered list — *overwrite all* / *overwrite all but the ones I name* / *answer my questions on the ones I name first* / *skip all* — since five differing documents no longer fit as options (conventions §7.3, and §7.13 for the shape).

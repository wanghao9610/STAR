...
Resolve the output mode before running `scripts/scan.sh --slim`; treat its plan frontmatter, sub-plan indexes, and placeholder counts as the readiness check's input, and its `metds/` artifact frontmatter as the input to Step 5's stale check. The run-log and `wkdrs/` lines the scan also prints are not read (Principle 1). If it fails, read the plan files directly and report the fallback.
...
**Where this run executes.** Apply the relocation rule in conventions §10.8 before Step 0 on the PLAN tier. A clear request for a named output, draft, or scoped overwrite satisfies that same decision; whether a choice remains unresolved is decided once before Step 0 under §10.8's fourth condition — an existing method document the request did not name for overwrite keeps the run here.
...
- **Output language** follows the plans: the root's `language:`; with several roots, the majority; a tie takes the language resolved under conventions §7.6.
...
Do not partition by document target: a leaf commonly feeds several documents, so the file lists would overlap and the same plan be read three or four times, and conventions §6.2 wants disjoint file ownership across passes without exception. Deciding which leaf feeds which document is itself the reading this step splits up (`extract_map.md` settles it from a leaf's §2 inputs, §3 steps and §4 deliverables, never from the title), and Step 1 opens no plan body, so no exact per-document file list exists before the collection passes run.
...
- **Missing** → write it.
- **Exists, generated by this skill** → compare against the fresh compile. Leave an unchanged file untouched. Show one section-level change list for substantive differences; an explicit request to update this output authorizes the listed overwrite, otherwise ask once via `star_questionnaire` under conventions §7.13.
...
- Reply in the language resolved under conventions §7.6; the documents follow the plans' `language` (Step 1), which may differ from the dialogue. Keep technical terms — metric names, module paths, dataset names — in English inside Chinese documents.

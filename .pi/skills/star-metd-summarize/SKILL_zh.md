...
description: >-
  ...
  /skill:star-metd-summarize、一次运行点名它是下一步动作， 或想把定稿的研究计划汇总 / 凝练成方法表述、产出 overview / dataset / framework / training
...
**规模**：步骤 1 只读了 frontmatter，所以计划正文是在这一步才进入本次运行的。小树（≤ ~15 个计划）通常由主 agent 直接读更省事。更大的树则**按计划**切成几遍只读收集，一次一遍，各自依据完整映射、一组互不相交的完整计划文件，与 `extract_map_zh.md` 里的提取格式约定。每一遍都在自己持有的计划上为全部五个目标做抽取，并给每段标上它所喂的 `target:`。

被它取代的是**按文档目标**切分，那种切法错在两处。一个叶子常常喂多个文档，文件清单必然重叠、同一份计划被读三四遍——而规约 §6.2 要求各遍之间文件所有权互不相交，是无条件的。更要命的是第二处：判定哪个叶子喂哪个文档，恰恰就是这一步要切分出去的那次阅读——`extract_map_zh.md` 规定这个判定要看叶子的 §2 输入、§3 步骤、§4 交付物，并明令禁止按标题猜，而步骤 1 已经不再打开任何计划正文。所以除非先把那一遍的活干完，否则根本给不出按文档划分的确切文件清单。

每一遍只提取并返回；不写文件，不做跨计划的冲突解析，也不编译任何一份文档。`overview` 尤其是最后才编译的，要用其余四份文档编译后的内容——但为它做的抽取和别的目标没有区别：持有某个叶子的那一遍照样要为它返回该叶子的 §1 目标。
...
步骤 2 切成几遍来读时,合并之前先对账:切分点名的每一份计划,都要能在某一遍的 `plans_read` 计数里、在它的 `unread` 列表里,或者作为尚未读到的余量出现(规约 §6.3)。标为 `unread` 的计划要在步骤 4 之前打开——没读到的计划不是缺口,绝不能作为 TODO 进入步骤 4。然后拿各遍的返回回去对源头,而不是对你自己的印象:它们按格式约定返回的是**改写成可独立成立**的段落,而这些段落要进的文档,其原则 2 恰恰把"编造"点名为最主要的失败方式。回到被引用的计划、被引用的 §,核对(a) 每一遍随机三段,以及 (b) **每一段**带数值或路径的事实——超参、split 规模、阈值,以及 `datas/` / `inits/` / `${CODE_NAME}/` 路径。只要有一段是它的来源里没有的,这一整遍的返回就作废:那一片重读一遍。报出核对了多少段、失败了多少段。抽样只是把风险压住,并不能消除它——所以数值和路径类的事实要逐条全查,因为正是它们会变成论文里的错数字。
...
- **不存在** → 直接写。
- **存在且由本 skill 生成**（有 `type:` + `generated:`）→ 与新编译的内容比对。无实质变化 → 保持文件原样并说明；不要空转 `generated` 日期。有实质变化 → 分节变更清单（每节一行：新增 / 改写 / 删除 / 未变，以及变了什么）先落到正文，再询问覆盖还是跳过——一份文档一个问题。超过两份文档有差异时，可合并为一个问题，对着编号列表问：*全部覆盖* / *覆盖其中几份（报编号）* / *全部跳过*——五份文档都有差异时已经放不进选项（规约 §7.3）。
...
- 五个确认点都用普通文本提问：就绪门槛的草稿豁免（对未完成的树编译草稿）、无法识别的 OPT、选哪个根子树（多根树）、每次覆盖已生成文档、以及挡路的手写文档。Pi 没有结构化提问工具，纯文本就是提问机制本身，不是退路；且覆盖任何已存在文件前仍必须取得明确批准——没有明确批准也绝不越过就绪门槛。

...
2. **广收集，先核实再报告。**可用时调用 `star_subagent`，设 `agent: "star-collector"`，READ 档值非空时传给 `model`，从未参与代码的上下文收集；数量按范围决定。每个受托者遵循 `references/review_rubric_zh.md`，绝不写入或给整体结论。工具不可用时本地做一次并记录限制。每条 blocker/major 进报告前都重读引用。
...
2. 解释参数，先匹配者生效：
   ...
   - 都不匹配 → 列出最接近的计划与路径候选，通过 `star_questionnaire` 直接问一个问题。
...
- **派几个由主 agent 定**（规约 §6.2）：并行调用 `star_subagent`，设 `agent: "star-collector"`，READ 档值非空时传给 `model`；每个拿到评分表、同一份规则摘要和确切文件清单。收集器不写文件、不越界、不定整体结论。约 50 个文件内可用一个，更大范围逐 package 按 10–15 个分组。
...
3. 应用获授权修复前，逐项点名 `file:line` 和具体变化。用户要求修复但范围仍未确定时，把具体合格清单摊在正文并通过 `star_questionnaire` 按规约 §7.13 只问一次；绝不反问只复核用户是否扩大任务。删除或覆盖需要其自己的具体授权，已有适用决定直接沿用。
...
- 已有明确修复授权在范围内跨严重度生效；不因发现是 `blocker` 或 `major` 重复询问。修复请求后的范围仍未定时，按规约 §7.13 把具体清单放在一次 `star_questionnaire` 上方；工具不可用时退回简洁纯文本。只复核请求绝不转成这次提问或任何写入。计划模式下报告跟随计划 frontmatter 的 `language`，否则跟随已解析回复语言。

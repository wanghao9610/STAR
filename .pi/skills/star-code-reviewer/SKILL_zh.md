...
5. **修复是例行的、不改行为的；问不问由严重度决定。** 报告之后走一轮修复，只覆盖 docstring、作用域内改名、未使用的 import、本项目引入的死代码。`minor` / `nit` 直接改，边改边点名；`blocker` / `major`，以及每一次删代码，先单独提问批准再写入——整张清单先摊到正文，再用一次提问定下来（规约 §7.13），标出推荐。每条写入后复检，复检不过就恢复原样。要问的那些绝不把用户看不见的清单拿去批准；绝不顺手"改进"相邻代码（AGENTS.md §3）。
...
4. 只留下可审的源码：Python 文件走完整评分表；范围内的 shell / YAML / 配置文件只查维度 D（路径与运行时）；`datas/`、`inits/`、`wkdrs/` 产物与生成文件不在范围内。审查前报出最终文件数；超过约 50 个文件时，先跑 Step 2 的全树筛查——它只是 grep 和 `wc`，不需要环境——再说明情况，并提议收窄（某个子包，或 diff 模式），同时用筛查结果指出证据已经在哪里。让用户去猜挑哪个子包，是更差的提议。
...
- **收集一律外派，范围多小都一样。** 跑到这条命令时，主 agent 的上下文已经不是这份代码的中立读者：它一直在讨论这份代码；范围内有文件是本轮对话此前写出或改过的，它重读时用的还是当初产生那段代码的那套推理。收集器的上下文两样都没见过，这正是要的效果——所以主 agent 从不自己收集问题项。报告的范围行写明：跑了几个收集器、文件在它们之间怎么分的；本会话写过范围内的文件时，一并写明代码出自本会话。
...
- **切分是自动的，不为它征求同意。** 跑这条命令，请求的就是这几遍收集——Step 3 正是这个技能写明要收集的地方，用户亲手敲的名字也好，agent 自己因任务明显匹配而接手也好（规约 §10.2），都一样。收集派给 `star_subagent`（`agent: "star-collector"`），按切分的顺序一遍一个；跑这条命令本身就是在请求这几遍收集，没有什么可问的。参与度档位同样不改变这一点：`high` 只是在开工前把切分方式说一声，绝不变成一个待答的问题（规约 §6.8）。
- **计划模式加维度 F**（主 agent 做，不放进任何一遍收集——它需要计划上下文）：§3 任务到代码的映射、§4 交付物在磁盘上核对、§5 支撑检查、EXEC_LOG 与代码交叉核对。

### Step 4：核实

先把文件数对上：发出去的每个文件，都要落在某个收集器的 `files_reviewed` 计数里，或者落在它的 `unknowns` 列表里；对不上的那些，连同 unknowns 一起补读一遍（conventions §6.3）——没人能解析的文件，绝不能被当成干净文件算过去。然后合并去重。每条 blocker/major：不论收集器给的 `confidence` 是什么，都重新打开被引用的文件与行，确认问题真实、规则确实适用——blocker 上标着 `low`，是更该去读它的理由，不是可以少当回事的理由。不成立的降一档或丢弃；重读也定不下来的 `low` 条目进 Unconfirmed。minor 抽查。值得一提但未确认的进报告的 **Unconfirmed** 列表——绝不计入结论统计。

...
2. **广收集，先核实再报告。** 收集一律交给只读收集遍——一个没参与过这份代码的上下文——但每条要进报告的 blocker/major 问题项，主 agent 都要重读被引用的代码确认；站不住的降级或丢弃。评价一份 review 看的是问题项的精度而不是数量——一条错误的 blocker 就足以让报告失去可信度。
...
5. **修复是例行的、逐项批准的、不改行为的。** 报告之后提供修复轮，只覆盖 docstring、作用域内改名、unused imports、本项目引入的死代码。每项先单独提问批准再写入——一次一条问题项（或一批同类项），标出推荐——写入后复检。绝不打包默批；绝不顺手"改进"相邻代码（AGENTS.md §3）。
...
4. 收敛到可审源码：Python 文件走完整评分表；范围内的 shell / YAML / 配置文件只查维度 D（路径与运行时）；`datas/`、`inits/`、`wkdrs/` 产物与生成文件不在范围内。审查前报出最终文件数；超过约 50 个文件时，先跑 Step 2 的全树筛查——它只是 grep 和 `wc`，不需要环境——再说明情况，并提议收窄（某个子包，或 diff 模式），同时用筛查结果指出证据已经在哪里。让用户去猜挑哪个子包，是更差的提议。
...
- **这里没有中立读者，报告要写明这一点。** 跑到这条命令时，主 agent 的上下文已经不是这份代码的中立读者：它一直在讨论这份代码；范围内有文件是本轮对话此前写出或改过的，它重读时用的还是当初产生那段代码的那套推理。另开一个收集器的上下文两样都没见过，这正是要的效果——而 Pi 给不出这样一个上下文。于是下面那份切分就在本上下文里一片一片地做，原则 2 要求的「每条 blocker/major 都回到被引行重读」就是用来顶替这份缺失的中立性的。报告的范围行写明：文件怎么分的、没有独立收集器读过它们；本会话写过范围内的文件时，一并写明代码出自本会话。
- **派几个由主 agent 定**，按工作量来（规约 §6.2）：只读收集遍，一次一遍，每个拿到评分表、Step 1 构建的评判依据摘要（同一块内容，逐字发给所有人）和确切的文件清单，按 `references/review_rubric_zh.md` 规定的结构化问题项格式返回。一遍只读收集绝不写文件、绝不越出自己的清单、绝不给整体结论打分。掂量的数：Step 0 已经提议收窄的那约 50 个文件以内，一遍就够；再多就按包/目录切分，每片 10–15 个文件——60 个文件的树分成四五遍，不是六十遍。往哪边偏都是主 agent 的判断，只要说得出为什么——本身连成一块、稍微长一点的包，宁可整块派出去，也不为凑片大小切开。
- **切分是自动的，不为它征求同意。** 跑这条命令，请求的就是这几遍收集——Step 3 正是这个技能写明要收集的地方，用户亲手敲的名字也好，agent 自己因任务明显匹配而接手也好（规约 §10.2），都一样。Pi 根本不提供委派能力，规约 §6.1 对此有直接答案：收集器契约照旧，由本 agent 按切分的顺序在本地逐片填；派不出去的事情，也就没有什么可问的（§6.7 点明了因此损失的东西——主 agent 没法自己给自己的第二意见——上一条要求把它写进报告）。参与度档位同样不改变这一点：`high` 只是在开工前把切分方式说一声，绝不变成一个待答的问题（规约 §6.8）。
- **计划模式加维度 F**（主 agent 做，不放进任何一遍收集——它需要计划上下文）：§3 任务到代码的映射、§4 交付物在磁盘上核对、§5 支撑检查、EXEC_LOG 与代码交叉核对。

### Step 4：核实

先把文件数对上：发出去的每个文件，都要落在某个收集器的 `files_reviewed` 计数里，或者落在它的 `unknowns` 列表里；对不上的那些，连同 unknowns 一起补读一遍（conventions §6.3）——没人能解析的文件，绝不能被当成干净文件算过去。然后合并去重。每条 blocker/major：不论收集器给的 `confidence` 是什么，都重新打开被引用的文件与行，确认问题真实、规则确实适用——blocker 上标着 `low`，是更该去读它的理由，不是可以少当回事的理由。不成立的降级或丢弃；重读也定不下来的 `low` 条目进 Unconfirmed。minor 抽查。值得一提但未确认的进报告的 **Unconfirmed** 列表——绝不计入结论统计。
...
2. 按报告顺序逐条走可修问题项——*照建议修* / *调整后修* / *跳过*，标出推荐，一次一条。同类超过 4 条（如 12 处缺 docstring）可合并为一问：*全修* / *选哪些（报编号）* / *全部跳过*。
...
- 修复轮的批准一律用普通文本提问——一次一条问题项（或一批同类项）。Pi 没有结构化提问工具，纯文本就是提问机制本身，不是退路：Pi 没有结构化提问工具，纯文本就是提问机制本身，不是退路：仍然一次一条，任何写入前先获明确批准。

...
- All fix-pass approvals go through `star_questionnaire` — one finding (or one same-type batch) per call: still one at a time, and an explicit approval is required before any write.

...
- **Collection has no neutral reader here, and the report says so.** By the time this command runs the main agent's context is not a neutral reader of this code: it has been discussing it, and where earlier turns of this conversation wrote or edited files now in scope it rereads its own code through the reasoning that produced it. A separate collector's context would have seen neither, which is the point — and Pi has none to offer. The partition below is therefore worked shard by shard in this context, and Principle 2's re-read of every blocker and major at its cited line is what stands in for the missing neutrality. The report's scope line records how the files were split, that no independent collector read them, and, when the session wrote files in scope, that the code was written in this session.
- **How many is the main agent's call**, sized to the work (conventions §6.2): read-only collection passes, run one at a time, each given the rubric, the review rule digest built at Step 1 — the same block, verbatim, for all of them — and its exact file list, returning the structured finding contract in `references/review_rubric.md`. A read-only pass never writes, never reviews outside its file list, never grades the overall verdict. The number to size against: one pass carries a scope up to the ~50 files where Step 0 already offers to narrow; past that, partition by package/directory in shards of 10–15 files, so a 60-file tree goes to four or five passes and not to sixty. Either side of that is the main agent's to take once it can say why — a package that hangs together and runs a little long stays whole rather than being cut to hit a shard size.
- **The partition is automatic; no approval is asked for it.** Running this command is the request for these passes — Step 3 is where this skill says it collects, whether the user typed its name or the agent picked the skill up on a task that plainly matched (conventions §10.2). Pi offers no delegation at all, which conventions §6.1 answers directly: the collector contract stands and this agent fills it locally, in the order the partition sets, with no question to ask about a dispatch that cannot happen (§6.7 names what is lost — the second opinion a main agent cannot give itself — and the bullet above records it in the report). The involve level moves nothing either: at `high` the partition is announced before the passes start, never put to a question (conventions §6.8).
...
2. Walk the eligible findings in report order — *apply as proposed* / *apply adjusted* / *skip*, recommendation marked, one finding per question. More than 4 same-type findings (e.g. 12 missing docstrings) may be batched into one question: *apply all* / *select which (say the numbers)* / *skip all*.
...
- All fix-pass approvals are plain-text questions — one finding (or one same-type batch) per message. Pi ships no structured question tool, so plain text is the mechanism rather than a fallback: still one at a time, and an explicit approval is required before any write.

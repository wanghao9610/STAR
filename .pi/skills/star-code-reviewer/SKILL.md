...
One message, four results — the conventions from the three `bash` calls with the `.env` line riding the first, and the six-dimension rubric with its finding format, which Step 1's digest and Step 3's collection work from, whole from its `read`. That line carries the reply language, the question level, and the three model keys this run and every delegate it dispatches take their model from (§10.8). The calls stay separate because each tool result carries its own size limit: a result past roughly 30 KB is written out to a file that costs a second round trip to read back — exactly the round trip the one message exists to avoid — and the conventions excerpt is about 61 KB in total, split 21, 21 and 19 across its three calls, while the `read` result arrives whole on its own budget. Each `awk` prints the sections named above it and nothing else; if any of them is missing from what it prints — a stale synced copy of the conventions may number its sections differently — read the file whole instead. Later steps use the rubric as loaded here — no separate read; the report template and Step 1's project-side reads load at their own steps, not here.
...
For this skill the fourth condition holds only under the `auto=unattended` grant: without it, blocker and major fixes and every code-deleting fix are asked at every level and turn on what the review finds, so a run started by hand stays here.
...
2. Interpret the argument, first match wins:
   ...
   - Nothing matches → list the nearest plan and path candidates and ask.
...
- **How many is the main agent's call**, sized to the work (conventions §6.2): read-only collection passes, run one at a time, each given the rubric, the review rule digest built at Step 1 — the same block, verbatim, for all of them — and its exact file list, returning the structured finding format in `references/review_rubric.md`. A pass never writes, never reviews outside its file list, never grades the overall verdict. One pass carries a scope up to the ~50 files where Step 0 already offers to narrow; past that, partition by package/directory in groups of 10–15 files, so a 60-file tree goes to four or five passes and not to sixty. Either side of that is the main agent's once it can say why — a package that hangs together and runs a little long stays whole rather than being cut to hit a group size.
- **The partition is automatic; no approval is asked for it.** Running this command is the request for these passes — Step 3 is where this skill collects, whether the user typed its name or the agent picked it up on a plainly matching task (conventions §10.2). Collection goes to `star_subagent` dispatches (`agent: "star-collector"`), one per pass in the order the partition sets, with no question to ask about a partition running this command already requested. The involve level moves nothing either: at `high` the partition is announced before the passes start, never put to a question (conventions §6.8).
- **Plan mode adds dimension F** (main agent, not the collection passes — it needs the plan context): the §3 task-to-code map, §4 deliverables on disk, §5 support, and the EXEC_LOG-vs-code cross-check.
...
2. **Severity decides what is asked.** An eligible `minor` or `nit` finding is applied unasked: the rubric already wrote the fix, it rewrites text in place, item 4 re-checks it, and git holds the version before it — a question per docstring buys nothing and spends the attention the report needs. An eligible `blocker` or `major` finding is normally asked first, at every involve level, and so is every fix that deletes code whatever its severity: a symbol nothing appears to reference may still be reached through a registry string. With `auto=unattended`, apply the recommended eligible behavior-preserving fixes of every severity without asking, but skip and route every deletion instead of authorizing it. At `involve=high` without that grant, the unasked half is asked too, batched by type — the level tightens what a skill takes on its own and never loosens it (§7.9).
3. Name the unasked fixes in the Step 6 digest before applying them (`file:line` and what changes), then put the asked ones on the page as one numbered list — `file:line`, severity, what changes — and settle it with **one** question over that list (conventions §7.13): *apply all as listed* / *apply all but the ones I name* / *answer my questions on the ones I name first* / *apply none*, recommendation marked. The user names findings by their numbers, so number the list on the page. Findings the user pulls out open a second round in the same shape. A fix that deletes code is never a row on that list — item 2 has it asked on its own.
...
- Git: read-only, plus the single optional fix commit staging only fix-pass files (conventions §1); `auto=unattended` selects that commit only when Step 0 found a clean tree. On a run's execution branch it lands on the branch, ahead of its merge (conventions §11); this skill still never switches branches.
...
- The fix-pass approvals that are asked — `blocker`/`major` fixes and every deletion — go through `star_questionnaire`: the whole list on the page, then one question over it (conventions §7.13), each deletion asked on its own; explicit approval is required before any write. What Step 7 applies unasked is named in the same reply that applies it.
...
- With `auto=unattended`, non-deleting eligible fixes and their clean-tree commit are pre-approved; deletion fixes are skipped and routed, never silently broadened.

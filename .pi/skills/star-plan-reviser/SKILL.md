...
2. **Collect wide, judge in the main agent.** Evidence gathering is delegated to **read-only collection passes**, one at a time — execution log / artifacts / code state — each returning the structured collector contract in `references/review_spec.md`. Collectors never write and never propose revisions; synthesis and judgment stay in the main agent.

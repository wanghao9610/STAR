...
2. If the target is absent or ambiguous, list concise candidates (prefix + slug + one-line state) and ask one direct question via `star_questionnaire` — prefer nodes with execution evidence (`exec_runs` non-empty) or known drift.

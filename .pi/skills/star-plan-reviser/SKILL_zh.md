...
2. 未给参数或匹配歧义时，列出候选（前缀 + slug + 一行状态）并询问——优先推荐有执行证据（`exec_runs` 非空）或已知失配的节点。
...
规模超过这个的：按 `references/review_spec_zh.md` 的收集器格式约定一次一遍地做只读收集——通常是 **log reader**（步骤状态、自报检查、"待用户执行"命令、方向性信号）、**artifact inspector**（§4 每个交付物：存在 / 大小 / 修改时间 / 低开销 合理性检查），以及当 §2–§3 涉及代码时的 **code inspector**（承诺的模块是否真的写出来了、与日志声称的改动是否一致）。
...
- 提问一律用纯文本（Pi 没有结构化提问工具，纯文本就是提问机制本身，不是退路）——仍然一次一条候选，仍然先获明确批准再写入。

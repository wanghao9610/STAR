...
2. **只有叶子可执行**。若 `PLAN_NAME` 命中一个有子节点的节点(`children:` frontmatter 非空),不要直接执行它:列出它的叶子(前缀 + slug + 一句话目标),用 `star_questionnaire` 让用户选执行哪一个(推荐依赖顺序中第一个就绪的),或提议按依赖顺序一次一个地执行它们。
...
**实现前先按 `references/design_check_zh.md` 跑设计检查。**调用 `star_subagent`，设 `agent: "star-auditor"`，PLAN 值非空时传给 `model`，只提供 EXEC_PLAN、叶子计划和根计划 §4。重开每条 `fail` 证据；只有研究范围、验收、关键输入或成本仍未解决时才通过 `star_questionnaire` 询问。
...
**把 Step 5 交给 EXEC 档。**记录与决定均已定下时，调用 `star_subagent`，设 `agent: "star-runner"` 并传非空 EXEC `model`，从 Step 5 恢复并携带控制令牌。不得改 EXEC_PLAN 或绕过计划级缺口；返回后重读日志。不可用时本地运行。

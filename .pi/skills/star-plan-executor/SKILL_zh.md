...
**把档位模型传给受托者。** 取 `pi` 条目，没有则取不带标签的备选，模型按 Pi 的 `provider/model` 写法填写。单任务把值传给 `star_subagent` 的 `model`，并行与串行分别传到相应 `tasks[]` / `chain[]` 项。它覆盖命名代理原有的模型；档位值为空则省略参数并保留原继承行为。盲读用 `star-auditor`，有边界的收集用 `star-collector`，执行动作用 `star-implementer`。阶段交接，或本 skill 某一步启动的另一个 skill，须用通用受托者 `star-runner`，其权限取自该 skill 和交办说明。每次派发均启动全新进程，后文的范围与写入限制照旧。已安装扩展没有 `model` 字段或模型不可用时，保持原执行路径；键已设则说明一条原因。派发被拒后，须确认它尚未开始工作才能退回本地。受托者记录自己会话的实际模型，不用请求别名或父会话的解析命令代替。
...
2. **让计划可见，再在范围内推进。**建立并展示具体 `EXEC_PLAN`。`/star-plan` 是用户的切换命令，本 skill 不能调用，也不要求用户先切换；只有实质决定仍未解决时才用 `star_questionnaire`。普通范围内实现与轻量验证已获授权。
3. **该委派时才委派。**有边界、相互独立的工作确有收益时调用 `star_subagent`：实现设 `agent: "star-implementer"`，只读勘察设 `agent: "star-collector"`，并按前文传档位 `model`。主 agent 负责集成与核验。
...
**本次运行在哪里执行。** 本次运行的档位是 PLAN；它留在启动它的会话里，用会话的模型跑。`STAR_PLAN_MODEL` 所指的模型不是会话正在跑的模型，或带有本宿主只能逐次派发时应用的深度时，在开头用一行说明——档位、该模型与深度，以及两种得到它们的办法：切换会话模型，或经 `star-auto` 启动这次运行（规约 §10.8）——然后在这里照常继续。带 `tier=` 的运行已按档位启动，不给这一行。档位变化是后文把执行—验证阶段（Step 5）交给 EXEC：判断交接前还缺哪些决定时，既有执行授权视为已定；带 `tier=` 的受托者不再次转交这一阶段。

### Step 0：定位目标计划

1. 解析 `PLAN_NAME`(slug / 数字前缀 / 完整文件名),与扫描摘要列出的计划匹配;它就是那份清单,不必再列一次目录。
2. **只有叶子可执行**。若 `PLAN_NAME` 命中一个有子节点的节点(`children:` frontmatter 非空),不要直接执行它:列出它的叶子(前缀 + slug + 一句话目标),用 `star_questionnaire` 让用户选执行哪一个(推荐依赖顺序中第一个就绪的);其余每个叶子各是一次独立的运行(规约 §10.4),在已有执行目标下等本次运行结束后再启动(§10.6)。
3. 若未给参数或匹配有歧义，列出可选计划并通过 `star_questionnaire` 询问。
4. 完整读取选定的子计划。
...
1. `/star-plan` 是用户的开关 只在计划仍有待用户决定的实质选择时使用；`low` 或目标、范围、验收、关键输入、成本与权限已定时，在当前模式起草，不制造批准门槛。
...
**实现前先按 `references/design_check.md` 跑设计检查。**调用 `star_subagent`，设 `agent: "star-auditor"`，PLAN 值非空时传给 `model`，只提供 EXEC_PLAN、叶子计划和根计划 §4。重开每条 `fail` 证据；只有研究范围、验收、关键输入或成本仍未解决时才通过 `star_questionnaire` 询问。

1. 在 commentary 展示具体计划。沿用已有授权，只为仍未解决的实质选择使用本宿主原生提问机制；`/star-plan` 是用户的开关 已启用时在决定解决后退出。提交、分支与 worktree 按 involve 档位处理；`auto=unattended` 仅覆盖不改变范围、关键输入、§5 验收和已批成本的战术偏差。
...
**把 Step 5 交给 EXEC 档。**记录与决定均已定下时，调用 `star_subagent`，设 `agent: "star-runner"` 并传非空、且不是本 run 所在模型别名（规约 §10.8）的 EXEC `model`，从 Step 5 恢复并携带控制令牌。不得改 EXEC_PLAN 或绕过计划级缺口；返回后重读日志。不可用时本地运行。
...
1. 按 `references/agent_dispatch_spec_zh.md` 调用 `star_subagent`：实现设 `agent: "star-implementer"`，只读勘察设 `agent: "star-collector"`，并传相应档位 `model`；提供确切 action 范围、解释器、检查和结构化返回契约。
...
**修正同步（战术信号）。**先应用已有具体决定，只把未决行摊在正文并通过 `star_questionnaire` 按规约 §7.13 问一次。`auto=unattended` 只覆盖不改变研究范围、关键输入、§5 验收和已批成本的推荐战术行；章节号或 ENRICHED 标签不会扩大授权。已授权行按 `references/plan_sync_rules_zh.md` 写回。触及 §1/§6、父计划或 kill-criterion 的是方向性信号，不在此同步。
...
以结果开头，说明验证证据、保留的任务工作区和 run 产物位置、等待启动授权或结果的命令、已同步修正及剩余风险。点出本 run 在任何重命令前启动的独立审查。确认的 blocker/major 与获授权修复经 `star-plan-executor <叶子>` 回来核验。执行分支审查干净后进入 `references/branch_rules_zh.md` 的合并授权点：已有具体授权沿用，否则通过 `star_questionnaire` 询问。点出未合并分支和 worktree；重型结果就位后由 `star-expt-analyst <叶子>` 对照 §5 分析。控制在约 500 字以内。

### Step 9：审查并返回执行目标

当前执行器仍负责用户的执行目标。重命令启动或合并前，通过本宿主的 skill 调用方式对本 leaf 运行 `star-code-reviewer`；已有审查覆盖当前代码版本和范围、且待启动时满足现有启动守卫，才直接复用。把既有执行目标和授权传入，并要求 reviewer 将证据与控制权交回这里。单独的只复核请求没有这条接续。
...
- 非交互运行中继续普通已授权实现与轻量验证。仍有必需决定且 `star_questionnaire` 不可用时，用纯文本呈现具体问题，只停依赖它的 action，并继续无关的已授权工作；沉默或时间经过都不是批准。

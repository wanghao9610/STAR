...
**Hand Step 5 to the EXEC tier.** With records and decisions settled, call `star_subagent` with `agent: "star-runner"` and the distinct EXEC `model` to read this manifest and resume from Step 5 with `involve=<level> tier=exec` and any valid `auto=unattended` grant. It may not edit EXEC_PLAN or improvise around a plan-level gap. Re-read EXEC_LOG on return; if unavailable, run Step 5 here.

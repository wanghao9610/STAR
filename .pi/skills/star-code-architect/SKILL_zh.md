...
展示架构摘要与带编号的迁移表。先应用针对这张表已经给出的清楚选择或批准。仍有未定条目时，通过 `star_questionnaire` 按规约 §7.13 对可见清单只问一次；只有已经定下的条目进入工作清单。"零迁移"是合法结果 → 直接跳到 C4。
...
- 只有实质选择或权限仍缺失时才通过 `star_questionnaire` 一次问一个具体问题；工具不可用时退回简洁纯文本。对同一仓库、架构、迁移表或动作已有的明确授权继续有效，不重复询问。

...
5. **主 agent 编排、复核，并亲自动手。**Pi 不提供子代理，所以下面每一处写着*派发*的步骤都按规约 §6.1 办：契约照旧，由主 agent 在本地按同样顺序、同样返回格式自己填。勘察是只读的，一次一个关注点；迁移一次一组，每组只写本组自己的文件。两者都是文件所有权互不相交、返回结构化结果。主 agent 亲自重跑每项检查（不信任自报的 pass），每验证完一组就打一个 git 检查点，重试 ≤2 次，仍失败则回滚。格式约定见 `references/orchestration_spec_zh.md`。
...
一次勘察一个关注点——结构与依赖、配置系统、数据管线、训练/评估入口、脚本与工具、测试与文档——每一遍都只读，各自按 `references/survey_spec_zh.md` 返回结构化报告（规约 §6.1：宿主不提供派发，主 agent 就按这个顺序、这个返回格式自己走完每一遍）。汇总成**仓库地图**：模块清单、依赖方向、排序后的可疑写法（只收会促成迁移项的可疑写法）。
...
以普通文本展示架构摘要与编号迁移表。随后确认：迁移项 ≤4 条时对着编号逐条问、让用户回复要批准的编号；更多时给 *全部批准* / *全部批准但排除（写出编号）* / *重新设计*。等到明确答复；只有获批条目进入工作清单。"零迁移"是合法结果 → 直接跳到 C4。

#### Step C3：执行迁移

把获批条目划分为**文件所有权互不相交**的组（`references/orchestration_spec_zh.md`）；有依赖的组按依赖顺序走，宿主不提供派发，相互独立的组也串行。一次做一组，格式约定为：范围原文照录（"只做这些条目"）、明确文件清单、只做例行移动 + import 修正——不顺手改别的——通过 `.env` conda 环境运行、结构化返回（`changed` / `ran` / `check` / `blockers`）。每组完成后**主 agent 亲自复核**（compileall、import 扫描、可跑的快速测试），然后提交：`star-code-architect: migrate <ids> — <summary>`，只暂存本 skill 涉及的路径。失败 → 把失败信息回传后重试 ≤2 次 → 仍失败：用 git 回滚该组路径，在迁移记录中把条目标 `blocked`，继续其他组。
...
- 两个确认点与所有提问都用普通文本——一条消息只问一题。Pi 没有结构化提问工具，纯文本就是提问机制本身，不是退路：仍一次一题，且任何跨确认点副作用都要先拿到明确的批准文字。

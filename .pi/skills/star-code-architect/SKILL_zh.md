...
勘察按关注点分——结构与依赖、配置系统、数据管线、训练/评估入口、脚本与工具、测试与文档——每项一次只读的 `star_subagent` 派发（`agent: "star-collector"`），各自按 `references/survey_spec_zh.md` 返回结构化报告。汇总成**仓库地图**：模块清单、依赖方向、排序后的可疑写法（只收会促成迁移项的可疑写法）。
...
以普通文本展示架构摘要与编号迁移表。随后确认：迁移项 ≤4 条时对着编号逐条问、让用户回复要批准的编号；更多时给 *全部批准* / *全部批准但排除（写出编号）* / *先解答我点名的几条* / *重新设计*——规约 §7.13 定的"先摊清单、再一次提问"。等到明确答复；只有获批条目进入工作清单。"零迁移"是合法结果 → 直接跳到 C4。

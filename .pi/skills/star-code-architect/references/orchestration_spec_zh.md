...
1. 只取确认点 2 批准的条目。
2. 分组时保证**文件所有权互不相交**——而且要算出来、不是假定：对每个候选项跑 `grep -rln "<该模块带点号的完整 import 路径>" ${CODE_NAME}`，这些命中加上该项要移动的文件，**就是**它的所有权集合；集合相交的项并成一组。用带点号的完整路径而不是裸模块名：并多了只损失一点分组粒度，绝不损失正确性。需要修 import 的位置恰恰是一组开工之后才发现的，所以这件事不能等到那时候——不先算，前后两组就可能改到同一个文件，按组 `git restore` 也就失效了：任何文件不得同属两组（被移动的文件和因移动而需改 import 的文件都算）。两条目争用同一文件时，合并成一组。

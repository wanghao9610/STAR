---
description: Compile the finished plan tree into paper-ready method documents
argument-hint: "[overview | dataset | framework | training | evaluation] [DESCRIPTION]"
---
Read `.pi/skills/star-metd-summarize/SKILL.md` in full and follow it as this run's instructions. This command is the skill's only entry point here: `.pi/settings.json` sets `enableSkillCommands: false`, so no `/skill:` command stands beside it. Everything that file says about the files it reads first, the conventions it reads, the involve level and its own steps applies unchanged.

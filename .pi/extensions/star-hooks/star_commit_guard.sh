#!/usr/bin/env bash
# STAR tool_call hook (Pi) — decline the git commands that break the
...
# .pi/extensions/star-hooks/index.ts. One copy per harness tree (conventions §1,
# "The guard"), each differing only in how its harness names the command on the
# way in and the decision on the way out. This one is also the guard's whole
...
deny() { # $1 = one-line reason, $2 = what to do instead (default: hand it to the user)
    printf '%s (declined by .pi/extensions/star-hooks/star_commit_guard.sh — %s)\n' "$1" "${2:-hand it to the user to run}"
    exit 1
}

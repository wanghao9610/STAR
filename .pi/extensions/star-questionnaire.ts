 ...
 *
 * Vendored from pi's own examples/extensions/questionnaire.ts (pi 0.84.1, MIT), with the
 * tool renamed to star_questionnaire so a user-level copy of the same example can stay
 * installed — pi refuses to start on a duplicate tool name.
 */
...
export default function questionnaire(pi: ExtensionAPI) {
	pi.registerTool({
		...
		renderCall(args, theme, _context) {
			...
			let text = theme.fg("toolTitle", theme.bold("star_questionnaire "));

...
- **`star_subagent`** — delegation. Its roster is `.pi/agents/`: `star-collector` (read-only, returns the form it was given), `star-implementer` (one step, under a written contract), `star-auditor` (blind second read, scores nothing else). Scope defaults to `project`, which is that roster.
...
- **`/star-plan`** — read-only exploration. This is the **user's** switch, not a tool you can call. A skill that needs a no-side-effects gate holds it itself.

...
- **[v0.1.33](https://github.com/wanghao9610/STAR/tree/v0.1.33)** (2026-08-11) — 一份计划现在可以被放弃而不必删除：`star-plan-reviser` 在该节点上写入 `dropped: <日期> — <原因>`，所有 skill 都按整棵子树继承来读这个字段，于是一行就把这个节点连同它的后代移出计数、覆盖检查与下一步动作。`star-flow-status` 把它们渲染成 `⊗`、括号里保留丢弃前的状态并排除在三个数之外，`star-plan-executor` 与 `star-plan-decomposer` 拒绝对它们动手，`star-metd-summarize` 也不从它们编译任何内容——而父计划保留 `children:` 条目与索引行，行上标 `— dropped <日期>`，所以「这条路试过」仍然读得到。丢弃不隐藏的是磁盘上还在的东西：仍依赖着被丢弃节点的活叶子、未合并的执行分支、它下面的 worktree，各自照旧得到一条失配标记。

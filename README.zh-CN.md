...
- **[v0.1.4](https://github.com/wanghao9610/STAR/tree/v0.1.4)** (2026-07-29) — skill 启动提速，已按四棵工具树双语移植。每个 skill 现在用一条消息完成开场加载：规约、模板等整个文件改经文件读取工具进入上下文，不再用 Bash 里的 `cat`（Bash 结果超过约 30 KB 会被转存成文件、多付一次往返，仅规约一个文件就越过这条线），探测 `STAR_LANG`/`INVOLVE` 的 `.env` 查询也合入同一次调用。`SKILL_zh.md` 不再在运行时加载——它仍是供人阅读的完整镜像；中文运行时按规约 §7.6 以中文回复，只读取 `_zh` / `.zh-CN` 资源，每个 skill 正文省去约 11 KB 的重复加载。`star-flow-status` 一次调用收齐全部输入（扫描结果与规范文件），聊天回复字数上限统一为 500。新增一项维护者检查固定这套开场加载形态，改一半会被点名报错；`wkdrs/` 加入 `.cursorignore`。

...
此外，可加上 `INVOLVE=low|medium|high` 设定 STAR skills 在决策前询问的程度——`low` 遇到需要判断的地方直接采用推荐项（并记录），`medium`（默认）按文档提问，`high` 逐步确认；红线、提交、删除等安全确认点在任何档位都会询问。若只想对单次运行生效，可在调用 skill 时附带同一参数——如 `$star-plan-executor 00 involve=low`。完整规则见[研究工作流规约](docs/mds/star-workflow/research-workflow-conventions.zh-CN.md#7-对话纪律) §7.7。
...
有六个 skill 在动手之前都要打开同一批计划、run 日志与报告——`star-flow-status`、`star-expt-digest`、`star-plan-decomposer`、`star-plan-executor`、`star-plan-reviser` 与 `star-metd-summarize`——因此各自用一个只读脚本一次收齐，即所用工具目录下各自的 `scripts/scan.sh`，而不是逐个打开文件。这是一次 shell 调用，所以 agent 第一次运行它时会请求授权。
...
- **[v0.1.5](https://github.com/wanghao9610/STAR/tree/v0.1.5)** (2026-07-30) — 开场装载不再逐个文件走计划树。又有四个 skill——`star-plan-decomposer`、`star-plan-executor`、`star-plan-reviser`、`star-metd-summarize`——改用 `star-flow-status` 与 `star-expt-digest` 早已共享的那个只读采集脚本收齐，而且这次调用折进它们开场那条消息本来就要发的 Bash 调用里，不另占一趟往返：每个计划的 frontmatter、它的 `## Sub-plans` 索引与占位符计数，以及每份运行日志的 frontmatter，全在一份结果里到手；每个 skill 都保留退回逐文件读取的退路，并且走了退路必须在回复里说明。headless 实测，每种配置跑三次，模型为 Claude Opus 5，测试项目含三份计划与一份运行日志：冷启动跑单个 skill 从 5.7 轮模型往返降到 4.0 轮，工具调用从 8.0 次降到 4.7 次，建立的上下文从 244k 降到 176k token，且整条轨迹里再没有任何一次工具调用触及 `metds/plans/`——过去那里要花掉一整轮往返逐个打开计划。每个 skill 的开场装载同时写明了什么时候可以跳过：同一轮对话里的第二个 skill，凡是此刻仍能逐字看到的部分就复用——同一种语言的规约且至少覆盖它点名的那些节、同样的参考文件、探测行给出的 `STAR_LANG` / `INVOLVE` 取值——唯独采集脚本的摘要不复用，它是文件在某一刻的快照，其间可能已有运行写过盘，所以每次都重新扫描一遍。这一段买的是确定性而非速度，实测把话说得很直白：基线里模型本来就每次都复用了上下文里的规约，而链式的两 skill 会话打成平手（两次合计 6.7 轮对 6.3 轮），因为第一个 skill 不再把计划正文拖进上下文供第二个白捡。新增一项维护者检查固定这一段——每个清单一段、按语言逐字一致、位于开场装载块之内、且不带裸 `§n`（第 20 项检查会把那里的 `§n` 读成"装载了哪些节"的声明）——随仓库分发的 Claude Code 放行规则现在覆盖全部六个采集脚本。
- **[v0.1.4](https://github.com/wanghao9610/STAR/tree/v0.1.4)** (2026-07-29) — skill 启动提速，已按四棵工具树双语移植。每个 skill 现在用一条消息完成开场加载：规约、模板等整个文件改经文件读取工具进入上下文，不再用 Bash 里的 `cat`（Bash 结果超过约 30 KB 会被转存成文件、多付一次往返，仅规约一个文件就越过这条线），探测 `STAR_LANG`/`INVOLVE` 的 `.env` 查询也合入同一次调用。其中两个 skill 把这次加载再收窄到自己真正用到的章节：`star-expt-digest` 与 `star-refs-reviewer` 在同一次 Bash 调用里用 `awk` 取出规约的相应章节，每次运行分别少读 25% 与 23%；两者都写明装载了哪些、不装载哪些又在自己文件的哪处复述，以及真需要时如何整份读回。其余十二个仍然整份读入，这是有意的（`star-flow-status` 早已只读它要汇报的那几节）：摘录必须走 Bash 才能成为摘录，而一个会提交、会提问、又要产出登记产物的 skill 用到的篇幅太大，取节后仍会回到转存阈值附近——逐 skill 的审计（每项拟省略的章节都过两轮对抗性复核）把四个看起来安全的候选送回了整份装载。`SKILL_zh.md` 不再在运行时加载——它仍是供人阅读的完整镜像；中文运行时按规约 §7.6 以中文回复，只读取 `_zh` / `.zh-CN` 资源，每个 skill 正文省去约 11 KB 的重复加载。`star-flow-status` 一次调用收齐全部输入（扫描结果与规范文件），聊天回复字数上限统一为 500。新增一项维护者检查固定这套开场加载形态，改一半会被点名报错；`wkdrs/` 加入 `.cursorignore`。
- **[v0.1.3](https://github.com/wanghao9610/STAR/tree/v0.1.3)** (2026-07-29) — `star-refs-reviewer` 新增 `survey` 模式：检索一个话题，按声明过的三层去读（8–12 篇精读、15–25 篇读摘要、其余只凭记录），写出独立的领域综述 `metds/refs/<slug>_survey.md`——分类体系带着写明的划分轴，对比表只许精读层来填，每条论断都引用本轮抓取的来源，`reference.bib` 与 `refs_index.md` 一概不碰。精读前只问一次（画像、分类轴、分层清单），`involve=low` 按推荐项代答，一次综述可以从头到尾无人值守。追加模式旁边多了 `add` 形式：一次收多篇——arXiv id、DOI、URL、标题随意混用，每个标题都解析成抓取到的记录、绝不猜。已按四棵工具树双语移植，综述模板与评分规则随 skill 就位。

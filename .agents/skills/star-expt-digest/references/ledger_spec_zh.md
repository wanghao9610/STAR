...
2. 每一行都抄自某条 trail 条目；只在头部行记了 `model_id` 的产物（规约 §8），则把那个 id 记为一次写入事件。带 frontmatter 却没有 `model_trail` 的产物是**缺口**，连同"为什么没有"（写于该字段存在之前，或某个 skill 漏写）列进 §5——绝不假定单模型，也绝不靠猜回填。
...
4. 填 `assets/model_ledger_template_zh.md`（英文：`assets/model_ledger_template.md`）写入 `wkdrs/digests/MODEL_LEDGER.md`。每次运行都整份重新生成：直接覆盖，`generated:` 写今天的日期。

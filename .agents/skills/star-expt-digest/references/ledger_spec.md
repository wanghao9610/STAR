...
2. Every row is copied from a trail entry, or, for an artifact that records only a header-line `model_id` (conventions §8), from that id as one write event. A frontmatter artifact with no `model_trail` is a **gap**, listed in §5 with why it has none (written before the field existed, or a skill that skipped it) — never assumed single-model, and never back-filled by guessing.
...
4. Fill `assets/model_ledger_template.md` (Chinese: `assets/model_ledger_template_zh.md`) into `wkdrs/digests/MODEL_LEDGER.md`. It is regenerated whole on every run: overwrite it, with `generated:` carrying today's date.

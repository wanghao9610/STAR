...
**不要为了补充报告而打开该 run 的原始日志。** 报告就是接口。绕到它背后去"核对"、或补上它遗漏的某个指标，属于逐 run 分析——那是 `$star-expt-analyst` 的活，且带着本 skill 不做的核实环节。看起来不对的报告要转交出去（`$star-expt-analyst <run dir>` 刷新它），而不是在这里改正。

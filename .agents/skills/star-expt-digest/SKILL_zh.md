...
动手前，用一条消息装齐本 skill 全部无条件的开场读取——也就是本文件后文回指的那次开场装载：两次 shell 调用（以项目根目录为工作目录），外加 `<本 skill 所在目录>/references/scope_spec_zh.md` 的一次文件读取，三者同发。

...
阅读与叙述；不执行、不分析 run、不评判 criteria、不修订计划、不翻转状态。digest 揭示出的、超出写入边界的事情一律路由出去：未分析的 run 交 `$star-expt-analyst`，过期台账交 `$star-expt-analyst aggregate`，未执行的叶子交 `$star-plan-executor`，被证伪的 claim 交 `$star-plan-reviser`，当前树态交 `$star-flow-status`。

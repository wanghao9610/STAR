...
每个 run 只从它最新的 `EXPT_ANALYSIS_<date>.md` 读取：run 判定、§5 记分卡压缩成一行、报告记录的关键指标连同来源与 split，以及它写明的任何 blocker/major 观察、方向性信号或 kill-criterion 命中。不要打开该 run 的原始日志去补充报告——报告就是接口，绕到它背后属于逐 run 分析，那是 `$star-expt-analyst` 的活，且带着本 skill 不做的核实环节。

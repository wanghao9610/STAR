...
<!-- Written by star-expt-digest ledger. This file is GENERATED by rolling up the model_trail of
     every listed artifact on disk — it is never hand-maintained, and editing it here changes
...
<!-- Write events per model, by artifact kind. Counts only — never a quality judgment, and never a
     rate ("X wrote more" is not "X did better"). The kinds are the conventions §8 output-table rows. -->

...
Per run, from its newest `EXPT_ANALYSIS_<date>.md` only: the run verdict, the §5 scorecard in one line, the headline metrics with the source and split the report records, and any blocker/major observation or plan-level finding it names. Do not open the run's raw logs — the report is the interface, and going behind it is per-run analysis, which is `$star-expt-analyst`'s job with its own verification.

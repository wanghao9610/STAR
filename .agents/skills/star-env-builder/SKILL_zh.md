...
description: >-
  ...
  ENV_REPORT.md 和版本快照写入 wkdrs/。只要用户调用 $star-env-builder，或要求 Codex 创建或重建
  项目的 conda 环境或 venv、解析并安装依赖、或验证运行环境时，都应使用本 skill。支持中英文双语
  工作。
---
...
6. **证据式验收。**亲自跑三层冒烟测试（`references/smoke_test_spec_zh.md`），报告"验证了什么"并附证据，而不是一句"能用了"（AGENTS.md §10）。Chats end, files do not：报告与版本快照写入 `wkdrs/env_<ENV_NAME>_<日期>/`。

...
2. **一道门，场景化追问。**唯一的门是安装计划批准（Step 4）：门前不装任何东西；门覆盖的内容之后自主执行。场景化问题——覆盖已有环境、CUDA 不匹配、uv 缺失、venv 后端遇到 conda 专属依赖——遇到时一次问一题：有 Codex 结构化用户输入工具时用该工具，否则用一句简洁的纯文本提问，都带推荐项，行动前等待明确答复。
...
以普通文本呈现：后端 + 环境名 + python 版本；采用的依赖来源；各类别包数与关键锁定；torch↔CUDA 匹配（探测到的驱动上限 vs 选定的 wheel 源）；大 wheel 的下载量级；conda.txt 项；已标记的不确定项（CUDA 不匹配、未解析 import、版本冲突）。随后问一个问题——有 Codex 结构化用户输入工具时用该工具，否则用纯文本：*批准并构建* / *调整（说明哪里）* / *中止*——等待明确答复。所有不确定项在此处解决——绝不悄悄带过。
...
- 门与所有场景化问题一次只问一题——有 Codex 结构化用户输入工具时用该工具，否则用简洁纯文本——都带推荐项，等待明确答复；安装计划必须先收到明确的批准文字才能开始安装。

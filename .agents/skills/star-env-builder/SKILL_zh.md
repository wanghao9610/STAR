...
2. **一道门，场景化追问。**唯一的门是安装计划批准（Step 4）：门前不装任何东西；门覆盖的内容之后自主执行。场景化问题——覆盖已有环境、CUDA 不匹配、uv 缺失、venv 后端遇到 conda 专属依赖——遇到时一次问一题：使用 `ask_user_question` 工具，仅在非交互的 `codex exec` 下改用一句简洁的纯文本提问，都带推荐项，行动前等待明确答复。
...
4. 走阶梯安装（uv > pip > conda；conda 仅在 conda 后端下、且仅限白名单）。需要源码编译的项留在 STOP line 上：把确切命令备好，不要跑。
...
- 门批准过的安装自主执行，包括框架级别的大下载。无论是否批准都在 STOP line 外：`sudo` 或系统包管理器（apt / brew）、驱动或 CUDA toolkit 的系统级安装、CUDA 源码编译（flash-attn 类构建）、超过约 10 GB 的下载、删除任何环境。这些以确切命令写进报告移交。
...
- 门与所有场景化问题一次只问一题——使用 `ask_user_question` 工具，仅在非交互的 `codex exec` 下改用简洁纯文本——都带推荐项，等待明确答复；安装计划必须先收到明确的批准文字才能开始安装。

...
6. **Evidence-based acceptance.** Run the three runnable-check layers yourself (`references/runnable_check_spec.md`) and report what was verified with evidence, not that it "works" (AGENTS.md §11). Chats end, files do not: the report and version list go to `wkdrs/env_<ENV_NAME>_<date>/`.

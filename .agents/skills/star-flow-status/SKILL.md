...
description: >-
  ...
  exec_status (plus each run's EXEC_LOG.md for step-level progress), then renders the tree with status,
  ...
  missing or out of date. Never writes. Use when the user invokes $star-flow-status, when a run names it
  as the next action, or asks for the status / overview / progress of their research or plans, what to

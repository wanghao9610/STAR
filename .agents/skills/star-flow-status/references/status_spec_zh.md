...
- `⚠` 需关注 —— **太粗、无法直接执行**，有两种情形：其自身 §3/§5 大量 `[TBD]` 的叶子，**或**带着 `finalized:` 却没有 `children:` 的总体计划——从未被拆解过，因而算作叶子（规约 §5.4），执行器会拒绝它。这是唯一的定义；第 3 层的资格判定一律依它，与生命周期规则最终渲染出哪个状态符号无关 → 建议 `$star-plan-decomposer`。

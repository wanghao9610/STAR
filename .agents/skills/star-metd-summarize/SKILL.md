...
description: >-
  Compile the finished plans into paper-ready method documents for the write-up. Invoked as
  ...
  change list. Compiles only a finished tree — every top-level plan finalized, every leaf done or abandoned — otherwise it stops and routes the gaps; a draft compile is an explicit choice. Use when the
  experiments are finished, the plans finalized, and the user invokes $star-metd-summarize. Supports
...
**Shared conventions.** Read `docs/mds/star-workflow/research-workflow-conventions.md` (Chinese: `research-workflow-conventions.zh-CN.md`) before acting: §1 git, §2 the STOP line, §3 `.env` runtime, §4 real dates, §5 plan-name resolution, §6 delegation, §7 dialogue, §8 the output table, §9 project layout. It is the baseline every STAR skill shares; this file states what is specific to this one, and wins wherever it is stricter. This read is the opening load — one message, not one shell call: the conventions file arrives as its own separate file read, never `cat`-ed into a shell command — a shell result past roughly 30 KB is written out to a file that costs a second round trip to read back, and the conventions file alone is past that limit. If this harness has no file-reading tool of its own, put `cat docs/mds/star-workflow/research-workflow-conventions.md` back into the shell command and accept that the result is written out. Alongside that read, the same message carries the two things only the shell can do here, in one call with the project root as the working directory:
...
**Scale**: Step 1 read frontmatter only, so this is the step that brings plan bodies into the run at all. A small tree (≤ ~15 plans) is usually simplest read directly. For a larger one, delegate whenever bounded, independent, read-only extraction over disjoint parts of the tree materially helps, partitioning **by plan** and calling `spawn_agent` with `agent_type: explorer` for each selected partition. Each agent gets the whole map, a disjoint set of whole plan files, and the extraction format in `extract_map.md`. Every collector extracts for all five targets from the plans it holds, and labels each passage with the `target:` it feeds.
...
- **Missing** → write it.
- **Exists, generated by this skill** (`type:` + `generated:` present) → compare against the freshly compiled content. No substantive change → leave the file untouched and say so; do not churn the `generated` date. Substantive change → show the section-level change list (one line per section: added / rewritten / removed / unchanged, and what changed) on the page first, then ask one direct question — overwrite or skip — per document. When more than two documents differ, they may be batched into one question over the numbered list — *overwrite all* / *overwrite all but the ones I name* / *answer my questions on the ones I name first* / *skip all* — the shape conventions §7.13 defines.

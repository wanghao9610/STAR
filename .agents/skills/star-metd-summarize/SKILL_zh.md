...
**规模**：步骤 1 只读了 frontmatter，所以计划正文是在这一步才进入本次运行的。小树（≤ ~15 个计划）直接读取。更大的树仅在对多个文档目标做有边界、相互独立、只读的提取确有帮助时才选择性委派——按**文档目标**切分，各自拿到映射、确切文件清单与 `extract_map_zh.md` 里的提取格式约定。收集器只提取并返回；不写文件，不做跨计划的冲突解析，也不编译 `overview`（它需要其余四份文档编译后的内容）。
...
步骤 2 分派出去的情况下,先抽查收集器再信它:它们按格式约定返回的是**改写成可独立成立**的段落,而这些段落要进的文档,其原则 2 恰恰把"编造"点名为最主要的失败方式。回到被引用的计划、被引用的 §,核对(a) 每个收集器随机三段,以及 (b) **每一段**带数值或路径的事实——超参、split 规模、阈值,以及 `datas/` / `inits/` / `${CODE_NAME}/` 路径。只要有一段是它的来源里没有的,该收集器这一整份返回就作废:要么重跑它,要么本地把它那一片读一遍。报出核对了多少段、失败了多少段。抽样只是把风险压住,并不能消除它——所以数值和路径类的事实要逐条全查,因为正是它们会变成论文里的错数字。

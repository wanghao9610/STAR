...
description: >-
  ...
  $star-metd-summarize、一次运行点名它是下一步动作， 或要求 Codex 把定稿的研究计划汇总 / 凝练成方法表述、产出 overview / dataset / framework /
...
**规模**：步骤 1 只读了 frontmatter，所以计划正文是在这一步才进入本次运行的。小树（≤ ~15 个计划）通常直接读取更省事。更大的树，只要对树上互不相交的若干片做有边界、相互独立、只读的提取确有帮助就委派——按**计划**切分，并对每个选中的分区调用 `spawn_agent`、使用 `agent_type: explorer`。每个子代理拿到完整映射、一组互不相交的完整计划文件，与 `extract_map_zh.md` 里的提取格式约定。每个收集器都在自己持有的计划上为全部五个目标做抽取，并给每段标上它所喂的 `target:`。

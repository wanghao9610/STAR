...
<!-- 只为"一行说不完"的 benchmark 各写一小节，来自根 §4 的指标与评测类叶子 §5 的阈值。有意义的提升
     幅度和指标本身一样重要：没有它，任何差值都能被说成提升。 -->

### 2.1 <Benchmark>

**Split 与协议。** <确切的 split；存在多个协议变体时说明用哪个>
**指标。** <计划写明的定义或实现——仅当不是标准那个才写>
**有意义的幅度。** <计划称之为真实提升的差值，以及这个数字的出处>

## 3. Baselines

<!-- 每个 baseline、它的数字将从哪来、以及是什么让这个对比公平。引用来的数字必须写明出处；"我们自己
...
<!-- 主张→消融的设计，来自根 §4 消融设计与 §5 kill-criteria：每条主张对应隔离它的那个变体，以及
     什么结果支持它、什么结果推翻它。"什么推翻它"这一列是在任何 run 之前就写好的——正是这一点让它成为
     一个检验。 -->

| # | 受检主张 | 变体 | 什么结果支持 | 什么结果推翻 |
|---|---|---|---|---|
| A1 | <…> | <…> | <…> | <…>（kill-criterion） |

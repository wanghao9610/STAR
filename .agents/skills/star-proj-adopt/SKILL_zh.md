...
一次只问一个问题——Codex 结构化用户输入工具可用时用它，否则用简洁纯文本——只问勘察定不下来的：哪个目录是 `CODE_NAME`、哪个解释器是 `PYTHON_HOME`、哪些已有目录是数据 / 权重 / 输出根。选项取自勘察结果并标出推荐项。这道门关上之前，什么都不写。
...
- 一次只问一个问题——Codex 结构化用户输入工具可用时用它，否则用简洁纯文本——并等待答复。三道门在任何跨门写入之前都必须拿到明确答复。

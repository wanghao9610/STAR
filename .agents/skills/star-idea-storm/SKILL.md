...
- Ask through the `ask_user_question` tool; fall back to plain text only in non-interactive `codex exec`, where human-input tools are unavailable — still one question at a time, and the two confirmation points — the set of directions to keep (Stage 2) and the decision (Stage 4) — always wait for an explicit answer.

...
5. **Deliverables have a name and a home** — §4 places outputs under the correct project directory (`wkdrs/<run>` for generated output, `datas/`, `inits/`, and `tasks/<plan-name>/` for the plan's own tool scripts — never `execs/`) with a run name that distinguishes this task/experiment, and names the artifacts, not just "results". A set of configurations lands one directory per cell under `wkdrs/<run>/cells/` with the metrics collected in `wkdrs/<run>/matrix.md`; a leaf that settles a configuration (hyper-parameters an ablation picked, a threshold fixed) writes the chosen values into a named file and gives its path, for a downstream leaf to cite in its §2.

6. **There is a real done-criterion** — §5 states a single check that proves completion (a passing test, a metric over a threshold, a specific output that exists), with the threshold tied back to the root's §4 metrics or §5 kill-criteria where relevant. If it can't be checked, it isn't a done-criterion. A metric-based done-criterion also states the seed/repeat policy it is measured under (or explicitly accepts single-seed as an MVP runnable check — open to a pilot leaf, or one whose `traces_to` names a §6 milestone rather than a §4 claim, written in the words the analyst will copy into the results table's `Seeds / n` cell, so the exemption travels with the number instead of staying behind in the plan) — `star-expt-analyst` scores exactly what is written. Three kinds of leaf take three shapes: one running a set of configurations is judged on the whole grid (every cell ran, the chosen cell clears the threshold on ≥N seeds), never on one cell; one building or curating data needs more than file counts and checksums — headline statistics, a hand check with its sample size written down, and an overlap / leakage check against the evaluation sets; a pilot leaf is judged on a recorded decision — what will be looked at and which decision each outcome triggers — its numbers staying provisional, out of the results table.

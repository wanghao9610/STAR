...
2. **先确认形状，再自动起草内容**。两个决策依次确认，每次一项，各带推荐项：**拆分轴**，然后是**子计划清单**。使用 `ask_user_question` 工具；仅在非交互的 `codex exec` 下问简洁纯文本问题。确认后，依据父计划自主起草每份子计划。真正缺失处标 `【待定】`；只有当某个步骤离开用户就无法写下时，才追问一个针对性问题。不要重新问父计划里已经记录的细节。
...
用一个问题提出 2–3 个轴，并推荐第一项。详情与如何选择见 `references/decomposition_axes_zh.md`。每个轴选项都要写清它意味着什么，而不只是它叫什么（约定 §7.3）：切分的形状、它蕴含的依赖形态（线性链 / 小 DAG / 基本独立），以及 Step 4 之后再换轴，就意味着对可能已带手工修订的文件重跑一次切分。
...
开头先挂上锚点（约定 §7.10）：刚选定的是哪个轴、它在这里切出了什么——"里程碑轴 → 4 个单元，一条线性链"。按选定的轴起草 N 个单元。每个含：简短标题、英文 `slug`、一句话目标、对应根计划的哪一节 / claim、**以及它依赖哪个（些）兄弟**。用普通文本展示清单——包含依赖边和由此得到的执行顺序——并询问用户确认、修改清单，还是调整粒度，并标出你推荐的一项。

...
Four lines a unit, and each stays one line: this is the sketch Step 4 expands into the six sections, not a draft of them. Then ask through the question tool — confirm, edit the list, or change granularity, with your recommendation marked — as the last thing that message does, the cards above it in that same message's text. The options carry the three answers and none of the list. Read the message back before it goes out: options with nothing above them mean the cards were skipped rather than shortened, which is how a run has ended before, so write them first and send both together.

...
<!-- 由 star-plan-reviser 生成（model_id: <模型 id，写入时由运行时自报；运行时未提供则写 "unrecorded"——见 docs/mds/star-workflow/model_id_spec.md>）。目标：metds/plans/<file>；节点类型：leaf | internal | root；

...
2. **广收集；判断留在主 agent。** 对多个 run 或产物集合进行有边界、相互独立、只读的检查确有帮助时就委派收集；满足条件时派一个只读子代理，跑在 READ 档的模型上（规约 §10.8），前提是这个宿主能指定受托者用哪个模型。受托者必须遵守 `references/review_spec_zh.md` 的收集器格式约定，绝不写入或提出修订。综合与判断留在主 agent。
...
6. **连带影响意识。** 一处修订可能让建立在旧文本上的工作失效。在征询任何改动**之前**先呈现反向 `depends_on` 边和派生的 children（报告 §6）；目标计划的一行目标变了就同步父计划 `## Sub-plans` 里对应那行；`updated` 一更新，过期提示自然在下游的 `star-flow-status` 浮现。
...
规模超过这个的：依据 `references/review_spec_zh.md` 中的收集器格式约定收集：**日志证据**（步骤状态、声称的检查、"待用户执行"命令（英文日志："Awaiting user"）、方向性信号）、**产物证据**（每个 §4 交付物：存在 / 大小 / 修改时间 / 低开销合理性检查），以及在 §2–§3 写明代码时的**代码证据**（承诺模块是否存在，是否与日志声称的修改一致）。按核心原则 2 收集；委派有帮助时，对每个选中的证据集合派一个只读子代理，跑在 READ 档的模型上。

...
规模超过这个的：依据 `references/review_spec_zh.md` 中的收集器格式约定收集：**日志证据**（步骤状态、声称的检查、"Awaiting user"命令、方向性信号）、**产物证据**（每个 §4 交付物：存在 / 大小 / 修改时间 / 低开销 合理性检查），以及在 §2–§3 写明代码时的**代码证据**（承诺模块是否存在，是否与日志声称的修改一致）。按核心原则 2 收集；委派有帮助时，对每个选中的证据集合派一个只读子代理。

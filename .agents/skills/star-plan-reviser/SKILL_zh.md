...
1. **证据先于观点。** 每条审查结论都带证据指针（文件路径、日志行、命令输出）。日志自报的 `done` 不等于完成——要对照磁盘上的产物核实，关键处可复跑廉价检查；绝不启动重实验（executor 的 STOP 线同样生效）。这是把项目的 Verification 规则（AGENTS.md §7）应用到计划本身。规则见 `references/review_spec_zh.md`。
2. **默认在本地收集；选择性委派。** 自行读取日志、工件和代码状态。仅当对多个 run 或工件集合进行有边界、相互独立、只读的检查确有帮助时才委派收集；受委派者必须遵守 `references/review_spec_zh.md` 的 collector 契约，绝不写入或提出修订。综合与判断留在主 agent。
...
2. 目标不存在或有歧义时，列出简洁候选（前缀 + slug + 一行状态），只问一个直接问题——优先推荐有执行证据（`exec_runs` 非空）或已知 drift 的节点。
...
依据 `references/review_spec_zh.md` 中的 collector 契约收集：**日志证据**（步骤状态、声称的检查、"Awaiting user"命令、strategy signal）、**工件证据**（每个 §4 交付物：存在 / 大小 / 修改时间 / 廉价 sanity 检查），以及在 §2–§3 点名代码时的**代码证据**（承诺模块是否存在，是否与日志声称的修改一致）。默认在本地收集；按核心原则 2 选择性委派。

交叉核对分歧——日志说 `done` 但产物缺失 → 该结论记为 **unverifiable**，不算 met。关键的廉价检查照样复跑；重的一律不跑。
...
1. 按报告顺序走候选，每条候选问一个直接问题：*照建议采纳* / *采纳但要改* / *跳过*——标出推荐；用户始终可以自由作答。**structural** 或 **strategic** 候选的选项是：*转给 `$star-plan-decomposer` 或 `$star-plan-coach`*（推荐）vs *仍在本文件做有界的文本修订*。绝不把整张候选清单打包成一个笼统问题。

...
收集器给出的 `suspect` 或 `inconsistent` 是线索，不是结论。它成为编号修订候选之前——每条候选都要问用户，而答案会改写计划文件——主 agent 亲自打开被引用的路径，确认该发现仍然成立（规约 §6.6）；确认时落在的那个 `path[:line]`，就是这条候选携带的证据。站不住的丢掉，或降格成 §5 里一条不引发任何改动的备注。

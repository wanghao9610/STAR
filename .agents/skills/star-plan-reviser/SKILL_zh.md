...
3. **每处改动由用户拍板。**审查发现整理成编号候选。先应用已获授权的点名改动；其余摆在页面上，按规约 §7.13 问一次。每次写入都必须追溯到明确指令或从可见清单接受的候选。
...
2. 去掉明确指令已经定下的候选，再按规约 §7.13 对剩余可见清单问一次。丢弃仍是 `references/revision_rules_zh.md` 管辖的独立实质动作；`exec_status` 重置与清除 `finalized:` 只有在其后果尚未获授权时才问。
...
- 只有修订、重置、重新定稿、丢弃、恢复或覆盖权限仍未解决时才一次问一个问题。只读报告请求以报告结束。计划正文与审查报告保持计划 frontmatter 的 `language`。

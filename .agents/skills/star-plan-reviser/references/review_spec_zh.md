...
所有收集器：只返回上面列出的字段，别的都不返回；宁报"unknown"不猜测；不提修订建议；对 `datas/`、`inits/`、`wkdrs/` 除读取外一概不碰。收集器给出的判定只是线索，要等主 agent 确认过才到得了用户面前（规约 §6.6）。
...
| `unmet` | 证据明确表明没做或失败 |
| `unverifiable` | 有说法无产物、产物无法低开销核验、或相互矛盾（日志 `done` 而产物缺失） |

绝不仅凭日志一面之词把 `unverifiable` 抬成 `met`。

**什么算低开销检查**：文件存在性 / 大小 / 开头几行、小型解析、校验和、单元测试量级的命令——大致一分钟内、不占 GPU、不花付费 API、不改动任何文件。超出即为重检查：不要跑；在报告里注明完整复核需要什么。

## 按节点类型圈定范围

- **叶子**：对它自己的 run 走完整三级。
- **根/内部**：不逐后代展开——直接读 children 的 frontmatter；只对确实存在的 run 派日志收集器，并行派发；用汇总后的信号审计本节点自己 §1–§6 的假设。
- **处处无执行证据**：纯文档审查——记分卡记 `unverifiable`/absent；偏差与候选依据计划文本和用户补充的信息。

## 报告各节

1. **目标回顾**——简要写目标；叶子逐字引用其 §5 完成判据，根/内部写明 finalized 状态与其依赖的关键主张/假设。
...
7. **修订候选**——编号；每条写明目标章节、改什么、证据、修改草案、影响范围。

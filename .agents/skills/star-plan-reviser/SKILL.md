...
Larger than that: gather per the collector contracts in `references/review_spec.md` — the **log evidence** (step statuses, claimed checks, "Awaiting user" commands, plan-level findings), the **artifact evidence** (each §4 deliverable: exists / size / mtime / cheap sanity), and, when §2–§3 name code, the **code evidence** (are the promised modules present and consistent with what the log claims changed?). Collect locally by default; when Core Principle 2 justifies delegation, call `spawn_agent` with `agent_type: explorer` for each selected evidence set.

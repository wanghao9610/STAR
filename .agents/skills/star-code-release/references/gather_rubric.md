...
3. Still unresolved → the row's action is `route`, not a guessed path. Creating a directory `codearc.md` does not name is `star-code-architect`'s decision; a release run that invents `${CODE_NAME}/utils/` amends the architecture spec without a confirmation point.

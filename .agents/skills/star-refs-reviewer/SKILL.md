...
description: >-
  ...
  id, DOI, or paper URL appends one paper. Every bib field is
...
Read locally by default; delegate selectively — only when several confirmed papers can be read independently and read-only, one paper per delegate, each returning the note collector contract in `references/refs_rubric.md` — not a filled template, which asks for fields only the writing session may set. Accept `depth: full` only where `depth_evidence` carries a real table caption and row; otherwise downgrade it. Write the files from the main agent, which also owns §5 (Relation to This Project) — that section needs the method context and is the reason the note exists. Each collector is given its own share of the per-host request budget written as a number, and returns `failures: [{host, error, retries}]`; the pages it fetches are cached under this run's own `raw/` prefix, one prefix per paper so no two collectors write to the same one (conventions §6.4). Steps 1 and 4 give their collectors the same three things — this step fetches from other hosts, not under other rules.
...
2. Organize by theme, following the bib's categories — merge or split only where the notes justify it. Per theme, one paragraph: what these works do and what they cannot do for this method, every claim drawn from that paper's own note, citekeys cited inline as `[@citekey]`. A paper that has a note is named as a link to it — `**[<ABBREV>](<ABBREV>.md)** [@citekey]` — with the file name copied from the note that exists on disk (`<ABBREV>.md`, carrying the `_<year>` suffix where Step 3 needed one), never rebuilt from the citekey; `related_work.md` sits in the same directory as the notes, so the bare file name is the whole path. A paper with no note is named plain — the link is what marks where a full read stands behind the sentence. Close with the positioning paragraph — what none of them do — grounded in the method source's §2.
...
5. Digest ≤400 words: themes written, citekeys cited vs entries total, thin-note gaps with the read-next list, and the boundary: compiled material, every claim traced to a note and nothing written from memory — voice, ordering, and final citation style belong to the writing tool.

### Step 10: Survey a field (survey mode only)

Write `metds/refs/<slug>_survey.md` — a standalone map of a field, read in tiers and organized by taxonomy. It needs no notes, no bib, not even a plan, and it writes neither `reference.bib` nor `refs_index.md`: promoting a paper it surfaced into the verified bib is one `$star-refs-reviewer <arxiv-id>` later. `<slug>` is the plan's slug when a plan is the source, else the topic kebab-cased to ≤5 words.
...
2. **Search wide.** Step 1's machinery as written — the same queries from the profile, the same delegation cap and contracts, the same budgets and caching per `references/source_policy.md` — but the pool is kept, not cut to 15: a survey cites breadth. Keep the count ledger as the pool moves: found → deduplicated → screened → tiered, recorded in the survey's Scope section.
3. **Screen and tier.** Three reading tiers, named by what they may support: **deep** — 8–12 papers at `method-and-results` or better, the only papers that may fill comparison-table rows or carry reproduced numbers; **abstract** — 15–25 papers at `abstract-and-intro`, enough to place a paper on a branch and characterize it in a clause (their abstracts are usually already in the search records — fetch a page only when one is not); **record** — the rest, cited from their records' facts (title, venue, year) for breadth and trends, never characterized, never in a table. Then the mode's one judgment call, in a single direct question: the profile, the taxonomy axis (and the axis rejected), and the tiered list with the deep tier marked as the recommendation. `involve=low` takes the recommendation unasked and logs it in the decisions record (conventions §7.8); this never loosens the full pass, whose core-set question stays mandatory.
4. **Read.** Read locally by default; delegate selectively — only when the deep- and abstract-tier papers can be read independently and read-only, at most 3, one paper per delegate, each given its own share of the per-host request budget written as a number per `references/source_policy.md` and one `raw/` prefix per paper, each returning the survey collector contract in `references/refs_rubric.md`. Below 6 papers to read, no delegation at all — read them in the main session.
...
8. **Digest.** Under about 400 words: the source and profile, the count ledger, the branches written, the self-check result, and the routing — each read-next paper is one `$star-refs-reviewer <arxiv-id>`; positioning goes to `$star-plan-coach` §2 (Related Work & Positioning); the full pass builds the notes and verified bib when the project needs them.

## State & File Rules

- Writes are confined to `metds/refs/**` (notes, `reference.bib`, `refs_index.md`, `related_work.md`, `<slug>_survey.md`) and the run cache `wkdrs/refs_<date>/raw/**`. Never touch `metds/plans/*`, the `metds/*.md` method notes, `metds/codearc.md`, `${CODE_NAME}/`, `.env`, `UPSTREAM.md`, or `LICENSE` / `CITATION*`.
...
- Ask the core-set confirmation as one direct question with recommendations marked, and require an explicit answer before any paper is read — even in headless or scripted runs. Survey mode's one question — profile, taxonomy axis, tiered list (Step 10.3) — is a judgment call: `involve=low` takes its recommendation unasked and logs it; what stays mandatory there at every level is Step 10.7's overwrite question. Never present a note as deeper than its `depth:` admits. Notes, the index, and a survey follow the method source's `language` (else the dialogue language); everything inside `reference.bib` stays English regardless.

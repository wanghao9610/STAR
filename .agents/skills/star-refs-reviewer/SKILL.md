...
Read locally by default; delegate selectively — only when several confirmed papers can be read independently and read-only, one paper per delegate, each returning a filled template. Write the files from the main agent, which also owns §5 (Relation to This Project) — that section needs the method context and is the reason the note exists.

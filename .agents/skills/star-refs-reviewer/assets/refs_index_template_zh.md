...
<!-- 由 $star-refs-reviewer 写出（model_id: <模型 id，写入时由运行时自报；运行时未提供则写 "unrecorded"——见 docs/mds/star-workflow/model_id_spec.zh-CN.md>）。本文件是 reference.bib 的审计线索：每条条目的出处都记在这里，

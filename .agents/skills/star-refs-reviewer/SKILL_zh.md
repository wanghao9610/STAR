...
精读默认在本地进行；只有当多篇已确认论文可以彼此独立、只读地处理时，才选择性委派——每个受托对象一篇，各自按 `references/refs_rubric_zh.md` 的笔记收集器返回格式返回——不是"填好的模板",模板里有些字段只有真正写文件的那次会话才能填。只有 `depth_evidence` 带着真实的表题和一行数据时才接受 `depth: full`,否则降级。文件由主 agent 写入，主 agent 也负责 §5（与本项目的关系）——这一节需要方法上下文，也正是这篇笔记存在的理由。

...
检索默认在本地进行；只有当各检索式分组彼此独立时才选择性委派——至多 3 组,每组拿到其中 2–3 条检索式、检索画像、以及**除过份数**的按 host 速率配额（写成具体数字）,并按 Step 4 那份候选返回格式返回——外加每条候选一段封顶的逐字 `abstract`（约 60 词）。这个字段不是可选的:主 agent 必须在本 skill 唯一的必答确认点上摆出约 15 条排好序的候选,并按 `refs_rubric_zh.md` 重排,而那份评分表的核心判据是与本方法的直接重叠度——只看标题、会议、年份和引用数根本判不了。重排是主 agent 的活:它自己一句话说不清理由的候选,直接丢掉。

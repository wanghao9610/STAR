...
担任这个家族的代码审计员。`$star-plan-executor` 为满足计划而写代码；`$star-plan-reviser` 对照执行证据审计**计划文本**。本 skill 审计**代码本身**：它是否遵守项目的成文规范？限定到某个计划时，它是否实现了计划的承诺？产出是一份落盘的、证据支撑的审查报告；可选地，加上逐项批准的机械修复。

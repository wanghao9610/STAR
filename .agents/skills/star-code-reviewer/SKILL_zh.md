...
2. **广收集，先核实再报告。** 默认在本地审查；仅当对多个包开展有边界、彼此独立、只读的审查确有实质帮助时，才选择性委派收集——每个受委派方遵循 `references/review_rubric_zh.md` 的问题项格式约定，绝不写入，也绝不给整体结论打分。每条要进报告的 blocker/major 问题项都要重读被引用的代码确认；站不住的降级或丢弃。评价一份 review 看的是问题项的精度而不是数量——一条错误的 blocker 就足以让报告失去可信度。
...
- **较大范围**：按核心原则 2 逐包处理、选择性委派——每个受委派方拿到评分表、Step 1 构建的评判依据摘要（同一块内容，逐字发给所有人）和确切的文件清单，按 `review_rubric_zh.md` 规定的结构化问题项格式返回。受委派方绝不写文件、绝不越出自己的清单、绝不给整体结论打分。

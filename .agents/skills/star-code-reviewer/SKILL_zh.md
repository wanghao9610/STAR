...
2. **广收集，先核实再报告。** 默认在本地审查；仅当对多个包开展有边界、彼此独立、只读的审查确有实质帮助时，才选择性委派收集，并调用 `spawn_agent`、使用 `agent_type: explorer`。每个受委派方遵循 `references/review_rubric_zh.md` 的问题项格式约定，绝不写入，也绝不给整体结论打分。每条要进报告的 blocker/major 问题项都要重读被引用的代码确认；站不住的降级或丢弃。评价一份 review 看的是问题项的精度而不是数量——一条错误的 blocker 就足以让报告失去可信度。
...
- **小范围**（≤ 约 20 个文件——diff 模式的审查通常都是）：逐个读文件，直接应用 `references/review_rubric_zh.md`。

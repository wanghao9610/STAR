...
- **小范围**（≤ 约 20 个文件——diff 模式的审查通常都是）：逐个读文件，直接应用 `references/review_rubric_zh.md`——除非范围内有文件是本轮对话此前写出或改过的。作者重读自己的代码，读到的是当初产生它的那套推理，所以此时收集照走较大范围那条路，范围再小也一样——一个 explorer 子代理带上全部文件清单就够，它的全新上下文没见过那套推理，这正是要的效果——并在报告的范围行写明：代码出自本会话，收集因此委派。

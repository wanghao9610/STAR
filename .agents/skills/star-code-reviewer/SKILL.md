...
- **Larger scope**: work package by package, delegating selectively per Core Principle 2 — each delegate gets the rubric, a review rule digest, and its exact file list, and returns the structured finding contract in `review_rubric.md`. Read-only delegates never write, never review outside their file list, never grade the overall verdict.

...
description: >-
  ...
  invokes $star-code-reviewer or asks Codex to review / audit code quality, check coding
  conventions or docstrings, or verify a plan's implementation in code. Supports bilingual
...
Serve as the family's code auditor. `$star-plan-executor` writes code to satisfy a plan; `$star-plan-reviser` audits the **plan text** against execution evidence. This skill audits the **code itself**: does it follow the project's written conventions, and — when a plan is in scope — does it implement what that plan promised? The product is a persisted, evidence-backed review report; optionally, individually approved mechanical fixes.

Review and polish; do not implement features, revise plans, reorganize the codebase, or run experiments. Route what the review surfaces beyond the write boundary: feature gaps to `$star-plan-executor`, plan-text divergence to `$star-plan-reviser`, structural reorganization to `$star-code-architect`, a broken environment to `$star-env-builder`.
...
2. **Find wide, verify before reporting.** Review locally by default; delegate collection selectively — only when bounded, independent, read-only review of several packages materially helps — with every delegate following the finding contract in `references/review_rubric.md`, never writing, and never grading the overall verdict. Re-read the cited code for every blocker/major finding before it enters the report; downgrade or drop what does not hold up. A review is judged by the precision of its findings, not their count — one wrong blocker costs the report its credibility.
...
- **Small scope** (≤ ~20 files — a diff-mode review usually is): read every file and apply `references/review_rubric.md` directly.
- **Larger scope**: work package by package, delegating selectively per Core Principle 2 — each delegate gets the rubric, a yardstick digest, and its exact file list, and returns the structured finding contract in `review_rubric.md`. Delegates never write, never review outside their file list, never grade the overall verdict.
...
- Keep chat replies under about 400 words (the report file does not count); ask fix-pass approvals one at a time and require an explicit answer before any write. Match the user's dialogue language; the report follows the plan's frontmatter `language` in plan mode (else the dialogue language); keep technical terms in English inside Chinese reports.

...
- **How many is the main agent's call**, sized to the work (conventions §6.2): read-only collectors, dispatched read-only, run in parallel, each given the rubric, the review rule digest built at Step 1 — the same block, verbatim, for all of them — and its exact file list, returning the structured finding format in `references/review_rubric.md`. Read-only collectors never write, never review outside their file list, never grade the overall verdict. The number to size against: one collector carries a scope up to the ~50 files where Step 0 already offers to narrow; past that, partition package by package, in groups of 10–15 files, so a 60-file tree goes to four or five collectors and not to sixty. Either side of that is the main agent's to take once it can say why — a package that hangs together and runs a little long stays whole rather than being cut to hit a group size.
...
- Ask the fix-pass approvals that are asked — `blocker`/`major` fixes and every deletion — as one list on the page settled in a single question over it (conventions §7.13), every deletion asked on its own, and an explicit answer required before any write; what Step 7 applies unasked is named in the same reply that applies it. The report follows the plan's frontmatter `language` in plan mode, else the dialogue language.

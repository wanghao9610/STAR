...
- **Larger scope**: work package by package, delegating selectively per Core Principle 2 — each delegate gets the rubric, the review rule digest built at Step 1 — the same block, verbatim, for all of them — and its exact file list, and returns the structured finding contract in `review_rubric.md`. Read-only delegates never write, never review outside their file list, never grade the overall verdict.

...
2. **Find wide, verify before reporting.** Delegate collection whenever bounded, independent, read-only review of several packages materially helps — the main agent sizes it — by calling `spawn_agent` with `agent_type: explorer`. Every delegate follows the finding contract in `references/review_rubric.md`, never writes, and never grades the overall verdict. Re-read the cited code for every blocker/major finding before it enters the report; downgrade or drop what does not hold up. A review is judged by the precision of its findings, not their count — one wrong blocker costs the report its credibility.
...
- **Small scope** (≤ ~20 files — a diff-mode review usually is): read every file and apply `references/review_rubric.md` directly — unless earlier turns of this conversation wrote or edited files now in scope. An author rereads its own code through the reasoning that produced it, so collection then goes through the larger-scope route even at this size — one `spawn_agent` explorer with the whole file list is enough, and its fresh context, which never saw that reasoning, is the point — and the report's scope line records that the code was written in this session and collection was delegated. Running this command is the request for that one explorer — Step 3 is where the skill the user invoked says it dispatches — so a standing instruction conditioned on the user having asked is satisfied here, and only here; the larger-scope fan-out below claims no such thing. One that bans delegation outright is not satisfied, and takes the route that follows. Where that explorer cannot be spawned — the host offers no delegation, or a standing instruction bans delegation outright (conventions §6.1) — ask once for this one dispatch; declined or unavailable, collection runs locally and the scope line records instead that the code was written in this session and no independent collector read it.
- **Larger scope**: work package by package, delegating selectively per Core Principle 2; for each selected package call `spawn_agent` with `agent_type: explorer`. Each agent gets the rubric, the review rule digest built at Step 1 — the same block, verbatim, for all of them — and its exact file list, and returns the structured finding contract in `review_rubric.md`. Read-only agents never write, never review outside their file list, never grade the overall verdict.

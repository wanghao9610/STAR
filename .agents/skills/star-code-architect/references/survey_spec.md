...
Work the areas sequentially by default; delegate a bounded area only when collaboration tools are available and it materially helps (areas are read-only, so concurrent delegation — at most 3 — is safe):

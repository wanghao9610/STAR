...
**Hand the migration to the EXEC tier.** Where §6 has `pending` items, the `STAR_EXEC_MODEL` value the opening load returned is non-empty, and this run is not itself a delegate already carrying a `tier=exec` token: dispatch one writing sub-agent on that model, briefed to read this skill's manifest in full and to resume from those items per `references/orchestration_spec.md`, carrying `involve=<level> tier=exec`. It runs C3's groups exactly as that spec writes them, moves each item to `done` or `blocked` in §6 as its group verifies, and returns when the last group is settled; it writes no other spec and does not go on to C4. C4 is the main run's, taken up from §6 once the delegate returns. Where the key is empty, or this harness has no delegate to hand the phase to, run C3 here exactly as before. Which tier a run belongs to is conventions §10.8's rule; this is only how this skill hands the phase across.

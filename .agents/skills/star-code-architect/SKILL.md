...
5. **Verify per group; delegate where it helps.** Delegate surveys and migrations wherever it helps: hand off a bounded, independent area or migration group whenever collaboration tools are available and delegation materially helps, and execute locally when it does not. When delegating, call `spawn_agent` with `agent_type: explorer` for a read-only survey or `agent_type: worker` for a migration, and give it the narrow contract in `references/orchestration_spec.md`; a migration agent writes only its own group's files. Either way: disjoint file ownership per group, re-run every check yourself (never trust a self-reported pass), commit a git checkpoint per verified group, retry ≤2, roll back what still fails.
...
Work through the topics in `references/survey_spec.md` — structure & dependencies, config system, data pipeline, train/eval entrypoints, scripts & tools, tests & docs — read-only, sequentially by default; when a bounded area meets the delegation rule, call `spawn_agent` with `agent_type: explorer`. Merge the area reports into the **repo map**: module inventory, dependency direction, ranked smells (only smells that would motivate a migration item).
...
- Ask one question at a time — the `request_user_input` tool, with concise plain text only in non-interactive `codex exec` — and wait for the answer. Any side effect past a confirmation point requires an explicit approval first.

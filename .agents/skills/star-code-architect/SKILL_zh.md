...
5. **逐组验证，该委派就委派。**勘察与迁移优先委派出去：协作工具可用、且某个有边界、相互独立的关注点或迁移组确有收益时就派，确实没有收益才留在本地做。委派时调用你的子代理工具：只读勘察使用只读子代理、跑在 READ 档的模型上（规约 §10.8）——前提是这个 harness 能指定受托者用哪个模型——迁移使用可写子代理、跑在 EXEC 档的模型上；并给它 `references/orchestration_spec_zh.md` 中的窄交办说明，迁移子代理只写本组自己的文件。无论是否委派：每组文件所有权互不相交，亲自重跑每项检查（不信任自报的 pass），每验证完一组就提交一次，重试 ≤2 次，仍失败则恢复该组文件。
...
按 `references/survey_spec_zh.md` 的关注点逐条处理——结构与依赖、配置系统、数据管线、训练/评估入口、脚本与工具、测试与文档——默认只读且顺序执行；有边界的关注点满足委派条件时，派一个只读子代理，跑在 READ 档的模型上。把各组报告汇总成**仓库地图**：模块清单、依赖方向、排序后的可疑写法（只收会促成迁移项的）。
...
把获批条目划分为**文件所有权互不相交**的组（`references/orchestration_spec_zh.md`）；有依赖的组按上游优先排序。逐组执行——默认本地；有边界的组满足委派条件时，派一个可写子代理跑在 EXEC 档的模型上——交办说明为：范围原文照录（"只做这些条目"）、明确文件清单、只做例行移动 + import 修正，不顺手改别的，通过 `.env` conda 环境运行、结构化结果（`changed` / `ran` / `check` / `blockers`）。每组完成后**亲自复核**（compileall、import 扫描、可跑的快速测试），然后提交：`star-code-architect: migrate <ids> — <summary>`，只暂存本 skill 涉及的路径。失败 → 把失败信息回传后重试 ≤2 次 → 仍失败：用 git 恢复该组路径，在迁移记录中把条目标 `blocked`，继续其他组。

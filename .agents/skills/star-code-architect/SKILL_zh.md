...
5. **逐组验证，选择性委派。**默认在本地执行勘察与迁移；只有协作工具可用且委派确有帮助时，才把有边界、相互独立的关注点或迁移组委派出去。委派时调用 `spawn_agent`：只读勘察使用 `agent_type: explorer`，迁移使用 `agent_type: worker`；并给它 `references/orchestration_spec_zh.md` 中的窄格式约定，迁移子代理只写本组自己的文件。无论是否委派：每组文件所有权互不相交，亲自重跑每项检查（不信任自报的 pass），每验证完一组就打一个 git 检查点，重试 ≤2 次，仍失败则回滚。

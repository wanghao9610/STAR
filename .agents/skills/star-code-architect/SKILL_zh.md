...
description: >-
  ...
  只读勘察。两条路径随后都会设计目标架构与迁移表，只执行用户批准的迁移——逐组验证、打 git 检查
  点——并把规范写入 metds/codearc.md，在 AGENTS.md 与 .cursor/rules/ 留下薄指针。当用户调用
...
给研究计划一个"代码的家"。上游的 `$star-plan-coach` 与 `$star-plan-decomposer` 分别拥有战略与可执行子计划；下游的 `$star-plan-executor` 在 `${CODE_NAME}/` 里实现计划步骤——但它假设代码库已经存在。本 skill 就负责产出它：一个可运行、已重命名、出处可追溯的 `${CODE_NAME}/` 代码库，外加一份权威架构规范（`metds/codearc.md`），让之后的每个智能体都知道代码该放哪。
...
2. **两道门，门间自主。**门 1：用户从打分候选中选定参考库。门 2：用户批准目标架构与迁移表。每道门只问一个问题——使用 `ask_user_question` 工具，仅在非交互的 `codex exec` 下改用一个简洁的纯文本问题——等到明确答复再跨过。两门之间和之后的工作自主推进、有界重试。门没有覆盖的事不做。
...
5. **逐组验证，选择性委派。**默认在本地执行勘察与迁移；只有协作工具可用且委派确有帮助时，才把有边界、相互独立的关注线或迁移组委派出去，并给每个受托者 `references/orchestration_spec_zh.md` 中的窄契约。无论是否委派：每组文件所有权互不相交，亲自重跑每项检查（不信任自报的 pass），每验证完一组就打一个 git 检查点，重试 ≤2 次，仍失败则回滚。
...
3. 既无计划也无 URL 时，问：*先跑 `$star-plan-coach`（推荐）* / *直接给 GitHub URL* / *现在口述主题据此检索*。
...
呈现 top 3–5，一个候选一行：一句话贴合理由、许可证、stars、最近更新、主要风险。推荐得分最高的一个，始终保留逃生选项（"都不合适——细化检索 / 从零起步"），并问用哪个。若以 URL 调用，也要展示该库的许可证、活跃度与风险，确认后再克隆。
...
以一次只读扫描补全 Step C1 需要的仓库地图（`references/survey_spec_zh.md` 轻量模式）——评分阶段已覆盖粗粒度结构；小仓库直接内联完成。

### 分支 B：整理已有代码库

#### Step B1：勘察

按 `references/survey_spec_zh.md` 的关注面逐条处理——结构与依赖、配置系统、数据管线、训练/评估入口、脚本与工具、测试与文档——默认只读且顺序执行；只有协作工具可用且确有帮助时，才委派有边界的关注面。把各线报告汇总成**仓库地图**：模块清单、依赖方向、排序后的坏味道（只收会促成迁移项的坏味道）。

### 汇合：架构、迁移、规范

#### Step C1：设计目标架构

由仓库地图 + 计划起草：目录布局（现状即基线——原则 3）、新代码放置规则、命名与风格约定（贴合上游风格，AGENTS.md §3）、计划组件落位映射（计划 §3 每个组件 → 目标路径，标 `exists` / `planned`），以及**迁移表**——逐条编号，每条含 `旧路径 → 新路径`、理由、风险级、绑定检查。保持精简。
...
把获批条目划分为**文件所有权互不相交**的组（`references/orchestration_spec_zh.md`）；有依赖的组按上游优先排序。逐组执行——默认本地，只有确有帮助时才委派有边界的组——契约为：范围原文照录（"只做这些条目"）、明确文件清单、只做机械移动 + import 修正，不顺手改别的，通过 `.env` conda 环境运行、结构化结果（`changed` / `ran` / `check` / `blockers`）。每组完成后**亲自复核**（compileall、import 扫描、可跑的快速测试），然后提交：`star-code-architect: migrate <ids> — <summary>`，只暂存本 skill 涉及的路径。失败 → 把失败信息喂回重试 ≤2 次 → 仍失败：用 git 回滚该组路径，在迁移记录中把条目标 `blocked`，继续其他组。
...
≤400 字：选定的仓库（附许可证说明）、落了什么在哪里、完成的改名 + 残留数量、迁移完成/受阻情况、写入的规范、验证证据、待用户执行的命令。**向下游交棒：**`$star-plan-executor <leaf>` 现在有代码可改了；`$star-flow-status` 查看全树。
...
- 一次只问一个问题——使用 `ask_user_question` 工具，仅在非交互的 `codex exec` 下改用简洁纯文本——并等待答复。任何跨门的副作用都必须先得到明确批准。

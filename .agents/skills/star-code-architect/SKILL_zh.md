...
**通用规约。** 动手前先读 `docs/mds/star-workflow/research-workflow-conventions.zh-CN.md`（英文：`research-workflow-conventions.md`）：§1 git、§2 红线、§3 `.env` 运行时、§4 真实日期、§5 计划名解析、§6 委派、§7 对话纪律、§8 产物登记表、§9 项目布局。那是所有 STAR skill 共享的基线；本文件只写本 skill 特有的部分，更严之处以本文件为准。这次阅读就是开场装载，以一条消息发出：规约文件经单独的一次文件读取读入，绝不 `cat` 进 shell 命令——shell 结果一旦超过 30 KB 左右就会被存成文件，要再读一次才拿得回来，而规约文件本身就超过这个上限——外加一次小的 shell 调用（以项目根目录为工作目录），做这里唯一只有 shell 才做得了的事，即这次运行的 `.env` 查询：`grep -sE '^(STAR_LANG|INVOLVE)=' .env || echo 'STAR_LANG / INVOLVE: unset'   # reply language, question level (§7.6, §7.7)`。若本宿主没有独立的文件读取工具，就把 `cat docs/mds/star-workflow/research-workflow-conventions.zh-CN.md` 放回那次 shell 调用，并接受结果被存成文件的代价。两次调用一起发出仍只花一趟往返——而它们也是本 skill 唯一的无条件装载：`references/` 与 `assets/` 下的每个文件都归属某个分支或步骤，留到引用它的步骤再读，不前置装载。
...
给研究计划一个"代码的家"。上游的 `star-plan-coach` 与 `star-plan-decomposer` 分别拥有总体方向与可执行子计划；下游的 `star-plan-executor` 在 `${CODE_NAME}/` 里实现计划步骤——但它假设代码库已经存在。本 skill 就负责产出它：一个可运行、已重命名、出处可追溯的 `${CODE_NAME}/` 代码库，外加一份权威架构规范（`metds/codearc.md`），让之后的每个智能体都知道代码该放哪。
...
2. **两个确认点，确认点之间自主。**确认点 1：用户从打分候选中选定参考库。确认点 2：用户批准目标架构与迁移表。每道确认点只问一个问题——使用 你的提问工具，仅在非交互运行下改用一个简洁的纯文本问题——等到明确答复再跨过。两个确认点之间和之后的工作自主推进、有限次重试。确认点没有覆盖的事不做。
...
5. **逐组验证，该委派就委派。**勘察与迁移优先委派出去：协作工具可用、且某个有边界、相互独立的关注点或迁移组确有收益时就派，确实没有收益才留在本地做。委派时调用 你的子代理工具：只读勘察使用 只读子代理，迁移使用 可写子代理；并给它 `references/orchestration_spec_zh.md` 中的窄交办说明，迁移子代理只写本组自己的文件。无论是否委派：每组文件所有权互不相交，亲自重跑每项检查（不信任自报的 pass），每验证完一组就提交一次，重试 ≤2 次，仍失败则恢复该组文件。
...
先数一下这份克隆有多少个 `.py` 文件。在轻量模式阈值以下：以一次只读扫描补全 Step C1 需要的仓库地图（`references/survey_spec_zh.md`）——评分阶段已覆盖粗粒度结构，小仓库直接内联完成。超过阈值：照原样走 Step B1 的各组，或只走 C1 真正需要的三组（结构与依赖、配置系统、训练/评测入口）。参考实现通常远在阈值之上，而这一遍是 C1 用来定架构和迁移表的唯一输入。

### 分支 B：整理已有代码库

#### Step B1：勘察

按 `references/survey_spec_zh.md` 的关注点逐条处理——结构与依赖、配置系统、数据管线、训练/评估入口、脚本与工具、测试与文档——默认只读且顺序执行；有边界的关注点满足委派条件时，派一个只读子代理。把各组报告汇总成**仓库地图**：模块清单、依赖方向、排序后的可疑写法（只收会促成迁移项的可疑写法）。
...
- 一次只问一个问题——使用 你的提问工具，仅在非交互运行下改用简洁纯文本——并等待答复。确认点之后每一次写文件、跑命令，都必须先得到明确批准。

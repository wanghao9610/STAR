...
给研究计划一个"代码的家"。上游的 `star-plan-coach` 与 `star-plan-decomposer` 分别拥有总体计划与可执行子计划；下游的 `star-plan-executor` 在 `${CODE_NAME}/` 里实现计划步骤——但它假设代码库已存在。本 skill 就产出它：一个可运行、已重命名、出处可追溯的 `${CODE_NAME}/` 代码库，外加一份权威架构规范（`metds/codearc.md`），让之后的每个智能体都知道代码该放哪。
...
5. **逐组验证，该委派就委派。**勘察与迁移在有帮助时委派：协作工具可用、且某个有边界、相互独立的关注点或迁移组确有收益时就派，没有收益就在本地做。委派时调用你的子代理工具：只读勘察使用只读子代理、跑在 READ 档的模型上（规约 §10.8）——前提是这个宿主能指定受托者用哪个模型——迁移使用可写子代理、跑在 EXEC 档的模型上；并给它 `references/orchestration_spec_zh.md` 中的窄交办说明，迁移子代理只写本组自己的文件。无论是否委派：每组文件所有权互不相交，亲自重跑每项检查（不信任自报的 pass；迁移的检查开销很小，所以本 skill 不走规约 §6.3 采信子代理检查证据的那条路），每验证完一组就提交一次，重试 ≤2 次，仍失败则恢复该组文件。
...
展示架构摘要与带编号的迁移表。先应用针对这张表已经给出的清楚选择或批准。仍有未定条目时，按规约 §7.13 对可见清单只问一次；只有已经定下的条目进入工作清单。"零迁移"是合法结果 → 直接跳到 C4。
...
把获批条目划分为**文件所有权互不相交**的组（`references/orchestration_spec_zh.md`）；有依赖的组按上游优先排序。逐组执行——有边界的组满足委派条件时，派一个可写子代理跑在 EXEC 档的模型上，否则本地执行——交办说明为：范围原文照录（"只做这些条目"）、明确文件清单、只做例行移动 + import 修正，不顺手改别的，通过 `.env` 的解释器运行（规约 §3）、结构化结果（`changed` / `ran` / `check` / `blockers`）。每组完成后**亲自复核**（compileall、import 扫描、可跑的快速测试），然后提交：`star-code-architect: migrate <ids> — <summary>`，只暂存本 skill 涉及的路径。失败 → 把失败信息回传后重试 ≤2 次 → 仍失败：用 git 恢复该组路径，在迁移记录中把条目标 `blocked`，继续其他组。
...
- 只有实质选择或权限仍缺失时才问一个具体问题。对同一仓库、架构、迁移表或动作已有的明确授权继续有效，不重复询问。

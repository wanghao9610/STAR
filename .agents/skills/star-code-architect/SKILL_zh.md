...
2. **两道门，门间自主。**门 1：用户从打分候选中选定参考库。门 2：用户批准目标架构与迁移表。每道门只问一个问题——Codex 结构化用户输入工具可用时用该工具，否则用一个简洁的纯文本问题——等到明确答复再跨过。两门之间和之后的工作自主推进、有界重试。门没有覆盖的事不做。
...
- 一次只问一个问题——Codex 结构化用户输入工具可用时用它，否则用简洁纯文本——并等待答复。任何跨门的副作用都必须先得到明确批准。

...
level: <n>                   # = 前缀长度（根 = 1）
traces_to: "<本子计划执行父计划的哪一节/哪条 claim，例如 §6 里程碑 1（MVP）；§4 claim 1>"
...
<!-- 用一两句话说明本子计划交付什么，并对应它所服务的父计划 claim/章节。明确写出 non-goals:
...
<!-- 证明本子计划完成的那一条检查: 一个通过的测试、一个越过阈值的指标、一个存在且正确的具体输出。
     相关时把阈值挂回父计划 §4 指标 / §5 kill-criteria。无法检查的,就不算完成判据。 -->

## 6. 局部风险与备选

<!-- 执行本子任务特有的风险(不是研究层面的风险——那些在父计划 §5)。什么会让这一步失败或卡住、
     需要盯的早期信号,以及局部备选方案。若与父计划 kill-criteria 有关联,标注出来。 -->

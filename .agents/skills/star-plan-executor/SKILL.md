...
4. Show a concise plan and expected side effects in commentary, and ask once whether to checkpoint each verified action as a git commit (recommended), naming any path that already carries uncommitted changes — those are never staged. Say what declining costs: without per-action commits every verified action stays uncommitted, so a later restore has only this run's own record of where each action began to work from (`references/agent_dispatch_spec.md`). Pause only for a material scope choice, a non-empty divergence table (confirm its rows with the user before executing), or a STOP-line action that requires user execution.
...
- In non-interactive `codex exec`, where `ask_user_question` is unavailable, fall back: present EXEC_PLAN as plain text and require an explicit plain-text approval before any side effect — still gate before executing, still stop before heavy experiments, still confirm in plain text before any change is written back into the sub-plan.

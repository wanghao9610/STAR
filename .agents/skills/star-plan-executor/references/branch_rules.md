...
4. Worktree: from the invoking checkout, `git worktree add <path> -b <run> <base>` — tree, branch, and fork point in one step, and no checkout switches. Pre-run uncommitted changes stay behind with their checkout; the tree starts out clean from `base:`, which is the isolation it was chosen for.
...
6. Choosing the branch chooses per-action commits with it: a branch with nothing committed has nothing to merge, so a commit-less branch run does not exist.

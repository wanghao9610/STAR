...
The operating procedure behind conventions §11. Step 3 decides and creates; Step 4 commits onto the branch; the State & File Rules end the branch — merge or discard. Every git command here is the executor's to run, except where a line hands it to the user.
...
2. The precondition was stated at Step 3's pause: nothing is running from this checkout — a live job re-reads switched files mid-run. A busy checkout means executing later, or the user preparing a `git worktree` and invoking the executor inside it.
3. `git switch -c <run>`. Pre-run uncommitted changes carry over untouched; they stay named as pre-existing and are never staged (conventions §1.4).
4. Choosing the branch chooses per-action checkpoint commits with it: a branch with nothing committed has nothing to merge, so a commit-less branch run does not exist.

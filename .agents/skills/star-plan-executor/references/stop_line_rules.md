...
- the **exact command**, invoked through `.env`'s interpreter (conventions §3; never system python) via the project's launch entry point (`execs/run.sh`) where one exists;

...
- Fail → record the failure in EXEC_LOG, and restore this action's files before retrying — minus anything named pre-existing — so the next attempt starts from a known tree. The baseline is the tree as it stood when this action began, not `HEAD`: checkpoint commits may have been declined, in which case earlier verified actions are uncommitted too and resetting to `HEAD` would erase work EXEC_LOG already records as `done`. Restore never reaches past this action's own files. Retry only when a concrete correction is available, at most twice. Otherwise the action is `blocked`, and its half-finished edits may still be worth something: ask before restoring them (conventions §7.7 — it gates a deletion), record which way it went, name the paths either way, and stop. No action ends leaving edits nobody has accounted for.

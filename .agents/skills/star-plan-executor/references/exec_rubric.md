...
5. **Runtime is the project env** — commands went through `.env`'s interpreter (conventions §3); no system python, no hardcoded local paths.
...
8. **Handoffs are runnable** — each "Awaiting user" command is complete (the `bash execs/scpts/<run>.sh` invocation, inputs, output path) and states what to bring back for verification.

...
**本次运行在哪里执行。** 本 skill 的档位是 PLAN，但整次运行从不交给受托者（规约 §10.8）：那里的第四条对它从不成立，因为合并确认点、Step 5 的待同步修正批次，以及以 `blocked` 结束的步骤其改动的去留，都取决于运行中发现了什么。它留在可与用户交互的会话；档位变化是后文把 Step 4（执行与验证）交给 EXEC。判断是否还缺必需决定时，既有执行授权视为已定；带 `tier=` 的受托者不再次转交这一阶段。

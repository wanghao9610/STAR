...
3. 若 EXEC_LOG 的 `Pending amendments` 非空，一次展示整批（全部同步 / 选择部分 / 跳过，标出你推荐的一项），按 `references/plan_sync_rules_zh.md` 把确认行写回（原地更新 §2–§5 + 添加 `## Revision History` + 更新 `updated`，然后勾掉各行）。仅限战术层：任何触碰 §1/§6、父计划或 kill-criterion 的内容都通过第 5 点的方向性信号转交，绝不回同步。

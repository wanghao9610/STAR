...
3. **选择性委派。** 默认在本地执行。只有 collaboration 工具可用，且有边界、相互独立的工作确实能从委派中受益时才委派。绝不为每个琐碎顺序步骤创建一个 subagent。给每个受委派者 `references/agent_dispatch_spec_zh.md` 中那份范围很窄的格式约定；主 agent 始终负责集成和重新运行检查。

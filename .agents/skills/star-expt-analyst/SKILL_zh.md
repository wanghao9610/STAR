...
1. **解读**：结果支持还是推翻 `traces_to` 里的主张？是否命中根计划 §5 的 kill-criterion，或否定了某个 MVP"低开销早期测试"？接受一个可疑的强结果之前，先跑评分表列出的泄漏检查——维度 C 分派出去的情况下，这些检查跑在各收集器返回的 `config_echo` 上，只有命中时才回去展开被引用的那段行号区间。明确写出这个 run 的局限（seed 数、split 规模、它没能显示什么）。

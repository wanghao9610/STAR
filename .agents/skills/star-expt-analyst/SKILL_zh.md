...
动手前把它合成一条消息装载——三次 shell 调用，以项目根目录为工作目录，一起发出；完整分析路径上，同一条消息里再加一次 file read：`<本 skill 所在目录>/references/analysis_rubric_zh.md`，即 Step 2–5 遵循的评分表；aggregate 与 watch 模式去掉评分表那次读取，各自的参考文件到点名它的步骤再装。
...
一条消息，三个 shell 结果——完整分析路径上，外加那次 file read 拿到的评分表。`.env` 那一行搭第一次调用的车：回复语言、提问档位，以及本次运行与它派出的每个子代理取模型所依据的三个模型键（§10.8）。三次调用分开发，是因为每个工具结果各有自己的尺寸上限：结果一旦超过 30 KB 左右就会被存成文件，要再读一次才拿得回来——恰是这条消息要省掉的那次往返——而规约摘录合计约 57 KB，三次调用分担 16、20、20。每个 `awk` 只打印它上面点名的节；若打印结果缺了哪一节——同步下来的旧规约可能节号不同——就退回整份读。
...
**把档位模型传给受托者。** 先取本次实际宿主对应的档位值，再检查当前委派接口。接口提供按次指定的 `model` 时，该档每次派发都传解析后的值；键为空则省略。指定模型若不能与完整上下文继承并用，就用全新上下文——接口有 `fork_turns` 时取 `"none"`——并给出自包含的交办说明。盲读无论模型键是否为空，都不继承产出该工作的对话。已配置的命名代理也可用，但其已安装模型必须与解析值一致。不跨厂商翻译模型名，不臆造工具参数，不另启 CLI 绕过缺失的接口。没有可用路径就留在这里；键已设时说明一条原因。派发被拒后，只有确认它尚未开始工作才能退回本地。技能的范围、提问点和写入限制照旧。受托者按自己的会话溯源记录实际模型；请求中的别名和父会话的解析命令都不是它的溯源。

## 角色

担任这个家族的结果审计员。`star-plan-executor` 产出 run——代码、产物，以及完成判据的二值判定；`star-code-reviewer` 审计产出它的代码；`star-plan-reviser` 对照执行证据审计**计划文本**。本 skill 审计**结果本身**：这个 run 产出了什么、跑完了没有、数字健不健康、是否达到计划的预期、以及它对计划 `traces_to` 的那条主张意味着什么。产出是一份写进文件的、证据支撑的分析报告。`star-expt-digest` 横向读取多份这样的报告，回答本阶段什么发生了变化；它从不重新打分，因此每个数字都归属于最早核实它的那份分析。

阅读与解读；不执行步骤、不修代码、不改计划、不翻计划状态。分析发现超出可写范围的问题，都转交出去：未完成或失败的步骤、判据已达标但还需终验，交 `star-plan-executor`；计划文本与现实不符交 `star-plan-reviser`；策略被推翻交 `star-plan-reviser` / `star-plan-coach` / `star-plan-decomposer`；疑似代码缺陷交 `star-code-reviewer`；环境损坏交 `star-env-builder`。
...
- **规模**：小 run（少量产物、没有超大日志）通常本地读取更省事。大的——日志文件很多，或日志大到读不完——**维度 C** 按核心原则 2 逐个日志文件处理、选择性委派：对每组选中的日志派一个只读子代理，跑在 READ 档的模型上；每个子代理拿到评分表、预期摘要和确切文件清单，按结构化观察格式约定返回。**维度 D 留在本地**：它那条来源权威阶梯把几个来源互相排序，只拿着一个文件的子代理没法应用它；而且指标来源本来就小，委派省下的，步骤 4 立刻又会花回来。只有一个例外：某个指标唯一的来源是子代理清单里那份超大日志中的某一行——那就由它连同 `source:` 返回这条指标行，步骤 4 照常复核。只读子代理绝不写文件、绝不越出清单、绝不给 run 的判定打分。

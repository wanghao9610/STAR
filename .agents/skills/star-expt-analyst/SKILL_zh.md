...
description: >-
  ...
  star-expt-analyst、一次运行点名它是下一步动作，或要求 agent 分析 / 解读实验结果、输出或产物，核对某个 run 是否达到预期或完成判据，读训练 日志或指标，或想知道一个跑完的
...
2. **广读，每个数字进报告前先核实。** 对许多或超大日志开展有边界、彼此独立、只读的读取确有实质帮助时就委派收集——派几个由主 agent 定——并派一个只读子代理。每个受委派方遵循 `references/analysis_rubric_zh.md` 的观察格式约定，绝不写入，也绝不给 run 的判定打分。每个数字、每条 blocker/major 观察进报告前，都要按引用重开那个文件、定位到那一行确认；站不住的降一档或丢弃。一个错数字就足以让报告失去可信度——而报告里的数字是会被抄进论文的。
...
- **规模**：小 run（少量产物、没有超大日志）通常本地读取更省事。大的——日志文件很多，或日志大到读不完——**维度 C** 按核心原则 2 逐个日志文件处理、选择性委派：对每组选中的日志派一个只读子代理；每个子代理拿到评分表、预期摘要和确切文件清单，按结构化观察格式约定返回。**维度 D 留在本地**：它那条来源权威阶梯是把几个来源互相排序（结果 JSON > 评测汇总 > TB event > 日志最后一行），只拿着一个文件的子代理根本没法应用它；而且指标来源本来就小，委派出去省下的，步骤 4 立刻又会花回来。只有一个例外：某个指标唯一的来源就是子代理清单里那份超大日志中的某一行——那就由它连同 `source:` 一起返回这条指标行，步骤 4 照常复核。只读子代理绝不写文件、绝不越出清单、绝不给 run 的判定打分。

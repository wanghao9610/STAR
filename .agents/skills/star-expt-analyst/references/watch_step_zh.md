...
4. 简短汇报，存活情况先行：还活着还是从何时起停滞、最近的进度行、任何致命或异常信号及其 `file:line`，以及一个下一步——继续等；致命信号 → 停掉任务、修复后经 `star-plan-executor <slug>` 重启；import 或环境错误 → `star-env-builder`。明说这个 run 尚未被打分：跑完后由完整流程（`star-expt-analyst <slug>`）来打。

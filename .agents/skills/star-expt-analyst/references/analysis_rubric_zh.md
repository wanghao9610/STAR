...
- **一组配置按整张网格读**：run 目录下有 `cells/` 时，先看 `matrix.md` 是否每格都有数、缺格算 blocked 而不算差结果；再对照 §5 判据。报选中格必须同时报同轴上的散布——只报最好那一格，等于把选择过程藏起来。多 seed 的格子报中位数与极差，不报最好的那一次。
- **开销对账**：EXEC_LOG 的"开销"一节与根计划 §4 的算力预算并排读一行——实际是预算的几成，超了多少。填着 `未记` 就照实说"这次没记"，不要跳过这一行：算力是这条流程唯一花得掉又要不回来的东西。根 §4 的主张→实验映射把这次 run 和某个 baseline run 放进同一张表时，这一行要并排写出两次 run 的实际开销，而不是只写这一次；差到一个数量级就记一条 `major` 观察。这是把两个已记录的数字摆在一起，不是给差值指派原因，所以它落在"绝不归因"那条规则之内，而不是之外。

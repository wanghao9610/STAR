...
description: >-
  ...
  metric — that command goes back to the user. Use when the user invokes $star-expt-analyst or
...
- Keep chat replies under about 400 words (the report file does not count); ask one direct question at a time where the workflow calls for it (which run to analyze, an ambiguous match) and require an explicit answer. Since the skill writes nothing outside its own report, there is no approval gate — but for the same reason, never state or imply that a plan, a status, or a log was changed. Match the user's dialogue language; the report follows the plan's frontmatter `language` (else the dialogue language); keep technical terms — metric names, log keys, file paths — in English inside Chinese reports.

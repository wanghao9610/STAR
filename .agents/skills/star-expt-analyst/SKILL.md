...
**Shared conventions.** Read `docs/mds/star-workflow/research-workflow-conventions.md` (Chinese: `research-workflow-conventions.zh-CN.md`) before acting: §1 git, §2 the STOP line, §3 `.env` runtime, §4 real dates, §5 plan-name resolution, §6 delegation, §7 dialogue, §8 the output table, §9 project layout. It is the baseline every STAR skill shares; this file states what is specific to this one, and wins wherever it is stricter. Load everything in one message, each file as its own file read: the conventions, and — on the full-analysis path — `<this skill's directory>/references/analysis_rubric.md`, the rubric Steps 2–5 follow; aggregate and watch modes drop the rubric read and load their own references at the step that names them. Keep the files out of the command: a shell result past roughly 30 KB is spilled to a file that costs a second round trip to read back, and the conventions file alone is past that limit — the shell is only for what only the shell can do, one small call sent in the same message. If this harness has no file-reading tool of its own, put `cat docs/mds/star-workflow/research-workflow-conventions.md <this skill's directory>/references/analysis_rubric.md` back as a second line of the command and accept the spill:
...
2. **Read wide, verify every number before it enters the report.** Delegate collection whenever bounded, independent, read-only reading of many or oversized logs materially helps — the main agent sizes it — by calling `spawn_agent` with `agent_type: explorer`. Every delegate follows the observation contract in `references/analysis_rubric.md`, never writes, and never grades the run's verdict. Re-open the cited file at the cited line for every number and every blocker/major observation before the report keeps it; downgrade or drop what does not hold up. One wrong number costs the report its credibility — and a number in a report gets quoted into a paper.

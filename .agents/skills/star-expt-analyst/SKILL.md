...
Before acting, load it in one message — three shell calls with the project root as the working directory, sent together, plus — on the full-analysis path — `<this skill's directory>/references/analysis_rubric.md` as its own file read in the same message, the rubric Steps 2–5 follow; aggregate and watch modes drop the rubric read and load their own references at the step that names them.
...
One message, three shell results — and, on the full-analysis path, the rubric from its file read. The `.env` line rides the first call: the reply language, the question level, and the three model keys this run and every delegate it dispatches take their model from (§10.8). The calls stay separate because each tool result carries its own size limit: a result past roughly 30 KB is written out to a file that costs a second round trip to read back — exactly the round trip the one message exists to avoid — and the conventions excerpt is about 59 KB in total, split 17, 21 and 21 across its three calls. Each `awk` prints the sections named above it and nothing else; if any of them is missing from what it prints — a stale synced copy of the conventions may number its sections differently — read the file whole instead.
...
**Passing a tier model.** Resolve the tier value for the harness actually running this skill, then inspect the available delegation interface. Where it exposes a per-dispatch `model`, pass the resolved value on every delegate of that tier; omit it when empty. If selecting a model is incompatible with a full context fork, use a fresh context — `fork_turns: "none"` where that field exists — and provide a self-contained brief. A blind read always starts without the producing conversation, even with an empty model key. A configured named agent is another valid route only when its installed model matches the resolved value. Do not translate model names between providers, invent a tool parameter, or start another CLI to bypass a missing interface. An unavailable route leaves the run here with one reason when the key is set. A rejected dispatch may fall back only after confirming it started no work. Preserve the skill's scope, question gates and write limits. A delegate records its own actual model from its own session provenance; the requested alias and the parent's resolver are not its provenance.
...
2. **Read wide, verify every number before it enters the report.** Delegate collection whenever bounded, independent, read-only reading of many or oversized logs materially helps — the main agent sizes it — by dispatching a read-only sub-agent on the READ tier's model (conventions §10.8), where the harness can name one. Every delegate follows the observation format in `references/analysis_rubric.md`, never writes, and never grades the run's verdict. Re-open the cited file at the cited line for every number and every blocker/major observation before the report keeps it; downgrade or drop what does not hold up. A number in a report gets quoted into a paper.
...
- **Scale**: a small run (a handful of artifacts, no oversized log) is read locally. For a large one — many log files, or logs too big to read whole — **dimension C** works log file by log file, delegating selectively per Core Principle 2: dispatch a read-only sub-agent on the READ tier's model for each selected file set; each agent gets the rubric, the expectations digest, and its exact file list, and returns the structured observation format. **Dimension D stays local**: its source-authority ladder ranks sources against one another, so an agent holding one file cannot apply it, and the metric sources are small enough that delegating saves nothing Step 4 does not spend again immediately. One exception: a metric whose only source is a line inside an oversized log on an agent's list — that agent returns the metric row with its `source:`, and Step 4 confirms it like any other. Read-only agents never write, never read outside their list, never grade the run's verdict.
...
- Ask one direct question at a time where the workflow calls for it (which run to analyze, an ambiguous match) and require an explicit answer. The skill writes nothing outside its own report, so there is no confirmation point — but never state or imply that a plan, a status, or a log was changed. The report follows the plan's frontmatter `language`, else the dialogue language.

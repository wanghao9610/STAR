...
**向下游交棒。** 定稿后，告诉用户推荐顺序：若 `${CODE_NAME}/` 还是空的，先给方法安一个代码家（`$star-code-architect`，它读的正是这份根计划）和运行环境（`$star-env-builder`），再用 `$star-plan-decomposer <slug>` 把战略落成可执行子计划——代码库已存在时写出的叶子能点到真实模块，而不是猜路径。子计划树建立后，`$star-flow-status` 给出整棵树的总览。并就计划文件提供一次提交提议（见状态与文件规则）。

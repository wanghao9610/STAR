...
阶段 2 的衔接：最接近的工作及其局限应当是读出来的，不是回忆出来的。若 `metds/refs/` 已有分析笔记与 `reference.bib`，就以它们为依据来写这一节并引用其 citekey。若没有，建议**先**抽身去跑 `$star-refs-reviewer`，再用 `$star-plan-coach <slug> related_work` 回来续写——凭记忆写定位，正是这一阶段要防的失败。用户若不愿意，就基于他已知的内容继续，并标出日后需要调研确认的部分。计划据 idea 文件起草时，其 §3 的扫描表为这一阶段点出首批候选——但那些只读到摘要深度：它们为调研指路，不能替代调研。

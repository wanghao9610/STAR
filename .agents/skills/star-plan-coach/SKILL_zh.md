...
2. **一次只问一个问题。** 每一道引导问题都一次只问一个——通过 `ask_user_question` 工具发出，仅在非交互的 `codex exec` 下退回到一个简洁的纯文本问题——等用户答完再发下一题。禁止在一条消息里一口气列出多道题。每道题附 2–4 个短而具体的候选选项，选项来自问题库并结合用户已说内容起草，并标出你推荐的一项——选项降低思考成本；同时注明用户可自由作答、不必限于选项。每答完 2–3 题，暂停并用一两句复述你听到的要点，再继续——这能及早暴露误解。例外：无法给出有意义候选的开放题（例如最初的研究主题）可不带选项。
...
- 阶段结束时：把该章节整理成 150–400 字的成文（是结构化的正文，不是问答记录），展示给用户，再确认（选项如："写入文件" / "需要修改"）；确认后写入计划文件对应章节，把 frontmatter `status` 中该节改为 `done`、下一节改为 `in_progress`，更新 `updated` 日期。

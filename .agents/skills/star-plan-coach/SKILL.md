...
Invocation: `$star-plan-coach [TOPIC]` — pass an optional topic or idea to seed a new plan, or no argument to resume an existing plan under `metds/plans/`.
...
2. **Ask one question at a time.** Use Codex's structured user-input tool when it is available and appropriate; otherwise ask one concise plain-text question and wait. For a choice, give 2–3 short, concrete options based on the question bank and what the user has already said, with a recommendation when one is clearly best. After every 2–3 answers, restate the key points in one or two sentences before continuing. Open questions such as the initial topic may be asked without options.
...
2. If a plan has any section whose `status` is not `done`, ask whether to continue it or start a new one. If continuing, resume from the first non-`done` section after summarizing completed sections in 2–3 sentences.
...
- At stage end: turn the section into 150–400 words of structured prose (not a Q&A log), show it, and ask whether to write it or revise it. After confirmation, write it to the plan file, set that section's `status` to `done` and the next to `in_progress`, and update `updated`.

Stage 2 handoff: the closest works and their limits are read, not recalled. If `metds/refs/` already holds analysis notes and a `reference.bib`, ground the section in them and cite their citekeys; if it does not, say once that `-refs-reviewer` builds that base — then continue with what the user knows rather than blocking on it.
...
**Hand off downstream.** Once finalized, tell the user the next step is to turn this strategy into executable sub-plans with `$star-plan-decomposer <slug>`, and that `$star-plan-status` gives an overview of the plan tree once it exists.
...
- Use structured input only when the current Codex surface exposes it; otherwise use plain text. In either case, ask only one coaching question at a time.

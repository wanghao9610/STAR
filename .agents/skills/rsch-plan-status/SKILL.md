...
description: >-
  ...
  staleness (parent edited after a child). Never writes. Use when the user invokes $rsch-plan-status, or
...
1. **Strictly read-only.** Never create, edit, or delete any file — not plans, logs, or frontmatter. Do not create a progress plan, delegate work, or ask interactive follow-ups. If the user wants to act on the report, point them at `$rsch-plan-coach`, `$rsch-plan-decomposer`, or `$rsch-plan-executor`.

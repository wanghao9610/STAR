...
3. Still unresolved → the row's action is `route`, not a guessed path. Creating a directory `codearc.md` does not name is `/star-code-architect`'s decision; a release run that invents `${CODE_NAME}/utils/` quietly amends the architecture spec without a confirmation point.
...
`plan_referenced` is not a veto but a consequence the user approves with open eyes. Plan text is not this skill's to edit, so every such row lands in the report's routing block for `/star-plan-reviser`.
...
Applies to: the files promoted this run, the entrypoints / configs / `execs/scpts/*.sh` the README prints, and the public API the README shows. Nothing else — the six-dimension audit of `${CODE_NAME}/` belongs to `/star-code-reviewer`, and a finding outside them is recorded for routing, not fixed.

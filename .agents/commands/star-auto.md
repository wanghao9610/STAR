...
2. Start what it names:
   ...
   - **Where the harness can name the model a delegate runs on**, every start above goes out on its tier's model directly (§10.8), that harness's own command wrapper saying how; the decisions record names the tier and the model with each start.

...
| `star-expt-analyst` | | Judge a run's results against its done-criteria; build the cross-run results table (`aggregate`); check a still-running job's health (`watch`) |
...
| `star-plan-executor` | | Execute a leaf sub-plan: plan, code, lightly validate, and record |
| `star-plan-reviser` | † | Revise a plan against execution evidence through one reviewed candidate list; drop or revive a direction |
| `star-proj-adopt` | † | Adopt an already-started project into STAR without disturbing it |
| `star-refs-reviewer` | | Build per-paper notes, a verified `reference.bib`, a field survey, or the related-work narrative |

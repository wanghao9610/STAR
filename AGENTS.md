...
- `.env` `STAR_PLAN_MODEL`, `STAR_EXEC_MODEL` and `STAR_READ_MODEL` name the model a skill run, and each delegate it dispatches, belongs on — research judgment, production, read-only work. Which run takes which, and when a run relocates itself to one of them, is `docs/mds/star-workflow/research-workflow-conventions.md` §10.8; an empty key changes nothing.

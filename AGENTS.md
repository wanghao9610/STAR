...
- `.env` `STAR_PLAN_MODEL`, `STAR_EXEC_MODEL` and `STAR_READ_MODEL` name the model a skill run, and each delegate it dispatches, belongs on — research judgment, production, read-only work. Each key holds one model name, or comma-separated `<harness>:<model>` entries, each optionally ending in `@<depth>` (the thinking depth); a run takes its own tree's entry and reads the key as empty where none is. A directly invoked run stays on the session's model: when the key for its manifest's `Tier:` is set, the first line of its final reply names that model and depth and the two ways to get them — switch the session's model, or start the run through `star-auto` — unless it carries `tier=` or its harness forked it onto its tier. An empty key changes nothing. The full rule is `docs/mds/star-workflow/research-workflow-conventions.md` §10.8; each host's mechanics are §13 (harness adapters).

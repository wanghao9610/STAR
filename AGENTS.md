...
- `.env` `STAR_PLAN_MODEL`, `STAR_EXEC_MODEL` and `STAR_READ_MODEL` name the model a skill run, and each delegate it dispatches, belongs on — research judgment, production, read-only work. Each key holds one model name, or comma-separated `<harness>:<model>` entries so one `.env` serves every tree — a run takes the entry tagged with the tree it runs from and reads the key as empty where none is. An entry may end in `@<depth>`, the thinking depth that tier's runs take where the harness can set one, so two tiers may name one model and differ only in depth. For a phase hand-off or a `star-auto` start, a configured depth the harness can apply per dispatch triggers a fresh delegate even when the model is unchanged. Which run takes which and how a host applies the model and depth is `docs/mds/star-workflow/research-workflow-conventions.md` §10.8: a directly invoked run stays on the session's model and says in one line when its tier asks for a model or depth the session lacks, `star-auto` starts each run on its tier's model, and an empty key changes nothing; each host's mechanics are §13 (harness adapters) of the same file.
...
- Replies and generated prose follow the human-writing contract in `docs/mds/star-workflow/research-workflow-conventions.md` §7: preserve research claims and the user's voice, diagnose formulaic prose from pattern clusters rather than isolated words, and never add facts or personality merely to sound human.

## 8. Project Layout

**Keep project files in their designated directories.**

- Core code in `${CODE_NAME}/`, as defined in `.env`; data and data-related files in `datas/`; model weights and weight-related files in `inits/`; generated output files in `wkdrs/`.
...
- `execs/` holds launcher scripts only: `run.sh`, `update.sh` and `configure.sh` at its root, per-run scripts in `execs/scpts/<run>.sh`. Anything that is not a launcher does not go there at all.
...
- Read the project root `.env` and use its `PYTHON_HOME`, or derive it from `CONDA_HOME` and `ENV_NAME` when `PYTHON_HOME` is empty. If `.env` is missing, create it from `.env.example` and fill in machine-specific values first.
...
- What travels with the repository goes to the versioned `.star/memory/`; a `machine:` scoped fact, true only here, goes to the git-ignored `.star/memory/local/`, as does any memory the user keeps off the repository.
- Where a memory disagrees with a file in the repository, the file wins.
- Types, file format, the index line the hooks build from each file, and how a memory is retired: `docs/mds/star-workflow/research-workflow-conventions.md` §12.

#!/usr/bin/env bash
# STAR PreToolUse hook (Claude Code) — decline the git commands that break the
...
deny() { # $1 = one-line reason
    printf '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"%s (declined by .claude/hooks/star_commit_guard.sh — hand it to the user to run)"}}\n' "$1"
    exit 0
}

...
- Involve (conventions §7.7). Coaching flattens the involve level (§7.9): `medium` and `high` are this file as written — the questions are the product, not overhead. `low` switches to draft-first: draft each stage's section from the seed material and what the user has already said (Core Principles item 4's "just draft it for me" made the default), present it, and still confirm once per section before writing; a section the user waves through without engaging keeps the honest "AI-drafted, pending confirmation" note. Step 0 resolution and the final rubric pass stay asked; the commit offer follows the level (conventions §1.5).

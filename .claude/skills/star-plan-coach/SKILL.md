...
3. If a plan has any section whose `status` is not `done`, ask whether to continue it (via AskUserQuestion: continue that plan / start a new one); if yes, resume from the first non-`done` section (before resuming, summarize completed sections in 2–3 sentences to restore context).

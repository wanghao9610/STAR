...
3. **先确认形状，再精读。** 精读是贵的那一步：把约 15 条排好序的候选（标题 / 会议 / 年份 / 引用数 / 一句话理由）用一次 AskUserQuestion（multiSelect，5–10 条标为推荐）交给用户，只读用户留下的。检索和分类不需要批准；精读和写笔记需要。
...
- 核心集确认是唯一的强制提问——一次 AskUserQuestion，multiSelect，标出推荐。不可用（headless / 脚本化）时退化为纯文本，精读前必须拿到明确答复。

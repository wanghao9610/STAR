...
≤400 字：方法来源与画像、写了哪些笔记（citekey → 文件）、条目数与类别表、自审计结果、待人工核对清单，以及路由——最接近工作的结论交 `/star-plan-coach` §2（相关工作与定位）去磨定位；以后单加一篇是 `/star-refs-reviewer <arxiv-id>`；`/star-refs-reviewer verify` 重查整个 bib；`/star-refs-reviewer synthesize` 把笔记合成为 `metds/refs/related_work.md`。

...
按 `references/refs_rubric_zh.md` 的核心论文标准排序，用一张表给出约 15 条（最相关在前），经一次 AskUserQuestion（multiSelect，5–10 条标为推荐）确认精读哪些。用户可以自己加论文——照常抓它们的记录。
...
8. **摘要。** ≤500 字：来源与画像、计数账、写了哪些支、自查结果，以及转交——补读清单逐篇 `/star-refs-reviewer <arxiv-id>`、或整单一次 `/star-refs-reviewer add …`；定位交 `/star-plan-coach` §2（相关工作与定位）；项目需要笔记和核验 bib 时走完整流程。

...
3. 既无计划也无 URL 时：若 `${CODE_NAME}/` 里已有真实代码，跳过这个问题——分支 B 负责整理既有代码，本就不需要计划，而这正是 `/star-proj-adopt` 转介进来的状态。否则用 AskUserQuestion 问：*先跑 `/star-plan-coach`（推荐）* / *直接给 GitHub URL* / *现在口述主题据此检索*。

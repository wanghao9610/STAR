...
**本次运行在哪里执行。** Step 0 前按规约 §10.8 在 PLAN 档应用迁移规则；判断是否还缺用户决定时，既有授权同样有效。架构设计留在 PLAN，已批准迁移按后文交给 EXEC。

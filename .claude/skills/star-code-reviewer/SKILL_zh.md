...
读 CLAUDE.md；若存在则读 `metds/codearc.md`（放置规则、命名约定、计划组件映射、§7 残留清单）；计划模式加读计划 §1–§6 与 `EXEC_PLAN.md` / `EXEC_LOG.md`。记下缺失的评判依据——没有 codearc.md 时，放置与命名检查退回 PEP 8 加周边代码的 upstream 风格（CLAUDE.md §3）。然后按 `references/review_rubric_zh.md` 组装**评判依据摘要**（评分表已随开头**通用规约**段的那次装载调用到达，不再单独读）：Step 3 逐字发给每个收集器的就是这一块。

...
- **Scale**: a small run (a handful of artifacts, no oversized log) is read by the main agent. For a large one — many log files, or logs too big to read whole — partition by file into read-only `Agent` subagents (`subagent_type: Explore`), at most 3 in parallel, each given the rubric, the expectations digest, and its exact file list, returning the structured observation contract. These read-only subagents never write, never read outside their list, never grade the run's verdict.

...
2. **Read wide, verify every number before it enters the report.** Collection may fan out to read-only `Agent` subagents (`subagent_type: Explore`), but the main loop re-opens the cited file at the cited line for every number and every blocker/major observation before the report keeps it; what does not hold up is downgraded or dropped. One wrong number costs the report its credibility — and a number in a report gets quoted into a paper.
...
- **Scale**: a small run (a handful of artifacts, no oversized log) is read by the main loop. For a large one — many log files, or logs too big to read whole — partition by file into read-only `Agent` subagents (`subagent_type: Explore`), at most 3 in parallel, each given the rubric, the expectations digest, and its exact file list, returning the structured observation contract. Collectors never write, never read outside their list, never grade the run's verdict.

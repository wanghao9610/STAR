...
调用方式：`star-expt-analyst [PLAN_NAME | RUN_DIR | aggregate [PLAN_NAME] | watch [PLAN_NAME | RUN_DIR]] [描述]`。先解析 `aggregate` 与 `watch`，再装载普通分析流程。计划经 `exec_runs` 定位 run，run 目录反查计划；目标未定时列出候选并询问。`watch` 仅在聊天中检查，不授权写文件或启动后续动作。
...
2. **广读，每个数字进报告前先核实。** 收集可以并行分派给只读 `Agent` subagent（`subagent_type: Explore`、`model:` 取从 `.env` 读到的 READ 档值，该键为空时不带这个参数），但每个数字、每条 blocker/major 观察进报告前，主 agent 都要按引用重开那个文件、定位到那一行确认；站不住的降一档或丢弃。报告里的数字是会被抄进论文的。
...
**本次运行在哪里执行。** 本次运行的档位随模式而定：普通分析为 PLAN，`aggregate` 与 `watch` 为 READ。它留在启动它的会话里，用会话的模型跑。该档的键（`STAR_PLAN_MODEL` 或 `STAR_READ_MODEL`）所指的模型不是会话正在跑的模型，或带有本宿主只能逐次派发时应用的深度时，在开头用一行说明——档位、该模型与深度，以及两种得到它们的办法：切换会话模型，或经 `star-auto` 启动这次运行（规约 §10.8）——然后在这里照常继续。带 `tier=` 的运行已按档位启动，不给这一行。`watch` 只在聊天中完成，结束后不启动写入型后续。
...
- **A——清点**：每个 §4 交付物记为 `present` / `missing` / `unexpected`，附轻量完整性检查（非空、可解析、大小合理）与布局符合度（CLAUDE.md §8）。
...
- 仅在工作流要求处使用 AskUserQuestion（分析哪个 run、匹配有歧义时）。不可用（无人值守 / 脚本化）时改用纯文本并要求明确答复。本 skill 除自己的报告外什么都不写，没有审批确认点——但绝不声称或暗示你改动了计划、状态或日志。
- 用按规约 §7.6 解析出的语言回复；该语言为中文时加载 `*_zh.md` 资源。报告跟随计划 frontmatter 的 `language`（否则取按规约 §7.6 解析出的语言）；中文报告里技术名词——指标名、日志键、文件路径——保留英文。

...
- 仅在工作流要求处使用 AskUserQuestion（分析哪个 run、匹配有歧义时）。不可用（headless / 脚本化）时退化为纯文本并要求明确答复。由于本 skill 除自己的报告外什么都不写，没有审批确认点——但同样地，绝不声称或暗示你改动了计划、状态或日志。

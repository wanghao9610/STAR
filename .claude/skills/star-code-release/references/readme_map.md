...
1. **A number is copied, never recomputed.** Every figure in §8 and §12 is transcribed from `wkdrs/results/results.md` exactly as that table carries it, with the run name it cites. A number the results table excluded (its `invalid` / `inconclusive` section) does not enter the README at all — not even with a caveat. If the results table is absent, §12 is omitted and the report routes to `/star-expt-analyst aggregate`.

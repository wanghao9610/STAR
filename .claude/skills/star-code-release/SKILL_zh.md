...
- 所有确认点都走 AskUserQuestion，一次一个问题：参数无法识别时的阶段选择、来源大面积缺失时的就绪决策、移入表的 确认点 1、每条打磨问题项、每处 README 小节变更、STAR README 或人工 README 的替换确认、以及中文 README 的提议。若该工具不可用（headless / 脚本化），退回纯文本，仍然一次一个，且任何写入前都必须拿到明确批准。

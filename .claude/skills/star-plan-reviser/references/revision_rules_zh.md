...
对总体计划章节做范围受限的文本修订仍属 local、是允许的：收紧一条 kill-criterion、给里程碑改期、记录某假设已被验证或已失败。"方法已死，换一个"不是一次编辑——那是一场 coaching 对话。
...
每次会话一个 `###` 块，真实日期（绝不编造），并在 skill 名之后写上本次编辑会话的 `model_id`——运行时报出的 id 原样抄录，没有则写 `unrecorded`（规约 §8）。这个逐条记录的 id 就是计划的模型归属依据：frontmatter 的 `model_id` 只写明最近一次写入者，每一次更早的修订出自谁手则保留在本节。每处改动一个要点：章节、改了什么、为什么、证据。`exec_status` 的重置、以及被清除的 `finalized:` 也记在这里；值得留痕的被拒候选可选记一笔（"用户保留 85 阈值，尽管未达标"）。丢弃同样是其中一条要点——`dropped: 2026-08-11 — 被 02 取代`——是什么终结了这个方向要写在这一条里，因为 frontmatter 那个字段只放得下一行。
...
一个已经放弃的方向——整条流程应当不再统计它、不再推荐它、不再在它上面继续搭东西。这个字段只写在做出决定的那个节点上，别处一律不写：所有 skill 都按继承来读它，所以一行就带走整棵子树；日后在这个节点下面新增的子节点也一并被丢弃，没有任何东西需要同步维护。

| 项目 | 规则 |
|---|---|
| 取值 | `dropped: <YYYY-MM-DD> — <一句话原因>`，日期取自系统时钟（规约 §4）。完整交代——试过什么、什么终结了它、由什么取代——写进同一次会话的 `## Revision History` 条目。 |
| 批准方式 | 本 skill 自己的一条候选，单独问、绝不打包：提问时点明随它一起变暗的东西——这个节点、每一个后代，以及它们的 run 本来欠着的后续。它**不**属于那些要转交给 star-plan-coach 的 strategic 候选：丢弃一个方向记录的是已经做出的决定，而选择由什么来取代它才是那场 coaching 对话。 |
...
两条入口，同一条规则。`<PLAN_NAME> drop` 走丢弃模式（见 `SKILL_zh.md`），它跳过审查，因为决定已经做出；在一次完整审查中浮现的丢弃则是 Step 4 的一条候选。两者写的都恰好是上面那三处，别的一概不写。

`exec_status: abandoned` 还是它一直以来的意思——一个叶子的执行结局。`dropped:` 说的是节点：它把这个节点连同其下的一切移出计数、覆盖检查和下一步动作。一个叶子可以同时带着两者，树上渲染的是计划级的那个字段。

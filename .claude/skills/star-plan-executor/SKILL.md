...
- Git: one commit per verified step, staging only the files that step touched — plus, on an execution branch, the run-record updates that step caused — and only when the confirmation point covered it; name pre-run dirty paths at the confirmation point (conventions §1). Branch create, merge, and discard happen only at their §11 confirmation points (`references/branch_rules.md`); never rebase an execution branch, and never delete one whose records have not first been carried to the base branch.

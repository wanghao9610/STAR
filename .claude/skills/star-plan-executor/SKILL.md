...
1. **Read before you write.** Always orient in `${CODE_NAME}/` first — read the modules/entrypoints the sub-plan's §2 names before planning any change. Produce a "current state vs §3 requirements" gap list. Never assume code exists; `code/` may be an empty codebase (only `.gitkeep`), in which case the plan builds the structure from scratch — or, better, start from a reference codebase first with `/star-code-architect`. Reference: `references/orient_checklist.md`.
2. **Plan behind a confirmation point, execute behind an agent.** The detailed executable plan (EXEC_PLAN) is produced in **plan mode** and approved via **`ExitPlanMode`** before anything is written or run. Execution is delegated to **subagents, one per step / step-group** — the main agent orchestrates and verifies; it does not edit code or launch jobs itself. Reference: `references/agent_dispatch_spec.md`.
...
4. **Right-sized.** An executable leaf can still be the wrong unit of work, and nothing re-checks that after the split: the plan may have been decomposed weeks ago, or written by hand. Apply here the sizing judgement the decomposer applies to its own drafts — one independently checkable chunk (the decomposer's own `references/subplan_rubric.md`, item 8). Every signal is in the text Step 0 already read, so the check costs no call of its own.

   - **Strong, any one is enough**: §5 states more than one independent check; §3 crosses the STOP line more than once, and each hand-back is a natural leaf boundary; §3 mixes acquiring data, building code, and running experiments in one unit, where a dataset is owed a leaf of its own.
   ...
   A lone weak signal is one sentence in the reply, never a question — interrupting a right-sized leaf costs more than missing an oversized one. Where the check fires, the split preview goes in the text above the call: 2–5 cards, each a slug, a one-line objective, and the done-criterion that unit would own, then the execution order in one line. Name it a preview and say what that means — no file written, no prefix assigned, no `depends_on` drawn, and `star-plan-decomposer` still picks its own axis and confirms its own list. Its cut lines are the signals that fired (one unit per done-criterion, per STOP-line crossing, per kind of work), not a fresh axis analysis; this skill does not decompose. Needing more than 5 cards is itself the finding — say that in a line instead of listing them. Then ask via AskUserQuestion: *split it first* (recommended once a strong signal fired) — the run ends here having written nothing, the command goes back as `/star-plan-decomposer <leaf> <the preview in one clause>` with the sketch carried as a lead (conventions §7.12), and execution resumes at `/star-plan-executor <the leaf it produces>` — or *execute it as it stands*, which costs: every STOP-line crossing stops the run and resumes it, one `blocked` step holds up everything behind it, and a failure re-runs the leaf entire. Executing part of the leaf is not offered — narrowing the scope is a decision nobody made (conventions §10.4).

   **Fresh runs only, and asked once.** A leaf whose `exec_runs` is non-empty, or with a run in flight on a branch or worktree, skips this check outright: splitting mid-run leaves that run's `wkdrs/<run>/` hanging off a node no executor revisits, the thing `star-plan-decomposer` stops to warn about. Where item 1 is already routing back to the decomposer, this rides in that question rather than following it — same destination, one round trip. Record the verdict and the signals in `EXEC_LOG.md`'s "Notes / decisions" when Step 4 creates it (conventions §7.8), so a resume does not reopen it and a later reader knows the leaf was flagged and run anyway.
...
2. When it returns, **the main agent re-runs the step's own check** to confirm (do not trust a self-reported pass without evidence). Pass → record it in `EXEC_LOG.md`, update the sub-plan's lightweight status, and — when the confirmation point approved per-step commits — commit this step's files; on an execution branch the commit also stages the run-record updates this step caused, because only commits merge (conventions §11.2). Fail → the main agent's own re-run is the evidence: read the `file:line` the failure names, and open the agent's full diff only when deciding `blocked`, or when the failure looks like a sub-plan-granularity problem (item 4). Restore this step's files before retrying; bounded retry (≤2) with the failure fed back; still failing → mark the step `blocked`, settle what happens to its edits (`agent_dispatch_spec.md`), and stop with the log.
3. **If the step is on the STOP line** (heavy experiment) → do **not** dispatch it to run; write the prepared command into EXEC_LOG's "Awaiting user" area, add a row to the "Cost" section with its expected cost (GPUs × hours, or call count and spend), and stop, handing it to the user. When the user returns with the result, fill the actual into the same row — that is the only place the root plan's §4 compute budget is reconciled; where the actual cannot be recovered write `unrecorded` rather than leaving it blank.
...
- If AskUserQuestion or plan mode is unavailable (headless / scripted), fall back: present EXEC_PLAN as plain text and require an explicit plain-text approval before anything is written or run — still stop for confirmation before executing, still stop before heavy experiments, still confirm in plain text before any change is written back into the sub-plan.

...
1. **完成判据确实被检查过**——子计划 §5 检查已跑,其证据(测试通过 / 指标越过阈值 / 产出存在)记录在 EXEC_LOG,而非只是声称。

2. **每一步都有真实检查结果**——EXEC_LOG 里不存在标了 `done` 却没有**主 agent**通过记录的步骤(只有 agent 自报不算)。
...
7. **改动是 精准克制 的**——代码 diff 都能追溯到 EXEC_PLAN 的步骤;没有对邻近代码做无关重构或"改进"(CLAUDE.md §3)。
...
10. **文档会引用的值都已捕获**——本轮敲定、子计划未写明、且某个 `metds/*.md` 章节会引用的值(超参、初始化、复现命令),要么在子计划里,要么作为一条 ENRICHED 行等待用户确认(`plan_sync_rules_zh.md`)。只有 EXEC_LOG 知道的值,`star-metd-summarize` 只会把它报成一个永久缺口。

...
1. **Resolve the runtime.** Read `.env`; get `CODE_NAME`, `CONDA_HOME`, `PYTHON_HOME`. If `.env` is absent, create it from `.env.example` and ask the user to fill machine-specific values before running anything — do not guess paths (CLAUDE.md §6).

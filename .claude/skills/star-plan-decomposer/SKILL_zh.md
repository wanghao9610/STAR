...
检查父计划的 frontmatter `status` 与正文。若关键章节（尤其是**方法**与**里程碑**）仍为 `pending`/`in_progress`，或布满 `[TBD]`，告知用户拆解会比较浅，并用 AskUserQuestion 提供选项：*仍然拆解（缺口在子计划里记为 `【待定】`）* / *先回到 `star-plan-coach` 把父计划补完*。尊重用户选择。

...
Invocation: `/star-flow-status [PLAN_NAME]`. This run's argument is substituted here at load time — `$ARGUMENTS` — and nothing between those backticks means none was given. This skill runs in its own context (`context: fork`), so that substitution is the only place the argument appears: there is no user turn behind it to re-read, and treating an argument that is there as absent silently answers a question nobody asked. With no argument, report the whole flow; with a slug / numeric prefix / filename, scope both the tree and the coverage checks to that plan's subtree. An `involve=<level>` token is stripped from the argument before `PLAN_NAME` is resolved (conventions §7.7); it changes nothing else here — this skill asks no questions.

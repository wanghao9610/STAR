...
Because this skill runs with `context: fork`, read its load-time arguments from `$ARGUMENTS`; an empty substitution means no argument. The fork has no user turn from which to recover them.
...
**READ-tier entry on this harness.** Before the scan, apply conventions §10.8 once: when the configured READ override is usable — either its model is not an alias of this run's, or the harness can apply its depth per dispatch — hand the complete run to one fresh READ-tier delegate with that model and supported depth, the original invocation, the scope this run already resolved (stated as settled), whether the user typed the invocation or the run was picked up (conventions §10.5), resolved language, `involve=<level> tier=read`, and any held grant; wait and relay its reply. A native READ fork or a run carrying `tier=read` skips this check. Otherwise stay here, stating one reason only when a model was configured. The run remains strictly read-only. Follow `references/status_spec.md` (Chinese: `references/status_spec_zh.md`) for status, coverage, and priority.

...
description: >-
  ...
  missing or out of date. Never writes. Use when the user runs star-flow-status, when a run names it as
  the next action, or asks for the status / overview / progress of their research or plans, what to work
  on or execute next, what still needs doing, how far a plan or its sub-plans have gotten, or to see the
...
Invocation: `star-flow-status [PLAN_NAME] [DESCRIPTION]`. This run's argument is substituted here at load time — `$ARGUMENTS` — and empty backticks mean none was given. This skill runs in its own context (`context: fork`), so that substitution is the argument's only appearance: no user turn behind it to re-read, and treating a present argument as absent silently answers a question nobody asked. With no argument, report the whole flow; with a slug / numeric prefix / filename, scope the tree and the coverage checks to that plan's subtree. Anything after `PLAN_NAME` is a description (conventions §7.12): in your own words, what this run is for. Prose resolving to no plan is description alone, not a missing target — report the whole flow, and say so in the reply's first line. A lone token that looks like a plan name and matches none is not a description: name it unmatched in that same line, since this skill never asks. A description steers what the report looks at hardest — which unknown is worth a second read, which line is worth quoting rather than counting, what the reason line addresses; it drops nothing the report owes, and never moves the next action, which the spec's priority order fixes down to the tie-break. An `involve=<level>` token is stripped before `PLAN_NAME` or the description is read (conventions §7.7); it changes nothing else here.

**READ-tier entry on this harness.** A run already started by a native context fork or carrying `tier=read` skips this entry gate. Otherwise, before the unconditional opening load, take only its `.env` lookup (reuse visible values when available), deferring the conventions reads and scan. Resolve READ for the actual harness and the current model from this session's provenance. A non-empty target different from a known current model may receive this whole run once through the model route described below, only when no question this mode or its possible findings would require is left. If deciding that would require a scan, keep the run here instead. Any failed condition keeps the run here, with one reason for a set key; reuse the `.env` values in the remaining opening load, so the lookup and scan each run only once. The delegate reads this manifest in full, receives the original invocation plus `involve=<level> tier=read`, the resolved reply language and any held `auto=unattended` grant, and performs the complete opening load itself. A status delegate is read-only; a digest delegate may write only the digest's allowed outputs. Wait and relay its reply; any artifacts carry the delegate's own provenance. This is the entry route for hosts without native skill forks, not a fan-out of the scan.

**Shared conventions.** `docs/mds/star-workflow/research-workflow-conventions.md` (Chinese: `research-workflow-conventions.zh-CN.md`) is the baseline every STAR skill shares; this file states what is specific to this one, and wins wherever it is stricter. What a read-only reporter acts on — §0 vocabulary, §5 plan-name resolution (only when there is a `PLAN_NAME` to resolve), §7's reporting rules (its preamble and items 1, 4, 5, 6, 11, 12; the question machinery in items 2–3, 7–10 and 13 governs skills that ask, and this one never asks), §9 project layout, and §10 the skill roster (this one is among the eight the agent may start unnamed; §10 is what bounds such a run) — arrives through Step 1's one load message. The rest stays out. §8 (the output table) is what the coverage checks audit against, cited, not loaded: the spec restates every filename and state field those checks read, and the rest of §8 — `model_id` / `model_trail` — governs producers, which this skill is not. §1 git, §2 the STOP line, §3 `.env` runtime, §4 real dates and §6 delegation govern skills that commit, run, or write; this one does none of that. Of §11's execution branches it needs only the listing Step 1's scan call carries and the reading rules the spec restates — the section itself stays with the skills that branch. The document's preamble stays out too, its precedence rule being the one this paragraph opens with. The conventions excerpt is about 25 KB in total, split 4, 1, 6 and 14 across Step 1's four prints. Read the whole file if you ever need one of them.
...
**One scan, then reason.** Step 1 collects everything this skill reads in one message, and Steps 2–9 work from what it returned. Do not re-open a file it already covered. Only two things earn a second read: a plan section you must quote rather than count, and a file the digest lists as present but does not print. This is the most-run skill in the flow, and extra round trips are what make it slow — a per-file read loop, or the load spread over separate messages.

### Step 1: Scan
Load everything in one message — two Bash calls, both with the project root as the working directory. First call, the conventions excerpts:
...
awk '/^## /{k=/^## (0)\./} k' docs/mds/star-workflow/research-workflow-conventions.md
awk '/^## /{k=/^## (5)\./} k' docs/mds/star-workflow/research-workflow-conventions.md
...
One message, three results: the conventions excerpts and the collector's digest from the two Bash calls, and the spec — `<this skill's directory>/references/status_spec.md` — from a `Read` sent in the same message. The first call opens with the `.env` lookup: the reply language, the question level, and the three model keys this run and any delegate it dispatches take their model from (§10.8). Several tool calls in one message cost one round trip between them, not one each; only a second message costs another. That is why the two commands stay separate: a Bash result past roughly 30 KB is written out to a file that costs a round trip to read back. Joined, the excerpts and the digest share one size limit, which a project with history overruns, losing both; separate, the excerpts are a fixed size and always arrive, and only the digest — the one part that grows with the history — can ever be written out. Keep the spec out of both commands for the same reason: another whole file, big enough to crowd either result toward the size limit. With no `PLAN_NAME` argument drop the `(5)` print — §5 exists to resolve one. The awk selects §7 by item number (the preamble plus items 1, 4, 5, 6, 11, 12); if it prints nothing — a stale synced copy of the conventions may number differently — load the whole section with `sed -n '/^## 7\./,/^## 8\./p'` instead. The last `awk` prints §9 and §10 and nothing else: §11, the execution branches, is deliberately not loaded — the `git branch` and `git worktree` lines above carry the live listings, and the spec restates how an execution branch and its worktree read. Every path the digest prints is relative to the project root. The digest is: every plan's frontmatter, its `## Sub-plans` index and its §3/§5 placeholder counts (`[TBD]` and `【待定】` together); every run log's frontmatter, its body counted per heading, and the dates in it; the frontmatter of every listed artifact outside a run directory; and a depth-1 listing of `metds/` and `wkdrs/`. Together with the spec and the conventions sections, that is the whole input for Steps 2–9.

`--slim` keeps this affordable once a project has history: it collapses the two parts of the digest that grow with the history rather than the plan tree — about a third off the digest on a 40-run project. A step table over six rows arrives as its header row, `[tally] N data rows`, and a value histogram per column — read `c3: done×7, blocked×1` as the step counts Step 3 needs; a column given as `N distinct` is a step name or a date, never a status. Six rows or fewer arrive unchanged. Un-ticked checkboxes and plan-level findings are never summarised, so an awaiting-user leaf still shows its exact command. An artifact inside a run directory prints no frontmatter: LISTING already carries its name and the date in it, which is all the follow-up checks read; the count of those left out is printed. Drop `--slim` only to read one run's steps line by line, paired with `--runs <that run>`.

`--slim` makes this rarely necessary, but on a very long history you can narrow further, and only when the question is scoped to a subtree: `--runs <dirs>` keeps the per-run body and dates line to those runs. Every run's frontmatter still prints, and PLANS, LISTING and DIRS stay project-wide — resolving the subtree needs every plan's `parent:`, and the unrecognized-files line counts across the whole project. A run outside the scope is named as omitted, never dropped silently.

The script gathers, never judges — it knows nothing about status symbols, coverage rows, the priority order, or which filenames the output table expects, so every rule stays in this file and in `references/status_spec.md`. Read what it prints as raw file content. If it is missing or fails, fall back to reading the files directly and say so in your reply. If you cannot resolve this skill's own directory, any copy in the repository will do — every `scripts/scan.sh` is byte-identical and CI enforces that: `bash "$(find . -path '*/skills/*/scripts/scan.sh' | head -1)"`.

If `PLAN_NAME` was given, resolve it and keep only that node and its descendants — every step below reports that subtree and nothing else. The scan stays project-wide: scoping a subtree needs every plan's `parent:` first, so a plan outside it is read to resolve the tree, then dropped, never rendered. The resolution rules, including what an ambiguous name does where there is no question to fall back on, are in the spec's Scope section.

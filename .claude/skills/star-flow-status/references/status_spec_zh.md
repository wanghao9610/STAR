...
给一句话理由和命令。若各层都给不出候选，说清是以下哪一种：不欠任何东西且每个叶子都已了结——整项研究已经完成，转交给 `/star-code-release`，或用 `/star-idea-storm` 开下一个题目；某个未满足的依赖（写明）；需拆解的粗糙叶子；或 `metds/plans/` 为空（转交给 `/star-plan-coach`；连想法也没有则 `/star-idea-storm`）。

...
A fifth, sixth and seventh come as a set: `STAR_PLAN_MODEL`, `STAR_EXEC_MODEL` and `STAR_READ_MODEL` name the model each tier of STAR work runs on — research judgment (plans, reviews, analyses, blind reads), implementation and production, and read-only scans, collectors and summaries — each value written the way your harness names a model, or as a full model id. A value is either one such name, used by whichever harness reads the key, or comma-separated `<harness>:<model>` entries — `claude`, `codex`, `cursor`, `dsh`, `kimi`, `pi`, `qwen`, the tags `STAR_HARNESSES` already uses — so one `.env` can serve every tree: a run takes the entry tagged with the tree it runs from, falls back to an untagged one, and reads the key as empty where it has neither, while a tag no tree answers to is ignored and leaves room for a harness added later. An entry may also end in `@<depth>` — `low`, `medium`, `high`, `xhigh`, `max`, or a positive integer — the thinking depth that tier's runs reason at wherever the harness can set one, so two tiers may name one model and differ only in depth. Claude Code reads the suffix from tier manifests synchronized by `bash execs/update.sh --models`; Codex passes a supported suffix as `reasoning_effort` on every subagent dispatch and needs no stamp or session restart, so `codex:gpt-6-astra@xhigh`, `codex:gpt-6-astra@high` and `codex:gpt-6-astra@low` can serve PLAN, EXEC and READ respectively. Cursor's `--models` stamp rewrites the same `@<depth>` as `id[effort=<depth>]` on that tier's named agent's `model:` field, unquoted, so `cursor:grok-4.6@xhigh` beside `cursor:grok-4.6@high` is one model at two depths. On a harness that can start a delegate on a named model, a run whose tier names a different model — or, on Codex, an explicitly configured per-dispatch depth — is handed to one delegate, and the artifacts record the model that wrote them; Claude Code's two forked READ skills carry theirs in the skill manifests, while Cursor and Qwen use named agent definitions synchronized from `.env` by `bash execs/update.sh --models`. All three keys ship empty, and empty means the harness default — the session's own model and effort, with nothing about how a skill runs changing. Full rule: [research workflow conventions](docs/mds/star-workflow/research-workflow-conventions.md#10-the-skill-roster) §10.8.
...
- `.claude/agents/`, `.cursor/agents/` and `.qwen/agents/` — the named `star-plan`, `star-exec` and `star-read` agents those three harnesses dispatch a tier's run to, stamped from `.env` by `bash execs/update.sh --models`: Cursor and Qwen take the tier's `model`, Claude Code its `effort`
...
The general forms are `bash execs/update.sh [--diff] [ref] [--harnesses LIST] [--skill NAME] [--force]`, `bash execs/update.sh [ref] [--harnesses LIST] --adopt`, and the offline `bash execs/update.sh --models`:
...
- `--models` synchronizes the configured models offline: Claude's two forked READ skills, its manifests' and three named delegates' thinking depth, plus the three named STAR agents in each of Cursor and Qwen. Each tree takes its own tagged entry, then an untagged fallback; empty values leave existing files alone. Codex reads its model and supported effort at dispatch, so the command only reports those values for Codex and no restart is needed. An ordinary update applies the static synchronization; start a new Claude, Cursor or Qwen session to load changed manifests or agents. The command runs alone, refused beside `--adopt`, `--diff` and `--skill`.
...
| --- | --- |
| Claude Code | Passes `model` per dispatch, and dispatches the delegate as the `star-plan`, `star-exec` or `star-read` agent carrying that tier's depth; an explicit depth triggers a delegate even when the model is unchanged. `--models` sets those agents, the manifests' depth, and the two forked READ skill manifests. |
...
| [Cursor](https://cursor.com/docs/subagents) | Selects `star-plan`, `star-exec` or `star-read`, whose model is synchronized by `--models`. |

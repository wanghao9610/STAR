...
Six skills open the same plans, run logs, and reports before doing anything else — `star-flow-status`, `star-expt-digest`, `star-plan-decomposer`, `star-plan-executor`, `star-plan-reviser` and `star-metd-summarize` — so each gathers them with one read-only script, `scripts/scan.sh` in its own directory inside your tool's directory, instead of opening each file. That is a shell call, so your agent asks to approve it the first time it runs.

Claude Code needs nothing on a fresh install — `.claude/settings.json` ships allow rules for exactly those six scripts and nothing else. A project adopted earlier keeps its own `settings.json` (`execs/update.sh` installs that file only when missing, never overwriting it), so add the rules there yourself:
...
| `$star-refs-reviewer` | Survey the work related to the method: read the closest papers into analysis notes and build a classified bibliography whose every entry is transcribed from a fetched record; `survey` reads a whole field in tiers and writes a standalone survey of it | `metds/refs/<ABBREV>.md`, `metds/refs/reference.bib`, `metds/refs/refs_index.md`, and `metds/refs/<slug>_survey.md` |
...
- **[v0.1.5](https://github.com/wanghao9610/STAR/tree/v0.1.5)** (2026-07-30) — The opening load stops walking the plan tree file by file. Four more skills — `star-plan-decomposer`, `star-plan-executor`, `star-plan-reviser`, `star-metd-summarize` — gather it with the same read-only collector `star-flow-status` and `star-expt-digest` already shared, and the call rides in the Bash call their opening message was already sending, so it costs no round trip of its own: every plan's frontmatter, its `## Sub-plans` index and placeholder counts, and every run log's frontmatter arrive in one result, and each skill keeps a fallback to direct reads that it has to declare in its reply. Measured headlessly, three runs per configuration on Claude Opus 5 against a fixture of three plans and one run log: a cold single-skill run drops from 5.7 to 4.0 model round trips, tool calls from 8.0 to 4.7, and the context it builds from 244k to 176k tokens, with no tool call touching `metds/plans/` at all where a whole round trip used to go to opening each plan. Every skill's opening load also now states when it may be skipped: a second skill in the same conversation reuses whatever it can still see verbatim — the conventions in the same language covering at least the sections it names, the same reference files, the probe's `STAR_LANG` / `INVOLVE` values — but never a collector digest, which is a snapshot of files a run may have written to since, so the scan runs again every time. That paragraph buys certainty rather than speed, and the measurement says so plainly: the model already reused the in-context conventions in every baseline run, and a chained two-skill session comes out level (6.7 to 6.3 round trips for the pair) because the first skill no longer drags plan bodies into context for the second to inherit. A new maintainer check pins the paragraph — one per manifest, uniform per language, inside the opening-load block, and carrying no bare `§n`, which check 20 would read as a claim about which sections load — and the shipped Claude Code allow rules now cover all six collector scripts.
- **[v0.1.4](https://github.com/wanghao9610/STAR/tree/v0.1.4)** (2026-07-29) — Faster skill startup, ported across the four tool trees in both languages. Every skill now opens with a single load message: whole files — conventions, templates, schemas — arrive through the file-reading tool instead of `cat` in Bash (a Bash result past roughly 30 KB spills to a file and costs a second round trip, and the conventions file alone is past that line), and the `.env` probe for `STAR_LANG`/`INVOLVE` rides in the same call. Two skills narrow that load further, to the sections they act on: `star-expt-digest` and `star-refs-reviewer` take an `awk` excerpt of the conventions in that same Bash call — 25% and 23% less per run — each naming what it loads, what it leaves out with where its own files restate it, and the way back to the whole file when a run needs it. The other twelve keep the whole-file read on purpose (`star-flow-status` already loaded only the parts it reports on): an excerpt has to ride in Bash to be an excerpt at all, and a skill that commits, asks, and writes a registered artifact needs so much of the document that the excerpt lands back at the spill line — a per-skill audit, with two adversarial passes over every proposed omission, sent four candidates that looked safe back to loading in full. `SKILL_zh.md` is no longer loaded at runtime — it stays a full mirror for human readers, while a Chinese run replies in Chinese per conventions §7.6 and reads only the `_zh` / `.zh-CN` resources, removing an ~11 KB duplicate of every skill body from every Chinese run. `star-flow-status` gathers its whole input — the scan plus the spec — in one call, and the chat-reply word cap unifies at 500. A new maintainer check pins this opening-load shape so a half-done edit fails loudly, and `wkdrs/` joins `.cursorignore`.
- **[v0.1.3](https://github.com/wanghao9610/STAR/tree/v0.1.3)** (2026-07-29) — `star-refs-reviewer` gains a `survey` mode: search a topic, read it in three declared tiers (8–12 papers deeply, 15–25 at abstract level, the rest from their records alone), and write a standalone field survey to `metds/refs/<slug>_survey.md` — a taxonomy under a stated division axis, a comparison table only the deep tier may fill, every claim citing a source fetched during the run, and neither `reference.bib` nor `refs_index.md` touched. One question comes before the reading (profile, axis, tiered list), and `involve=low` answers it by its recommendation, so a survey can run unattended end to end. The append mode grows an `add` form beside it: several papers in one call — arXiv ids, DOIs, URLs, and titles mixed freely, every title resolved to a fetched record and never guessed. Ported across the four tool trees in both languages, with the survey template and its grading rules beside the skill.
- **[v0.1.2](https://github.com/wanghao9610/STAR/tree/v0.1.2)** (2026-07-28) — One wording rule, in `AGENTS.md` §7 and conventions §7.11: write the action, not its name. It holds in any language and covers the prose a run writes into files, not only chat. All fifteen skills are audited against it across the four tool trees: what was a "gate" is now named for what it does — a confirmation point where the run stops and asks, or the test a document has to pass — and the Chinese copies lose the coined terms the English side had already dropped (收编, 落盘, 落地, 台账 and four more). The same rule now settles the case it left optional: a literal that is nothing but a pointer — `§4`, `C4`, a run name — carries a few words of what it points at, in parentheses, on first use in every chat reply, because the reader of one message does not scroll back.
<details>
<summary>Earlier releases</summary>

- **[v0.1.1](https://github.com/wanghao9610/STAR/tree/v0.1.1)** (2026-07-27) — Faster `$star-flow-status`: the plan tree is scanned once and cached, and `$star-expt-digest` reads the same scan. Adds `STAR_LANG` to pin the language of replies and newly generated workflow documents; `execs/update.sh` now syncs `AGENTS.md` as well, and takes `--force` to update over uncommitted changes and hook registration configs. The Claude Code and Codex provenance hooks now read the model id when a skill records it, so a model switched mid-session is no longer written down as the one the session opened with.

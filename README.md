...
A fifth, sixth and seventh come as a set: `STAR_PLAN_MODEL`, `STAR_EXEC_MODEL` and `STAR_READ_MODEL` name the model each tier of STAR work runs on — research judgment (plans, reviews, analyses, blind reads), implementation and production, and read-only scans, collectors and summaries — each value written the way your harness names a model, or as a full model id. On a harness that can start a delegate on a named model, a run whose tier names a model other than the session's own is handed to one delegate on that model, and the artifacts record the model that wrote them; the two read-only skills carry theirs in the skill manifest instead, stamped there from `.env` by `bash execs/update.sh --models`. All three ship empty, and empty means the harness default — the session's own model, with nothing about how a skill runs changing. Full rule: [research workflow conventions](docs/mds/star-workflow/research-workflow-conventions.md#10-the-skill-roster) §10.8.
...
Two things an existing project therefore picks up by hand, both new in v0.2.18. The three model keys: `.env.example` reaches a project at `--adopt` time only, not on an update, and your `.env` is yours in any case, so add `STAR_PLAN_MODEL`, `STAR_EXEC_MODEL` and `STAR_READ_MODEL` to it yourself — [upstream's `.env.example`](.env.example) carries the comment that explains them. And the sub-agent registration: a `.claude/settings.json` written before it has no `SubagentStart` block, the note names it as the missing `SubagentStart delegate context` hook, and the fix is to copy that block's two hook commands out of upstream's file into yours.
...
- `--models` restamps `STAR_READ_MODEL` from `.env` into the two skill manifests that fork into a context of their own — `star-flow-status` and `star-expt-digest`, both languages — and touches nothing else, offline, with no clone and no network. Every other skill is given its model when a run starts it, which a manifest cannot express. An empty key stamps nothing and leaves the manifests as they shipped; an ordinary update ends with the same step, so a stamp survives one. It runs alone, refused beside `--adopt`, `--diff` and `--skill`.

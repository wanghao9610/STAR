...
- `.cursor/agents/` and `.qwen/agents/` — the named `star-plan`, `star-exec` and `star-read` agents those two harnesses dispatch a tier's run to, their `model` stamped from `.env` by `bash execs/update.sh --models`
...
- `--models` synchronizes the configured models offline: Claude's two forked READ skills, plus the three named STAR agents in each of Cursor and Qwen. Each tree takes its own tagged entry, then an untagged fallback; empty values leave existing files alone. An ordinary update applies the same synchronization. Run it after changing the model keys and start a new session to load changed agents. It runs alone, refused beside `--adopt`, `--diff` and `--skill`.
...
| --- | --- |
| Claude Code | Passes `model` per dispatch; `--models` also sets the two forked READ skill manifests. |
| [Codex](https://learn.chatgpt.com/docs/agent-configuration/subagents#choosing-models-and-reasoning) | Passes `model` when the runtime exposes it; uses a fresh context when a full fork cannot accept an override. |
...
- **[v0.3.0](https://github.com/wanghao9610/STAR/tree/v0.3.0)** (2026-09-04) — Three `.env` keys decide which model does which kind of STAR work: `STAR_PLAN_MODEL` for research judgment, `STAR_EXEC_MODEL` for implementation and production, `STAR_READ_MODEL` for read-only scans and summaries — all empty upstream, and an empty key leaves every run exactly as it was. Conventions [§10.8](docs/mds/star-workflow/research-workflow-conventions.md) holds the rule and the roster gains a Tier column, with the exceptions written beside it: a mode can override its skill's tier — `star-expt-analyst aggregate` and `watch`, plus `star-code-release check`, on READ, `star-refs-reviewer synthesize` and `star-proj-adopt backfill` on PLAN — and two runs change tier partway through, `star-plan-executor` handing its execute-and-verify loop and `star-code-architect` its approved migrations to one delegate on EXEC while the judgment around them stays on PLAN. The architect now writes `metds/codearc.md` the moment the migration table is approved, every item `pending`, which is what makes that hand-over resumable. A run started in the main session relocates itself once, never recursing and never while a confirmation point is still owed to you; `star-auto` applies the same rule from outside, starting each run on its tier's model directly. The decision itself is a paragraph at the head of every relocating manifest's Workflow, the session's model read through the provenance resolver, so a skill typed into a session on another model moves by itself. Inside a run the model follows the work — collectors on READ, implementers on EXEC, independent blind reads on PLAN, in place of the `model: sonnet` the manifests used to name — and a harness that cannot name a delegate's model ignores all three keys. A key holds either one model name or comma-separated `<harness>:<model>` entries, so one `.env` can name a model per tree, and a tag no tree answers to is ignored rather than misread — room for a harness added later. Cursor, Qwen Code and Pi now reach their tier's model through named STAR agents rather than a per-call parameter none of them documents: `.cursor/agents/` and `.qwen/agents/` each carry `star-plan`, `star-exec` and `star-read`, Pi gains `star-runner` and a `star_subagent` whose single, parallel and chain schemas take a model, and `bash execs/update.sh --models` stamps all of them from `.env` beside the two Claude manifests — a run whose named agent is missing, stale or not yet loaded says so and keeps its original route. `.github/scripts/check_model_routing.sh` covers that routing offline in CI. `bash execs/update.sh --models` stamps `STAR_READ_MODEL` into the two manifests that fork, offline, and both session hooks now run again on Claude Code's `SubagentStart`, so an artifact a delegate wrote records the delegate's own model. An existing project adds three things by hand: the three keys, since `.env.example` reaches a project only at `--adopt` time; the `SubagentStart` registration in a kept `.claude/settings.json`, which the updater's note names; and the tier bullet in `AGENTS.md`, which an update keeps rather than overwrites, silently, because a project that has written its own instructions owns them.

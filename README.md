...
Two hooks run at the start of a session: one records the model id skills write into every artifact, the other puts the [project memory](#project-memory) index in front of the agent. Claude carries a third that is not a session hook: while `.env` reads `INVOLVE=low` it answers the permission prompt before a file edit, and at every other level it does nothing. It ships registered in `.claude/settings.json` like the other two.

...
- **AI-friendly project guidance and research workflows** shared across Codex, Claude, Kimi Code, Cursor, Pi, and Qwen Code, with support for both English and Chinese.
...
├── .pi/hooks/              # Session hooks for Pi, wired by .pi/extensions/star-hooks.ts
...
| Pi | nothing to pre-approve — Pi ships no permission prompts; sandbox it instead if you want the command restricted |
...
| Pi | `/skill:star-<name>` | `/skill:star-plan-coach open-vocabulary detection` |
...
- `.claude/hooks/`, `.codex/hooks/`, `.cursor/hooks/`, `.kimi-code/hooks/`, `.pi/hooks/`, `.qwen/hooks/`, and the two files that register them where registration is not automatic — `.kimi-code/hooks.example.toml` and `.pi/extensions/star-hooks.ts` — the model-id provenance, project memory, and involve-gate hooks
...
- Delete the tool directories you will not use. Each of `.agents/` (Codex), `.claude/`, `.cursor/`, `.kimi-code/`, `.pi/` and `.qwen/` is a self-contained copy of the same fifteen skills, ~150 files each; keep the one your agent reads and `rm -rf` the rest. On Pi this is worth doing rather than optional: Pi reads `.agents/skills/` as well as `.pi/skills/`, and deleting `.agents/` removes the name collision that `.pi/APPEND_SYSTEM.md` otherwise has to talk the agent out of.
...
- **[v0.1.44](https://github.com/wanghao9610/STAR/tree/v0.1.44)** (2026-08-13) — A sixth skill tree, `.pi/`, brings the fifteen skills to [Pi](https://github.com/earendil-works/pi): `/skill:star-*`, Pi's own lowercase built-ins (`read`, `bash`, `edit`, `write`, `grep`, `find`, `ls`), and three hooks wired by a TypeScript extension Pi discovers itself — no registration file to merge, and a `model_id` that cannot go stale, since the extension reads the live model before each prompt and re-injects on every `/model`. Pi ships no sub-agents, no plan mode and no permission prompts, so this tree is the first where all three are substituted rather than renamed: questions are plain text, the executor holds its own no-side-effects gate, and every dispatch becomes conventions §6.1's local fill. The involve gate is deliberately absent — it answers a permission prompt, and there is none — and `.pi/APPEND_SYSTEM.md` settles which copy to follow, since Pi reads `.agents/skills/` too.

...
Two hooks run at the start of a session: one records the model id skills write into every artifact, the other puts the [project memory](#project-memory) index in front of the agent. Claude and Codex carry a third hook that is not a session hook: while `.env` reads `INVOLVE=low` it answers the permission prompt before a file edit, and at every other level it does nothing. It ships registered in `.claude/settings.json` and `.codex/hooks.json` like the other two. Cursor and Kimi Code do not carry it: Cursor has no hook that fires before a file edit, and Kimi's `PermissionRequest` only observes the prompt it fires beside. All four carry a further hook, also not a session hook, and this one runs at every level: `star_commit_guard.sh` declines the git commands the [conventions](docs/mds/star-workflow/research-workflow-conventions.md) §1 forbids — blanket or forced staging, the history rewrites, and a commit whose staged files exceed 10 MB. Claude, Codex and Kimi Code run it on `PreToolUse` matching `Bash`; Cursor on `beforeShellExecution`, which is where a shell command is decided there. It is the floor under `INVOLVE=low` answering the commit offer itself: what it declines is yours to run.
...
- `.claude/hooks/`, `.codex/hooks/`, `.cursor/hooks/`, `.kimi-code/hooks/`, and `.kimi-code/hooks.example.toml` — the model-id provenance, project memory, and involve-gate hooks
...
- **[v0.1.21](https://github.com/wanghao9610/STAR/tree/v0.1.21)** (2026-08-07) — Delegating stops being the exception. [§6](docs/mds/star-workflow/research-workflow-conventions.md) read "execute locally by default", which made every fan-out something a run had to justify; it now hands the call to the main agent and says that keeping a large read out of the main context is the ordinary way to run such a step, not an exception that owes an argument. The flat "at most three run at once" goes with it — how many run is sized to the work — and so does §6.5's reservation of file-changing delegates to `star-plan-executor` and `star-code-architect`: any skill may send one, because the written dispatch contract was the part that mattered, and a skill with no spec of its own states the same terms in the step that dispatches. `star-flow-status` loses "no subagents", the roster's one outright ban, while staying strictly read-only and still never writing. Across the four trees 76 occurrences of "at most 3 in parallel" and 26 further caps in other shapes lose the number, and 36 size thresholds phrased as instructions — "a small tree is read by the main agent" — become guidance the main agent may go either side of once it can say why, since a `SKILL.md` wins wherever it is stricter and would otherwise have overridden the policy above it. What is kept is everything that was never about caution: disjoint file ownership among concurrent delegates, the main agent re-running every check and owning every judgment, read-only delegates that write nothing, and above all the per-host request budget (§6.9) — the reason a fetch fan-out is bounded at all, split into numbers or serialized, and now the one place a fixed count still earns its keep. Retry caps and the "at most 3 EXEC_PLAN actions" that sizes a single delegate's workload are untouched, neither having ever counted delegates. `.agents` was ported rather than copied, as ever: it inlines its delegation policy into each skill's Core Principles instead of loading §6, so some 25 sentences of "locally by default; delegate selectively" were flipped one at a time in Codex's `spawn_agent` vocabulary. Item numbering in §6 is deliberately untouched — 152 citations of §6.N across the trees would have shifted silently under a deletion. Separately, the Claude manifests gain `argument-hint` and turn-scoped `allowed-tools`, with `Write` granted beside `Edit` wherever a skill creates files instead of editing them — `EXEC_PLAN.md` and `EXEC_LOG.md`, the digest, the five method documents, the reviewer's and analyst's reports — while `README.md` keeps `Edit` alone so overwriting a whole README still asks, and `star-flow-status` grants neither. `allowed-tools` pre-approves and never restricts: a tool left off the list still runs and merely prompts, the field that removes one from the pool being `disallowed-tools`, which no manifest here uses. `.github/CONTRIBUTING.md` records why the two keys stay out of the other three trees — Cursor's frontmatter table is closed at five keys, Kimi keeps unknown keys and silently ignores them, and Codex removed per-skill permissions on purpose while its marketplace validator rejects `argument-hint` outright — and Cursor's `Read` and `Shell` stop being this repository's descriptive guess, the hooks and CLI permissions pages having since published both. One citation older than all of this is corrected alongside it: the `.agents` aggregate spec pointed at the executor's Core Principle 2, "make the plan visible", where it meant the delegation rule at 3 — the only cross-skill principle citation in that tree, and the only one of 28 that did not resolve.
- **[v0.1.20](https://github.com/wanghao9610/STAR/tree/v0.1.20)** (2026-08-06) — A leaf that edits existing code can now run on its own branch, and the base branch stays canonical until the change earns its merge. The conventions gain [§11](docs/mds/star-workflow/research-workflow-conventions.md): when `star-plan-executor`'s gap list shows an EXEC_PLAN modifying files the codebase already has, the plan-approval confirmation point recommends executing on `exec/<run>` — a branch named for the run, forked from whatever branch the checkout is on, never assumed to be `main` — while §1.3's "no branch switches" keeps holding everywhere else. Diff size is argument, never trigger, and choosing the branch chooses per-step checkpoint commits with it, because only commits merge: each step's commit carries the run-record updates it caused, so code and evidence travel together. Until the merge, everything the run wrote exists only on the branch, which is a semantics rather than a gap — read from the base branch the leaf is simply not done yet, so a dependent leaf stays blocked with no new check written anywhere — and `star-flow-status` lists `exec/*` branches, gains a coverage row that recommends the merge once review findings settle, and skips an in-flight leaf in its next-runnable tier, so an unmerged run reads as a pending contribution rather than lost work. The merge is a mandatory confirmation point at every involve level, reached when the leaf is `done` and the run's newest code review holds no unsettled blocker or major finding — for an experiment leaf, `done` already contains outputs produced from the branch checkout. Squash is the default, so the base branch stays linear with one commit per leaf and the step story in `EXEC_LOG.md`; keeping the step commits is the stated alternative; drift is synced by merging the base branch *in* — rebase stays forbidden — conflicts stop the run and are handed over by name; the leaf's light checks re-run on the base branch after, and the branch's deletion is its own question. Discarding is the path that keeps negative results: `wkdrs/<run>/*.md` and the sub-plan's run entry, verdict included, are committed to the base branch before the branch may go, because a dead end's records are evidence `star-plan-reviser` and `star-expt-digest` still read. Around the executor: `star-code-reviewer` reviews a branch run as its `<base>...HEAD` diff and lands fix commits ahead of the merge, `star-expt-digest` names unmerged branches among its gaps, `star_commit_guard.sh` gains arms for the spellings that break this in one keystroke (`branch -D`/`-f`, `switch -C`/`-f`, `checkout -B`/`-f`), and any skill about to commit on an `exec/*` branch that is not its target offers to switch back first. Worktrees stay the user's: a base checkout that must stay live during a branch run — a training job still reading it — is a `git worktree` the user prepares and invokes the executor inside; skills act on the checkout they are invoked in and never create or remove worktrees. The same release retires the size figures nothing checked: four skills' "the whole file is 35 KB" asides — 34 in the Chinese editions, the two languages having rotted to different wrong numbers while the conventions file actually passed 47 KB — go numberless in all four trees, the way the executor always phrased it, and the flow-status spec-size aside follows. What keeps a number is what something holds: the ~30 KB spill threshold itself, and the digest/refs excerpt claims that check 20e re-measures against the selector's actual bytes on every run, refreshed to 27 KB.
- **[v0.1.19](https://github.com/wanghao9610/STAR/tree/v0.1.19)** (2026-08-06) — The Kimi tree stops naming two tools Kimi Code CLI does not have. Its thirty manifests called the file reader `ReadFile` and the terminal `Shell`, where that harness publishes `Read`, `Write`, `Edit`, `Grep`, `Glob`, `ReadMediaFile` and `Bash` — the same two words Claude uses. Forty-six lines across thirty files now name `Read` and `Bash`, and forty-five of them come out byte-identical to their Claude counterparts, which is what a tool name that never needed adapting looks like. The rename it undoes came from [v0.1.8](https://github.com/wanghao9610/STAR/tree/v0.1.8), the release that checked every tree against its own harness's published tool list: `.cursor` and `.agents` genuinely had inherited Claude's `Bash` unadapted, and `.kimi-code` was swept in with them on the same suspicion rather than on Kimi's list, which names both tools exactly as Claude does. That is the more expensive direction of the same defect, because it leaves behind a record saying the name was checked — this repository's contributor guide and that release's own change-log entry both did — and nobody re-checks a checked name. The contradiction became legible on disk at [v0.1.17](https://github.com/wanghao9610/STAR/tree/v0.1.17), where `.kimi-code/hooks.example.toml` registers the commit guard against `PreToolUse` matching `Bash` — the name every manifest beside it had stopped using — and nobody read the two together. What the same pass got right holds unchanged, now with its citation: Kimi's `Agent` tool takes `subagent_type`, whose built-in values are `coder` (the default), `explore` and `plan`, and the tree dispatches `explore` for read-only collection at thirty-two sites and `coder` for the ten that write files; `plan` goes unused because plan approval runs through `EnterPlanMode` / `ExitPlanMode`, which Kimi has, and its structured questions take `multi_select` where Claude writes `multiSelect`. Check 23 in the consistency script now pins, per tree, the file reader, the terminal, and the `subagent_type` values each harness publishes — check 22 had validated those values for Codex alone, so a Kimi dispatch written `general-purpose` would have passed every check in the repository. The same pass re-read `.cursor` against Cursor's own documentation and deliberately changed nothing there: that harness publishes capabilities rather than tool identifiers — "Read files", "Run shell commands", a page titled Terminal — so this tree's `Read` and `Shell` are descriptive names the repository chose, and renaming them to the community-reported `read_file` and `run_terminal_cmd` would move the guess rather than end it. What the check cannot stand behind is written down instead, in the contributor guide and beside the check itself: Cursor publishes the `Task` tool but not `subagent_type`. Its built-in subagents are `explore`, `bash` and `browser`, which left this tree's ten file-writing dispatch sites naming `generalPurpose` — a type the current documentation does not list, and one none of the three built-ins succeeds. Those sites now set no type at all and carry the write surface their contract already gave them, `explore` stays wherever the delegate is read-only, and check 23 admits no other value there. Shipping a custom subagent was the alternative: Cursor loads them from `.cursor/agents/*.md` — and, for compatibility, `.claude/agents/` and `.codex/agents/` — where frontmatter `name` is the identifier the `Task` tool hints at and `readonly: true` withholds file edits and state-changing shell commands, which would make the read-only contract a mechanism rather than an instruction, at the price of an artifact to keep in step across four trees. It stays available and unused. Worth keeping straight while reading either tree: in Cursor's vocabulary `Bash` names a subagent, not a terminal.
<details>
<summary>Earlier releases</summary>

- **[v0.1.18](https://github.com/wanghao9610/STAR/tree/v0.1.18)** (2026-08-06) — A paper's architecture figure now lands in the note that reads it. `star-refs-reviewer`'s analysis note may carry one image in its Method section: the figure that shows the method as a whole, copied from the paper's own arXiv HTML rendering. Which figure that is comes from the captions and never from the numbering — a paper's first figure is as often a teaser, a qualitative sample, or a dataset diagram as it is the architecture — so the caption has to say the figure shows this paper's own method, its architecture, framework, pipeline, or overview, and several captions qualifying resolves to the one the method section refers to first. Absence is a first-class answer, and its two causes are told apart in one line: a theory, dataset, or analysis paper that has no such figure, and a paper arXiv has not rendered — roughly anything before 2024, and anything whose submitted LaTeX did not convert. A results plot promoted into the empty slot is neither, and the rubric grades it as a failure. One source only, and deliberately narrow: the arXiv HTML page is where a caption, its number, and its image are machine-readable and bound to each other, a PDF would have to be rasterized by a tool this skill is forbidden to install, and a project page, a repository README, a blog, or a search result leaves a figure whose paper is an assumption. What is fetched is written as fetched — never re-encoded, cropped, or rescaled — into `metds/refs/figs/`, where the file is its own cached payload rather than a second copy beside the run cache, and the line beneath the image carries the figure number, the caption's first sentence, the image URL and the fetch date: the same origin-beside-the-artifact rule `% src:` gives a bib entry, so a figure copied out of the note can still be traced. Choosing and fetching are the main agent's, like the GitHub call beside them; a reading collector returns the page's figures as candidates and neither picks nor downloads one. One image per note, skipped along with the note on a re-run, and none at all in `survey` mode, which writes no note to put one in.
- **[v0.1.17](https://github.com/wanghao9610/STAR/tree/v0.1.17)** (2026-08-05) — The commit offer stops being a question you have to answer. Until now [§7.7](docs/mds/star-workflow/research-workflow-conventions.md) listed it among the mandatory confirmation points, so `INVOLVE=low` — the level whose whole purpose is to stop asking — still halted at the end of every run to ask whether to commit what it had just written. It is now a judgment call like any other, and the answer it recommends is to commit: `medium` and `high` ask exactly as before and declining stays valid, `low` takes it unasked, and either way the final reply names every commit made with its message and file count, so the levels that ask keep their veto and the level that does not keeps its review. The reclassification is that mandatory class read against its own rationale — everything else in it is irreversible or costly (the STOP line, a deletion, an overwrite, plan approval), while a local commit publishes nothing, §1.3 having always forbidden pushes, and one made at the wrong moment costs a `git reset --soft`. What the confirmation actually guarded was never the commit but the staging beside it, and that is where the guarding moves. §1.4 — a path already dirty when the run started is never staged — now names the mechanism it always needed, a `git status` snapshot taken at the start of the run, because at `low` nobody reads the file list. And a new hook in all four tool trees, `star_commit_guard.sh`, declines the spellings that make §1 expensive to undo: blanket or forced staging (`add -A`, `add .`, `add -u`, `add -f`, `commit -a`), the history rewrites §1.3 names (`commit --amend`, `rebase`, `reset --hard`, `filter-branch`), and any commit whose staged files exceed 10 MB — a checkpoint or a dataset in history being the one costly mistake a research repository can make, since clearing it back out needs exactly those rewrites. `push` is deliberately absent: nothing here makes a skill likelier to push, and a user who asks for one directly should get it. The guard is a floor under the prose and never a replacement for it — it reads one shell line at a time and cannot resolve quoting, so a flag written after a commit message is past where it stops reading, and silence is its answer to everything it cannot read confidently. It reaches every harness, where the involve gate stopped at two, and for that gate's own reason inverted: the gate needs an event that can *allow*, while this one only ever denies, which all four can express. Claude, Codex and Kimi Code answer `PreToolUse` matching `Bash`; Cursor answers `beforeShellExecution`, where a shell command is decided there, and takes the documented `permission: deny` and exit code 2 together, since a build that does not read the JSON still reads the exit code. Codex takes `PreToolUse` rather than the `PermissionRequest` its involve gate uses, because that one fires only where the CLI would have waited on you — a command the project already trusts never reaches it, and a guard is worth nothing on the commands nobody was going to be asked about; the allow a released `codex-cli` rejected is not what this hook returns. Kimi's documented block shape omits the `hookEventName` the other two carry, so its copy omits it rather than sending a field the parser was not shown, and since Kimi loads no project-level config, `.kimi-code/hooks/install.sh` writes that entry into the global config the way it already writes the other two. `star-plan-coach` and `star-plan-decomposer` drop the commit offer from what they always ask; `star-plan-executor` keeps its checkpoint question, which rides along on a plan approval that stops at every level anyway.
- **[v0.1.16](https://github.com/wanghao9610/STAR/tree/v0.1.16)** (2026-08-05) — The involve gate reaches Codex. `.codex/hooks/star_involve_gate.sh` answers `PermissionRequest` — the event that fires just before the CLI waits on you, and the one that takes `decision.behavior` — with an allow for `apply_patch` while `.env` reads `INVOLVE=low`. It is not the Claude block ported across: Codex reports an edit as the patch envelope in `tool_input.command` rather than as a path field, so the paths come from the envelope's own `*** Add File:` and `*** Update File:` headers, and every one of them must sit inside the project and outside the dot-directories at its root before the hook answers. A patch that deletes or renames keeps its prompt — deletion is asked at every level, and Claude's `Edit` and `Write` cannot delete either, so the two harnesses admit exactly the same set of changes. A Bash command that reached the hook through a mis-set matcher carries no envelope at all, so that same requirement declines it rather than letting a shell command through the gate built for edits. `PreToolUse` documents an allow too, but a released `codex-cli` rejected `permissionDecision: allow` as unsupported, and `PermissionRequest` is the event built for approving, so approval goes through that one. Codex loads project hooks only once the `.codex/` layer is trusted and each hook approved, and a changed hook needs approving again, so `/hooks` in the Codex CLI is what arms this one in a project that already had the other two. Two harnesses of four, by capability rather than by choice: Cursor has no hook that fires before a file edit at all — `afterFileEdit` is a notification with no response type, and the events that do return `allow`/`deny`/`ask` gate shell and MCP calls, which this gate deliberately leaves alone — while Kimi Code's `PreToolUse` documents `deny` and no allow, and its `PermissionRequest` is marked observation-only, so it cannot answer the prompt it fires beside. `execs/update.sh` now names the involve gate for `.claude/settings.json` and `.codex/hooks.json`, the two configs that can carry it, and stays quiet about the two that cannot.
- **[v0.1.15](https://github.com/wanghao9610/STAR/tree/v0.1.15)** (2026-08-05) — `INVOLVE=low` now reaches the permission prompt, not only the questions a skill asks. A third Claude hook, `.claude/hooks/star_involve_gate.sh`, answers `PreToolUse` for `Edit`, `Write` and `NotebookEdit` with an allow decision while `.env` reads `INVOLVE=low`, so a run at that level stops asking before each file it writes. It moves permission prompts and nothing else, which is [§7.7](docs/mds/star-workflow/research-workflow-conventions.md)'s own division carried down to the harness: the STOP line, commit offers, deletions and overwrites, and plan approval are questions a skill asks, and a hook that answers permission prompts cannot reach them — they hold at `low` exactly as they hold at `high`. Two limits keep it narrower than `acceptEdits` mode, which it otherwise resembles: a path outside the project keeps its prompt, and so does every dot-directory at the project root — `.git`, `.claude`, `.star`, the other tool trees — whose contents are project machinery rather than the code a run is editing. `Bash` is not in the matcher at all, because what a command touches does not follow from its name; shell approval stays with the `permissions.allow` rules, where each entry says what it admits. The level is read from `.env` on every call, so changing it takes effect in the session already running, and silence is the hook's answer to everything else — a project with no `.env`, or one at `medium`, is a project where nothing changed. The gate is Claude-only in this release: it exists to answer a prompt Claude Code raises, and the other three hook trees are untouched. A project that keeps its own `.claude/settings.json`, which the updater still never overwrites, receives the script without the registration that arms it, so `execs/update.sh` now names the involve gate among the hooks a kept config does not register. The same release closes three gaps in what a run leaves behind. `reference.bib` carries its own provenance: every entry gains a `% src: <record URL> (fetched YYYY-MM-DD)` line above it, the same URL and date as its `refs_index.md` row, so copying the file into a paper repository no longer leaves the trail behind — and a paper with no fetchable record is named in a `%% Needs manual check` block at the end instead of being dropped, as a comment line holding no entry type, because a `%` line that begins a record is the one comment BibTeX reads and silently swallows what follows. A paper note's headline number now travels with its dataset, metric, and setting on one self-contained line, and a claim about what a method requires or cannot do names where the paper states it — a section, a table, or a short quote — because both are read a second time by the writing-side companion, which quotes lines out of the note and audits manuscript sentences against them. And `§10.6`'s pickup rule learns the mixed case: ownership is per command, not per block, so a run ending with `/star-code-reviewer <leaf>` recommended above a STOP-line command only the user can clear runs the review rather than printing the pair, since standing beside a command the user must clear does not make it one of those.
- **[v0.1.14](https://github.com/wanghao9610/STAR/tree/v0.1.14)** (2026-08-04) — Eight of the fifteen skills may now start themselves. Until this release every skill was guarded against implicit invocation, so a run began only when you typed its name — including the runs the files already determine: the review of a leaf that just finished, the analysis of the run it produced, the next ready leaf. The conventions gain [§10](docs/mds/star-workflow/research-workflow-conventions.md), a roster of all fifteen that marks seven with † — adoption, the topic, the plan, how it splits, how it is revised, what the code is built on, what gets released — and the mark means slash-only, because a decision reached on an agent's own initiative is a decision nobody made. `star-code-architect` earns it twice over: which reference implementation the project starts from shapes everything built on top of it, and its trigger fires about once in a project's life, so taking it unasked saves nobody anything. The roster is the source of truth and the four per-harness guards follow it — `disable-model-invocation: true` in the Claude, Cursor and Kimi manifests, `allow_implicit_invocation: false` in Codex's `agents/openai.yaml` — with CI checking both directions, since a † whose guard is missing runs unrequested on exactly the harness that forgot it, and a guard carrying no † withholds a skill the roster says may be picked up. Being picked up changes nothing about how the run then behaves: the STOP line, commit offers, deletions and overwrites, and every mandatory confirmation point hold exactly as they do when you type the name, and the involve level still moves judgment calls only — it never decides whether a skill may start. Three rules bound a run nobody asked for: an unresolved target is asked about rather than guessed, one skill per invocation with one unit of work inside it, and the run says so before it begins and leaves a line in the decisions record. Permission alone moved nothing, because every contract around those eight still handed the reader a command — `§7.5` asks each reply to close with the routing, and the status skill's tier 3 prints the executor's command — so `§10.6` closes the gap: where a run ends naming a next action that is one of the eight and whose target is settled, the agent runs it instead of printing it, after that run has ended and under the same three limits. The seven keep the printed command, because typing it is the decision they exist to leave with you. The guide's unattended list had been read as a triggering rule and contradicted the roster; the two questions are now separated at the top of that section — who may start a skill is one, how far a run gets before it needs you is the other, and a skill the agent started stops at exactly the points one you typed would. The eight descriptions name that second trigger source as well, since a description is what the harness matches to decide whether to surface a skill at all and every one of them opened on "Use when the user runs /star-&lt;name&gt;". It was paid for out of prose rather than trigger words — no synonym a user might type was dropped — because seven of the eight sat at 1005–1022 of the 1024-character manifest limit.
